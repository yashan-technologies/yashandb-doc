...
- [CSV文件导出](CSV文件导出)：从数据库导出按指定表结构排列的数据，并通过--csv命令选项指定生成CSV文件。生成的CSV文件可通过[yasldr](../yasldr/00yasldr)工具或[LOAD DATA](../../开发手册/SQL参考手册/SQL语句（yashan模式）/LOAD DATA)语句导入到数据库（同构或异构）。
