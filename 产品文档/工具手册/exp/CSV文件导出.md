...
| *[--lob]*                 | 设置LOB数据的导出方式，可选值lls/csv，默认值lls：<br>lls表示将LOB数据另外导出至一个单独的文件，当前CSV文件中记录该单独文件名称，文件名称格式见[LOAD DATA](../../开发手册/SQL参考手册/SQL语句（yashan模式）/LOAD DATA)中lls_column_clause描述<br>csv表示将LOB导出到当前CSV文件 |
...
| 选项                         | 含义                                                                                                                                                                                                                                                      |
|----------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *[--fields-enclosed-by]*   | 指定单个字符作为文件中各字段的包围符，该字符取值范围是ASCII码1-126对应字符，换行符、空格除外。默认值`"`，支持用字符、十六进制表示<br/>注意：<br/>\* number、bool、rowId、bit类型导出后不带包围符<br/>\* 如果字符类型（char、nchar、varchar、nvarchar）或者时间日期类型（date、time、timestamp、timestamp_ltz、timestamp_tz）的字段中出现了指定的包围符，会使用两个包围符对数据转义后导出 |
