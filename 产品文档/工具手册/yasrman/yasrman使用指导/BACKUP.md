...
压缩属性，规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE)语句中相关描述。

### ENCRYPTION

加密属性，规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE)语句中相关描述。

### FORCE

是否强制备份，规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE)语句中相关描述。

### FORMAT

备份集文件名称（含路径），规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE)语句中相关描述。
...
并行度，规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE)语句中相关描述。

### SECTION SIZE

文件分片规格，规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE)语句中相关描述。
...
- 使用CLUSTER关键字用于分布式部署的数据库备份，表示对分布式集群中的所有CN节点、DN主节点和MN主节点发起备份。执行过程中任何一个节点备份出现异常时，所有节点备份都将失败，本命令抛出错误。

### FULL

全量备份，规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE.html#full)语句中相关描述。

### INCREMENTAL LEVEL

增量备份，规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE.html#incremental)语句中相关描述。

### TAG

备份集的唯一标识，用于区分备份集，规则与定义同[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE)语句中相关描述。
...
```shell
$ yasrman sys/sys@192.168.1.2:1688 -c "backup tablespace user tag 'user_bak''" -D /home/yashan/catalog

$ yasrman sys/sys@192.168.1.2:1688 -c "backup tablespace user,system tag 'sys_bak'" -D /home/yashan/catalog
```
...
基于SEQUENCE、SCN或时间指定需要备份的归档日志范围，规则与定义同[BACKUP ARCHIVELOG](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP ARCHIVELOG)语句中相关描述。
...
配合SEQUENCE、SCN或时间指定备份起/止点，规则与定义同[BACKUP ARCHIVELOG](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP ARCHIVELOG)语句中相关描述。
