...
- YashanDB数据库：用于TPC-H测试的数据库，建议23.2.8.100及后续补丁版本，安装操作请查阅[分布式部署](../../安装和升级/安装部署/YashanDB服务端安装（命令行）/分布式部署)。TPC-H测试的参数推荐特性暂时只在23.2大版本受支持。
...
YashanDB在分布式部署下支持TPC-H测试的参数推荐，通过[参数推荐高级包](../../开发手册/PL参考手册/内置高级包/DBMS_PARAM)来设置调优后的参数。
