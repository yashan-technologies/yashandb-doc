...
>
> - 密钥管理功能不适用于分布式部署，在分布式部署中请直接创建加密对象。
...
- 在单机部署和共享集群部署中，创建加密对象前必须先完成密钥管理相关配置，包括创建钱包、开启钱包、设置主密钥等。
...
在单机部署或共享集群部署环境中，部分场景需要先[配置并开启钱包](密钥管理.html#configuringwallet)：
...
具体操作请查阅CREATE TABLESPACE的[encryption_clause](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE TABLESPACE.html#encryptionclause)子句。
...
具体操作请查阅CREATE TABLE的[table_encryption_clause](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE TABLE.html#tableencryptionclause)子句。
...
具体操作请查阅CREATE TABLE的[column_encryption_clause](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE TABLE.html#columnencryptionclause)子句。
...
具体操作请查阅ALTER TABLE的[column_encryption_clause](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER TABLE.html#columnencryptionclause)子句。
