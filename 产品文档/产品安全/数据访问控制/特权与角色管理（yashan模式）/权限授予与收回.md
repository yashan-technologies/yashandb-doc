...
| 基于角色间接授权 |满足如下条件之一：<br/>* 具备GRANT ANY ROLE系统特权。<br/>* 具备SECURITY_ADMIN安全管理员角色。  | 用户<br/>角色（不能使用WITH OPTION）<br/>在分布式部署中，无法对角色授予角色 | 已存在的所有角色 |
...
3. 执行[GRANT](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/GRANT)语句，对另一个用户或角色发起授权。
...
在分布式部署中，无法对角色授予角色，因此也不存在对角色收回角色。
...
2. 执行[REVOKE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/REVOKE)语句，对另一个用户/角色发起授权收回操作。
