...
当用户通过CREATE [OR REPLACE] FUNCITON语句创建自定义函数时，发生的行为和创建存储过程基本一致，但自定义函数多了返回值信息。
...
- 定义包头（PACKAGE HEAD）
    ...
    PACKAGE HEAD用于声明公有（PUBLIC属性）变量、公有类型、公有游标和公有子过程体对象（存储过程、自定义函数）。自定义高级包创建成功后，公有变量、公有类型、公有游标和公有子过程体对象将可以作为自定义高级包的成员被其他外部程序所引用。

- 定义包体（PACKAGE BODY）
    
    PACKAGE BODY用于定义过程体以及私有（PRIVATE属性）变量，PACKAGE BODY也可以直接使用公有变量。私有变量则只能在该PACKAGE BODY中使用，无法被外部其他过程体引用。

PACKAGE HEAD和PACKAGE BODY通过不同语法分别定义，YashanDB对HEAD和BODY存在一定约束，但YashanDB不严格要求HEAD和BODY的创建顺序，即使在创建HEAD前先定义BODY，仍然允许创建自定义高级包，但会因为HEAD声明不存在而抛出编译错误，该自定义高级包也无法被执行和调用，直至完成其HEAD创建。约束如下：

- HEAD和BODY内部不允许出现同名的变量、存储过程、自定义函数声明。

- HEAD中只能对存储过程或自定义函数对象进行声明，BODY中只能对其进行过程体定义，且HEAD中声明的对象，必须在BODY中有对应的定义。
