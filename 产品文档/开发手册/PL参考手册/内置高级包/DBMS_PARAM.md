...
| table_type| 主要业务的表类型，可选[HEAP,TAC,LSC]，默认为HEAP<br>只在分布式部署环境中且表类型为LSC时，可设置为TPCH，否则会报错 |
...
| scene| 参数调优的场景，可选[NORMAL,TPCH]，默认为NORMAL<br>当使用TPCH场景进行推荐时，必须确保所有节点都执行参数推荐，否则可能重启时可能会因为连接参数不匹配而导致失败 |
