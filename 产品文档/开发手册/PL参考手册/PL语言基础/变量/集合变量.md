...
```ebnf+diagram
syntax::= variable_name (varray_type|nested_table_type|indexby_table_type) ";"
```
...
expr1/expr2为一个YashanDB认可的[通用表达式](../../../SQL参考手册/通用SQL语法/expr)，其结果必须为一个非负数字（为NULL时表示extend(0)）。expr2省略时，表示拓展空值，expr1省略时，表示拓展1个。
