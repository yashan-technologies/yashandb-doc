...
```ebnf+diagram
syntax::= MOUNT|OPEN [READWRITE|RESETLOGS|UPGRADE|READONLY|READ(ONLY|WRITE)]
```
...
MOUNT是数据库挂载物理文件后的状态，OPEN是数据库打开后的状态，详细数据库启动介绍请参考[实例启停](../../../数据库管理/基本数据库管理/实例启停)章节。 

YashanDB的OPEN方式分为：

- READWRITE(READ WRITE)：以读写模式打开数据库，单机部署、共享集群部署和分布式部署下默认的主库打开方式，禁止物理备库使用该方式打开数据库。
- READONLY(READ ONLY)：以只读模式打开数据库，限制数据库只读，不产生任何redo，物理备库默认使用该方式打开数据库，不适用于共享集群部署、分布式部署。
...
执行此操作后该数据文件所属表空间也会被offline，表空间内的所有数据文件的状态都是recover。
...
SIZE指新增redo日志文件大小。BLOCKSIZE指redo日志文件的块大小，默认为4096，可手动指定为512的整数。redo日志实际创建的文件大小是BLOCKSIZE的整数倍，若指定大小不是整数倍，则实际创建大小向上取整。
PARALLEL可以指定创建redo日志文件的并行度，取值范围为1到8。不指定时系统根据文件大小自适应并行度，例如文件不超过1G时的并行度为1，文件超过128G时的并行度为8，文件大小在1G到128G之间时的并行度为4。
...
ALTER DATABASE ADD LOGFILE '/home/yasdb/YASDB_DATA/dbfiles/redo6' SIZE 72355840 BLOCKSIZE 512;
ALTER DATABASE ADD LOGFILE '/home/yasdb/YASDB_DATA/dbfiles/redo7' SIZE 72355840 PARALLEL 4;
ALTER DATABASE ADD STANDBY LOGFILE '/home/yasdb/YASDB_DATA/dbfiles/redo8' SIZE 72355840 BLOCKSIZE 512;
```
...
ALTER DATABASE DROP STANDBY LOGFILE '/home/yasdb/YASDB_DATA/dbfiles/redo8';
```
...
> - 可能导致备库不可用，后续可通过build database修复，请查阅[备库异常修复](../../../高可用/备库异常修复)。

示例

```sql
ALTER DATABASE CLEAR LOGFILE '/home/yasdb/YASDB_DATA/dbfiles/redo5';
```
...
该语句用于执行主备库之间的切换。主备库详细操作描述请参考[高可用手册](../../../高可用/YashanDB高可用概述)。
...
手动清理归档日志的筛选条件由ARCH_CLEAN_IGNORE_MODE参数决定，清理条件的具体描述见[归档管理](../../../数据库管理/基本数据库管理/归档管理)章节。 
...
数据库级附加日志对选定类型的所有用户表生效，可以通过动态视图[V$DATABASE](../../../参考手册/系统视图/动态视图/V$DATABASE)查看数据库级别的附加日志生效状态。若目标表开启了[加密](./CREATE TABLE.html#columnencryptionclause)，数据库级附加日志仍会根据选定表类型显示生效状态但实际redo里并不会记录该表相关的附加信息。
...
该语句用于配置数据库全库闪回功能。开启全库闪回后，数据库允许在没有备份集的情况下[闪回](FLASHBACK.html#databaseclauses)到指定时间点。
...
- 若存在[永久还原点](CREATE RESTORE POINT)，将无法关闭全库闪回功能，需先手动[删除](DROP RESTORE POINT)所有永久还原点。

- 关闭全库闪回功能时，会删除所有普通（非永久）还原点。

> **Note**：
>
> 全库闪回会对数据库性能造成一定影响（估计降低8%左右），请根据实际使用场景按需配置是否开启。

#### 10.2. 备库全库闪回

备库闪回目前仅适用于以下特定场景：

备库需要failover单独执行一段时间业务后重新闪回到failover前的时间点，重新与主库建立连接。

> **Note**：
>
> - 备库只能在MOUNT状态下开启闪回。
>
> - 备库开启闪回的同时必须要创建[永久还原点](CREATE RESTORE POINT)。
>
> - 备库开启闪回后，在执行failover之前必须把所有的archive_dest_n和replication_addr都设为空。
...
- 只能在主实例（即V$INSTANCE视图中INSTANCE_ROLE字段为MASTER_ROLE的实例）上执行本语句，且主实例需为OPEN状态。若为HA环境，则只能在主集群的主实例上执行本语句。
