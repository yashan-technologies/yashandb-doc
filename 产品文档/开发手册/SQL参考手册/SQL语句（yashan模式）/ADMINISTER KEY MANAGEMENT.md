...
该语句用于进行钱包管理操作，包括创建新钱包、打开或关闭已有钱包。
...
- 若先配置WALLET_ROOT参数再创建钱包，keystore_location必须指定为WALLET_ROOT路径的子目录`$WALLET_ROOT/tde`，或直接省略keystore_location由系统默认在WALLET_ROOT路径下创建一个子目录`tde`。

- 若先创建钱包再配置WALLET_ROOT参数，建议直接将keystore_location配置为规划好的WALLET_ROOT路径，并新增子目录`/tde`存放ewallet.p12文件，否则在创建完钱包后[打开钱包](#openkeystore)前需要将相关文件移至`$WALLET_ROOT/tde`目录下。
...
```sql
-- 配置WALLET_ROOT参数，需重启数据库使之生效
ALTER SYSTEM SET WALLET_ROOT='/data/yashan/wallet' scope=spfile;

-- 创建钱包，不指定keystore_location会自动创建子目录/tde
...
该语句用于打开`$WALLET_ROOT/tde`目录下的钱包，只能通过IDENTIFIED BY keystore_password指定密码进行钱包信息认证。

该语句的使用规则如下：

- 需确保已正确配置WALLET_ROOT参数、已生成ewallet.p12等钱包相关文件且存放在`$WALLET_ROOT/tde`目录。
...
该语句用于关闭WALLET_ROOT的子目录`tde`下的钱包，只能通过IDENTIFIED BY keystore_password指定密码进行钱包信息认证。
...
该语句用于管理主密钥，包括[创建并激活主密钥](#setkey)、[创建但不激活主密钥](#createkey)以及[激活主密钥](#usekey)。

执行该语句前，应该先[打开钱包](#openkeystore)，否则必须指定FORCE KEYSTORE关键字。
...
该语句用于激活已经存在的主密钥，key_id需指定为目标主密钥的ID（可通过查询V$ENCRYPTION_KEYS视图的KEY_ID字段获取）。
