...
该语句用于指定要更新数据的对象，可以为表名称（包括本地数据库的表或[远端表](../通用SQL语法/dblink)）、表分区名称或视图名称，可对其指定一个别名。
...
--更新多个列字段（方式二）
UPDATE area SET (area_name,DHQ) = ('华西','Shaanxi') WHERE area_no='09';
```
