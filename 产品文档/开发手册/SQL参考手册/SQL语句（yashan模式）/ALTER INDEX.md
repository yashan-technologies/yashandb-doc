...
```ebnf+diagram
syntax::= ALTER INDEX [schema"."] index_name (INITRANS integer|VISIBLE|INVISIBLE|UNUSABLE|COALESCE [CLEANUP] [ONLY]|NOPARALLEL|PARALLEL integer|NOLOGGING|LOGGING |modify_partition|modify_subpartition|rebuild_clause|RENAME TO new_name|reclaim_index_clause)
```
...
```ebnf+diagram
syntax::= REBUILD [(PARTITION partition_name)|(SUBPARTITION subpartition_name)|(NOREVERSE|REVERSE)] [(TABLESPACE tablespace_name|INITRANS integer|PCTFREE integer|ONLINE|(NOCOMPRESS|COMPRESS (integer))|(LOGGING|NOLOGGING)|(NOPARALLEL|PARALLEL integer)) {" " (TABLESPACE tablespace_name|INITRANS integer|PCTFREE integer|ONLINE|(NOCOMPRESS|COMPRESS (integer))|(LOGGING|NOLOGGING)|(NOPARALLEL|PARALLEL integer))}]
```
...
建库时默认会创建一个每天凌晨2点清理全库包含孤儿键值对索引的定时任务PMO_DEFERRED_GIDX_MAINT_JOB，它会调用[DBMS_PART](../../PL参考手册/内置高级包/DBMS_PART)中的CLEANUP_GIDX_JOB。
...
#### 10.8. NOCOMPRESS|COMPRESS

该语句用于语法兼容，无实际含义。

#### 10.9. NOLOGGING|LOGGING

该语句用于语法兼容，无实际含义。

#### 10.10. NOPARALLEL|PARALLEL
...
### 11. RENAME TO new\_name

该语句用于指定修改索引的名称。  

指定的新名称不能为空且必须符合YashanDB的[对象命名规范](../基本SQL元素/标识符)。
