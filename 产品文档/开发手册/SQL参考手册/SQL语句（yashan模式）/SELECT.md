...
在分布式部署中，[全局动态视图](../../../参考手册/系统视图/动态视图/00动态视图)相关的操作均在各节点本地执行，最后再汇总至CN。
...
```ebnf+diagram
syntax::= ((query_name|subquery)[sample_clause][flashback_query_clause][t_alias][pivot_clause]) {"," ((query_name|subquery) [sample_clause][flashback_query_clause][t_alias][pivot_clause])}
```
