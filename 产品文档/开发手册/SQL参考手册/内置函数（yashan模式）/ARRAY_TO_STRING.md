...
ARRAY_TO_STRING函数将数组（[数组变量](../../PL参考手册/PL语言基础/变量/集合变量)，[数组对象](../../PL参考手册/PL对象/自定义类型)，[数组类型](../数据类型（yashan模式）/用户自定义类型)）array_var的成员以split_string作为分隔符进行连接，若数组成员中存在NULL且replace_string非空，则将NULL成员替换为replace_string后进行连接，函数返回一个VARCHAR类型的字符串。
