...
BIN_TO_NUM函数用于实现二进制向十进制的转换，将一个或多个[expr](../通用SQL语法/expr)组合表示的二进制位向量串转换成与其等效的数值，返回一个NUMBER类型的数据。
...
--对于NUMBER数据，函数按去小数位取整后进行十进制转换，对于浮点型小数，函数按四舍五入取整后进行十进制转换。以下示例仅在主备或集群多活部署形态下HEAP表中支持，列存产品暂不支持如需示例SQL。
