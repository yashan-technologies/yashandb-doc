...
优化器可以根据系统统计信息动态调整执行计划，使用hint则对执行计划进行了唯一指定，所以应该被谨慎使用，只有在已获取到足够的统计信息，确认改变执行路径是对性能更优的方式，才建议给语句增加hint。当语句涉及的表结构、业务场景或数据量等信息发生变化时，应通过[查看执行计划](../SQL语句（yashan模式）/EXPLAIN)等方式，重新审视该hint是否合适。
...
<span id="bulkload" name="bulkload" class="yaslink"></span>
...
- 开启自动提交时，会禁止用BULKLOAD提示项，可以通过设置参数ENABLE_BULKLOAD_AUTO_COMMIT打开BULKLOAD提示项。
- BULKLOAD提示项与[SAVEPOINT](../SQL语句（yashan模式）/SAVEPOINT)功能不能同时使用。
- BULKLOAD提示项不能在[INSERT ON DUPLICATE UPDATE](../SQL语句（yashan模式）/INSERT)语句中使用。
- 同一个事务内，使用BULKLOAD提示项进行插入后，不允许再对该表进行除插入（但不能为INSERT ON DUPLICATE UPDATE）、查询外的其他DML操作。
...
SET AUTOCOMMIT ON;
INSERT /*+ BULKLOAD */ INTO area  VALUES ('07','华东','Shanghai');
YAS-03728 action not allowed : autocommit on do bulkload insert
   
-- 开启自动提交后，设置参数ENABLE_BULKLOAD_AUTO_COMMIT打开bulkload
ALTER SYSTEM SET ENABLE_BULKLOAD_AUTO_COMMIT=TRUE;
INSERT /*+ BULKLOAD */ INTO area  VALUES ('07','华东','Shanghai');
Succeed

SET AUTOCOMMIT OFF;
-- 事务内，area表使用BULKLOAD提示项进行插入后，再对area表进行更新会提示报错
INSERT /*+ BULKLOAD */ INTO area  VALUES ('08','华东','Shanghai');
UPDATE area SET area_name='华南' WHERE area_no='02';
YAS-03728 action not allowed : bulkloading table update/delete/merge

-- 此时只会回滚UPDATE语句
COMMIT;
-- 提交事务后查看area表可以发现('08','华东','Shanghai')数据已成功插入
...
<span id="deduplicate" name="deduplicate" class="yaslink"></span>

### DEDUPLICATE提示项

使用DEDUPLICATE提示项时，无需指定表名，该提示项仅适用于LSC单表插入或BULKLOAD导入。表示对有唯一约束、主键约束的LSC表进行批量插入时，若插入数据与已有数据冲突，则进行去重处理，将冲突的数据对应的行替换成新插入的行。

示例（LSC表）

```sql
--未使用DEDUPLICATE提示项时，插入重复数据报错违反唯一约束
INSERT INTO area  VALUES ('05','华南','Shenzhen');
YAS-02030 unique constraint violated
--使用DEDUPLICATE提示项后，插入重复数据成功，查询表的AREA_NO='05'的数据已经被替换成新插入的数据
INSERT /*+ DEDUPLICATE */ INTO area  VALUES ('05','华南','Shenzhen');
...
<span id="max_workers_per_exec" name="max_workers_per_exec" class="yaslink"></span>

### MAX\_WORKERS\_PER\_EXEC提示项
