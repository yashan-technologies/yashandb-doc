...
- 分布式部署中建Sharded表时，如果同时有多个唯一约束键，例如主键或唯一键，则限制所有的唯一约束键必须至少有一个公共字段 （默认的分区键取唯一约束键的公共子集中的第一个恰当的数据类型的字段），如不满足则返回错误。
...
指定索引属性的语句，其语法与[CREATE INDEX](../SQL语句（yashan模式）/CREATE INDEX)中的index_attr_clause一致。
...
创建索引的语句，除无法创建倒序索引之外，其余语法与[CREATE INDEX](../SQL语句（yashan模式）/CREATE INDEX)一致。
...
INSERT INTO area VALUES ('06','华北华北华北华北华北1','tianjing');
