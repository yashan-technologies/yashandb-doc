YashanDB支持通过[yasboot](../../../工具手册/yasboot/00yasboot)工具对分布式部署中的DN组进行扩/缩容，执行具体操作前请仔细阅读相关[注意事项](../扩缩容注意事项)。
...
3. 结合实际场景，执行相应操作。
    ...
    >  **Note**：
    >
    > 对于3节点及以上的DN组，HA_ELECTION_ENABLED参数是强制开启的，如需要关闭，可在扩容成功后修改该参数。
...
4. 若缩容时指定了--with-host参数（即移除了空服务器），缩容完成后需要更新hosts.toml文件的[[host]]内容，删除对应服务器相关信息，以免升级时仍用旧的hosts信息。