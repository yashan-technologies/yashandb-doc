...
|    格式           |     说明        |
|--------------------|-----------------------|
| username/password| 连接并登录数据库的用户名和密码。<br/>如果在yashan模式下创建用户时用户名未使用双引号包围，使用yasql连接时大小写不敏感。<br/>如果用户名或密码中含有特殊符号（下划线除外），可能在输入命令时需要对特殊符号进行一定规则的改写，详情可查阅[yasql使用指导](../../../工具手册/yasql/yasql使用指导)。  |
