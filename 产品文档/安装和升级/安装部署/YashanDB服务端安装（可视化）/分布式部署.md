...
在【数据库建库参数】页面，可参考[分布式配置文件](../../../工具手册/yasboot/配置文件/分布式配置文件)按需增/删/改各节点组的参数，确认信息无误后，单击【下一步】。

![de_create_param_next](./image/de_create_param_next.png)

## 步骤6：设置配置参数

在【数据库节点参数】页面，可按需增/删/改各节点的参数，确认信息无误后，单击【保存并下一步】。

![de_node_param_next](./image/de_node_param_next.png)
...
若DN节点数量配置为2（即DN节点采用一主一备规格），需使用基于yasom的[仲裁选主](../../../工具手册/yasboot/yasboot命令介绍/yasboot election)才能实现DN组内节点的主备自动切换。
