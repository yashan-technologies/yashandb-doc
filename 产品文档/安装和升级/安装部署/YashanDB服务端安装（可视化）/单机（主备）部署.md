...
1. 在数据库配置区域，可以配置以下功能：
   ...
   - 创建cgroup：开启表示创建用于YashanDB CPU资源管理的cgoup目录，并需填写服务器其他配置区域的cgroup目录。仅需安装可开启CPU资源管理的单机数据库（非级联备）时需配置该参数，具体请查阅[配置资源管理](../../../数据库管理/资源管理/配置资源管理)。
...
1. 如需调整节点相关配置，在节点规模区域单击【编辑】，根据实际情况调整相关配置，单击【确定】保存信息。
   ...
   - 部署模式：即语法模式，可根据业务需求选择YashanDB模式（即后文的yashan模式）或MySQL兼容模式（即后文的mysql模式）。若选定为yashan模式，安装后无法直接切换为mysql模式，只能卸载重装。
...
在【数据库建库参数】页面，可参考[单机配置文件](../../../工具手册/yasboot/配置文件/单机配置文件)按需增/删/改建库参数，确认信息无误后，单击【下一步】。

![se_create_param_next](./image/se_create_param_next.png)

## 步骤6：设置配置参数

在【数据库节点参数】页面，可按需增/删/改各数据库实例的参数，确认信息无误后，单击【保存并下一步】。

![se_node_param_next](./image/se_node_param_next.png)
...
2. （可选）创建数据库用户并赋权，更多操作请查阅[用户管理](../../../产品安全/身份标识与鉴别/用户/00用户)。
