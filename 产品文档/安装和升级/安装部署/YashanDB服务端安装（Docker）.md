...
```shell
$ docker load -i yashandb-image-{版本号}-linux-x86_64.tar.gz
```
...
yashandb                                   yashandb-image-{版本号}-linux-x86_64   2d27c4f55115   29 hours ago   912MB
```
...
```shell
$ docker run -d -p 1688:1688 -v /home/yashan/yashandb:/data/yashan -e SYS_PASSWD=password --name yashandb yashandb:yashandb-image-{版本号}-linux-x86_64
```
...
| DB_BLOCK_SIZE  | 数据块大小                                                   |
| YAS_{建库参数} | 支持所有[建库参数](../../工具手册/yasboot/建库参数)，格式为`YAS_参数名`（例如YAS_REDO_FILE_SIZE表示建库参数REDO_FILE_SIZE），此处配置过的参数建库后不可修改 |
...
2. 使用[yasql](../../工具手册/yasql/00yasql)工具连接数据库，查看实例状态。

    ```shell
    $ yasql sys/********@0.0.0.0:1688
    YashanDB SQL Enterprise Edition Release {版本号} aarch64

    Connected to:
    YashanDB Server Enterprise Edition Release {版本号} aarch64 - Linux
...
进入容器后，可使用[yasql](../../工具手册/yasql/00yasql)、[yasboot](../../工具手册/yasboot/00yasboot)等工具连接或管理数据库。
