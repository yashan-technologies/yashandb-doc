...
| COL# | INTEGER | 列ID |
| BUCKET_COUNT | BIGINT | 直方图的bucket数量 |
| ROW_COUNT | INTEGER | 直方图的行数 |
| NULL_COUNT | BIGINT | 列的空行数 |
| MINIMUM | BIGINT | bucket数量最小值 |
| MAXIMUM | BIGINT | bucket数量最大值 |
| DIST_COUNT | BIGINT | 列的distinct数量 |
| LOWVAL | RAW(1000) | 列的最小值 |
| HIGHVAL | RAW(1000) | 列的最大值 |
| DENSITY | FLOAT | 列的密度 |
| AVG_LENGTH | INTEGER | 列的平均长度 |
| MAX_LENGTH | INTEGER | 列的最大长度 |
