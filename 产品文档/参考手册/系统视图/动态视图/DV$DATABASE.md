...
| LOG_MODE | VARCHAR(16) | 是否开启归档<br>\*   ARCHIVELOG：开启归档<br>\*   NOARCHIVELOG：关闭归档 |
| OPEN_MODE | VARCHAR(16) | 启动模式<br>\*   READ_ONLY：只读模式<br>\*   READ_WRITE：读写模式 |
...
| RCY_POINT | VARCHAR(32) | 数据库启动时，日志恢复点。格式{rst}_{asn}_{blockid}_{lfn} |
| FLUSH_POINT | VARCHAR(32) | 数据库当前日志刷盘点。格式{rst}_{asn}_{blockid}_{lfn} |
| RESET_POINT | VARCHAR(32) | 数据库日志重置点。格式{rst}_{asn}_{blockid}_{lfn} |
