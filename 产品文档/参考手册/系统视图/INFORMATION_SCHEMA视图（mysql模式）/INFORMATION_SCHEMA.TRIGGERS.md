...
| ACTION_ORDER               | VARCHAR(20)    | trigger操作在同一个表上具有相同EVENT_MANIPULATION和ACTION_TIMING值的触发器列表中的序号位置|
...
| CREATED                    | VARCHAR(20)    | trigger的创建日期和时间                                              |
| SQL_MODE                   | VARCHAR(20)    | trigger创建时生效的SQL_MODE，触发器将在该模式下执行                            |
| DEFINER                    | VARCHAR(64) | 子句中命名的帐户DEFINER（通常是创建trigger的用户），格式如'user_name'@'host_name'  |
| CHARACTER_SET_CLIENT       | VARCHAR(20)    | 创建trigger时系统变量CHARACTER_SET_CLIENT的会话值                       |
| COLLATION_CONNECTION       | VARCHAR(20)    | 创建trigger时系统变量COLLATION_CONNECTION的会话值                       |
| DATABASE_COLLATION         | VARCHAR(20)    | 与trigger关联的数据库的排序规则                                          |