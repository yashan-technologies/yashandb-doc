...
- 备份数据库：执行[BACKUP DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP DATABASE)语句对整个数据库进行备份，通过DBA_BACKUP_SET视图可以查询生成的数据库备份集信息。

- 备份归档日志：执行[BACKUP ARCHIVELOG](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/BACKUP ARCHIVELOG)语句对数据库的归档日志文件进行备份，通过DBA_ARCHIVE_BACKUPSET视图可以查询生成的归档日志备份集信息。
