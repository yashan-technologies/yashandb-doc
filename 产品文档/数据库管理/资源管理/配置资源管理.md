YashanDB资源管理通过内置高级包[DBMS_RESOURCE_MANAGER](../../开发手册/PL参考手册/内置高级包/DBMS_RESOURCE_MANAGER)和相关配置参数提供对物理资源的配置能力。
...
-- 该计划指令指定CPU共享份额为50，最大使用上限为10%，用户内存使用上限为20%，会话使用上限为5%
...
-- 该计划指令指定CPU共享份额为100，最大使用上限为100%，用户内存使用上限为50%，会话使用上限为10%，资源使用组中50%的用户内存预留给每个会话
EXEC DBMS_RESOURCE_MANAGER.CREATE_PLAN_DIRECTIVE(
...
如果对应用户不存在，请先创建用户，具体操作请查阅[创建用户](../../产品安全/身份标识与鉴别/用户/管理用户（yashan模式）)。
...
1. 在[YashanDB单机部署](../../安装和升级/安装部署/YashanDB服务端安装（命令行）/单机（主备）部署)或[YashanDB分布式部署](../../安装和升级/安装部署/YashanDB服务端安装（命令行）/分布式部署)的`Step1：生成部署文件`中，增加`--create-cgroup`选项，用于创建cgroup目录。
...
>**Note**：
>
>- [删除用户](../../产品安全/身份标识与鉴别/用户/管理用户（yashan模式）)时，该用户的映射关系会被删除。
