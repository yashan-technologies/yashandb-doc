在日常使用过程中，数据库管理员可按需增/删/改自定义表空间，关于表空间管理的详细语法描述请参考开发手册[CREATE TABLESPACE](../../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE TABLESPACE)、[ALTER TABLESPACE](../../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER TABLESPACE)、[DROP TABLESPACE](../../开发手册/SQL参考手册/SQL语句（yashan模式）/DROP TABLESPACE)。
...
SELECT * FROM GV$TABLESPACE WHERE name = UPPER('tablespace名称') ORDER BY group_id;
SELECT * FROM GV$DATAFILE WHERE name LIKE '%datafile名称' ORDER BY group_id;
SELECT * FROM GV$DATABUCKET WHERE url LIKE '%bucketfile名称' ORDER BY group_id;
```
