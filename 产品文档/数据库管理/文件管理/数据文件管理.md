...
- redo：redo日志文件
- temp、swap、users、sysaux、system、undo、users_aim-0（分布式部署独有）：表空间数据文件；TSS_*_CHUNK_*_FILE_0（分布式部署独有）：表空间集下包含了chunk的表空间的数据文件
...
单机部署和共享集群部署中可直接使用[ALTER DATABASE](../../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER DATABASE)语句，分布式部署中推荐使用[yasboot](../../工具手册/yasboot/yasboot命令介绍/00yasboot命令介绍)运维工具。
