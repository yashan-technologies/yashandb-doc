...
- 数据迁移平台-YMP：YMP（YashanDB Migration Platform）是YashanDB提供的数据库迁移产品，支持数据源类型包括Oracle、MySQL、DM、PG及YashanDB，支持元数据和数据一键迁移到崖山数据库。具体的适配版本可参考[YMP产品规格](https://doc.yashandb.com/ymp/23.4/zh/Product-Introduction/Specifications.html)。
...
4. 以ymp用户将[快速体验YashanDB](快速体验YashanDB)中部署的数据库安装包和YMP安装包yashan-migrate-platform-{版本号}-linux-x86-64.zip上传到/home/ymp路径，解压后执行安装命令。
   ...
   ```bash
   # su - ymp
   $ unzip yashan-migrate-platform-{版本号}-linux-x86-64.zip
   $ cd /home/ymp/yashan-migrate-platform/
   $ sh bin/ymp.sh install --db /home/ymp/yashandb-{版本号}-linux-x86_64.tar.gz
   $ sh bin/ymp.sh status
   YMP is running, pid is 7747.
   Built-in database is used, pid is 7617.
   ```

部署完成后，可通过 http://YMP_IP:PORT/ 访问YMP服务页面，PORT默认值为8090。首次登录时需要重置登录密码，初始账户名和密码为admin/admin。
