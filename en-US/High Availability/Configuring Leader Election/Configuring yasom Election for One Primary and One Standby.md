...
For unified terms, the primary database in a ISC Distributed Cluster Deployment environment refers to the main node of the DN group, and the standby database refers to the standby node of the DN group. In a YAC Deployment environment, the primary database refers to the main cluster, and the standby database refers to the standby cluster.

## Prerequisites

Ensure that [OS Authentication](../../Database Administration/Basic Database Management/OS Authentication Configuration) functionality is enabled (it is enabled by default when following standard installation procedures).
...
- [OS Authentication](../../Database Administration/Basic Database Management/OS Authentication Configuration) must be enabled (it is enabled by default when following standard installation procedures) to use the election functionality properly.
...
| ----------------------- | ------ | ---------- | --------------------------- |
| FailoverThreshold            | 9             | [2,1000]   | Timeout for the standby database heartbeat (unit: seconds); if this time is reached, yasom will execute the failover switch process. |
...
- When the primary database fails, yasom will check if the standby database connection has timed out; when the duration exceeds the FailoverThreshold parameter value, yasom will issue a failover command to the standby database, allowing it to assume primary status.
...
#### In a ISC Distributed Cluster Deployment, how do I configure and manage the election for certain DN groups?
