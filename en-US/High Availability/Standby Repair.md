For certain specific exceptional scenarios, YashanDB provides quick repair functionality to ensure the smooth operation of a highly available environment. For syntax definitions and descriptions, please refer to [BUILD DATABASE](../Development Guide/SQL Reference Manual/SQL Statements/BUILD DATABASE).
...
>- The `build database incremental` statement can also be used to repair gaps in cascade standbys.
>- The primary database can repair the standby database GAP by specifying the IP address.
>- The `build database incremental` statement can only be executed within the same primary/standby or cascade standby cluster, and cannot cross-cluster to repair the GAP.

### Repair via Synchronous Standby

If synchronous standbys have been configured in the high availability environment, or if there is a standby database with only a small gap from the primary, the standby database can be used to repair a much lagging standby database, minimizing the impact on the primary database and business operations.
...
-- Use the synchronous standby to execute build database to repair the standby database GAP (standby_addr is the IP address of the abnormal standby database)
...
If there is no synchronous standby deployed in the high availability environment and all standby databases are significantly lagging behind the primary database, the gap must be repaired through the primary database.
...
> - The `build database repair` can also be used to repair cascade standbys.
...
```
<span id="buildpdb" name="buildpdb"></span>
...
When standby PDBs encounter abnormal situations, unified operations can also be performed through the CDB root by executing [BUILD PLUGGABLE DATABASE](../Development Guide/SQL Reference Manual/SQL Statements/BUILD PLUGGABLE DATABASE) to quickly rebuild and recover single or multiple PDBs.
