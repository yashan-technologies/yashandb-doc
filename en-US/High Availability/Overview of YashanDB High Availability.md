...
- Rich high availability deployment options:
  ...
  - Standalone primary/standby deployment: The primary and standby databases are deployed on different servers connected to the same switch, ensuring low network latency and avoiding single points of failure.

  - Standalone cascade standby deployment: An asynchronous standby database deployed at a different location reduces bandwidth load on the primary database and ensures high availability for disaster recovery.
  ...
  - YAC Deployment: Multiple instances use an active-active deployment approach, ensuring high availability for cluster databases through client TAF technology and server-side automatic failover capabilities.
...
In YAC Deployment, it is referred to as **primary cluster**, where multiple instances of the primary cluster provide online services simultaneously and transfer logs to the master instance of the standby cluster.
...
In YAC Deployment, it is referred to as **Standby Cluster**, where the master instance of the standby cluster starts multiple threads to receive logs from all instances of the primary cluster and apply them.
...
- Physical standby database: Uses the redo apply mechanism to maintain synchronization with the primary database by physically replaying redo data received from the primary database. A physical standby database is a physical copy of the primary database, with identical disk database structures block by block, and completely identical database architecture (including indexes) to the primary database.

- [Logical standby database](./Logstdby Standby/00Logstdby Standby): Uses the SQL Apply mechanism to first convert redo data received from the primary database into SQL statements, then executes the corresponding SQL statements. A logical standby database contains the same logical information as the primary database, meaning both contain exactly the same business data and metadata, but allows differences in physical storage structures (such as data file distribution and index organization forms).
...
The choice to enable [*yasom* election](./Configuring Leader Election/Configuring yasom Election) is available as needed.
...
##  High Availability in CDB

When YashanDB is deployed as a cdb (configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), it supports PDB-level primary-standby replication mechanisms. By establishing PDB-level primary-standby relationships between multiple independent CDBs, it provides flexible disaster recovery solutions for CDB environments while ensuring data high availability, achieving true cross-container data protection.
...
- Association constraint: The establishment of PDB primary-standby relationships between CDBs depends on the connection configuration between primary and standby CDB roots. Only after the primary and standby CDB roots complete the necessary association configuration can PDBs establish primary-standby relationships with corresponding PDBs in another CDB, ensuring architectural consistency and security.

- Metadata synchronization: The primary CDB and standby CDB maintain complete synchronization of PDB count and metadata.

###  PDB-Level High Availability

In primary-standby deployed CDBs, tenant-level refined high availability assurance capabilities are provided:

- Fault isolation: PDB-level independent replication and primary-standby switching mechanisms achieve true fault isolation. When a PDB fails, it does not affect the normal operation of other PDBs in the same CDB, ensuring business independence and continuity for each tenant in CDB environments and avoiding the risk of cascading failures.

- Precise recovery and resource optimization: Supports independent fault detection, recovery, and primary-standby switching operations per PDB without restarting or recovering the entire CDB. This approach reduces fault recovery time while avoiding unnecessary resource consumption, improving overall system resource utilization efficiency.

- Tenant-autonomous high availability services: Each PDB can configure independent high availability strategies and primary-standby switching thresholds based on its business importance. Critical tenants can configure more sensitive fault detection and faster switching responses, while general tenants can adopt relatively relaxed high availability configurations to achieve differentiated service assurance.

- Flexible disaster recovery strategies: Supports configuring different disaster recovery strategies and recovery targets for different PDBs, enabling both active-active deployment within the same city and remote disaster recovery to meet disaster recovery requirements under different business scenarios.

- Improved operations efficiency: Administrators can perform independent high availability maintenance and testing for specific PDBs without affecting other tenants' normal services, significantly enhancing management efficiency and maintenance flexibility in CDB environments.

YashanDB supports configuring differentiated high availability strategies for different PDBs:
...
- Automatic primary election strategy: Each PDB can configure personalized automatic primary election strategies to achieve business-level flexible disaster recovery.
...
###  Primary-Standby Operations Management

In primary-standby deployed CDBs, YashanDB provides complete operations management capabilities for PDBs:

- Metadata synchronization management: PDB creation and deletion operations can only be executed on the primary CDB and are automatically synchronized to the standby CDB, ensuring metadata consistency between CDBs.

- Localized operations: Except for PDB creation and deletion operations, both primary and standby CDBs can independently execute daily operations management for PDBs, improving operational efficiency, such as opening and closing PDBs. Common localized operations include building standby PDB and primary-standby switching.

####  Building Standby PDB

When abnormal situations such as standby PDB failures occur, the standby PDB needs to be rebuilt. PDB construction can be performed by PDB administrators themselves or managed uniformly through the CDB root. For specific operations, please refer to [Standby PDB Abnormal Repair](./​​Standby Repair.md#buildpdb).

####  Primary/Standby Switching

To meet different business scenario requirements and improve management efficiency while simplifying operations, the following primary-standby switching methods are supported:

- CDB-level global switching: Execute Switchover or Failover through the CDB root to achieve unified role conversion for the entire CDB (the CDB root and all PDBs). The final switching result is that all PDBs on the new primary CDB root become primary databases, suitable for large-scale maintenance scenarios.

  ![](./image/CDBswitch.png)

- PDB-level precise switching: Through CDB root, perform directed operations on specified single or multiple PDBs for independent Switchover or Failover, or directly connect to the target PDB to execute Switchover or Failover.
