...
## ARCHIVE\_DEST\_* Parameter Configuration
...
<span id="swgroups" name="swgroups" class="yaslink"></span>
...
3. Execute the switchover operation on NODE2-1 to initiate the promotion.
...
<span id="failovergroups" name="failovergroups" class="yaslink"></span>
...
4. Start all nodes in the old primary replication group Group 1 to the NOMOUNT stage and sequentially disable their leader election functionality, following the order of first the standby databases and then the primary database, i.e., disable the corresponding configuration for NODE1-2, NODE1-3, and finally NODE1-1.

   ```shell
   -- Disabling order: first standby databases then primary databases 
   -- Execute on NODE1-2
   $ yasboot node start -c yashandb -n 1-2 -m nomount
   $ yasql sys/********
   SQL> ALTER SYSTEM SET HA_ELECTION_ENABLED=FALSE;
   
   -- Execute on NODE1-3
   $ yasboot node start -c yashandb -n 1-3 -m nomount
   $ yasql sys/********
   SQL> ALTER SYSTEM SET HA_ELECTION_ENABLED=FALSE;
   
   -- Execute on NODE1-1
   $ yasboot node start -c yashandb -n 1-1 -m nomount
   $ yasql sys/********
   SQL> ALTER SYSTEM SET HA_ELECTION_ENABLED=FALSE;
   ```
...
<span id="failoveringroup" name="failoveringroup" class="yaslink"></span>
