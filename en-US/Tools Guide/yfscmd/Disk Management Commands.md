...
- It is necessary to ensure that after deleting disk(s), the following quantity requirements are still met. If these requirements cannot be met, YFS will prevent the deletion operation (the operation will not succeed):

    - The number of remaining failure groups meets the DiskGroup redundancy requirements.
    
    - When the redundancy level is Normal or High, the number of disks in each remaining failure group is consistent.
