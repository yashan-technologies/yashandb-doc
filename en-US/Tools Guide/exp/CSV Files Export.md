...
| *[--lob]*                   | Set the export method for LOB data; selectable values are lls/csv; default value is lls:<br> lls indicates that LOB data is exported separately to a single file, and the name of that file is recorded in the current CSV file; the file name format is described in the [LOAD DATA](../../Development Guide/SQL Reference Manual/SQL Statements (yashan Mode)/LOAD DATA) lls_column_clause<br> csv indicates that LOB is exported to the current CSV file |
