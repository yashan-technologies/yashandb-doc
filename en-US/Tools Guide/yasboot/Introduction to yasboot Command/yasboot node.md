...
| *-n, --node-id*   | The ID of the target node (e.g., `1-2`. You can view the database information via the `yasboot cluster status` command and take the numeric string before the colon of `nodeid`) (required) |
| *-q, --query*     | Query single or multiple parameters in the node configuration (supports fuzzy matching) |
...
When YashanDB is deployed as a CDB (configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), this command will close all PDBs on the target node simultaneously.  
...
When YashanDB is deployed as a CDB (configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), this command will start the PDBs that start with the CDB root (i.e., PDBs created without specifying `--policy` or with `--policy` set to `automatic`) on the target node simultaneously.  
...
When YashanDB is deployed as a CDB (configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), this command will close all PDBs on the target node, but will only restart those PDBs that start with the CDB root (i.e., PDBs created without specifying `--policy` or with `--policy` set to `automatic`).
...
| --------------- | ------------------------------------------------- |
| *-c, --cluster*   | The cluster name of YashanDB (required) |
| *-n, --node-id*   | Node ID to be promoted to primary (e.g., 1-1, can be viewed via the `yasboot cluster status` command, no need for colon and the following number) (required) |
| *--pdb*      | This option is only applicable to CDB<br/>Specify the name of PDB(s) to be promoted, must be PDB(s) on the specified node. Multiple names should be separated by commas. Specifying `all` indicates promoting all standby PDBs on the specified node to primary<br/> In a CDB, if this option is not specified, a unified role conversion of the entire CDB (the CDB root and all PDBs) will be performed. The final switching result is that all PDBs on the specified node become primary PDBs  |
...
| *-c, --cluster*   | The cluster name of YashanDB (required) |
| *-n, --node-id*   | Node ID to be promoted to primary (e.g., 1-1, can be viewed via the `yasboot cluster status` command, no need for colon and the following number) (required) |
| *--pdb*      | This option is only applicable to CDBs<br/>Specify the name of PDB(s) to be promoted to primary, must be PDBs on the specified node. Multiple names should be separated by commas. Specifying `all` indicates promoting all standby PDBs on the specified node to primary<br/> In a CDB, if this option is not specified, a unified role conversion of the entire CDB (the CDB root and all PDBs) will be performed. The final switching result is that all PDBs on the specified node become primary databases  |
| *-w, --nowait*    | Do not wait for the execution result after running |
| *-d, --child*     | Display task and subtasks information   |
| *--disable*       | Disable the display of running progress  |
| *-u, --username*  | Specify the database user. If not specified, the default user sys is used |
...
| --------------- | ---------------------------------------- |
| *-c, --cluster*   | The cluster name of YashanDB (required) |
| *-n, --node-id*   | The ID of the node to be deleted (e.g., `1-2`. You can view the database information via the `yasboot cluster status` command and take the numeric string before the colon of `nodeid`). Only one ID is allowed.<br/>To delete multiple nodes at once, please use the `--node-ids` parameter.|
| *--node-ids*      | List of node IDs to delete (e.g., `1-3,1-4`. You can view the database information via the `yasboot cluster status` command and take the numeric string before the colon of `nodeid`). One or more IDs are allowed to be specified, separated by commas. |
...
| *-d, --child*     | Display task and subtasks information   |
| *--disable*       | Disable the display of running progress  |
| *-p, --password*  | Password corresponding to the `sys` user of the database<br/>If [OS authentication](../../../Product Security/Identity Identification and Authentication/OS Authentication/00OS Authentication) (enabled by default after installation) is activated, no password needs to be specified |
