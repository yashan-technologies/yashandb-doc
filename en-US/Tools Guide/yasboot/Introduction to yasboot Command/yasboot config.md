...
This command is used to generate the scale-out configuration files: hosts_add.toml and ${cluster}_add.toml.
...
| *-g, --group-id*       | Group ID (can be seen via the `yasboot cluster status` command, the value before the hyphen in nodeid is the group ID, e.g., `1-1:1` has a group ID of 1), defaults to 1 |
...
| *--vips*              | When expanding YACs, if the original cluster has configured VIP, you need to specify the VIP configuration information list for scaled-out instances through this parameter (format: IP address/subnet mask/network card name, e.g., 192.168.60.4/255.255.255.0/ens192), the number of VIPs must match the number of scaled-out nodes, and multiple VIP configuration information separated by commas `,` |
...
> **Note**:
>
> If the `--ssl-path` and `--ssl-protocol` parameters were used during deployment, these two parameters must also be used when generating configuration for scale-out otherwise, the scale-out operation will fail.
...
***Example***3 in Distributed Cluster Deployment, for expanding nodes on new servers, generating hosts_add.toml and ${cluster}_add.toml files
