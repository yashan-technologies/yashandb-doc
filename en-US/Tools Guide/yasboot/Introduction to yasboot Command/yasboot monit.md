...
| --------------- | ---------------------------------- |
| *-c, --cluster*  | The cluster name of YashanDB (required parameter) |
| *-n, --node-id* | The ID of the target node (e.g., `1-2`. You can view the database information via the `yasboot cluster status` command and take the numeric string before the colon of `nodeid`) |
| *-h,--help*          | View help information for the current command  |
...
| --------------- | ---------------------------------- |
| *-c, --cluster*  | The cluster name of YashanDB (required parameter) |
| *-n,--node-id*   | Node ID (e.g., 1-1, can be viewed via the `yasboot cluster status` command, do not include the colon and subsequent numbers) (required parameter) |
| *--pdb*      | Only applicable to CDBs. When specifying a PDB name, it indicates monitoring that PDB individually. When omitted, it indicates monitoring all PDBs on the specified node simultaneously          |
| *-w, --nowait*   | Do not wait for the command result after execution |
| *-d, --child*    | Display task and subtasks information   |
| *--disable*      | Suppress the display of the task progress bar |
| *-h,--help*          | View help information for the current command  |
...
| *-c, --cluster*  | The cluster name of YashanDB (required parameter) |
| *-n,--node-id*   | Node ID (e.g., 1-1, can be viewed via the `yasboot cluster status` command, do not include the colon and subsequent numbers) (required parameter) |
| *--pdb*      | Only applicable to CDBs. When specifying a PDB name, it indicates stopping monitoring for that PDB only. When omitted, it indicates stopping monitoring for all PDBs on the specified node simultaneously         |
| *-w, --nowait*   | Do not wait for the command result after execution |
| *-d, --child*    | Display task and subtasks information   |
| *--disable*      | Suppress the display of the task progress bar |
| *-h,--help*          | View help information for the current command  |
