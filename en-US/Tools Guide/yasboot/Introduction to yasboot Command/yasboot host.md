...
<span id="cgroup" name="cgroup"></span>

## host cgroup create

This command is used to create a database resource management cgroup directory.

|Option |Meaning |
| --------------------- | ------------------------------------------------------------ |
| *-c,--cluster*         | Cluster name (mandatory parameter)                              |
| *--host-id*            | Server ID, which can be queried via the `yasboot cluster status` command; defaults to all servers where the database is deployed |
