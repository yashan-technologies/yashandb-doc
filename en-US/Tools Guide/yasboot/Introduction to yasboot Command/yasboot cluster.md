...
| *-t, --toml*         | Path to the cluster configuration file `{cluster_name}.toml` (required parameter) |
...
|Field |Meaning |
| --------------------- |-----------------------------------------------------------|
| hostid | The name of the server where the current database node is located |
| nodeid | ID information of the current database node, consisting of two parts (separated by a colon):<br />*  `n - m` is the actual node ID. In scenarios where the node ID needs to be specified, only this part of the value is required<br />\* `x` is the sequence number of this node in the current environment |
| node_type | Deployment form of YashanDB<br />\* se: Non-CDB in Standalone deployment<br />\* ce: Non-CDB in YAC Deployment or Distributed Cluster Deployment<br />\* de: Non-CDB in ISC Distributed Cluster Deployment<br />* cdb: CDB |
| pdb_name | Name of the container<br />This field only exists in CDB |
| pid | The ID of the core process of the current database node<br />When the value of this field is `off`, it indicates that the database instance is in a closed state |
| instance_status | The startup status of the current database node instance<br />* started: Started to the NOMOUNT stage<br />\* mounted: Started to the MOUNT stage<br />\* open: In the OPEN state |
...
| *-f, --force*        | Force stop flag (kill -9), default is false |
| *--disable*          | Disable the display of the task progress bar |
| *-s, --stop-mode*    | Database shutdown method, options [normal&#124;immediate&#124;abort], default is immediate (YACDistributed Cluster Deployment cannot use this temporarily) |
| *-u, --username*  | Specify the database user. If not specified, the default user sys is used |
| *-p, --password*  | Password for the database user <br/>If the `sys` user is used and [OS authentication](../../../Product Security/Identity Identification and Authentication/OS Authentication/00OS Authentication) (enabled by default after installation) is activated, no password needs to be specified   |
| *--wait-timeout*     | Command execution timeout (hidden parameter) |
| *--wallet-password*  | Database wallet password |
...
| *-n, --new-password*   | New sys password (required), must comply with [password complexity](../../../Product Security/Identity Identification and Authentication/Password Authentication/Password Policy.md#password_complexity) and [password policy](../../../Product Security/Identity Identification and Authentication/Password Authentication/Password Policy) requirements<br/>If the password contains special characters related to OS commands (e.g., `@`, `/`, `.`, `!`, `$`, `'`, etc.), they must be escaped |
| *-w, --nowait*         | Do not wait for the execution command result after running |
| *-d, --child*          | Display information about tasks and subtasks |
| *--disable*            | Disable the display of the task progress bar |
...
| *--disk-config*              | The disk information required to upgrade YAC from version 23.2.x.x and earlier to version 23.4.x.x and above, refer to [YAC upgrade configuration file](../Configuration Files/YAC Upgrade Configuration File) |
