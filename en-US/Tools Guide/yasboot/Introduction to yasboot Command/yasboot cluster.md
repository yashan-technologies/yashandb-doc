...
| *--yfs-force-create* | Force create diskgroup for yfs (only applicable to YACs) |
...
> **Note**: 
>
> When specifying --ignore-hostname, *yasboot* will ignore the server names and directly generate default strings (yas1, yas2, ...) as cluster node names for deployment. **It is not recommended** to do this, as using server names as cluster node names is more convenient for management.
...
| *-d, --child*       | Display information about tasks and subtasks |
| *--disable*         | Disable the display of the task progress bar |
| *-s, --stop-mode*   | Database shutdown method, options [normal&#124;immediate&#124;abort], default is immediate (YAC Deployment cannot use this temporarily) |
| *-u, --username*  | Specify the database user. If not specified, the default user sys is used |
| *-p, --password*  | Password for the database user <br/>If the `sys` user is used and [OS authentication](../../../产品安全/身份标识与鉴别/操作系统认证/00操作系统认证) (enabled by default after installation) is activated, no password needs to be specified   |
| *--wait-timeout*    | Command execution timeout (hidden parameter) |
...
| ------------------ | ---------------------------------------------------- |
| *-c, --cluster*      | The cluster name of YashanDB (required) |
| *-m, --start-mode*   | The mode for starting the database instance, options [open&#124;nomount&#124;mount], default is open (YAC Deployment cannot start in mount mode temporarily) |
| *-w, --nowait*       | Do not wait for the execution command result after running |
| *-d, --child*        | Display information about tasks and subtasks |
| *--disable*          | Disable the display of the task progress bar |
| *-u, --username*  | Specify the database user. If not specified, the default user sys is used |
...
| *-n, --new-password*   | New sys password (required), must comply with [password complexity](../../../产品安全/身份标识与鉴别/密码认证/密码策略（yashan模式）.html#password_complexity) and [password policy](../../../产品安全/身份标识与鉴别/密码认证/密码策略（yashan模式）) requirements<br/>If the password contains special characters related to OS commands (e.g., `@`, `/`, `.`, `!`, `$`, `'`, etc.), they must be escaped |
| *-w, --nowait*         | Do not wait for the execution command result after running |
| *-d, --child*          | Display information about tasks and subtasks |
| *--disable*            | Disable the display of the task progress bar |
...
| *--keep-primary*  | Retain the identity of the original primary node after rolling upgrade |
| *-w, --nowait*    | Do not wait for the execution command result after running |
| *-d, --child*     | Display information about tasks and subtasks |
| *--disable*       | Disable the display of the task progress bar  |
| *-u, --username*  | Specify the database user, which must be the `sys` user; if not specified, the default user `sys` is used |
| *-p, --password*  | The password corresponding to the `sys` user of the database<br/>If [OS authentication](../../../产品安全/身份标识与鉴别/操作系统认证/00操作系统认证) (enabled by default after installation) is activated, no password needs to be specified |
| *--disk-found-path*| YAC disk discovery path (default is /dev/yfs) |
| *--system-data*   | YAC system data disks, supports multiple inputs, separated by commas |
| *--wait-timeout*  | Command execution timeout (hidden parameter) |
| *--logic-stdby-sync-timeout* | Timeout for waiting for logical standby database synchronization in seconds (default is 600) |
| *--continue*      | For continuation of upgrades after failure between incompatible versions under special circumstances |
