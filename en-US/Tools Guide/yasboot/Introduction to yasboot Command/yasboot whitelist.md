...
| *-g, --group-id*  | The ID of the target group (e.g., `1`. You can view the database information through the `yasboot cluster status` command and take the number before the hyphen of `nodeid`)<br/> This option is mutually exclusive with the `-n, --node-id` option |
| *-n, --node-id*   | The ID of the target node (e.g., `1-2`. You can view the database information through the `yasboot cluster status` command and take the numeric string before the colon of `nodeid`)<br/> This option is mutually exclusive with the `-g, --group-id` option |
