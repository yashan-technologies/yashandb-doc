...
| *--cpu-limit*           | Maximum percentage limit of CPU usable by the server, defaults to 100           |
...
| *--scale-factor*        | Data scale for test scenarios such as TPCH, defaults to 100, indicating 100G test data                     |
...
The command is used to generate a configuration file for YAC Deployment based on the input options.
...
| *-p,--password*         | SSH login password |
| *-N,--no-password*      | SSH passwordless login |
| *--ip*                  | Deployment IP address, separate multiple IPs with a comma `,`            |
| *--port*                | SSH connection port of the server                       |
| *-i, --install-path*    | Database installation path, supports numbers, letters (case-sensitive) and some symbols (`/`, `-`, `_`, `.`)          |
| *-su,--sudo-username*   | SSH user with sudo privileges (used to execute commands that require sudo privileges, such as creating cgroup directories)           |
| *-sp,--sudo-password*   | Password for the SSH user with sudo privileges                 |
| *--host*                | Server connection information expression                       |
| *-L,--local*            | Whether it is a local deployment (no installation package needed), defaults to false, IP defaults to 127.0.0.1                 |
| *--IPv6*                | Whether to use IPv6 for local deployment, defaults to IPv4            |
| *--listen-cidr*         | Listen address of the database (defaults to the same IP segment)              |
...
| *--node*                | Scale of nodes in YAC type, defaults to 2               |
| *--replica-cidr*        | Primary/standby cluster replication link address (defaults to the same IP segment)            |
| *--inter-cidr*          | Network communication link address between instances within a YAC (defaults to the same IP segment)|
| *--data*                | Data disk in YAC, separate multiple data disk information with a comma `,`         |
...
| *--system-data*         | System disk of YAC, separate multiple system disk information with a comma `,`       |
...
| *--begin-port*          | Starting port 1688                      |
| *--data-path*           | DATA directory of the database instance, supports numbers, letters (case-sensitive) and some symbols (`/`, `-`, `_`, `.`)     |
| *-d, --deploy-mode*     | Deployment mode for ISC Distributed Cluster Deployment:<br>\* MINI: Minimum scale deployment, deployed 1MN 1CN 3DN on the same server<br>\* NORMAL: Regular deployment, based on the following node scale               |
| *-f, --force*           | Whether to force deploy the database, forcing means it will not check whether the current server's running state can deploy                      |
...
| *--ssl-protocol*        | SSL protocol, only supports SSL                  |
| *--ssl-path*            | Directory where the SSL key is located                    |
| *--create-cgroup*       | Whether to create resource management cgroup directory (defaults to no)        |
| *--cgroup-path*         | Resource management cgroup directory (defaults to `/sys/fs/cgroup`), when enabled, at least one of -u, -p or -su, -sp must be specified           |
| *--recommend-param*     | Whether to enable recommended parameter functionality                     |
| *--memory-limit*        | Maximum percentage limit of memory usable by the server, defaults to 80          |
| *--cpu-limit*           | Maximum percentage limit of CPU usable by the server, defaults to 100        |
