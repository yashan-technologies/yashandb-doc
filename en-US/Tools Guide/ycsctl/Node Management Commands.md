### ycsctl start ycs \<-m mode\> \<-p password\>
...
```shell
ycsctl start ycs <-m mode>
```

**mode**

The startup mode of the database instance, with optional values of NOMOUNT, MOUNT, or OPEN.

**password**

The [wallet password](../../Product Security/Encryption/Storage Encryption/Key Management) for key management of the database instance. When there are existing encrypted objects in a YAC and you need to start the database directly to MOUNT or OPEN mode, it must be specified; otherwise, the database instance will fail to start.

### ycsctl start instance \<-m mode\> \<-p password\>
...
> - The contents of the script to stop the database instance usually are: *yasql* logs into the database instance and executes the "shutdown immediate" command. To enhance security, it is recommended to complete the [OS authentication configuration](../../Database Administration/Basic Database Management/OS Authentication Configuration) and use yasql for [password-free login](../yasql/User Guide for yasql).
...
> **Note**: 
>
> For detailed information on YCS OS Watcher, please refer to [OS Load Monitoring](../../共享集群/集群服务管理/操作系统负载监控).
