...
```shell
$ yasldr -H
YashanDB Loader Enterprise Edition Release {version number} x86_64 431125c
...
- In a ISC Distributed Cluster Deployment, the user must also have the SELECT_CATALOG_ROLE role and privileges on the system tables ROUTE$, NODE_INFO$, DIST$, OBJ$, DISTCOL$, TAB$, USER$, PARTOBJ$, and PARTCOL$.
...
>
> - In a ISC Distributed Cluster Deployment, after completing the data import using the bulkload mode, it is recommended to perform operations such as compacting and collecting statistics on the table to improve subsequent query performance.
...
```bash
YashanDB Loader Enterprise Edition Release {version number} x86_64 a533f66
