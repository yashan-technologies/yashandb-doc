...
```ebnf
= LOBFILE "(" filler_column_name [format "fvecs"] ")" [terminated_by_eof].
```
...
When importing FVECS binary vector data files, the `format "fvecs"` keyword must be specified. Each FVECS file can only contain one vector.
