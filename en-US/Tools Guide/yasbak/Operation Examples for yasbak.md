...
>
>- The IPs in the DIN\_ADDR and REPLICATION\_ADDR of the B cluster's configuration file must be consistent with LISTEN_ADDR, and the port numbers of DIN\_ADDR and REPLICATION\_ADDR must be completely consistent with those of Cluster A; otherwise, the cluster cannot be restored properly.
>
...
2. Enter the installation directory and check all nodes of the database to determine if all nodes have archiving mode (ARCHIVELOG) enabled. If it is not enabled, please refer to [Archive Management](../../Database Administration/Instance Management/Archive Management) to enable archiving.
