...
| Data File Size           | Minimum Value      | 128  * DB_BLOCK_SIZE<br />Depending on the configured data block size, the minimum data file size can be 1MB, 2MB, or 4MB |
| Data File Size           | Maximum Value      | 64M * DB_BLOCK_SIZE<br/>Depending on the configured data block size, the maximum data file size can be 512GB, 1TB, or 2TB |
...
|Parameter Name |Parameter Type |Parameter Value |
| ------------------------ | ---------------- | :------------------------------------------------------------------------- |
| Log File Size            | Minimum Value   | 6MB <br>The minimum value is affected by DB_BLOCK_SIZE, MAX_SESSIONS, and REDO_BUFFER_SIZE parameters <br> The minimum value can be referenced with the formula: `DB_BLOCK_SIZE * MAX_SESSIONS * 8 + REDO_BUFFER_SIZE / 2` |
...
|**Parameter Name** |Parameter Type |Parameter Value |
| ---------------------- | ------------ |-----------------------------------------|
| Physical Standby Database  | Maximum Standby Database Count | Standalone Deployment: 32<br>YAC Deployment: 32 (Standby Cluster)<br>ISC (Integration Storage and Compute) Distributed Cluster Deployment: 4 (Standby Node) |
| Logical Standby Database    | Maximum Standby Database Count | Standalone Deployment: 32<br>YAC Deployment: No logical standby cluster<br>ISC Distributed Cluster Deployment: No logical standby database |
| Cascade Standby              | Maximum Cascade Count | Standalone Deployment: Unlimited<br>YAC Deployment: No cascade standby cluster<br>ISC Distributed Cluster Deployment: No cascade standby |
...
| Number of FailureGroups     | Global Maximum Value  | 2048                                             |
| Number of FailureGroups     | Maximum Value per DiskGroup | 16                                            |
| Number of FailureGroups | Minimum Value per DiskGroup | Depends on the redundancy configuration of the DiskGroup:<br />* For redundancy level External: 1<br />\* For redundancy level Normal: 2 for data DiskGroups and 3 for system DiskGroups<br />\* For redundancy level High:  3 for data DiskGroups and 5 for system DiskGroups |
| Number of Disks              | Global Maximum Value  | 65535                                            |
| Number of Disks              | Maximum Value per DiskGroup | 8192                                         |
| Number of Disks               | Minimum Value per FailureGroup | 1                                    |
| Number of Disks | Maximum Value per FailureGroup | 1024 |
| Allocation Unit (AU) Size                 | Default Value         | 1M                                               |
...
| Maximum Number of Files in per DiskGroup | Maximum Value      | 1 million                                   |
...
| Maximum Disk Size            | Maximum Value         | 4G * AU_size<br>When the AU size uses the default value, the maximum single disk size is 4PB |
| Minimum Disk Size            | Minimum Value         | 32 * AU_size<br/>When the AU size uses the default value, the minimum single disk size is 32M |
