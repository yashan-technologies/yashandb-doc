...
- Materialized Views:
  ...
  - Supports local materialized views. Remote materialized views support data synchronization via DBLink.
- BTree Indexes:
  ...
  - Supports update global indexes syntax.
- Synonyms:
...
YashanDB currently implements over 120 built-in functions. For the specific list and description, please refer to [Built-in functions](../../Development Guide/SQL Reference Manual/Built-in Functions/00Built-in Functions).
...
| String Functions     | ASCII, CHR, ASCIISTR, CHARTOROWID, CONCAT, INITCAP, INSTR, INSTRB, LEFT, LENGTH, LENGTH2, LOWER, LPAD, LTRIM, NLSSORT, POSITION, REPLACE, RIGHT, RPAD, RTRIM, SPLIT, STRPOS, SUBSTR, SUBSTRB, SOUNDEX, TRIM, TRANSLATE … USING, UNISTR, UPPER, empty_LOB, … | Supported| Supported       |
| Regex Functions       | REGEXP_LIKE, REGEXP_COUNT, REGEXP_INSTR, REGEXP_REPLACE, REGEXP_SUBSTR, … | Supported| Supported       |
| Conversion Functions   | BIN_TO_NUM, CAST, CONVERT, NLS_LOWER, NUMTODSINTERVAL, NUMTOYMINTERVAL, ROWIDTOCHAR, RAWTOHEX, TRANSLATE, TO_CHAR, TO_DATE, TO_DSINTERVAL, TO_NUMBER, TO_MULTI_BYTE, TO_SINGLE_BYTE, TO_CLOB, TO_TIMESTAMP, TO_YMINTERVAL, … | Supported| Supported       |
| Conditional Processing Functions | COALESCE, DECODE, NULLIF, NVL, NVL2, …                      | Supported| Supported       |
...
| Time Functions         | AT TIME ZONE, ADD_MONTHS, CURRENT_DATE, CURRENT_TIMESTAMP, FROM_TZ, EXTRACT, LAST_DAY, NOW, NEXT_DAY, OVERLAPS, TO_TIMESTAMP_TZ, TZ_OFFSET, … | Supported| Supported       |
| JSON Functions          | JSON, JSON_ARRAY_GET, JSON_ARRAY_LENGTH, JSON_EXISTS, JSON_FORMAT, JSON_PARSE, JSON_QUERY, JSON_SERIALIZE, JSON_TABLE, JSON_VALUE, … | Supported| Supported       |
| XML Functions | EXTRACTVALUE, EXISTSNODE, XMLAGG, XMLPARSE, XMLSEQUENCE, XMLTABLE | Supported | Supported |
| Random Functions        | RANDOM                                                           | Supported| Not Supported   |
| Other Functions         | DUMP, GREATEST, LEAST, HEXTORAW, ORA_HASH, RATIO_TO_REPORT, SQLCODE, SQLERRM, SYS_CONNECT_BY_PATH, … | Supported| Supported       |
...
- Cursors
  - BULK COLLECT
  - Explicit cursors support FOR UPDATE statements
  - Supports CURRENT OF functionality
- Stored Procedures
  - Procedure body encryption wrap functionality
- Triggers
  - Supports row-level and statement-level triggers.
  - Supports INSTEAD OF DML triggers
...
- Supports multiple characters as separators

**Operational Tools**

- Supports 10046 trace events.
...
- Supports default sampling ratio auto_sample_size

**Tablespaces**

- Supports ONLINE/OFFLINE.
- Supports RENAME.
- Supports tablespace backup and recovery.

**Reliability**

- Supports database flashback, flashback DML operations, flashback query functionality.