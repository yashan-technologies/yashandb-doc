...
As shown in the above example, by comparing the actual SELECT results with the values returned by EXPLAIN, one can assess the accuracy of Rows estimation. For specific usage, please refer to [EXPLAIN](../../../Development Guide/SQL Reference Manual/SQL Statements (yashan Mode)/EXPLAIN).

#### autotrace

Enabling autotrace directly can substitute for the above comparison process. For specific usage, please refer to [SET AUTOTRACE](../../../Development Guide/SQL Reference Manual/SQL Statements (yashan Mode)/SET AUTOTRACE).
