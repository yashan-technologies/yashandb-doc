...
| RESULT COLLECTOR      | Indicates batch execution and the opening of pipeline parallelism. | Row                     | Only for Batch |
| CLUSTER COORDINATOR  | Marker for multi-node parallel execution in cluster deployment. Indicates all operators below this marker run in parallel across nodes. | Row       | Only for Batch        |
...
| HNSW INDEX SCAN                         | HNSW index scan when excuting vector queries. | Row                     | ×        |
...
| AFFINITY PART SCAN ALL | Affinity-based full partition scan. | Row         | Only for Batch        |
| AFFINITY PART SCAN ITERATOR | Affinity-based partition group scan. | Row        | Only for Batch        |
| AFFINITY PART SCAN SINGLE | Affinity-based single partition scan. | Row       | Only for Batch        |
