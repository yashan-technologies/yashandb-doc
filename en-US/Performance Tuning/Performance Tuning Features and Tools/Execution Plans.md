...
An execution plan defines the order and algorithm by which the SQL engine runs a SQL statement. The execution plan can be queried in the *yasql* client using the [EXPLAIN](../../Development Guide/SQL Reference Manual/SQL Statements/EXPLAIN) command or the [AUTOTRACE](../../Tools Guide/yasql/User Guide for yasql.md#autotrace) command.
