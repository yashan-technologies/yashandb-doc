...
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Single cluster<br>primary-standby cluster | CDB / Non-CDB (must be non-CDB when deploying primary-standby clusters)<br>yashan mode / mysql mode<br>Single network segment / multiple network segments<br>SCAN and VIP |
...
The SCAN and VIP functions of YAC can be used in combination, and can be specified for installation through the following options in the corresponding command of [Step 1: Generate Configuration File] in the example installation process. However, when the CDB mode is specified, they cannot be simultaneously designated as SCAN mode.
...
```toml
...
scan_port = "6688"          # SCAN listening port, defaulting to the port value of LISTEN_ADDR.
...
When the use of SCAN or VIP functions is not yet determined, the above options can be omitted when installing YashanDB. YashanDB supports adding, modifying, and deleting SCAN or VIP functions on existing clusters.
...
1. Execute the yasboot package ce gen command to generate the configuration file. For the complete set of parameters for this command, please refer to the [yasboot](../../All Manuals/Tools Guide/yasboot/Introduction to yasboot Command/yasboot package) chapter. 
...
| Parameters under [group.ycsconfig]            | These parameters are all [YCS configuration parameters](../../All Manuals/Database Administration/Cluster Management/Cluster Parameter Configuration) and can be manually adjusted. |
| Parameters under [group.yfsconfig]            | These parameters are all [YFS configuration parameters](../../All Manuals/Database Administration/Storage Management/YFS Management/YFS Configuration) and can be manually adjusted. |
...
1. Execute the yasboot package ce gen command to generate the configuration file. For the complete set of parameters for this command, please refer to the [yasboot](../../All Manuals/Tools Guide/yasboot/Introduction to yasboot Command/yasboot package) chapter. 
    ...
    The values following the --ip option should be entered in primary-standby order. Multiple IP addresses are entered one by one, separated by commas. Consecutive IP addresses can be entered using range format, for example, 192.168.1.[2-3].

    In addition, the --group option value should enter the number of YACs, the --node option value will be used as the number of nodes in the primary cluster, and the --standby-node option value will be used as the number of nodes in the standby cluster.

    Primary-standby clusters cannot be deployed in CDB mode.
