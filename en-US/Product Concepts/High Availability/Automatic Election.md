...
Node priority is determined by a configuration parameter, with higher values indicating higher priority for the node. During the election process, Raft ensures that, while maintaining data consistency, high-priority nodes in the cluster are preferred to become the leader.
...
The arbitration modes are divided into zero data loss mode and normal mode: 
...
The priority of a node is determined by its order in the candidate list. The earlier a node appears in the list, the higher its priority. During arbitration, while ensuring data consistency, every effort is made to make the high-priority nodes become the new primary.
