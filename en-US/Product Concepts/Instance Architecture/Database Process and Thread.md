...
- Database Writer (DBWR) Thread
   ...
   - Write-back optimization: The DBWR thread attempts to merge multiple write-back operations of data blocks to reduce the number of disk I/O operations and improve write performance.
...
Internal network communication-related threads start in ISC Distributed Cluster Deployment and YAC Deployment, including:
...
In the ISC Distributed Cluster Deployment scenario, the capability of the server core process YASDB can be divided into the following three types of processes:

- MN Process

   The Management Node (MN) process is primarily responsible for metadata management, cluster management, and distributed transaction coordination.

- CN Process

   The Coordinator Node (CN) process provides interfaces, receives user requests, generates distributed query plans, distributes these to the DN, and aggregates execution results.

- DN Process

   The Data Node (DN) process is responsible for storing data and executing query plans issued by the CN.

### Main Threads

- Cluster Management Thread (CM_SERVICE)

   The CM_SERVICE thread is principally involved in handling internal messages and task scheduling for distributed cluster management modules. It starts during the OPEN phase of database startup and has a lifecycle consistent with the database instance. Specific functionalities include:

   - Node exception detection task scheduling.

   - Node primary/standby switch task scheduling.

   - Node information push and pull task scheduling.

   - Node join and exit cluster message handling.

- CM Task Worker Thread (CM_WORKER)

   The CM_WORKER thread mainly executes tasks assigned by the CM_SERVICE. It starts during the OPEN phase of database startup and has a lifecycle consistent with the database instance. Specific functionalities include:

   - CM exception detection.

   - CM active pull of node metadata.

   - CM active push of node metadata.

   - Node primary/standby switch task handling.

- Distributed Task Scheduling Thread (TASK_SERVICE)

   The TASK_SERVICE thread is primarily responsible for handling operational management tasks or internal tasks. It starts during the OPEN phase of database startup and has a lifecycle consistent with the database instance.

- Distributed Task Worker Thread (TASK_WORKER)

   The TASK_WORKER specifically executes tasks sent out by TASK_SERVICE. It starts during the OPEN phase of database startup and retains a lifecycle consistent with the database instance.

- Distributed Metadata Management Service Thread (MM_SERVICE)

   The MM_SERVICE thread is responsible for managing global metadata. It starts during the OPEN phase of database startup and maintains a lifecycle consistent with the database instance. Specific functionalities include:

   - Regularly checking for exceptions and generating relevant tasks for the PUB_SERVICE thread.

   - Handling requests for metadata synchronization, such as syncing global dataOid, etc.

- Distributed DDL Exception Push Thread (PUB_SERVICE)

   The PUB_SERVICE thread's primary functionality is to handle recovery instructions for distributed DDL exceptions distributed by the MM_SERVICE. It starts during the OPEN phase of database startup and maintains a lifecycle consistent with the database instance.

- Distributed Metadata Management Client Thread (LMM_SERVICE)

   The LMM_SERVICE thread interacts with the MM_SERVICE thread, responsible for synchronizing server-side metadata. It starts during the OPEN phase of database startup and has a lifecycle consistent with the database instance.

- Distributed Transaction Coordination Thread (TM_SERVICE)

   The TM_SERVICE thread is primarily responsible for regularly discovering and recovering undecided transactions during the OPEN phase of database startup, with a lifecycle consistent with the database instance.

- GTS Service Thread (GTS_SERVICE)

   The GTS_SERVICE thread mainly synchronizes global SCN across all instances in the distributed cluster. It starts during the OPEN phase of database startup and has a lifecycle consistent with the database instance.

- GTS Client Thread (LGTS_SERVICE)

   The LGTS_SERVICE thread is responsible for receiving global SCNs synchronized by the GTS_SERVICE thread. It starts during the OPEN phase of database startup and has a lifecycle consistent with the database instance.
