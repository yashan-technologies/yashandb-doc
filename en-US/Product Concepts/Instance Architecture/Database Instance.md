...
- **OPEN**: The instance is started, and the database is in an open state.
  ...
  - If it is an ISC Distributed Cluster Deployment, load and start the distribution-related capabilities.
...
The database redo log files record all changes made to the instance, with each database instance having a redo thread (which means multiple redo threads exist in YAC Deployment and Distributed Cluster Deployment).
...
- In Standalone, YAC and Distributed Cluster Deployment, when all database instances experience an abnormal shutdown (e.g., server power failure or database shutdown abort) during the first opening.

- In YAC and Distributed Cluster Deployment, if an instance closes abnormally, remaining instances will execute instance recovery.
