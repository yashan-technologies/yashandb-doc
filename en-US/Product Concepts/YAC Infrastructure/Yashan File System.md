...
A group of YFS-related threads running on each server in a YAC is called a YFS instance. The YFS instance runs in the same process (YCS process) as the YCS instance. In a production environment, typically only one YFS instance runs on each server.
