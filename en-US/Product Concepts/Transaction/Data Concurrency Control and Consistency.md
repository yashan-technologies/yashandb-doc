...
| update t1 set id = -2 where id = 2;         |                    | Session 1 updates the id of the second record to -2.         |
|                          | update t1 set id = id * 10 where id = 2;                    | Session 2 tries to update the same record, causing a transaction wait. |
| commit;       |                  | After session 1 commits, session 2 successfully updates.  |
|                          | select * from t1;          | The query result for session 2 is -1 and -20.                         |
