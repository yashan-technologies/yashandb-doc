...
- Define Package Header (PACKAGE HEAD)
    
    PACKAGE HEAD is used to declare public (PUBLIC attribute) variables, public types, public cursors, and public sub-procedures (stored procedures, UDFs). After the successful creation of package, public variables, public types, public cursors, and public sub-procedures can be referenced as members of package by external programs.

- Define Package Body (PACKAGE BODY)
    
    PACKAGE BODY is used to define the procedure body and private (PRIVATE attribute) variables. The PACKAGE BODY can also directly use public variables. Private variables can only be used within that PACKAGE BODY and cannot be referenced by external procedures.

PACKAGE HEAD and PACKAGE BODY are defined using different syntax. YashanDB imposes certain constraints on HEAD and BODY, but it does not strictly require the order of creation. Even if the BODY is defined before the HEAD is created, it is allowed to create the package, but will throw a compilation error due to the absence of the HEAD declaration, and the package cannot be executed or called until the HEAD creation is completed. The constraints are as follows:

- HEAD and BODY cannot contain variables, stored procedures, or UDF declarations with the same name.

- Only stored procedures or UDF objects can be declared in the HEAD. Only their procedure bodies can be defined in the BODY, and the objects declared in the HEAD must have corresponding definitions in the BODY.
