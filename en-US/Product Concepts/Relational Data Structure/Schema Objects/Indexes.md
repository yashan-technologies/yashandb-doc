...
RTree index is an extension of BTree in n-dimensional space, a multidimensional indexing structure specifically designed for spatial data queries, primarily used for processing geographic spatial data, multidimensional range queries, and nearest neighbor queries.

YashanDB's spatial index is a variant of R-tree (Rectangle Tree), built upon n-dimensional rectangles (typically two-dimensional rectangles or three-dimensional cubes) to construct indexes, managed in a tree structure, where each key value represents a rectangular area.

The key values stored in Rtree are the MBRs (Minimum Boundary Rectangles) of Geometry objects. Using spatial clustering, adjacent Geometry objects are grouped together to form higher-level nodes; at the next higher level, these nodes are clustered based on their MBRs to form even higher-level nodes, continuing until all Geometry objects compose a root node.

Rtree index retrieval involves giving a target rectangle and comparing the spatial relationships (whether there are common points or containment) between the bounding rectangles of each key on the RTree and the target rectangle, thereby selecting rows that meet the conditions.

## Full-Text Index

Full-Text Index is a specialized index structure based on text content analysis, specifically designed for fuzzy queries of unstructured/semi-structured text data. It is mainly applicable to scenarios such as document retrieval, content search, and log analysis, enabling quick query operations like keyword matching, fuzzy search, and boolean search.

The full-text index in YashanDB takes the words after text segmentation as the core construction units. The text content in the fields is split into independent words (i.e., tokens) by a tokenizer. An index mapping is established for each token, recording the position where the token appears in the text to achieve fast text content matching.
...
The BITMAP index is an efficient index structure based on bitwise operations, specifically designed for data queries on low - cardinality columns (i.e., columns with a small number of distinct values). It is mainly applicable in scenarios such as data warehouses and report analysis, enabling fast query operations like filtering, counting, and intersection/union operations.

In YashanDB, the BITMAP index uses binary bits as the basic storage unit. It maps a corresponding bitmap to each unique value in the column. Each bit in the bitmap corresponds to a row of data in the table. A bit value of 1 indicates that the column value of that row of data is equal to the corresponding unique value, and a bit value of 0 indicates inequality.

The BITMAP index is stored and organized by the unique values of the column. Each unique value corresponds to an independent bitmap, and all bitmaps together form a complete index. For null values, a separate bitmap is allocated for management to ensure that null - valued data is not missed during queries. During the index construction process, the bitmap data is automatically compressed to reduce storage space occupancy.

The retrieval of the BITMAP index is achieved through bitmap bitwise operations. Given a query condition, the bitmap corresponding to the unique value is first located, and then bitwise operations such as AND, OR, and NOT are used to quickly filter out the bit positions corresponding to the rows that meet the conditions, and then map them to the specific data rows in the table, greatly improving the query efficiency for low - cardinality columns.
