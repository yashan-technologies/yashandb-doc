...
Control Groups: A resource management mechanism provided by the Linux kernel. It aggregates (or divides) a series of tasks and their subtasks into different groups, and thereby limits, records, and isolates the resource usage of different groups.
...
DN: In YashanDB's ISC Distributed Cluster Deployment, DN is responsible for storing data and executing query plans issued by CN. The DN group provides high availability, with primary-standby relationships among nodes, achieving data consistency between nodes using the Raft protocol.
...
<span id="kpara" name="kpara"></span>

### KP Relationship

Key Preserved Relationship

Key Preserved (KP) Relationship: The result set of a subquery can uniquely correspond to one row in the base table, which indicates that the base table has a KP relationship.
...
MN: In YashanDB's ISC Distributed Cluster Deployment, MN is responsible for managing distributed cluster nodes, metadata management services, and distributed transaction services. Nodes within the MN group have a primary-standby relationship and achieve inter-node consistency through the Raft protocol.
...
​​NVMe Qualified Name:​​ A standardized naming system in the NVMe protocol used to uniquely identify controllers, subsystems, or namespaces. It complies with RFC 4122 specifications and adopts a hierarchical namespace structure.
...
Virtual IP: In YAC, virtual IPs can migrate between faulty instances and healthy instances, making instance-level failures transparent to users.
