...
YashanDB provides audit management in the following areas:
...
The audit log storage mode is specified by the `AUDIT_RECORD_TABLE` parameter, which can be either system tables in the SYSAUX tablespace (default, `AUDIT_RECORD_TABLE = TRUE`) or external files (`AUDIT_RECORD_TABLE = FALSE`). 

To prevent system tables from overusing the SYSAUX tablespace, a warning threshold can be configured via the `AUDIT_RECORD_THRESHOLD` parameter. If the threshold is exceeded, an audit log entry with action "AUDIT RECORD THRESHOLD" is recorded to alert users to clean up logs. If usage exceeds 80%, the system automatically switches audit logs to external files. If usage drops to ≤80%, it switches back to system tables. Each switch logs an entry with action "AUDIT RECORD SWITCH".  

###  Audit Log Viewing Method

For audit logs stored in system tables, users can query the `UNIFIED_AUDIT_TRAIL` view to retrieve and sort detailed operation records by date, audit type, object name, operating user, etc.

Audit logs stored in external files cannot be viewed directly through database views. Access them via the file system. Default path: $YASDB_DATA/log/audit. Filename format: audit-yyyymmddhhmmss.aud.

###  Audit Log Cleanup Method
