...
![3-key](./image/3-key.png)

- Wallet: A virtual container used for the independent external storage and management of database security credentials, enabling key sharing between servers in a database cluster.

    - Local Key Management: Persist the Master Encryption Key in a local wallet file on the database server.
    
    - Third-party key management: Integrate with a key management service provided by a third party, which protects the Master Encryption Key, e.g., Tencent Cloud KMS.
