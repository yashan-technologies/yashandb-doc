...
6. Select the plugins to install, using commas `,` to separate multiple values.
    ...
    | 3/gis  |  Install plugins for built-in [GIS](../../All Manuals/Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/GIS Function/00GIS Function) function  |
    ...
    | 7/listagg  |  Install plugins for built-in [LSFA_LISTAGG](../../All Manuals/Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/LSFA_LISTAGG) function(corresponding functionality is not applicable to YAC Deployment, no need to specify installation of this plugin separately)  |
    | 8/xml  |  Install plugins for built-in [XML](../../All Manuals/Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/XML Function/00XML Function) function  |
...
3. (Optional) Create a database user and grant permissions. For more operations, please refer to [User Management](../../All Manuals/Product Security/Identity Identification and Authentication/User/00User).
...
Based on the deployment scale of the database (e.g., single cluster, 1 primary and 1 standby), the parameters that need to be specified when generating the configuration file may vary slightly, as shown in the table below. For a complete list of parameters, please refer to the [yasboot package ce gen command](../../All Manuals/Tools Guide/yasboot/Introduction to yasboot Command/yasboot package.html#ce).
...
2. Adjust the relevant parameters in the configuration file according to actual needs:
    ...
    - The database cluster configuration file (example yashandb.toml): in the [group.config] section, all [database creation parameters](../../All Manuals/Tools Guide/yasboot/Database Creation Parameters) can be set. In the [[group.diskgroup]] section, configure data disk information (e.g., redundancy, data disk grouping information, etc.). In the [group.systemdiskgroup] section, configure system disk group settings (**immutable post-installation**). In the [group.ycsconfig] section, set [YCS parameters](../../All Manuals/YashanDB for Cluster/Yashan Cluster Service/YAC Configuration). In the [group.yfsconfig] section, set [YFS parameters](../../All Manuals/YashanDB for Cluster/Yashan File System/YFS Configuration). In the [group.node.config] section, set all [configuration parameters](../../All Manuals/Reference Manual/Configuration Parameters) for YashanDB.
...
For detailed information about environment variables, please refer to [Initial Environment After Installation > Environment Variables](../../All Manuals/Installation and Upgrade/Installation and Deployment/安装后初始环境/环境变量).
