...
The parameters required to generate the configuration file vary slightly depending on the size of the database to be deployed. The commonly used parameters are shown in the table below. For the complete list of parameters, please refer to the [yasboot package de gen command](../../All Manuals/工具手册/yasboot/yasboot命令介绍/yasboot package.html#de).
...
When the DN group has 1 primary 1 standby deployment, if automatic switching between primary/standby nodes in the DN group is required, it is necessary to enable the [election](../../All Manuals/高可用/自动选主配置/一主一备yasom仲裁选主) based on yasom.
