...
- Configure the monit daemon and set to start automatically on boot
    ...
    This operation only configures the database to start automatically. If [SCSI persistent reservation I/O Fencing functionality](../../All Manuals/Database Administration/Cluster Management/IO Fencing/SCSI IO Fencing) has been enabled, the startup of the *ycsrootagent* process should also be configured. Please refer to [Configure Boot Autostart](../Initial Environment after Installation/Configuring Boot Autostart) for steps.
