...
3. (Optional) If you need to deploy a primary/standby high-availability environment, click [Add] in the upper right corner of the server list to add other server information, then click [Confirm] to save the configuration, and click [Verify All] to check correctness.
   ...
   - Installation path: Database installation path, recommended to configure to the planned [HOME directory](../Pre-Installation Preparation/Creating Users and Directories.html#HOMEDirectory)/{version_number} (there is no check for the existence of the/{version_number} subdirectory; it will be created automatically during installation), supports numbers, letters (case-sensitive), and some symbols (`/`, `-`, `_`, `.`), for example, /data/yashan/yasdb_home/{version_number}.

   - Log path: [Running Log Directory](../Pre-Installation Preparation/Creating Users and Directories.html#run_log_path), for example, /data/yashan/log.
...
1. If you need to adjust node-related configurations, click [Edit] in the [Node scale] area. Adjust the relevant configurations based on actual conditions and click [Confirm] to save the information.
   ...
   - Begin port: Fill in the starting value of the database listening port. If there are multiple listening ports, the system will calculate it according to the [Port Partitioning Rules](../Pre-Installation Preparation/Preparing the Networks.html#openports); the default value is 1688.
