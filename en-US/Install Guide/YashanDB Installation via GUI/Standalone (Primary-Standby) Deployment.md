...
3. (Optional) If you need to deploy a primary/standby high-availability environment, click [Add] in the upper right corner of the server list to add other server information, then click [Confirm] to save the configuration, and click [Verify All] to check correctness.
   ...
   - Log path: [Running Log Directory](../../All Manuals/Installation and Upgrade/Installation and Deployment/安装前准备/创建用户和目录.html#run_log_path), for example, /data/yashan/log.
...
1. If you need to adjust node-related configurations, click [Edit] in the [Node scale] area. Adjust the relevant configurations based on actual conditions and click [Confirm] to save the information.
   ...
   - Begin port: Fill in the starting value of the database listening port. If there are multiple listening ports, the system will calculate it according to the [Port Partitioning Rules](../../All Manuals/Installation and Upgrade/Installation and Deployment/安装前准备/网络准备.html#openports); the default value is 1688.
   ...
   - Whether to enable recommended parameters: When the recommended configuration is enabled, yasom will call the [DBMS_PARAM](../../All Manuals/开发手册/PL参考手册/内置高级包/DBMS_PARAM) advanced package to generate recommended parameters, overriding parameters of the same name. It is defaulted to be enabled. When enabled, the following parameters also need to be configured:
...
On the [DB creation parameters] page, refer to [Database Creation Parameters](../../All Manuals/工具手册/yasboot/建库参数) to add/delete/edit corresponding parameters as needed. After confirming the information is correct, click [Next step].
