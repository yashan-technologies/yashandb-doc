...
4. In the pop-up dialog, configure other server information and click [Confirm] to save the configuration.
   ...
   - Installation path: The database installation path, recommended to be configured as the planned [HOME directory](../../All Manuals/Installation and Upgrade/Installation and Deployment/安装前准备/创建用户和目录)/{version_number} (this does not check whether the /{version_number} subdirectory exists; it will be created automatically during installation), supports numbers, letters (case-sensitive), and certain symbols (`/`, `-`, `_`, `.`), e.g., /data/yashan/yasdb_home/{version_number}.

   - Log path: [Run log directory](../../All Manuals/Installation and Upgrade/Installation and Deployment/安装前准备/创建用户和目录), e.g., /data/yashan/log.
...
1. In the pop-up node scale configuration dialog, adjust the relevant configurations based on the [actual planning](../../All Manuals/Installation and Upgrade/Installation and Deployment/安装前准备/服务器准备) for the number of instances, and click [Confirm] to save the information.
   ...
   - Begin port: Enter the starting value for the database listener port. If there are multiple listener ports, the system will calculate them based on the [port allocation rules](../../All Manuals/Installation and Upgrade/Installation and Deployment/安装前准备/网络准备.html#openports), with the default value being 1688.
...
On the [DB creation parameters] page, refer to [Database Creation Parameters](../../All Manuals/工具手册/yasboot/建库参数) to add/delete/edit corresponding parameters as needed, refer to [YFS Configuration](../../All Manuals/数据库管理/存储管理/集群文件系统管理/YFS参数配置) to add/delete/edit YFS parameters as needed, refer to [YAC Configuration](../../All Manuals/数据库管理/集群管理/集群参数配置) to add/delete/edit YCS parameters as needed, and after confirming the information is correct, click [Next step].

## Step 6: Set Configuration Parameters

On the [DB node parameters] page,  refer to [Configuration Parameters](../../All Manuals/参考手册/配置参数) to add/delete/edit corresponding parameters for each database instance as needed, and after confirming the information is correct, click [Save and Go to the Next Step].
