...
4. In the pop-up dialog, configure other server information and click [Confirm] to save the configuration.
   ...
   - Installation path: The database installation path, recommended to be configured as the planned [HOME directory](../Pre-Installation Preparation/Creating Users and Directories.html#HOMEDirectory)/{version_number} (this does not check whether the /{version_number} subdirectory exists; it will be created automatically during installation), supports numbers, letters (case-sensitive), and certain symbols (`/`, `-`, `_`, `.`), e.g., /data/yashan/yasdb_home/{version_number}.

   - Log path: [Run log directory](../Pre-Installation Preparation/Creating Users and Directories.html#run_log_path), e.g., /data/yashan/log.
...
1. In the pop-up node scale configuration dialog, adjust the relevant configurations based on the [actual planning](../Pre-Installation Preparation/Preparing the Servers) for the number of instances, and click [Confirm] to save the information.
   ...
   - Begin port: Enter the starting value for the database listener port. If there are multiple listener ports, the system will calculate them based on the [port allocation rules](../Pre-Installation Preparation/Preparing the Networks.html#openports), with the default value being 1688.
...
On the [DB creation parameters] page, refer to [YAC Deployment Configuration File](../../All Manuals/Tools Guide/yasboot/Configuration Files/YAC Deployment Configuration File) to add/delete/edit creation parameters as needed, refer to [YFS Configuration](../../All Manuals/Database Administration/Storage Management/YFS Management/YFS Configuration) to add/delete/edit YFS parameters as needed, refer to [YAC Configuration](../../All Manuals/Database Administration/Cluster Management/Cluster Parameter Configuration) to add/delete/edit YCS parameters as needed, and after confirming the information is correct, click [Next step].
