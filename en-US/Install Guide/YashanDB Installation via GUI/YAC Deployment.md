>**Note**:
>
> The visual installation currently cannot configure a YAC VIP. If you need to use VIP, please [manually deploy via command line](../YashanDB Installation via CLI/YAC Deployment.html#Manual).
...
4. In the pop-up dialog, configure other server information and click [Confirm] to save the configuration.
   ...
   - Installation path: The database installation path, recommended to be configured as the planned [HOME directory](../Pre-Installation Preparation/Partitioning Directories)/{version number} (this does not check whether the /{version number} subdirectory exists; it will be created automatically during installation), supports numbers, letters (case-sensitive), and certain symbols (`/`, `-`, `_`, `.`), e.g., /data/yashan/yasdb_home/{version number}.

   - Log path: [Run log directory](../Pre-Installation Preparation/Partitioning Directories), e.g., /data/yashan/log.
...
1. In the pop-up node scale configuration dialog, adjust the relevant configurations based on the [actual planning](../Pre-Installation Preparation/Preparing the Servers) for the number of instances, and click [Confirm] to save the information.
   ...
   - Begin port: Enter the starting value for the database listener port. If there are multiple listener ports, the system will calculate them based on the [port allocation rules](../Pre-Installation Preparation/Configuring the Pre-Installation Environment), with the default value being 1688.
...
6. In the [Node configuration] Area, adjust the following configurations as needed:
   ...
   - Click the node group (as shown in the figure ceg1) to modify the disk-related configuration for that node group.

   - Change the Node Scale: Add or delete nodes/node groups. For example, click [Add a New node Group] to add a standby cluster; click [Add] next to the node group (as shown in the figure ceg1) to add an instance to that cluster; click the delete icon next to the instance name (as shown in the figure ceg1-1) to delete that instance from the cluster.

   - Expand the database instance list, click the instance name (as shown in the figure ceg1-1) to view instance details, and adjust configurations as needed.
...
On the [DB creation parameters] page, refer to [YAC Deployment Configuration File](../../All Manuals/Tools Guide/yasboot/Configuration Files/YAC Deployment Configuration File) to add/delete/edit creation parameters as needed, refer to [YFS Configuration](../../All Manuals/YashanDB for Cluster/Yashan File System/YFS Configuration) to add/delete/edit YFS parameters as needed, refer to [YAC Configuration](../../All Manuals/YashanDB for Cluster/Yashan Cluster Service/YAC Configuration) to add/delete/edit YCS parameters as needed, and after confirming the information is correct, click [Next step].
...
1. Use the [yasql](../../All Manuals/Tools Guide/yasql/User Guide for yasql) tool to connect to the database and check the instance status.
...
2. (Optional) Create a database user and grant privileges. For more operations, please refer to [User Management](../../All Manuals/Product Security/Identity Identification and Authentication/User/00User).
