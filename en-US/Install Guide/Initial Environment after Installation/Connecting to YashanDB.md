...
|Format |Description |
|--------------------|-----------------------|
| username/password| Username and password for connecting and logging into the database.<br/>If the username is not enclosed in double quotes when created in Yashan mode, the connection via *yasql* is case-insensitive.<br/>If the username or password contains special symbols (excluding underscores), certain rules may need to be applied for entering those symbols; for details, refer to the [yasql User Guide](../../All Manuals/工具手册/yasql/yasql使用指导). |
...
- Ensure that the user has the [CREATE SESSION privilege](../../All Manuals/Product Security/Data Access Control/Privilege and Role Management (yashan Mode)/System Privileges) (or [CONNECT role](../../All Manuals/Product Security/Data Access Control/Privilege and Role Management (yashan Mode)/Roles)).
...
- **Login**
    ...
    - **{database_name}**: Optional, specifies the default database for subsequent operations in the current session after connecting to YashanDB. If not specified, no database will be entered, and you can specify it using the [USE](../../All Manuals/Development Guide/SQL Reference Manual/SQL Statements (mysql Mode)/USE) statement after connecting.
