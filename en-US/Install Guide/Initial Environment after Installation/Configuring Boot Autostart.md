...
| ycsrootagent process | [SCAN](../../All Manuals/共享集群/集群服务管理/SCAN管理) enabled, [VIP](../../All Manuals/共享集群/集群服务管理/VIP管理) enabled or [I/O Fencing functionality based on SCSI persistent reservations](../../All Manuals/共享集群/集群服务管理/IO Fencing/基于SCSI持久预留的IO Fencing) | Provides corresponding services for I/O Fencing functionality based on SCSI persistent reservations. <br/> This process cannot be managed by YAC node start/stop commands and cannot be monitored by the yascsm process. During installation, *yasboot* will automatically attempt to start *ycsrootagent* with sudo; if this process encounters exceptions or if the server restarts, it needs to be manually started with sudo (requires root privilege). |
