...
| --- | --- | --- |
| Operating System | CentOS 7.6 and above<br/>openEuler 22.03 LTS<br/>KylinOS V10 (ARM architecture: recommended using V10 SP3 2303 kernel 52.33, Haiguang CPU: recommended using V10 SP3 2303 kernel 52.45) | If large-page memory is to be used, the required Linux kernel version is 2.6 or above<br>If NVMe oF protocal based TCP is to be used, the required Linux kernel version is 5.0 or above |
...
| Network Card | Recommended to use more than 2 private network cards, using Linux network bonding technology to bond cards for redundancy, configured with one IP address<br>If deploying a YAC and using VIP functionality, ensure all servers in the same cluster configured for VIP have the same card names<br>If deploying a distributed cluster, all cards must be RDMA in storage network, running normally with matching driver installed. | -                                                            |
...
> **Caution**:
>
> YAC supports two methods of [I/O Fencing](../../All Manuals/YashanDB for Cluster/Yashan Cluster Service/IO Fencing/00IO Fencing), with recommendations to use [SCSI Persistent Reservation-based I/O Fencing](../../All Manuals/YashanDB for Cluster/Yashan Cluster Service/IO Fencing/SCSI IO Fencing), but this method has certain requirements on the shared storage's capabilities. During the installation process, the [fenceScsiCheck script](../../All Manuals/YashanDB for Cluster/Yashan Cluster Service/IO Fencing/SCSI IO Fencing.html#fenceScsiCheck_usage) will automatically be invoked to check whether the shared storage and other devices/environments meet the requirements.
>
> If the fenceScsiCheck script detects an error, it will not block the installation, but YAC's I/O Fencing method will be configured as the [In-Transit I/O Protection Algorithm](../../All Manuals/YashanDB for Cluster/Yashan Cluster Service/IO Fencing/IO Protection Algorithm). If persistent reservation-based I/O Fencing is still required, please make adjustments according to the corresponding error prompts.
>
...
| Server 1    | 192.168.1.2   | Primary   |
| Server 2    | 192.168.1.3   | Standby   |
| Server 3    | 192.168.1.4   | Cascading Standby |
...
| Server 4 (Datacenter B) | 192.168.10.5  | Standby Group: 2-1:4 (Asynchronous Standby) |
| Server 5 (Datacenter B) | 192.168.10.6  | Standby Group: 2-2:5 (Cascade Standby) |
| Server 6 (Datacenter B) | 192.168.10.7  | Standby Group: 2-3:6 (Cascade Standby) |
...
[VIP](../../All Manuals/YashanDB for Cluster/Yashan Cluster Service/VIP) is an optional extended functionality in YAC Deployment, and if used, it should follow these rules:
...
- VIP must be in the same subnet, the same network card as the instance listening address (LISTEN_ADDR).
...
| Server 3    | 192.168.10.4  | 192.168.10.64 | Standby Cluster Database Instance 1 |
| Server 4    | 192.168.10.5  | 192.168.10.65 | Standby Cluster Database Instance 2 |
...
|Server Name |Server IP |Role |
| ---------- | ----------- | ------------------------------------- |
| Server 1    | 192.168.1.2  | CN 1  |
| Server 2    | 192.168.1.3  | CN 2            |
| Server 3    | 192.168.1.4  | DN 1 |
| Server 4    | 192.168.1.5  | DN 2 |
| Server 5    | 192.168.1.6  | DN 3 |
