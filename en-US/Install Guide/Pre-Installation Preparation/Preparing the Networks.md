...
- The database listening port will serve as the initial value for calculating other internal communication ports.
  ...
  - If the SQL mode is specified as [mysql mode](../../All Manuals/Product Overview/Compatibility/Compatibility with MySQL), a set of listening ports for mysql protocol usage should also be planned, with a default value of 1690 (Standalone (primary/standby) deployment) or 1691(YAC deployment) and each standby database defaulting to +3, and so on.
