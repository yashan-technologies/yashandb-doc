...
To perform a [rolling upgrade](./Upgrade Procedure/Rolling Upgrade), ensure the current database meets the following conditions:
...
- Database operates in yashan mode.
    ...
    If the current environment operates in mysql mode, use [offline upgrade](./Upgrade Procedure/Offline Upgrade) instead.
...
- Database contains no LSC tables.
    ...
    If LSC tables exist in the current database, [rolling upgrade](./Upgrade Procedure/Rolling Upgrade) only applies to compatible versions. For cross-version upgrades across incompatible versions, please use [offline upgrade](./Upgrade Procedure/Offline Upgrade).
...
If the default backup path is to be used, there is no need to manually create the path, but it must be ensured that the upgrade operation user has permission to create the path, and the parent directory ${new_version_YASDB_HOME} where the default backup path is located has sufficient capacity.

| Deployment Form | Backup Location |
|--------------------|-------------|
| Standalone Deployment<br/>ISC Distributed Cluster Deployment | ${New_version_YASDB_HOME}/upgrade_tmp/backup on each server |
| YAC Deployment | ${New_version_YASDB_HOME}/upgrade_tmp/backup on the server hosting the master instance |
...
Before upgrading, please check and terminate the daemons on the servers where the instances reside. If auto-start at boot has been configured, you need to temporarily [disable auto-start](../Installation and Deployment/Initial Environment after Installation/Configuring Boot Autostart.md#disable).
...
After the upgrade is complete, please restore the relevant configurations as needed. For specific operations, please refer to [one-primary/one-standby yasom election](../../High Availability/Configuring Leader Election/Configuring yasom Election for One Primary and One Standby).
...
Query the database status information through the [yasboot cluster status](../../Tools Guide/yasboot/Introduction to yasboot Command/yasboot cluster) command, and confirm the instance status based on the value of the instance_status field.
...
Query the status of corresponding instances through the [GV$INSTANCE](../../Reference Manual/System Views/Dynamic Performance Views/GV$INSTANCE) or [V$INSTANCE](../../Reference Manual/System Views/Dynamic Performance Views/V$INSTANCE) views.
...
Query the detailed information of the DiskGroup through the [show diskgroup](../../Tools Guide/yfscmd/Status Inspection Commands) command, and confirm its status based on the value of the `stat` field.
...
Query the status of corresponding instances through the [GV$YFS_DISKGROUP](../../Reference Manual/System Views/Dynamic Performance Views/GV$YFS_DISKGROUP) views.
