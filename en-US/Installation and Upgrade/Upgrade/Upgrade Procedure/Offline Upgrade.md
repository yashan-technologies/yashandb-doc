...
- After a successful upgrade, you need to determine whether any deprecated parameters are in use based on the running log warnings. Please refer to the parameter initialization section example in [Parameter Configuration](../../../Database Administration/Instance Management/Database Parameter Configuration). If deprecated parameters are in use, check the introduction of the corresponding parameters in [Configuration Parameters](../../../Reference Manual/Configuration Parameters) to determine if the parameter has become deprecated due to renaming and whether it is necessary to adjust the deprecated parameter to the corresponding new parameter. If the new parameter is not configured, the value of the deprecated parameter will be automatically converted to the new parameter value.
...
2. Check whether yasom and yasagent processes exist in the current environment and their versions.
    ...
    # Execute the -h command to obtain current version information
    ...
    ```shell
    $ {the bin directory of yasom}/yasboot process yasom stop -c yashandb -t /home/yashan/install/hosts.toml
    stop yasom successfully

    $ {the bin directory of yasom}/yasboot process yasagent stop -c yashandb -t /home/yashan/install/hosts.toml
...
7. Modify the management configuration file join_demo.toml as needed and save it.
   ...
   [primary_config]
     ...
     yasdb_home = "/opt/yasom/yashandb/yashandb"					# Change to the HOME directory of the primary database.
     node_path = "/opt/yasom/yashandb/data/yashandb/db-1-1"		# Change to the DATA directory of the primary database (must be the actual value obtained by running the "yasdb -D" command).
...
2. Execute the corresponding command based on the database deployment form to upgrade the database.
    ...
    - Standalone Deployment, YAC Deployment (upgrading between versions 23.4.x.x and above), ISC Distributed Cluster Deployment 
        ...
        If you need to use a [custom backup path](../Pre-Upgrade Preparation.md#space), the --backup-path parameter must also be specified.
    ...
    - YAC Deployment (from 23.2.x.x and earlier to 23.4.x.x and above)
        ...
        |Parameter |Description |
        |--------------------|--------------------------------------|
        | --disk-config       | Specify [YAC upgrade configuration file](../Pre-Upgrade Preparation.md#sys-data) |
...
- Enable or Disable Certain Configurations
  ...
  If certain configurations were temporarily adjusted to smoothly execute the upgrade (such as disabling yasom election, disabling auto-start), they should be restored as needed after the upgrade (for example, re-enable yasom election, re-enable auto-start). 

  If the functionality supported by the new version is disabled by default after the upgrade (for example, SCAN or VIP functionality for YAC), you can manually configure and enable the corresponding functionality according to the process (for example, [configure SCAN for YAC](../../../Database Administration/Cluster Management/SCAN Management.md#scan_configuration)).

  >**Note**:
  >
  > Starting from version 23.2.1.x, the yasom election functionality depends on enabled [OS Authentication](../../../Product Security/Identity Identification and Authentication/OS Authentication/00OS Authentication). If yasom election was enabled in versions below 23.2.1.x (which will be temporarily disabled during the pre-upgrade preparation), you must enable it again after upgrading to version 23.2.1.x or above. You should first check and enable the operating system authentication to ensure the proper functioning of the election functionality.