...
- Enable or Disable Certain Configurations
  ...
  >**Note**:
  >
  > Starting from version 23.2.1.x, the yasom election functionality depends on enabled [OS Authentication](../../../Database Administration/Basic Database Management/OS Authentication Configuration). If yasom election was enabled in versions below 23.2.1.x (which will be temporarily disabled during the pre-upgrade preparation), you must enable it again after upgrading to version 23.2.1.x or above. You should first check and enable the OS authentication to ensure the proper functioning of the election functionality.
