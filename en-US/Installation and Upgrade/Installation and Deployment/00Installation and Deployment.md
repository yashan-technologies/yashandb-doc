...
> - This chapter is a guide for installing YashanDB in a new environment. If YashanDB is already running in an environment, an upgrade, rather than a fresh installation, is needed. Please refer to [Upgrade](../Upgrade/00Upgrade) for specific operations.
>- YashanDB has been adapted to multiple [CPU architectures](./Pre-Installation Preparation/Preparing the Servers), and its installation configuration and upgrade operations are identical across different devices, including the required commands and corresponding graphical interface. 
...
2. Execute the installation program. YashanDB provides three installation methods:
   ...
   - [Interactive Command Installation](./YashanDB Server Installation (Interactive)/00YashanDB Server Installation (Interactive)): Interactive command installation requires logging into the server, and the configuration is completed through interaction with the commands during the installation process.
...
Both command line and graphical installation of YashanDB are based on the [yasboot tool](../../Tools Guide/yasboot/00yasboot), with the main execution flow being:
...
2. Prepare the deployment environment for all nodes, including automatically creating the HOME directory and running log directory, synchronizing the installation package to all servers, and decompressing it, etc.
   ...
   > **Note**: 
   >
   > A node refers to an operating environment that can provide a certain level of service capability (management, computing, storage, etc.). In Standalone/YAC/Distributed Cluster Deployment, one server constitutes one node.
