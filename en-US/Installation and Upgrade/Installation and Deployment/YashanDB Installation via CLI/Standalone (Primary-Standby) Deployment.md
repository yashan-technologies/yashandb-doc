...
1. Check the operating system configuration.
...
3. Select the plugins to install, using commas `,` to separate multiple values.
    ...
    | 7/listagg  |  Install plugins for built-in [LSFA_LISTAGG](../../../Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/LSFA_LISTAGG) function  |
...
| -su,--sudo-username<br/>-sp,--sudo-password | Specify the SSH username and password with sudo privileges. If the installation user already has sudo privileges, this parameter can be omitted. <br/> During the installation process, some operations may use or verify sudo privileges.  | - |
...
| --replica-cidr      | During high-availability deployment, the network segment for the replication link between primary and standby databases; this configuration will be used for the REPLICATION_ADDR parameter. The format is `192.168.1.0/24` <br/> If a different network segment is planned for the database in production, this parameter needs to specify the planned private network segment. <br/> If no different network segment is planned, this parameter can be omitted. | Omit |
...
| --plugins | Plugins to be installed, separate multiple options with a comma `,`, optional options and their meanings are as follows:<br>* all: Install all plugins, default value<br>* none: Do not install plugins<br>* dblink: Install plugins for DBLink related functionality<br>* gis: Install plugins for built-in [GIS](../../../Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/GIS Function/00GIS Function) function<br>* listagg: Install plugins for built-in [LSFA_LISTAGG](../../../Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/LSFA_LISTAGG) function<br>* s3: Install plugins for S3 bucket related functionality<br>* udf: Install plugins for [ UDF ](../../../Development Guide/PL Reference Manual/PL Objects/User-Defined Functions) related functionality<br>* xml: Install plugins for built-in [XML](../../../Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/XML Function/00XML Function) function  |  Omit (default: all) |
| -m, --mode | Specify the syntax mode of the database, optional options [yashan,mysql]<br/>* yashan: Indicates deployment in yashan mode; if omitted, it defaults to this value, and once installed in yashan mode, it cannot be switched directly to mysql mode.<br/>* mysql: Indicates deployment in mysql mode | Omit | 
|**Node Scale-Related Parameters** |
| --group             | Only required during dual repli-group primary-standby deployment, specifying the number of groups | Omit or specify as 2      |
| --node | During high-availability deployment, the number of primary and standby database nodes (excluding the number of cascading standbys). If omitted, it defaults to 1. For example, for a deployment of 1 primary and 2 standby, this parameter needs to be set to 3. <br/> If it is a primary/standby deployment of dual replication groups, this parameter indicates the number of nodes in the primary group | Specify as needed |
| --standby-node      | Only required during dual repli-group primary-standby deployment, indicating the number of nodes in the standby group       | Specify as needed                        |
| --cascade-node      | Number of cascading standby nodes                      | Specify as needed                      |
| --cascade-parent    | Index of the primary standby database for the cascading standby        | Specify as needed                             |
|**Database Performance-Related Parameters** | Specify as needed; omit if not required | |
...
| --scene           | Scenario for parameter tuning, optional [NORMAL,BATCH,TPCH], defaults to NORMAL. For OLTP business primarily using HEAP tables, if performance is poor during periodic batch scenarios, consider using BATCH scene parameters for tuning.          |
...
1. Execute the yasboot package se gen command to generate the configuration file.
