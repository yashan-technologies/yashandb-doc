...
Currently, interactive deployment cannot configure the VIP of YAC. If you need to use this functionality, please proceed with [manual deployment](#Manual). 
...
1. Check the operating system related configuration.
...
5. Configure shared storage information.
    ...
    After entering all shared storage information, the optimal I/O Fencing method will be selected for YAC.
...
6. Select the plugins to install, using commas `,` to separate multiple values.

    |Option |Description |
    | ------------ |------------|
    | 1/none  |  Do not install plugins  |
    | 2/all  |  Install all plugins, default value  |
    | 3/gis  |  Install plugins for built-in [GIS](../../../Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/GIS Function/00GIS Function) function  |
    ...
    | 7/listagg  |  Install plugins for built-in [LSFA_LISTAGG](../../../Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/LSFA_LISTAGG) function(corresponding functionality is not applicable to YAC Deployment, no need to specify installation of this plugin separately)  |
    | 8/xml  |  Install plugins for built-in [XML](../../../Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/XML Function/00XML Function) function  |
...
4. Configure the required directories for the database based on the [directory partitioning](../Pre-Installation Preparation/Partitioning Directories) plan:
    ...
    YASDB_HOME will be: /data/yashan/yasdb_home/{version}
    ```
...
Based on the deployment scale of the database (e.g., single database, 1 primary and 2 standby), the parameters that need to be specified when generating the configuration file may vary slightly, as shown in the table below. For a complete list of parameters, please refer to the [yasboot package ce gen command](../../../Tools Guide/yasboot/Introduction to yasboot Command/yasboot package.html#ce).
...
|--ip | Specify the IP addresses of the database server, multiple IPs separated by commas.  | [Set of Server IP Addresses](../Pre-Installation Preparation/Preparing the Servers), for example 192.168.1.2,192.168.1.3  |
|--port | Specify the SSH service port of the database server | 22    |
| -su,--sudo-username<br/>-sp,--sudo-password | Specify the SSH username and password with sudo privileges. If the installation user already has sudo privileges, this parameter can be omitted. <br/> During the installation process, some operations may use or verify sudo privileges.  For example: During the installation of YAC, the current environment's compliance with SCSI I/O Fencing requirements will be automatically detected, requiring sudo privileges. (Detection errors will not block installation)         | - |
...
| --data             | Specify the bound [data disk](../Pre-Installation Preparation/Partitioning Directories.html#lun) path<br/> During primary/standby cluster deployment, the correct standby cluster data disk configuration cannot be automatically generated (By default, both primary/standby cluster configurations in the configuration file use the --data specified value), manual editing of the toml file is required for configuration | /dev/yfs/data0 |
| --disk-found-path   | Specify the disk found path, i.e., the parent directory of the data disk | /dev/yfs |
| --system-data      | Specify the bound [system disk](../Pre-Installation Preparation/Partitioning Directories.html#lun) path. The system disk group redundancy will be automatically set based on the specified number of system disks<br/> During primary/standby cluster deployment, it is not possible to automatically generate the correct standby cluster system disk configuration (By default, both primary/standby cluster configurations in the configuration file use the --system-data specified value), manual editing of the toml file is required for configuration | /dev/yfs/sys0,/dev/yfs/sys1,/dev/yfs/sys2 |
...
| --inter-cidr       | Internal network communication link address segment of YAC, this configuration will be used for CLUSTER_INTERCONNECT and INTER_URL parameters. The format is `192.168.1.0/24`<br/> If a different segment is planned for the database in the production environment, this parameter must specify the planned private network segment<br/> If no different segment is planned, this parameter can be omitted        | Omitted |
| --replica-cidr     | During primary/standby cluster deployment, this parameter represents the subnet of the replication link between the primary and standby clusters, which will be used for the REPLICATION_ADDR parameter. The format is `192.168.1.0/24`<br/> If a different segment is planned for the database in the production environment, this parameter must specify the planned private network segment<br/> If no different segment is planned, this parameter can be omitted         | Omitted |
| --vips             | VIP configuration information list of YAC, format: IP address/subnet mask/network card name, e.g., 192.168.60.4/255.255.255.0/ens192<br/> The number of VIPs should match the number of instances. Multiple VIPs are separated by a comma `,<br/> During the primary/standby deployment, it cannot automatically generate the VIP configuration for instances in the standby cluster, manual editing of the toml file is required | [Server VIP address set](../Pre-Installation Preparation/Preparing the Servers), e.g., 192.168.1.62/24/ens192,192.168.1.63/24/ens192 |
...
2. Adjust the relevant parameters in the configuration file according to actual needs:
    ...
    - The database cluster configuration file (example yashandb.toml): in the [group.config] area, all [database creation parameters](../../../Tools Guide/yasboot/Database Creation Parameters) can be set. In the [[group.diskgroup]] area, configure data disk information (e.g., redundancy, data disk grouping information, etc.). In the [group.ycsconfig] area, set [YCS parameters](../../../YashanDB for Cluster/Yashan Cluster Service/YAC Configuration). In the [group.yfsconfig] area, set [YFS parameters](../../../YashanDB for Cluster/Yashan File System/YFS Configuration). In the [group.node.config] area, set all [configuration parameters](../../../Reference Manual/Configuration Parameters) for YashanDB.
    ...
    [[group]]
      ...
      [[group.node]]    # Node configuration.
        ...
        vip = 192.168.1.62/24/ens192   # VIP configuration for YAC instance.
...
2. Adjust the relevant parameters in the configuration file according to actual needs:
    ...
    - The database cluster configuration file (example yashandb.toml): in the [group.config] area, all [database creation parameters](../../../Tools Guide/yasboot/Database Creation Parameters) can be set. In the [[group.diskgroup]] area, configure data disk information (e.g., redundancy, data disk grouping information, etc.). In the [group.ycsconfig] area, set [YCS parameters](../../../YashanDB for Cluster/Yashan Cluster Service/YAC Configuration). In the [group.yfsconfig] area, set [YFS parameters](../../../YashanDB for Cluster/Yashan File System/YFS Configuration). In the [group.node.config] area, set all [configuration parameters](../../../Reference Manual/Configuration Parameters) for YashanDB.
         ...
         During primary/standby cluster deployment, it is impossible to automatically generate the correct standby cluster system disk and data disk configurations (The default configuration file for the primary/standby cluster uses the --data and --system-data specified values), VIP configurations must be manually modified in the configuration file yashandb.toml.
