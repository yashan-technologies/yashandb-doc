...
- Time zones must be consistent
   ...
   ## If time zones are different, corresponding adjustments need to be made
...
| ---------------- | ---------- |------------------------------- | -----------------|
| shmmax | 50%-90% of physical memory, and greater than the database SGA | vi /etc/sysctl.conf add: `kernel.shmmax = 24696061952`, execute `sysctl -p` to make the configuration take effect | Maximum size of a single shared memory segment (in bytes) |
| ‌shmall | Calculated as shmmax / PAGE_SIZE (typically 4KB) | vi /etc/sysctl.conf add: `kernel.shmall = 6029312`, execute `sysctl -p` to make the configuration take effect | Total number of shared memory pages in the system |
| shmmni | Typically default 4096, can be increased for high concurrency scenarios | vi /etc/sysctl.conf add: `kernel.shmmni = 4096`, execute `sysctl -p` to make the configuration take effect | Total number of shared memory segments allowed in the system |
...
<span id="CGroups" name="CGroups"></span>

## Enable CGroups (Optional)

CGroups is a resource-limiting mechanism provided by the Linux kernel. To use [CPU resource management](../../../Database Administration/Resource Management/00Resource Management), you must ensure that the server has this feature enabled. Most mainstream Linux distributions (e.g., CentOS/RHEL 7+) enable CGroups by default. You can follow the steps below to further confirm and optionally enable it.

1. Confirm whether CGroups are enabled.

   ```bash
   # mount | grep -E "cgroup|cgroup2"
   ## If this command returns no output, it indicates that CGroups are not enabled

   ## Check whether CGroups are disabled due to kernel boot parameters
   # sudo cat /etc/default/grub

   ## Check whether the GRUB_CMDLINE_LINUX line contains cgroup_disable information — if so, it indicates that CGroups are disabled by kernel boot parameters.
   ```

2. Enable CGroups as needed.

   - Scenario 1: CGroups are disabled due to kernel boot parameters

      ```bash
      ## Modify the kernel boot parameters — remove the cgroup_disable configuration from the GRUB_CMDLINE_LINUX line
      # vim /etc/default/grub

      ## Regenerate the GRUB configuration (please use the actual grub.cfg path —— may be /boot/efi/EFI/[system_name]/grub.cfg)
   ...
   - Scenario 2: CGroups are disabled, but not due to kernel boot parameters
      ...
      # mount | grep -E "cgroup|cgroup2"
      ```
      Based on the CGroups version supported by the OS kernel and the CGroups version to be mounted (optional: v1, v2, or v1 + v2 hybrid mode), add the following content to the `/etc/fstab` file:

      ::: tabs
      == Mount CGroup v1 
```text
tmpfs          /sys/fs/cgroup      tmpfs   mode=755,nosuid,nodev,noexec        0 0
...
If it is an openEuler system, you do not need to add the `tmpfs` line listed below.
