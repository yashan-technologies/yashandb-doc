To ensure the performance of YashanDB, it is recommended to adjust the following operating system parameters on all servers where YashanDB is installed. Especially during ISC Distributed Cluster Deployment, failure to perform these adjustments may result in extreme scenarios that exhaust system resources during subsequent usage, potentially leading to service outages or business operation stalls and errors.

## Set Server Hostname (YAC)

When deploying a YAC, server names within the same cluster cannot be the same.
