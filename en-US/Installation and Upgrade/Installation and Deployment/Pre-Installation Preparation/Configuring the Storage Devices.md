...
If it is a primary-standby cluster deployment, LUNs must be planned and bound separately for the primary cluster and standby cluster.
...
|Steps    |YAC Deployment |Distributed Cluster Deployment |
| ----------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Disk partitioning | Partition LUNs according to YFS disk planning                | Partition namespaces according to YFS disk planning          |
| Mounting disks    | Mount to all servers via multipath                           | Mount to all DN servers                                      |
| Binding disks | Perform binding on all servers | - |
| Disk mapping      | -                                                            | Perform mapping on all DN servers                            |
| Connecting disks  | -                                                            | Perform connection on all CN servers                         |
...
- **System Disk**: Used for cluster management. The absolute paths of these disks (generated by subsequent configurations in this chapter) will serve as the value of the `--system-data` parameter, separated by commas, such as `--system-data /dev/yfs/sys0,/dev/yfs/sys1,/dev/yfs/sys2`. 
    ...
    Only supporting 1, 3, or 5 disks of the **same size**, with each disk having a minimum capacity of 1G (if the number of system disks is 1 and AU Size needs to be configured to 32M, the system disk size must be at least 2G). The number of planned system disks will affect the [redundancy](../../../Product Concepts/YAC Infrastructure/Yashan File System.md#redundancy) of the system DiskGroup, with 1, 2/3, or 5 system disks corresponding to EXTERNAL, NORMAL, or HIGH redundancy.
...
- **Data Disk**: Used to store cluster business data. The absolute paths of these disks (generated by subsequent configurations in this chapter) will serve as the value of the `--data` parameter, separated by commas, such as `--data /dev/yfs/data0,/dev/yfs/data1,/dev/yfs/data2`.
	...
	Considering the redundancy of data DiskGroups, the number of data disks can be planned as a multiple of the actual demand. Data disks can also be added later during operation according to circumstances.
...
## Binding Disks for YAC Deployment

Please perform the following operations on **all servers** in the YAC Deployment. Upon completion of the operations described under this heading, the storage device configuration for YAC Deployment is complete. Please skip the rest of this document's content.
...
## Storage Configuration for Distributed Cluster Deployment

In Distributed Cluster Deployment, you need to log in to all DN servers as root user or a user with sudo privileges, and map all NVMe-oF disks that have been mounted on the servers.

Assume that the NVMe SSD has been partitioned and mounted to the DN servers as shown below:

|NVMe-oF Disk Path |Disk Usage |Host Server | Namespace Path Planning (Example Value) |
|--------|--------------------|------------------|--------|
...
The namespace path is a sample value. Please adjust it according to the actual business requirements. However, when naming, please note:

- The full-path character length shall not exceed 31 bytes.
- The namespace paths will all be used as the corresponding parameter values of the yasboot package ce gen command. If there are any modifications, please use the actual values when executing the yasboot package ce gen command.

Mapping of NVMe-oF disks requires installing nvme-cli or nvmetcli tools. This chapter uses nvmetcli tool as an example.  
...
11. Log in to other DN servers and repeat all the above steps to complete NVMe-oF disk mapping on all DN servers.

## Connecting Disks for Distributed Cluster Deployment
...
6. Log in to other CN servers and repeat the above steps to complete the disk connection.
