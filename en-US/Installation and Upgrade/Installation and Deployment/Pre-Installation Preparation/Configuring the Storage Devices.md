...
- **System Disk**: Used for cluster management. The absolute paths of these disks (generated by subsequent configurations in this chapter) will serve as the value of the `--system-data` parameter, separated by commas, such as `--system-data /dev/yfs/sys0,/dev/yfs/sys1,/dev/yfs/sys2`. 
  
   Only supporting 1, 2, 3, or 5 disks of the **same size**, with each disk having a minimum capacity of 1G (if the number of system disks is 1 and AU Size needs to be configured to 32M, the system disk size must be at least 2G). The number of planned system disks will affect the [redundancy](../../../Product Concepts/YAC Infrastructure/Yashan File System.md#redundancy) of the system disk group, with 1, 2, 3, or 5 system disks corresponding to EXTERNAL, NORMAL, or HIGH redundancy.
	
    The system disk configuration **cannot be modified after installation**. Please determine the appropriate configuration specifications (number of disks, disk capacity) based on your requirements at this step and proceed with the subsequent operations. If uncertain, please consult our technical support.

- **Data Disk**: Used to store cluster business data. The absolute paths of these disks (generated by subsequent configurations in this chapter) will serve as the value of the `--data` parameter, separated by commas, such as `--data /dev/yfs/data0,/dev/yfs/data1,/dev/yfs/data2`.

    Supporting 1 or more disks. For detailed configuration specifications, please refer to [YFS Specifications](../../../Product Overview/Specifications/Physical Specifications.md#YFS). If the redundancy level of the DiskGroup is planned to be Normal or High, both the available **capacity** of all disks and the **number** of disks in each FailureGroup in the same DiskGroup need to be consistent. Please plan the quantity and capacity according to actual business needs.
...
- The bind directory and disk name used by the same LUN on each server in the same YAC must remain consistent.
...
>
>  - When using other methods to bind disks, the disk permissions may change after server restart, which could cause the database to fail to start (such issues will be classified as [ERROR][errno=05579] in the [run.log](../../../Database Administration/Operation Monitoring/Log Management/Runtime Log Management)). If you encounter such problems, manually adjust the disk permissions (owner: installation user, permissions: at least 0666).
...
|NVMe-oF Disk Path |Disk Usage |Host Server | Planned Namespace Path (Example Value) |
|--------|---------------------|-----------------|--------|
| /dev/nvme0n1 | System Disk | 172.16.1.4 | /dev/nvme1n1  |
| /dev/nvme0n2 | System Disk | 172.16.1.4 | /dev/nvme2n1  |
| /dev/nvme0n1 | System Disk | 172.16.1.5 | /dev/nvme3n1  |
| /dev/nvme0n3 | Data Disk | 172.16.1.5 | /dev/nvme4n1  |
...
3. Enter the directory where the nvmfAdd.sh script is located, check the script execution permissions, and execute the script to map NVMe-oF disks.  
   ...
   The subsystem port ID must be a positive integer, and the listening address must be an IP address within the subnet of [Network Planning](./Preparing the Networks) (For example, 172.16.1.4 in the table above). The transport port must be the port number opened according to the [Port Planning](./Preparing the Networks.md#openports).
...
## Binding Disks for YAC (Distributed Storage)/Distributed Cluster Deployment
...
In Distributed Cluster Deployment, you need to log in to each CN server as root user or a user with sudo privileges, and bind all successfully connected NVMe-oF disks.
...
|------------|--------------------------------------|--------|
| /dev/nvme1n1 | System Disk | /dev/yfs/sys0 |
| /dev/nvme2n1 | System Disk | /dev/yfs/sys1 |
| /dev/nvme3n1 | System Disk | /dev/yfs/sys2 |
| /dev/nvme4n1 | Data Disk | /dev/yfs/data0 |
