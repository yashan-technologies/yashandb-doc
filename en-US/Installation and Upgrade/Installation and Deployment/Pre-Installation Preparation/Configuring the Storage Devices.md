...
- **System Disk**
    ...
    Used for cluster management, only supporting 1, 3, or 5 disks of the **same size**, with each disk having a minimum capacity of 1G (if the number of system disks is 1 and AU Size needs to be configured to 32M, the system disk size must be at least 2G). The absolute path of these disks will serve as the value of the --system-data parameter, separated by commas, such as `--system-data /dev/yfs/sys0,/dev/yfs/sys1,/dev/yfs/sys2`. The absolute path is generated by the configuration described below in this document.
	
	The number of planned system disks will affect the [redundancy](../../../Product Concepts/YAC Infrastructure/Yashan File System.md#redundancy) of the system disk group, with 1, 3, or 5 system disks corresponding to EXTERNAL, NORMAL, or HIGH redundancy.
