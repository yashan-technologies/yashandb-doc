...
- Storage Network (Exclusive to YAC): Mainly used for YAC instances to access shared storage.
...
|Address |Description |Suggested Subnet |
|--------------------|-------------|-----------------|
| LISTEN_ADDR<br/>VIP  | Both are used to connect to the database, providing external database services <br/> VIP is only applicable to YAC Deployment, and must be in the same subnet and on the same network card as LISTEN_ADDR | Public Network         |
