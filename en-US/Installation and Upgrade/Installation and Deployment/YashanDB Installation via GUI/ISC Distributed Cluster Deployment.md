...
4. In the pop-up dialog box, configure the information for other servers and click [Confirm] to save the configuration.
   ...
   - Installation path: The database installation path, recommended to be configured as the planned [HOME directory](../Pre-Installation Preparation/Creating Users and Directories.md#HOMEDirectory)/{version_number} (this does not check whether the /{version_number} subdirectory exists; it will be created automatically during installation), supporting numbers, letters (case-sensitive), and special symbols ( `/`, `-`, `_`, `.`), e.g., /data/yashan/yasdb_home/{version_number}.

   - Log path: [Log directory](../Pre-Installation Preparation/Creating Users and Directories.md#run_log_path), e.g., /data/yashan/log.
...
1. In the pop-up dialog for node scale configuration, adjust the relevant configurations based on the [actual planning](../Pre-Installation Preparation/Preparing the Servers) for the number of nodes, and click [Confirm] to save the information.
   ...
   - Begin port: Fill in the starting value for the database listening port. If there are multiple listening ports, the system will calculate based on the [port allocation rules](../Pre-Installation Preparation/Preparing the Networks.md#openports), with a default value of 1688.
