...
5. In the yasagent configuration area, adjust the following configurations as needed:
   ...
   - Containing nodes: Displays the database instance information deployed on each server. Instances can be dragged to adjust their distribution. Recheck the NVMe protocol configuration after operation.
...
6. In the [Node configuration] Area, adjust the following configurations as needed:
   ...
   - Adjust node scale: add/delete CN nodes. Click the [+] icon to the right of the node group name to add CN nodes to the cluster; click the delete icon to the right of the instance name (for example, ceg1-1) to delete that node. Recheck the NVMe protocol configuration after operation.
...
> **Note**:
>
> After deployment, yasom will generate hosts.toml and yashandb.toml files in the directory `/home/yashan/install/conf/CE_DE/yashandb`, where yashandb is the database name, and this directory is the installation directory.
