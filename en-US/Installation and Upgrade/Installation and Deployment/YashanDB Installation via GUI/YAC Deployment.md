...
4. In the pop-up dialog, configure other server information and click [Confirm] to save the configuration.
   ...
   - Installation path: The database installation path, recommended to be configured as the planned [HOME directory](../Pre-Installation Preparation/Creating Users and Directories.md#HOMEDirectory)/{version_number} (this does not check whether the /{version_number} subdirectory exists; it will be created automatically during installation), supports numbers, letters (case-sensitive), and certain symbols (`/`, `-`, `_`, `.`), e.g., /data/yashan/yasdb_home/{version_number}.

   - Log path: [Run log directory](../Pre-Installation Preparation/Creating Users and Directories.md#run_log_path), e.g., /data/yashan/log.
...
1. In the pop-up node scale configuration dialog, adjust the relevant configurations based on the [actual planning](../Pre-Installation Preparation/Preparing the Servers) for the number of instances, and click [Confirm] to save the information.
   ...
   - Whether to enable recommended parameters: When the recommended configuration is enabled, yasom will call the [DBMS_PARAM](../../../Development Guide/PL Reference Manual/Built-in Advanced PL Packages/DBMS_PARAM) advanced package to generate recommended parameters, overriding parameters of the same name. It is defaulted to be enabled. When enabled, the following parameters also need to be configured:
      ...
      - Table type: Select the default table type of the database and modify the database configuration parameters.
