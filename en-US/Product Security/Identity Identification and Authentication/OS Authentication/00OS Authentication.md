OS authentication means that YashanDB directly reuses the OS user authentication for login to the server, eliminating the need for database-level authentication. This allows users to connect directly to the database instance corresponding to the local YASDB_DATA environment variable without needing a YashanDB user password (or any password).
