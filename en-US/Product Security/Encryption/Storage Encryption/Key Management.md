...
- Plan and create a storage path for the wallet file on each database server. The YashanDB install user (i.e., yashan) must have full privileges for the corresponding path. This document takes the path /data/yashan/wallet as an example.
