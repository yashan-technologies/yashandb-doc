...
In YashanDB, trusted channels can currently be configured for communication between database clients and servers, primary-standby replication networks, and the internal interconnect bus (DIN) in ISC Distributed Cluster Deployment. The specific encryption protocols supported by each channel are shown in the table below.
...
- If enabling database client <-> server trusted channel:
    ...
    - Database parameters related to various server-side certificate paths must be correctly configured; otherwise, the database startup will fail.
    ...
    - Clients must correctly configure certificates to connect to the database.
