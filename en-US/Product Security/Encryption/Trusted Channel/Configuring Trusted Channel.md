Before configuring trusted channels, please first understand their [configuration rules](00Trusted Channel.md) and select the trusted channels to be configured and the appropriate encryption protocol based on actual requirements.

After configuring trusted channels, subsequent attention should be paid to the validity period of the corresponding certificates, and certificates should be [replaced](./Managing Certificates.md#expire) in a timely manner before they expire.
...
- [SSL protocol related certificates](./Managing Certificates.md#getSSLCert) have been generated.
...
3. Enable the SSL trusted channel switch and configure the server certificate paths.
    ...
    > **Warn**:
    >
    > After turning on the client <-> server trusted channel, database parameters related to various server - side certificate paths must be configured correctly. Otherwise, the database will fail to start.
    >
    > If the SSL trusted channel is enabled and the [server configuration parameter file](../../../Database Administration/Storage Management/Database File Management/Configuration Parameter File and Password File Management) (yasdb.spfile) is used, an additional check will be performed on the consistency between the server-side certificate file and the records in yasdb.spfile when starting the database. If this check fails, the database startup will also fail.
...
<span id="tlcp" name="tlcp"></span>

###  Using TLCP Protocol

#### Preparation

- [TLCP protocol related certificates](./Managing Certificates.md#getTLCPCert) have been generated.
...
<span id="dinssl" name="dinssl"></span>
...
3. Enable the DIN trusted channel in ISC Distributed Cluster Deployment and configure the server certificate paths.
