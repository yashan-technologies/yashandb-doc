...
1. Create the physical standby database. If the standby database is created later than the primary database (for example, in a scaling scenario), it must first be initialized based on the primary database data.
...
1. Enable supplemental logging for the YashanDB database ([database-level supplemental logging](../../Development Guide/SQL Reference Manual/SQL Statements/ALTER DATABASE.md#supplementallogclauses) or [table-level supplemental logging](../../Development Guide/SQL Reference Manual/SQL Statements/ALTER TABLE.md#supplementaltablelogging)).
