...
After enabling the separation of duties, you can further require management users to declare their role when logging into the database via *yasql* (default: not enforced; if needed, please contact our technical support). In addition to the existing authentication requirements, login is only permitted if the specified role matches.

## Configuring the Separation of Duties

>**Note**:
>
> It is recommended to plan and confirm in advance whether to use the separation of duties. If needed, enable it during installation/database creation. It is not recommended to frequently switch permission modes. If the separation of duties was not enabled during database creation but is needed later, you should first clean and consolidate the existing users' permission sets before enabling the separation of duties.
