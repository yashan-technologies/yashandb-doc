...
- Object operation privileges: Manage objects such as tables, indexes, views, sequences, synonyms, UDT, stored procedures, triggers, custom libraries, jobs, database links, and materialized views.
...
| --- | --- |
| ALL PRIVILEGES                | Has all system privileges (except SELECT ANY DICTIONARY) |
...
| DEBUG CONNECT SESSION | For compatibility only, no actual meaning |
...
|Privilege Name |Description |
|------------------------------| --------------------------- |
| SELECT ANY DICTIONARY | Has the privilege to query/access any object in the sys schema |
| **Tables**:                        |                                                         |
...
| LOCK ANY TABLE | Has the privilege to lock any table in the database (except sys schema) |
| **Indexes**:                      |                                                         |
...
| DEBUG ANY PROCEDURE | Has permission to debug any procedure or function (except sys schema) |
| **Triggers**:                    |                                                         |
...
| **Jobs**: |  |
| CREATE ANY JOB | Has the privilege to create and manage jobs under any schema (except sys schema), i.e., calling DBMS_SCHEDULER and DBMS_JOB. |
| CREATE JOB | Has the privilege to create jobs in the user's own schema (i.e., calling DBMS_SCHEDULER.CREATE_JOB, DBMS_JOB.SUBMIT) |
| MANAGE SCHEDULER | For compatibility only, no actual meaning |
| **Database Links**:              |                                                         |
