...
| -------------- | ------------------------------------------------------------ |
| DBA            | * When the separation of duties is not enabled: Has nearly all privileges (except SHUTDOWN).<br>* When the separation of duties is enabled: Permissions are restricted — cannot perform DML operations on unprivileged database objects (tables, indexes, etc.), and cannot perform GRANT or REVOKE operations for permission management. For specific permissions, please refer to [Separation of Duties](../Separation of Duties). |
| PDB_DBA            | Exists only when YashanDB is deployed as a CDB (with configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), with CREATE SESSION privilege.   |
...
- **Privilege Set**: Grant ( [GRANT](../../../Development Guide/SQL Reference Manual/SQL Statements/GRANT)) a certain privilege/role to the target role.
...
1. Log in to the database using a user with the corresponding privileges.
    ...
    In a CDB, creating common roles requires connecting to the CDB root, while creating local roles requires connecting to the target PDB. 

2. Execute the [CREATE ROLE](../../../Development Guide/SQL Reference Manual/SQL Statements/CREATE ROLE) statement to create a custom role.

    ```sql
    CREATE ROLE reader;
    ```

3. Execute the [GRANT](../../../Development Guide/SQL Reference Manual/SQL Statements/GRANT) statement to grant the target privilege to the newly created role.
...
1. Log in to the database using a user with the corresponding privileges.
    ...
    In a CDB, deleting common roles requires connecting to the CDB root, while deleting local roles requires connecting to the target PDB. 

2. Execute the [DROP ROLE](../../../Development Guide/SQL Reference Manual/SQL Statements/DROP ROLE) statement to delete a specific custom role.
