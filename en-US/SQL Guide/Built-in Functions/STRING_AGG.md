...
The STRING_AGG function concatenates multiple rows of data, with a separator between them, returning a single row of CLOB type string. This function has similar functionality to the [GROUP_CONCAT](GROUP_CONCAT) function.
...
- General expression [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr)

- A subquery that returns a single row and a single column.

The value of the string must be a character type or another type that can be converted to a character type (LOB types support implicit conversion), but it cannot be of type JSON, NVARCHAR, NCHAR, NCLOB, or time zone types.
