...
The COUNT function counts the number of record rows based on the given parameter [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr). This function returns a BIGINT value and will never return NULL.
...
When the OVER keyword is specified, COUNT will act as a [window function](../../All Manuals/Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/00Built-in Functions (yashan Mode).html#WindowFunction) and supports sliding windows, returning the count of multiple rows.
...
General syntax for KEEP aggregate functions. For more details, see [KEEP aggregate function](../../All Manuals/Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/00Built-in Functions (yashan Mode).html#KeepAggregateFunction).
