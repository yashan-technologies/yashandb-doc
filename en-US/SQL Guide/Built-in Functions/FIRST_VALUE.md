...
FIRST_VALUE is a [window function](../../All Manuals/Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/00Built-in Functions (yashan Mode).html#WindowFunction) and supports sliding windows. This function calculates the window data set based on the given window conditions and returns the value of [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) corresponding to the first row of that set. If the first value is NULL and IGNORE NULLS is not specified, the function returns NULL.
