...
The BIN_TO_NUM function is used to convert binary to decimal. It converts a binary bit vector string represented by one or more [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) combinations into its equivalent numeric value, returning a NUMBER type data.
...
This function supports a maximum of 65535 expr as parameters. However, if the computed value exceeds the range of number representation, an error is reported, and all expr values must be 0/1 or rounded to 0/1; otherwise, an error occurs. The rounding rules are:
