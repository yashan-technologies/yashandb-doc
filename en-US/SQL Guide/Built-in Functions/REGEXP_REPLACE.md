...
REGEXP_REPLACE is a regular expression function used to match the target string `regexp` in the source string `[expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr)` according to the regular pattern, and replaces the `occurrence`-th occurrence of the matched `regexp` substring with the `replace` string, finally returning the modified complete string, with a return value of type VARCHAR. If no match is found, the function returns the source string `expr`.
