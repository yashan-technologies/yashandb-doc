...
The array dimension, that is, the array nesting depth, which is a [general expression](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) that must evaluate to an INT type (an error is raised if it exceeds the INT value range). The calculation rules for this value are as follows:
