...
REGEXP_INSTR is a regular expression function that is used to match the target string regexp in the source string [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) according to the regular pattern, returning the return_opt (start or end) position of the subexpr-th substring matched to regexp at the occurrence-th match. The return value is of BIGINT type. If there is no match, the function returns 0.
