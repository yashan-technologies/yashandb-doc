...
LAST_VALUE is a window function that operates on HEAP tables and supports sliding windows. This function calculates the set of window data based on the specified window conditions and returns the value of [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) corresponding to the last row of that set. If the last value is NULL and IGNORE NULLS is not specified, the function returns NULL.
