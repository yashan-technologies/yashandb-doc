...
The value of [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) can be numeric or a character type that can be converted to a NUMBER type (if the conversion fails, it returns an Invalid number error). For other types, the function returns type not supported.
