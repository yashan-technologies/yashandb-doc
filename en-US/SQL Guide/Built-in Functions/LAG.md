...
LAG is a [window function](../../All Manuals/Development Guide/SQL Reference Manual/Built-in Functions (yashan Mode)/00Built-in Functions (yashan Mode).html#WindowFunction). This function provides access to the row located at a specified physical offset before the current cursor position based on a set of rows returned by the window query, and returns the value corresponding to [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) for that row.
