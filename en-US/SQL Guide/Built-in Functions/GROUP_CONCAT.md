...
The string must be of character type or other types that can be converted to character type, but JSON, NVARCHAR, NCHAR, NCLOB types, and timezone types are not allowed.

If the string is NULL, the function returns NULL.

The string parameter can be:

- [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr)
