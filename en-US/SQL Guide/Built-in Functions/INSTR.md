...
The INSTR function searches for the target string `sub_character` starting from the `position`th character of the source string `expr` [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) and returns the position of the `occurrence`-th occurrence of `sub_character`. The return value is of BIGINT type. If not found, it returns 0.
