...
RLIKE_FILTER is a regular expression function that matches the target string `regexp` against the source string [expr](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/expr) according to the regular pattern and returns the match result. The return value is Boolean type, returning TRUE for a successful match and FALSE otherwise.
