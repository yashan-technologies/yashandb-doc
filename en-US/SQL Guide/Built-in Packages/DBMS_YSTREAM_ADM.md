...
- It is recommended to enable supplemental logging ([database-level](../../All Manuals/Development Guide/SQL参考手册/SQL语句/ALTER DATABASE.html#supplementallogclauses) or [table-level](../../All Manuals/Development Guide/SQL参考手册/SQL语句/ALTER DATABASE.html#addsupplementalloggingclause)) before creating the YStream server. If not enabled, only supplemental information from existing logs can be parsed, and enabling supplemental logging after an existing YStream server will result in an error.
...
| STARTED | Indicates the YStream server has been started, waiting for the [YStream client](../../All Manuals/Development Guide/YStream参考手册/00YStream参考手册) to connect | The instance that starts the YStream server allows:<br/>* [ADD_TABLES](#add_tables): Add parsing table names and schemas<br/>* [STOP](#stop): Stop the YStream server<br/><br/> When the instance that starts the YStream server is offline, other instances in the cluster allow:<br/>* [STOP](#stop): Stop the YStream server<br/>* [TEST](#test): Test if the start_scn of the YStream server is appropriate |
| RUNNING | * Indicates that the [YStream client](../../All Manuals/Development Guide/YStream参考手册/00YStream参考手册) has connected, and the YStream server is running on the current instance<br/>* Indicates that the current instance is executing the [TEST](#test) function | The instance that starts the YStream server allows:<br/>* [ADD_TABLES](#add_tables): Add parsing table names and schemas.However, it is not possible to narrow the scope of parsing objects for a running YStream server whose parsing objects are all tables. (You need to stop the server first and then modify the parsing scope)<br/>* [STOP](#stop): Stop the YStream server<br/><br/> When the instance that starts the YStream server is offline, other instances in the cluster allow:<br/>* [STOP](#stop): Stop the YStream server |
...
- For the status requirements of the target YStream server for this operation, please refer to [Status Introduction](#STATUS).
  ...
  - If you need to add existing tables to the parsing scope of a Ystream service, it is not recommended to perform the operation when the service is in the STARTED or RUNNING state. Otherwise, you must ensure that the [table-level supplemental logging](../../All Manuals/Development Guide/SQL参考手册/SQL语句/ALTER TABLE.html#addsupplementalloggingclause) of the target tables are not enabled before executing ADD_TABLES. And after executing ADD_TABLES, you need to manually enable the table-level supplemental logging of these tables so that the Ystream service can parse this table.
...
The START function is used to start an existing YStream server. Only after the YStream server has been started can the [YStream client](../../All Manuals/Development Guide/YStream参考手册/00YStream参考手册) establish a connection.
