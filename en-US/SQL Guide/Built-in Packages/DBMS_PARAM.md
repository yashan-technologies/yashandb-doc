...
| os_memory_limit | Memory limit percentage, defaults to 100, which allocates all memory. It is recommended to set according to requirements. |
| os_cpu_limit    | CPU limit percentage, default is 100, which allocates all CPU. |
...
| scene           | The scenario for parameter tuning, optional [NORMAL,BATCH,TPCH,AIM], default is NORMAL.<br>When the business primarily involves OLTP with HEAP tables and experiences poor performance in periodic batch scenarios, consider using the BATCH scenario parameters for tuning.<br>This can only be set to TPCH in an ISC Distributed Cluster Deployment with table type LSC, otherwise an error will occur. When using TPCH scenario tuning, all nodes must execute parameter recommendations; otherwise, it may fail during restart due to connection parameter mismatches.<br>When business uses distributed in-memory databases, it can be configured for AIM tuning recommendations |
| scale_factor    | The data scale for test scenarios such as TPCH, default is 100, representing 100G test data. The range is [1, 100000] and is only used to assist in recommending some parameter values when memory is sufficient; in cases of insufficient memory, priority will be given to avoid over-allocation, and this parameter will be automatically downgraded. |
| mms_limit       | The percentage of process memory limit used by the memory table space in the memory database, defaulting to 80%, with the remaining 20% allocated for computational memory allocation. If the mms occupies too much memory, the system will calculate based on the database's minimum available memory of 1.5GB. This parameter takes effect when scene is set to AIM and only applies to DN in distributed memory databases. The valid range is [0, 100]. |
