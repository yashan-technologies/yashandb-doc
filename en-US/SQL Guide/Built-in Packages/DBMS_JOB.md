The DBMS_JOB package provides a set of built-in stored procedures for creating and managing [scheduled jobs](../../All Manuals/Development Guide/PL Reference Manual/PL Objects/Scheduled Jobs). It is important to note that all operations performed on scheduled jobs by invoking these stored procedures only take effect after executing COMMIT.
...
- The instance value can be obtained in YAC Deployment by querying the INSTANCE_NUMBER field of the V$INSTANCE view, while in other deployment modes the instance value is always 1.
