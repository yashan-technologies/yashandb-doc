...
The RUN_LEVEL option is only implemented when using the *yasldr* tool for data import and cannot be specified directly through the LOAD DATA statement. Specific operations can refer to the [User Guide for yasldr](../../All Manuals/Tools Guide/yasldr/User Guide for yasldr) section.
...
-- Log file content
YashanDB Loader Release {version} x86_64 ff1fe7a
...
This statement specifies the directory where files created during splitting will be stored when run_level is set to split. Since SPLIT mode is implemented only in the *yasldr* tool, the specific usage can be found in the [User Guide for yasldr](../../All Manuals/Tools Guide/yasldr/User Guide for yasldr) section.
