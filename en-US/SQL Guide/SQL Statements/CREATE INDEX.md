...
- Only one [ST_Geometry type](../../All Manuals/Development Guide/SQL Reference Manual/Data Types (yashan Mode)/ST_GEOMETRY Type) is allowed in the specified column at a time. Other types are not restricted. If you need to create RTREE indexes for multiple ST_Geometry type columns in the same table, you must execute the statement again and specify different columns.
