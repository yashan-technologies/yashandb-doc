...
- When performing a full database flashback, the database must be in MOUNT state. After flashback is complete, you must [ALTER DATABASE OPEN RESETLOGS](ALTER DATABASE.html#startupclauses) to reset the redo timeline.
...
- The original table name (the ORIGINAL_NAME column in the [DBA_RECYCLEBIN](../../All Manuals/Reference Manual/System Views/DBA Views/DBA_RECYCLEBIN) view)

- The BIN$-prefixed system-generated object name (the OBJECT_NAME column in the [DBA_RECYCLEBIN](../../All Manuals/Reference Manual/System Views/DBA Views/DBA_RECYCLEBIN) view) when the target table entered the recycle bin
