...
For detailed operation descriptions on backup recovery, please refer to [Backup Recovery](../../All Manuals/数据库管理/备份与恢复/00备份与恢复).
...
In YAC/Distributed Cluster Deployment, specifying a [YFS](../../All Manuals/数据库管理/存储管理/集群文件系统管理/00集群文件系统管理) path allows data to be backed up to shared storage; specifying a regular disk path backs data up to the server where the instance resides.
