...
In ISC Distributed Cluster Deployment, users can execute this statement, but it can only back up the node where it is located, so it is not recommended. To back up a distributed cluster, the [yasrman](../../All Manuals/Tools Guide/yasrman/00yasrman) tool should be used.
...
For detailed operation descriptions on backup recovery, please refer to [Backup Recovery](../../All Manuals/Database Administration/Backup and Recovery/00Backup and Recovery).
...
In YAC/Distributed Cluster Deployment, specifying a [YFS](../../All Manuals/Database Administration/Storage Management/YFS Management/00YFS Management) path allows data to be backed up to shared storage; specifying a regular disk path backs data up to the server where the instance resides.
