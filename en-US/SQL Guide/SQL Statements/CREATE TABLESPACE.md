...
In addition to custom tablespace creation on demand, YashanDB has a built-in set of basic tablespaces. For related information, please refer to the [tablespace_clause](CREATE DATABASE.html#tablespaceclause) of CREATE DATABASE or [Tablespace Management](../../All Manuals/Database Administration/Tablespace Management/00Tablespace Management.html#build-in).
...
>
> If a node failure occurs during the execution of this statement in ISC Distributed Cluster Deployment, please refer to [User Tablespace Management](../../All Manuals/Database Administration/Tablespace Management/General Tablespace Management).
...
This statement is used to specify the name of the tablespace to be created. This cannot be omitted and must comply with YashanDB's [object naming conventions](../../All Manuals/Development Guide/SQL Reference Manual/Basic SQL Elements/Identifiers).
...
Specifies the name of the data file. If the name does not include a path, the system uses the default data file path.

In ISC Distributed Cluster Deployment, data file paths are only allowed to use relative paths.

Data file paths in a YAC must be of [YFS](../../All Manuals/YashanDB for Cluster/Yashan File System/00Yashan File System) type.

For data files created in a YAC without specifying a complete YFS path, the default storage location is +DG0/dbfiles.

##### 5.1.2. size

Specifies the size of the data file, adhering to the general [size_clause](../../All Manuals/Development Guide/SQL Reference Manual/General SQL Syntax/size_clause) definition.
