...
- In YAC Deployment, different types of file storage paths have different requirements:
  ...
  - DATAFILE: Must be a [YFS](../../All Manuals/共享集群/集群文件系统/00集群文件系统) path. You can use filename without path (e.g., `yashan`) or the full disk group path (e.g., `+DG0/dbfiles/yashan`). When using filename without path, the actual file creation will take the default disk group path `+DG0/dbfiles`.
