...
In ISC Distributed Cluster Deployment, only the following syntax is supported. It is recommended to use [*yasboot*](../../All Manuals/Tools Guide/yasboot/Introduction to yasboot Command/00Introduction to yasboot Command) to manage the database.
...
```ebnf+diagram
syntax::= ALTER DATABASE (startup_clause|database_file_clauses|logfile_clauses|standby_database_clauses|upgrade_clauses|repair_database_clauses|delete_archivelog_clauses|double_write_file_clauses|supplemental_log_clauses|flashback_database_clauses|add_instance_clauses)
```

**[startup\_clause](#startupclause)::=**
...
MOUNT is the state after the database mounts physical files, while OPEN is the state after the database is opened. For detailed database startup information, please refer to the [Instance Startup and Shutdown](../../All Manuals/Database Administration/Instance Management/Instance Startup and Shutdown).
...
- **READONLY**: The database is opened in read-only mode, restricting the database to read-only operations without generating any redo. 
  ...
  - In Standalone Primary/Standby Deployment, the physical standby database opens in read-only mode by default.
...
After this operation, the tablespace to which the data file belongs will also be offline, and the status of all data files within the tablespace will be recover.
...
> - May cause the standby database to become unusable; subsequent recovery can be done through building the database. Refer to [Repairing Abnormal Standby Database](../../All Manuals/High Availability/​​Standby Repair).
...
This statement is used to switch between the primary and standby databases. For detailed operations of primary and standby databases, refer to the [High Availability](../../All Manuals/High Availability/Overview of YashanDB High Availability).
...
- This statement can only be executed on a standby database or the master instance of a standby cluster. 

    - DATABASE_ROLE=STANDBY in the GV$DATABASE or V$DATABASE view indicates a standby database/standby cluster.

    - INSTANCE_ROLE=MASTER in the GV$INSTANCE view indicates a master instance.

- When YashanDB is deployed as a CDB (configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), if this statement is used when connecting to the standby CDB root, it indicates performing a global switchover, covering both the CDB root and all PDBs. If this statement is used when directly connecting to a specific standby PDB, it indicates performing a PDB-level switchover. 


- Both primary and standby database/clusters must be in OPEN state. For YAC/Distributed Cluster, the primary cluster requires at least the master instance to be in OPEN state (no requirements for other instances).  

- The redo transmission link between primary and standby is normal, and the standby database log synchronization status is normal. The primary database can query the V$ARCHIVE_DEST_STATUS view or the standby database can query the V$REPLICATION_STATUS view to confirm the corresponding information.

> **Note**: 
>
> In YAC/Distributed Cluster Deployment, the performance of switchover is affected by checkpoints, and it will wait for non-instance 1 to complete a full checkpoint before switching.
...
- The current standby database/standby cluster must be in OPEN state. For YAC/Distributed Cluster, the standby cluster requires at least the master instance to be in OPEN state (No special requirements for other instances, just keep them in OPEN state).  
...
The conditions for manually cleaning up archive log files are determined by the ARCHIVELOG_DELETION_POLICY or ARCH_CLEAN_IGNORE_MODE parameters. For specific descriptions of the cleanup conditions, refer to [Archive Management](../../All Manuals/Database Administration/Instance Management/Archive Management).
...
Database-level supplemental logging takes effect on all selected types of user tables, and the effective state can be viewed through the dynamic view [V$DATABASE](../../All Manuals/Reference Manual/System Views/Dynamic Performance Views/V$DATABASE). If the target table has [encryption](CREATE TABLE.html#columnencryptionclause) enabled, the database-level supplemental logging will still display the effective status based on the selected table types, but relevant supplemental information will not be recorded in redo.

For table-level supplemental logging, please refer to [ALTER TABLE](ALTER TABLE.html#addsupplementalloggingclause).

If you wish to use the [YStream server](../Built-in Packages/DBMS_YSTREAM_ADM), you must first enable supplemental logging (either at the library or table level) before creating the YStream server; otherwise, it will result in an error.
...
- In YAC/Distributed Cluster Deployment, full database flashback operations can only be performed on the master instance (INSTANCE_ROLE = MASTER_ROLE in the [GV$INSTANCE](../../All Manuals/Reference Manual/System Views/Dynamic Performance Views/GV$INSTANCE) view).  
...
> **Caution**:
>
> For [YAC instance expansion](../../All Manuals/Installation and Upgrade/Scalability/Scalability of YAC Deployment/Scaling of Cluster Instances), it is recommended to use the *yasboot* tool for a one-click command. **It is not recommended** to execute this statement separately to add database instances.

The execution of this statement must follow the rules below:

- It is applicable only to YAC/Distributed Cluster Deployment.
- It can only be executed on the master instance (i.e., the instance where the INSTANCE_ROLE field in the V$INSTANCE view is MASTER_ROLE), and the master instance must be in OPEN state. If in HA environment, this operation can only be performed on the master instance of the main cluster.
