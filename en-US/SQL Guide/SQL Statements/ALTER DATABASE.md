...
MOUNT is the state after the database mounts physical files, while OPEN is the state after the database is opened. For detailed database startup information, please refer to the [Instance Startup and Shutdown](../../All Manuals/数据库管理/实例管理/实例启停).
...
This statement is used to switch between the primary and standby databases. For detailed operations of primary and standby databases, refer to the [High Availability](../../All Manuals/高可用/YashanDB高可用概述).
...
> - In YAC/Distributed Cluster Deployment, this operation must be executed on the master instance of the standby cluster. Other surviving instances of the standby cluster should remain in OPEN state.
...
The conditions for manually cleaning up archive log files are determined by the ARCHIVELOG_DELETION_POLICY or ARCH_CLEAN_IGNORE_MODE parameters. For specific descriptions of the cleanup conditions, refer to [Archive Management](../../All Manuals/数据库管理/实例管理/归档管理).
...
- In YAC/Distributed Cluster Deployment, full database flashback operations can only be performed on the master instance (INSTANCE_ROLE = MASTER_ROLE in the [GV$INSTANCE](../../All Manuals/参考手册/系统视图/动态视图/GV$INSTANCE) view).  
...
This statement is used to add a new database instance to a YAC. The instance id in the statement can be obtained by viewing the [GV$INSTANCE](../../All Manuals/参考手册/系统视图/动态视图/GV$INSTANCE) view.

> **Caution**:
>
> For [YAC instance expansion](../../All Manuals/安装和升级/扩缩容/共享集群扩缩容/集群实例扩缩容), it is recommended to use the *yasboot* tool for a one-click command. **It is not recommended** to execute this statement separately to delete database instances.
