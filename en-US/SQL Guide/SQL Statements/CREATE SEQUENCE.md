...
This statement is used to specify the name of the sequence number generator to be created. It is mandatory and must comply with YashanDB's [object naming conventions](../../All Manuals/Development Guide/SQL Reference Manual/Basic SQL Elements/Identifiers).
...
Sequence numbers are divided into two types: CURRVAL and NEXTVAL, which are specifically described in the [pseudo-columns](../../All Manuals/Development Guide/SQL Reference Manual/Basic SQL Elements/Pseudo Columns) chapter.
...
- In YAC/Distributed Cluster Deployment, if ORDER is specified, the sequence numbers will remain ordered across all instances; if NOORDER is specified, and the number of pre-allocated sequence numbers is greater than 1, then the sequence numbers will remain ordered within each instance.

***Example*** for YAC/Distributed Cluster Deployment

In a two-instance YAC/Distributed Cluster Deployment, Instance 1 executes:
...
In YAC/Distributed Cluster Deployment, pre-allocation is at the instance level, with each instance pre-allocating a segment of sequence numbers in order; the sequence numbers are ordered within the instance but not globally across the cluster.
