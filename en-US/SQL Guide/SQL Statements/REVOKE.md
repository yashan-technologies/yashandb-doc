...
REVOKE is used to revoke privileges granted to a user or role. The revocable items include [system privilege SYSTEM PRIVILEGE](../../All Manuals/Product Security/Data Access Control/Privilege and Role Management/System Privileges), [object privilege OBJECT PRIVILEGE](../../All Manuals/Product Security/Data Access Control/Privilege and Role Management/Object Privileges), and [role ROLE](../../All Manuals/Product Security/Data Access Control/Privilege and Role Management/Roles).
...
Regular users can only revoke privileges that they have granted. Even in cases of cross-user granting specified with WITH GRANT|ADMIN OPTION, they cannot revoke privileges from other users. Users with the SECURITY_ADMIN security administrator role can revoke any privilege.
...
For specific details on the privilege management system in YashanDB, please refer to [Privilege and Role Management](../../All Manuals/Product Security/Data Access Control/Privilege and Role Management/00Privilege and Role Management).
...
```ebnf+diagram
syntax::= (revoke_system_privilege|revoke_object_privilege|revoke_role) [CONTAINER "=" (CURRENT|ALL)]
```
...
- Regular users with the GRANT ANY PRIVILEGE privilege.

- If an regular user has obtained privilege for transfer via the WITH ADMIN OPTION statement, that user can revoke the privileges they have granted.
...
- Regular users with the GRANT ANY OBJECT PRIVILEGE privilege.

- If an regular user has obtained privilege for transfer via the WITH GRANT OPTION statement, that user can revoke the privileges they have granted.
...
The name of the object; must specify an already created table or [UDT](../../All Manuals/Development Guide/PL Reference Manual/PL Objects/User-Defined Types).
...
- Regular users with the GRANT ANY ROLE privilege.
...
```sql
REVOKE DBA FROM c##sale1 CONTAINER = ALL;
```
