...
- If a failure occurs during a transaction, the thrown exception message is "The transaction must be rerun." When this exception appears, it is necessary to explicitly execute a rollback; otherwise, executing any statement or commit operation within the transaction will result in an error. After rollback, the entire transaction can be re-executed as needed.
- If a failure occurs outside of a transaction, the thrown exception message is "Cannot be safely called repeatedly." When this exception appears, it is possible to choose to re-execute the target statement directly or abandon the execution, neither of which will impact subsequent operations.
