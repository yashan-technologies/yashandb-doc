...
|  | database_name | Database name, mandatory. This parameter is for compatibility only, has no actual meaning, and does not validate the parameter value. |
...
To configure the auto-commit property at the client level, you can either specify the appropriate parameter during connection or call the [setAutoCommit](../../Status of Support for JDBC Interfaces/java.sql.Connection) interface after establishing the connection.
