...
|  | host:port | * host: The network address of the server where the database resides, which can be an IPv4 address, an IPv6 address, or a domain name. In YAC deployment, if [SCAN](../../../Database Administration/Cluster Management/SCAN Management) or [VIP](../../../Database Administration/Cluster Management/VIP Management) has been configured, it can also be the corresponding domain name or IP address.<br>\* port: The listening port of the YashanDB server. If not adjusted during installation, the default is 1688.  <br /><br />If the database and JDBC driver are installed on the same server (i.e., connecting to local host), host can be omitted, for example `jdbc:yasdb://:1688/yasdb` |
|  | database_name | Database name, mandatory. <br> * When connecting to YashanDB deployed as a non-CDB, this parameter is only for compatibility, has no practical business meaning, and the system does not verify its parameter value. <br> * When connecting to YashanDB deployed as a CDB, you must use a JDBC driver of version v1.10.2 or above and specify [the correct target container name](../../../Database Administration/​​Multitenant Management/​​Managing PDBs​​/​View PDBs). For example, the CDB root is cdb$root.   |
...
The connection string can also be used for the following scenarios:

```java
// Scenario 1: Connecting to a local database
jdbc:yasdb://:1688/yashan

// Scenario 2: Connecting to YAC with SCAN enabled
jdbc:yasdb://scan.example.com:1688/yashan

// Scenario 3: Connecting to YAC with VIP enabled
...
| --------------- | ------------------------------------------------------------ |
| connectTimeout  | Timeout for creating socket connection when establishing connection, in seconds. Default is 10 seconds when not configured, configured via info attribute or URL. |
| socketTimeout   | Timeout for client waiting for server return data during TCP communication after obtaining connection, in seconds. Default is 0 seconds (waiting indefinitely) when not configured, configured via info attribute or URL. |
| loginTimeout    | Timeout for client waiting for server return data during login authentication after socket is successfully created, in seconds. Default is 300 seconds when not configured, configured via info attribute or URL. |
| SQL statement execution timeout | Configured via the Statement interface setQueryTimeout. Default has no timeout (waiting indefinitely) when not configured. |
...
|Parameter |Description |
| --------- | ---------------------- |
| serverMode           | Specify the connection mode. The available values are as follows:<br />\* shared: The default value, indicating the connection to shared server mode, allowing client connections to use a shared thread pool (actual use of a shared thread pool depends on relevant YashanDB server configuration parameter [MAX_REACTOR_CHANNELS](../../../Reference Manual/Configuration Parameters.md#max_reactor_channels)).<br />\* dedicated: Indicates the connection to the dedicated server mode, meaning each client connection request must allocate a separate thread (not affected by relevant YashanDB server configurations). |
...
- YashanDB in mysql mode: Whether transactions are automatically committed depends on the server-side configuration, controlled by the [system variable AUTOCOMMIT](../../../Reference Manual of mysql Mode/System Variables/List of System Variables In mysql Mode). If the server is not configured to auto-commit but the client is, the final behavior will still be non-auto-commit. To avoid discrepancies between actual behavior and expectations or partial configurations, it is recommended that the auto-commit settings on both the server and client sides be kept consistent.

To configure the auto-commit property at the client level, you can either specify the appropriate parameter during connection or call the [setAutoCommit](../YashanDB JDBC Reference/Status of Support for JDBC Interfaces/java.sql.Connection) interface after establishing the connection.
