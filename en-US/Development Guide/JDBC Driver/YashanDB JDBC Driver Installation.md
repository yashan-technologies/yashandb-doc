...
## Install YashanDB JDBC Driver via Maven

Configure the yashandb-jdbc library using the Maven manager in your project.

The yashandb-jdbc is published on [The Maven](https://central.sonatype.com/search?q=yashandb-jdbc&smo=true) and has the following Group ID and artifactId:
...
## Offline Installation of YashanDB JDBC Driver

1. Refer to the [YashanDB Software Package List](../../Installation and Upgrade/Installation and Deployment/Pre-Installation Preparation/Downloading Software Packages) to obtain the YashanDB JDBC driver installation package yasdb-jdbc-*version*.jar, for example, yasdb-jdbc-{version}.jar.

2. Download the jar file to a local path, e.g., /path/yasdb-jdbc-{version}.jar.

3. Add the YashanDB JDBC driver to the JVM environment:

    - **For Java applications running from the command line** (using Linux as an example)

    ```shell
    # Pre-adding method
    export CLASSPATH=/path/yasdb-jdbc-{version}.jar:${CLASSPATH} 

    # Adding at JVM runtime, Jdbcexample is the application name
    java -cp .:/path/yasdb-jdbc-{version}.jar Jdbcexample
    ```

    - **For Java applications running in an IDE**

    Each IDE product will provide an interface for configuring external libraries. Follow the prompts to add the YashanDB JDBC driver jar file to the project. For instance, in IDEA:
