...
2. Download the YashanDB client installation package and unzip it to a local path, for example, `D:\yasdb-driver-c`.

   After unzipping the installation package, the following folders can be obtained:

   - bin: Executable files for the C driver; there will be no bin folder if the installation package corresponds to a 32-bit system.
...
Set `D:\yasdb-driver-c\lib` in the Windows environment variable PATH. The specific operations are:
...
## Step 3: Download the ODBC Driver Installation Package

1. Obtain the YashanDB ODBC driver installation package according to the installation manual [YashanDB Software Package List](../../../../Installation and Upgrade/Installation and Deployment/Pre-Installation Preparation/Downloading Software Packages).
...
## Step 4 (Optional): Check Dependencies (32-bit)
...
1. Check if 32-bit versions of ucrtbased.dll and vcruntime140d.dll are included in the `C:\Windows\SysWOW64` folder.
   ...
   - If present, proceed directly to [Configure Data Source](#datasource).
   
   - If not, the ucrtbased.dll and vcruntime140d.dll files need to be obtained and saved to the `C:\Windows\SysWOW64` folder.

2. Obtain the above dynamic link library files yourself, for example, by installing Visual Studio.
...
2. Fill in the data source information.
   ...
   - Url: The complete URL of the data source; please refer to the [C Driver yacConnect function](../../C Driver/C Driver Interfaces/Connection & Authorization & Initialization Functions/yacConnect) for the url parameter. After configuring the URL parameter, the Server and Port parameters will no longer be effective (optional).
