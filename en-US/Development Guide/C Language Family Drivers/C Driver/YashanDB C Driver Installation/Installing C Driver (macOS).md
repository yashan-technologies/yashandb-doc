This chapter uses macOS 26.4.1 on Apple Silicon architecture, cmake 3.16 or above, and the YashanDB client installation package yashandb-client-xx.xx-darwin-arm64.tar.gz as an example to introduce the installation and configuration process of the YashanDB C driver in this environment.

## Environment Requirements

The YashanDB C driver supports macOS on Apple Silicon architecture.

## Step 1: Download the C Driver Installation Package

1. From the [YashanDB Official Website Download Center](https://download.yashandb.com/download), or contact our technical support to obtain the corresponding software package. 

2. Download and extract the YashanDB client installation package to a local path, for example, /Users/yashandb/yasdb-driver-c/.

   After extracting the installation package, the files required for the C driver can be obtained:

   * Header files for C driver: Located in the include folder.

   * Library files for C driver: Located in the lib folder.

## Step 2: Set Up Dynamic Library Dependency Path

1. Edit the bashrc file:

   ```shell
   $ vi ~/.zshrc
   ```

2. Add a new line to increase the LD_LIBRARY_PATH search path pointing to the folder where the C driver's library files are located:

   ```shell
   export DYLD_LIBRARY_PATH=$DYLD_LIBRARY_PATH:/Users/yashandb/yasdb-driver-c/lib
   ```

3. Save and exit.

4. Refresh the system variable configuration.

   ```shell
   $ source ~/.zshrc
   ```

At this point, the YashanDB C driver has been successfully installed. For character set-related configuration adjustments, please refer to [Character Set Configuration](../../../../Database Administration/Instance Management/Database Character Set Configuration).

## Step 3 (Optional): Create a C/C++ Language Project

The following example project depends on the cmake environment and requires the cmake component to be installed in advance.

1. Create a project folder yacli_c_project in any path.

2. Create a file CMakeLists.txt in yacli_c_project.

3. Modify the CMakeLists.txt file to contain the following content and save:

   ```cmake
   cmake_minimum_required(VERSION 3.16)
   project(example)
   
   set(CMAKE_CXX_STANDARD 11)
   set(CMAKE_CXX_STANDARD_REQUIRED ON)
   set(BUILD_USE_64BITS on)
   set(CMAKE_CONFIGURATION_TYPES "Debug" CACHE STRING "" FORCE)
   set(CMAKE_BUILD_TYPE "Debug" CACHE STRING "" FORCE)
   
   # set src code
   SET(LIBHELLO_SRC 
   ./cexample.c
   )

   add_executable(yasctest ${LIBHELLO_SRC})
   target_link_libraries(yasctest /Users/yashandb/yasdb-driver-c/lib/libyascli.so)
   target_include_directories(yasctest PUBLIC /Users/yashandb/yasdb-driver-c/include)
   ```

4. Copy the cexample.c from the [YashanDB C Driver Usage Examples](../YashanDB C Driver Usage Examples/00YashanDB C Driver Usage Examples) to the current project path.

   The directory structure of yacli_c_project is as follows:
   
   * CMakeLists.txt

   * cexample.c

5. Generate the cmake project in the current path:

   ```shell
   cmake .
   ```

6. Compile in the current path:

   ```shell
   make
   ```

7. Run yasctest:

   ```shell
   ./yasctest
   ```
