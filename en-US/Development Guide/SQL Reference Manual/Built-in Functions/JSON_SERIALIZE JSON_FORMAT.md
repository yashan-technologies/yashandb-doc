...
The JSON_SERIALIZE function converts binary [json](../General SQL Syntax/json) data into a string in either pretty or compact format, with the length of the serialized string not exceeding the limit defined by the `returning_clause`.
...
The EXTENDED keyword is used to set the output of JSON data in extended mode, see [JSON extended format output](../General SQL Syntax/json).
