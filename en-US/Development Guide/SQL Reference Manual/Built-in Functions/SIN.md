...
The SIN function returns the sine value of the given parameter, where the parameter is an angle expressed in radians, and its magnitude is unrestricted (limited only by the range defined by its data type).

The rules of this function are as follows:

- The value of [expr](../General SQL Syntax/expr) can be numeric or a character type that can be converted to NUMBER type (a conversion failure results in an Invalid number error). For other types, the function returns type not supported.
