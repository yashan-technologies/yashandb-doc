...
REGEXP_LIKE is a regular expression function that matches the target string `regexp` against the source string [expr](../General SQL Syntax/expr) according to the regular pattern and returns the match result. The return value is of Boolean type, returning TRUE when the match is successful, otherwise returning FALSE.
