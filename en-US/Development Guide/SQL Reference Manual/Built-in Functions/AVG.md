...
The value of [expr](../General SQL Syntax/expr) must be numeric or convertible to NUMBER type (failure to convert returns an Invalid number error). Other types are not supported by the function return type.
...
When the OVER keyword is specified, AVG functions as a [window function](00Built-in Functions.md#WindowFunction) and supports sliding windows, returning the average values of multiple rows.

**analytic\_clause**

General syntax for window functions, for details see [window function](00Built-in Functions.md#WindowFunction).
...
The general syntax for KEEP aggregate functions can be found in [KEEP aggregate function](00Built-in Functions.md#KeepAggregateFunction).
