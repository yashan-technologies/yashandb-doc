```ebnf
listagg = LISTAGG "(" [DISTINCT|UNIQUE|ALL] expr ["," separator] [listagg_overflow_clause] ")"
...
When the OVER keyword is specified, LISTAGG acts as a [window function](00Built-in Functions.md#WindowFunction), concatenating rows within each group window, where the concatenation result of each row in the group window is the same.
