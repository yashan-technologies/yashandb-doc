...
LAG is a [window function](00Built-in Functions.md#WindowFunction). This function provides access to the row located at a specified physical offset before the current cursor position based on a set of rows returned by the window query, and returns the value corresponding to [expr](../General SQL Syntax/expr) for that row.
