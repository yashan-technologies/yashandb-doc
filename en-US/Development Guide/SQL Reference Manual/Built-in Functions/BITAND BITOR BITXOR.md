...
The values of [expr](../General SQL Syntax/expr)1 and [expr](../General SQL Syntax/expr)2 can be of types TINYINT, SMALLINT, INT, BIGINT, NUMBER, TIME, or character data that can be converted to NUMBER type (a conversion failure will return a type conversion error). For other types, a type not supported error will be returned.
...
SQL> select bitand(time('00:00:00'),1) from dual;
