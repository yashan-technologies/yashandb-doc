...
- YDBC_BUFFER_SIZE
    ...
    The memory pool size driven by the yex_server sandbox process. The default value is 64M, and the value must be a positive integer or 0 with a valid range of [32M,1T]. If the value is adjusted to below the minimum of 32M, no error will be reported, but the actual effect still uses the minimum value. 
