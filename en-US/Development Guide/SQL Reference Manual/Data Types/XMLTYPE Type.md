...
- For [XML data processing functions](../Built-in Functions/XML Function/00XML Function), methods, and advanced packages, if not specially stated, accessing external resources is not supported by default.

### Usage Method

The XMLTYPE type can be generated using the OBJECT method or [XMLPARSE](../Built-in Functions/XML Function/XMLPARSE) function, and can be queried using the syntax `table_alias.column_name.attribute_name`.
...
existsNode() checks if a specified node exists in XML data. Returns `1` if exists, `0` otherwise. `xml.existsNode()` is equivalent to the [EXISTSNODE function](../Built-in Functions/XML Function/EXISTSNODE).
...
extract() retrieves an XML fragment based on an XPath, concatenating all matched nodes. Returns XMLTYPE. `xml.extract()` is equivalent to the [XMLEXTRACT function](../Built-in Functions/XML Function/XMLEXTRACT).
