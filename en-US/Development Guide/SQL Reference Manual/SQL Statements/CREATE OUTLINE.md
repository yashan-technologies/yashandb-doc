...
```ebnf
= CREATE [OR REPLACE] [PUBLIC] OUTLINE [outline_name] (([FROM [PUBLIC] source_outline] [FOR CATEGORY category_name] [ON statement]) | ON sql_id USING HINT hint).
```
...
category\_name can either be an existing category name or a newly specified category name. category\_name cannot include schema names and must comply with YashanDB's [object naming conventions](../Basic SQL Elements/Identifiers).

For the same SQL statement, it cannot be specified multiple times in the same category.

### ON statement

Specifies the SQL statement corresponding to the OUTLINE. The SQL statement must be a SELECT statement that adheres to the corresponding syntax requirements.
