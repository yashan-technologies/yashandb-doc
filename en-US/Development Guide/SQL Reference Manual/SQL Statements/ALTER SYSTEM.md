...
This statement is used to terminate a session and roll back any uncommitted transactions in that session, releasing the locks generated by that session. Afterward, running SQL statements in that session will prompt a connection error message.

KILL SESSION supports the following three syntax forms.

#### Terminate a Single Session

Terminate the specified session using SID and SERIAL# parameters. Executing this command requires that the session SID and SERIAL# be provided.
...
#### Batch Terminate Sessions (WITH TRANSACTION)

Terminate all sessions, including sessions that are executing transactions but not including internal connections (for example, sessions established by YStream clients).  This statement will wait for all sessions with active transactions to complete before returning the execution result.

> **Note**:
>
> Executing this statement requires DBA privileges.

**count**

Can optionally specify the maximum number of sessions to terminate. If omitted, all sessions that meet the criteria will be terminated by default.

***Example*** for Standalone/YAC/Distributed Cluster Deployment

```sql
-- Terminate all sessions that meet the criteria
ALTER SYSTEM KILL SESSION WITH TRANSACTION;

-- Terminate up to 10 sessions
ALTER SYSTEM KILL SESSION WITH TRANSACTION 10;
```

#### Batch Terminate Sessions (WITHOUT TRANSACTION)

Terminate sessions that currently have no active transactions. Sessions with active transactions and internal connections (for example, sessions established by YStream clients) will be skipped directly.
...
-- Terminate up to 5 sessions without active transactions
...
This statement is used to swapout all Temp Blocks in the Buffer Cache of the database. speeding up the recycle of Temp Blocks.
