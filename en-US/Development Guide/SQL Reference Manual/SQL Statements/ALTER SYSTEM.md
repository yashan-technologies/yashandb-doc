...
This statement is used to modify the database's [configuration parameters](../../../Reference Manual/Configuration Parameters). 

ALTER SYSTEM SET modifies configuration parameters and prints descriptive information of the changes to the RUN LOG. 

In ISC Distributed Cluster Deployment, it is still recommended to use the YCM graphical interface or the [yasboot](../../../Tools Guide/yasboot/Introduction to yasboot Command/yasboot config) command line to modify configuration parameters.
...
This statement is only applicable to CDBs (with configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), used to specify the container scope for the current operation, which can be set to take effect globally or locally within the current container. It can be omitted.
...
- CURRENT: Default value, indicates configuring the corresponding parameters for the currently connected database, i.e., local configuration. If specified as CURRENT when connecting to the CDB root, it may still be global configuration. Its effectiveness rule is to first modify the parameter value of the CDB root, then distribute it to all PDBs in the CDB for inheritance.

- ALL: When connecting to the CDB root, it indicates forcibly modifying the parameters of the CDB root and all its PDBs, restoring the inheritance relationship between PDB and the CDB root. When connecting to PDB operations, it has no special meaning and is equivalent to CURRENT.

- A PDB name: Indicates configuring the corresponding parameters for that PDB, which is equivalent to directly connecting to that PDB for local configuration. If connecting to a PDB for operations, only its own name can be specified.

***Example*** for Standalone/YAC/Distributed Cluster Deployment

```sql
-- CDB global parameters, must connect to the CDB root for modification
...
-- Common parameters, assuming connection to the CDB root for global modification
...
-- Both the CDB root and PDBs with inheritance relationship will take effect
SHOW PARAMETER checkpoint_timeout;
NAME                  VALUE   
--------------------- ------- 
...
ALTER SYSTEM SET data_buffer_size=128M scope=both;
...
> **Note**: 
>
> - TYPE cannot be specified in NOMOUNT/MOUNT mode; it defaults to modifying the configuration parameters of the local node.
>
> - For nodes not in OPEN status, even if they fall within the TYPE specified range, their configuration parameters cannot be modified across nodes.
>
> - The above description pertains to the rules when executing from a connected CN node; if connected directly to other nodes, TYPE cannot be specified and defaults to modifying the configuration parameters of the local node.
...
Specify the increment for the Data Buffer, which must be an integer value. The unit can be B/K/M/G/T/P/E. The size limit set by this statement is constrained by the following two values:
...
- STOP: Sets the connection listener into maintenance mode. In this mode, it maintains existing connections but blocks new connections, permitting only local UDS connections to access the database until it is switched back to normal operation mode or the database is restarted.
