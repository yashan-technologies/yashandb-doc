...
In an ISC Distributed Cluster Deployment, when executing this statement, the system checks for any abnormal unrecoverable DDL statements. If such statements exist, an error will be reported, and this statement can only be executed after the corresponding DDL has been successfully recovered.
...
```ebnf
= DROP TABLESPACE SET tablespace_set_name INCLUDING CONTENTS.
```
