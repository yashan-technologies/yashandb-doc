...
*  [query_containers](#querycontainers)：containers() operator, can aggregate queries on tables, views, etc., only applicable to CDBs. Using this operator on the CDB root can aggregate data related to homonymous query objects from all PDBs, i.e., execute global queries. Using this operator on PDB always queries local data, equivalent to not using this operator. 
...
In the scenario of vector data querying, specify whether the current query is an exact query or an approximate query for vector values.

- EXACT: Exact query, which is the default when not specified.
- APPROX: Approximate query, which calculates and returns the vector most similar to the input vector based on the set distance function.
- APPROXIMATE: Synonym for APPROX.
