...
In ISC Distributed Cluster Deployment, operations related to [global dynamic views](../../../Reference Manual/System Views/Dynamic Performance Views/00Dynamic Performance Views) are executed locally at each node and are finally aggregated to CN.
...
```ebnf
= ((query_name|subquery)[sample_clause][flashback_query_clause][t_alias][pivot_clause|unpivot_clause]) {"," ((query_name|subquery) [sample_clause][flashback_query_clause][t_alias][pivot_clause|unpivot_clause])}.
```
...
Columns obtained from the [pivot_clause](#pivotclause) cannot be used as projections in the recursive part of a recursive CTE.
...
-- Using PIVOT for row-column transformation in the WITH statement
...
DISTINCT is completely equivalent to UNIQUE.
...
* [expr](../General SQL Syntax/expr): An expression
* CASE statement
* `udt_expr`: For Object UDT columns, the format of `udt_expr` is `table_alias.attribute_name`; for Varray UDT and Nested Table UDT columns, the format is `table_alias.*`. For detailed usage methods and examples, see [User UDT](../Data Types/User-Defined Types).
...
Can be used to query Varray UDT or Nested Table UDT data, for details refer to [User UDT](../Data Types/User-Defined Types). 
...
The table() can be used for queries, for specific descriptions of table functions, refer to  [Built-in Table Functions](../Built-in Functions/00Built-in Functions.md#tableFunc).
...
This statement is used to specify the [condition](../General SQL Syntax/condition) for the query.
...
When the sorting key is a [subquery](../Basic SQL Elements/Subquery), the following rules must be followed:

- It must be a scalar subquery, meaning a subquery that returns a single row and a single column (scalar value).

- When querying the LSC and TAC tables, the sorting key must be a non-correlated subquery.
...
When executing an `ORDER BY` SQL statement, if an index is created on the sorting column(s), the database can leverage an index scan to avoid explicit sorting, thereby improving query performance.

- In yashan mode, NULL values are placed at the end by default, and using NULLS LAST avoids the need for explicit sorting;
- In mysql mode, NULL values are placed at the end by default, and using NULLS FIRST avoids the need for explicit sorting;
...
If the query is performed on a view or the query statement contains nested subqueries, `FOR UPDATE` has the following scenario restrictions on the parent query, view, and subqueries:

- Parent and subqueries cannot both be multi-table join queries at the same time;

- When a subquery is a multi-table join, the parent query cannot contain GROUP BY, aggregate functions, or OFFSET clauses;

- The parent query does not support `FOR UPDATE` in the following scenarios:
...
- Views and subqueries do not support `FOR UPDATE` in the following scenarios:
  ...
  - DISTINCT
  - FULL JOIN
  - Queries on DBLINK tables
  - References to parent query fields
  - ROWNUM
  - Window functions
  - CONNECT BY
  - Pivot/unpivot expressions
  - Projection columns containing subquery expressions
...
The column\_name must identify an actual column name and cannot be an alias.
