...
```ebnf
= FLASHBACK (database_clauses|table_clauses).
```

**[database\_clauses](#databaseclauses)::=**

```ebnf
= DATABASE (TO (SCN scn|TIME timestamp|RESTORE POINT restore_point_name)).
```

**[table\_clauses](#tableclauses)::=**

```ebnf
= TABLE table_name (TO (SCN scn|TIMESTAMP timestamp| BEFORE (DROP [RENAME TO new_name]|TRUNCATE)) | (PARITITION|SUBPARTITION) part_name TO BEFORE TRUNCATE).
```

<span id="databaseclauses" name="databaseclauses"></span>

### database\_clauses

This statement is used to perform database flashback.
...
- Ensure that the [database flashback](ALTER DATABASE.md#flashbackdatabaseclauses) functionality is enabled.

- Currently, file-related DDL (such as tablespace DDL, data file DDL, etc.) and NOLOGGING objects cannot be flashed back.
- Operations on archived log files and redo log files cannot be flashed back.
...
- When performing database flashback, the database must be in the MOUNT stage. After the flashback is completed, corresponding post-operations must be carried out. For detailed database flashback operations, please refer to [Database Flashback](../../../Database Administration/Fault Handling/Flashback/Database Flashback/00Database Flashback).

    - If the flashback is performed in a non-primary-standby HA deployment, [ALTER DATABASE OPEN RESETLOGS](./ALTER DATABASE.md#startupclauses) must be executed to reset the redo timeline.

    - If flashback is performed on the primary database/primary cluster in primary-standby ha deployment, [ALTER DATABASE OPEN RESETLOGS](./ALTER DATABASE.md#startupclauses) must be executed to reset the redo timeline of the primary database/primary cluster. Also, flashback or rebuild of the standby database/standby cluster should be carried out to make it catch up with the primary database. 

    - If flashback is performed on the standby database/standby cluster in primary-standby ha deployment, the redo log apply will be automatically stopped, and you need to manually [enable the apply](./ALTER DATABASE.md#start_apply).
...
<span id="tableclauses" name="tableclauses"></span>
...
- FLASHBACK TABLE TO SCN is not allowed in scenarios where DDL that changes the table structure occurred from the specified SCN point to the present, such as:
...
The table name for Flashback recovery can be specified as:
