...
```ebnf
= enable_xfmr_clause|mcol_ttl_clause|mcol_ability_clause|force_xfmr_clause.
```
...
**[inline_constraint](../General SQL Syntax/constraint)** 
...
```ebnf
= MODIFY (column_name (dataType|DEFAULT default_expr|identity_clause|inline_constraint) 
...
```ebnf
= ADD (((PARTITION [partname] add_range_partition_clause)
...
```ebnf
= DROP PARTITION (partname {"," partname}) [update_index_clause].
```
...
```ebnf
= DROP SUBPARTITION (subpartname {"," subpartname}) [update_index_clause].
```
...
~~~ebnf
= MODIFY (PARTITION | SUBPARTITION) partname (shrink_space_clause | add_subpartition_clause).
~~~

**[split\_table\_partition](#splittablepartition)::=**

```ebnf
= SPLIT PARTITION partname
...
```ebnf
= MERGE PARTITIONS partition_or_key_value ((partition_or_key_value) {"," (partition_or_key_value)} | "TO" partition_or_key_value) [INTO PARTITION [partname] [table_partition_description]] [UPDATE [GLOBAL] INDEXES].
```
...
```ebnf
= MERGE SUBPARTITIONS subpartition_or_key_value ((subpartition_or_key_value) {"," (subpartition_or_key_value)} | "TO" subpartition_or_key_value) [INTO SUBPARTITION [partname] [table_partition_description]] [UPDATE [GLOBAL] INDEXES].
```
...
```ebnf
= EXCHANGE (PARTITION | SUBPARTITION) (part | subpart | FOR '(' (partition_key_value) {"," (partition_key_value)} ')' ) WITH TABLE [schema'.'] table_name [ (INCLUDING | EXCLUDING) INDEXES ] [ (WITH | WITHOUT)  VALIDATION] [UPDATE [GLOBAL] INDEXES] [CASCADE].
```
...
```ebnf
= ADD "(" out_of_line_constraint {"," out_of_line_constraint} ")" . 
```

**[out\_of\_line\_constraint](../General SQL Syntax/constraint)** 
...
```ebnf
= RECLAIM SEGMENT [PRIMARY_SPACE tablespace_name PARTITION partition_name] TABLESPACE tablespace_name  [OBJNO_REUSE object_id SEG_FILE file_id SEG_BLOCK block_id] [COLSEGS "(" DATAOBJ data_objectid TYPE type ID id ENTRY entry FLAG flag ")"] 
...
```ebnf
= MODIFY identity_column_name [dataType] (identity_clause |(DROP IDENTITY)).
```

<span id="altertableproperties" name="altertableproperties"></span>

### alter\_table\_properties

This statement is used to modify a series of properties of a table.

<span id="RENAMETO" name="RENAMETO"></span>

#### RENAME TO

This statement is used to modify the table name. The specified new name cannot be empty and must comply with YashanDB's [object naming conventions](../Basic SQL Elements/Identifiers).
...
<span id="supplementaltablelogging" name="supplementaltablelogging"></span>

#### supplemental\_table\_logging

This statement is used to enable or disable supplemental logging properties at the table level. Table-level supplemental logging only applies to the current table. For database-level supplemental logging, refer to [ALTER DATABASE](ALTER DATABASE.md#supplementallogclauses).

In ISC Distributed Cluster Deployment, users cannot perform this operation.

To use the [YStream server](../../PL Reference Manual/Built-in Advanced PL Packages/DBMS_YSTREAM_ADM), supplemental logging (either at the database level or table level) must be enabled before creating the YStream server. If the YStream server is created first and then supplemental logging is enabled, an error will occur.

<span id="addsupplementalloggingclause" name="addsupplementalloggingclause"></span>

##### add\_supplemental\_logging\_clause
...
The current enabled supplemental logging information can be viewed through the views [dba\_log\_groups](../../../Reference Manual/System Views/DBA Views/DBA_LOG_GROUPS), [all\_log\_groups](../../../Reference Manual/System Views/ALL Views/ALL_Views), and [user_log_groups](../../../Reference Manual/System Views/USER Views).
...
<span id="dropsupplementalloggingclause" name="dropsupplementalloggingclause"></span>

##### drop\_supplemental\_logging\_clause
...
<span id="shrinkspaceclause" name="shrinkspaceclause"></span>

#### shrink\_space\_clause
...
<span id="lscpropertiesclause" name="lscpropertiesclause"></span>

#### lsc\_properties\_clause
...
<span id="enablexfmrclause" name="enablexfmrclause"></span>
...
<span id="forcexfmrclause" name="forcexfmrclause"></span>
...
This statement is used to modify the parallel property of the table, which specifies the degree of parallelism when querying the table. The syntax is the same as the [parallel_clause](CREATE TABLE.md#parallelclause) in CREATE TABLE.
...
<span id="altercolumnclause" name="altercolumnclause"></span>

### alter\_column\_clause

This statement specifies operations on column fields of the table.

<span id="addcolumnclause" name="addcolumnclause"></span>
...
This specifies the data type of the column field. Refer to YashanDB's [data type descriptions](../Data Types/00Data Types).
...
- For newly added LSC table columns, the default value cannot be specified if the column is of LOB type; for other types, the default value cannot be defined as a [Sequence pseudo-column](../Basic SQL Elements/Pseudo Columns).
...
This clause is used to define inline constraints when adding a column field. Refer to general SQL syntax [constraints](../General SQL Syntax/constraint) for detailed descriptions.
...
###### column\_encryption\_clause

This clause is used to define encryption attributes for the column being added, following the same rules as [CREATE TABLE](CREATE TABLE.md#columnencryptionclause).
...
<span id="renamecolumnclause" name="renamecolumnclause"></span>

#### rename\_column\_clause

This statement is used to rename a column field. The new specified name cannot be empty and must comply with YashanDB's [object naming conventions](../Basic SQL Elements/Identifiers).
...
<span id="modifycolumnclause" name="modifycolumnclause"></span>
...
Modifying the attributes of columns where [full-text index](CREATE INDEX.md#SEARCH) is located is not allowed.

##### dataType

This modifies the data type of the column field to one recognized by YashanDB [data types](../Data Types/00Data Types).
...
- **Other non-empty columns with original data types of character types**: Modifications to different character types are allowed, but still must follow these rules:
    ...
    - HEAP tables do not allow modification of data types across storage modes. For example, in HEAP tables, it is not allowed to modify the column data type from VARCHAR(3200) (with standard string storage) to VARCHAR(8004) (which will automatically convert to LOB storage). TAC tables, on the other hand, allow such operations. For an introduction to storage methods, refer to [character types](../Data Types/Character Types).
...
SQL> INSERT INTO area VALUES ('01','EastChina','Shanghai','');

1 row affected.
SQL> INSERT INTO area VALUES ('02','WestChina','Chengdu','');

1 row affected.
SQL> INSERT INTO area VALUES ('03','SouthChina','Guangzhou','');

1 row affected.
SQL> INSERT INTO area VALUES ('04','NorthChina','Beijing','');

1 row affected.
SQL> INSERT INTO area VALUES ('05','CentralChina','Wuhan','');
...
SQL> ALTER TABLE area MODIFY owner DEFAULT 'zq';

Succeed.
SQL> INSERT INTO area VALUES ('06','CentralChina','Changsha','cc');

1 row affected.
SQL> INSERT INTO area VALUES ('07','CentralChina','Hengyang','');

1 row affected.
SQL> INSERT INTO area VALUES ('08','CentralChina','Ezhou',default);
...
SQL> ALTER TABLE area MODIFY owner DEFAULT null;

Succeed.

SQL> INSERT INTO area VALUES ('10','CentralChina','Enshi','');
...
This modifies the constraints of a column field, following the syntax rules defined in general [constraints](../General SQL Syntax/constraint).
...
- Cannot execute modify PRIMARY KEY on column fields that do not meet the requirements for creating primary keys. For the requirements for creating primary keys, refer to [constraints](../General SQL Syntax/constraint).
...
- Cannot execute modify FOREIGN KEY on column fields that do not meet the requirements for creating foreign keys; for the requirements for creating foreign keys, refer to [constraints](../General SQL Syntax/constraint).
...
<span id="altertablepartition" name="altertablepartition"></span>
...
<span id="addtablepartition" name="addtablepartition"></span>
...
<span id="addrangepartitionclause" name="addrangepartitionclause"></span>
...
<span id="addlistpartitionclause" name="addlistpartitionclause"></span>
...
<span id="addhashpartitionclause" name="addhashpartitionclause"></span>
...
<span id="droptablepartition" name="droptablepartition"></span>
...
<span id="droptablesubpartition" name="droptablesubpartition"></span>
...
<span id="truncatetablepartition" name="truncatetablepartition"></span>
...
<span id="truncatepartclause" name="truncatepartclause"></span>
...
<span id="truncatetablesubpartition" name="truncatetablesubpartition"></span>

#### truncate\_table\_subpartition
...
<span id="setpartitionclause" name="setpartitionclause"></span>

#### set\_partition\_clause
...
<span id="modifypartitionclause" name="modifypartitionclause"></span>
...
<span id="addsubpartitionclause" name="addsubpartitionclause"></span>

##### add\_subpartition_clause
...
<span id="splittablepartition" name="splittablepartition"></span>
...
You can specify whether to update indexes when splitting a partition. This can be omitted; if omitted, it means the indexes will not be updated during the split.

- Specifying UPDATE INDEXES means to update both local partition indexes and global indexes when splitting a partition.

- Specifying UPDATE GLOBAL INDEXES means to update global indexes when splitting a partition to keep them valid. If not specified, global partitions may become invalid:

    - If the original partition has no data, or the data in the original partition can be completely divided into one new partition, then splitting the partition will not change the status of the global index.

    - If the original partition has data but cannot be completely divided into one new partition, the global index will become invalid after splitting the partition and needs to be manually [rebuilt](./ALTER INDEX.md#rebuildclause) to restore it.
...
<span id="mergetablepartitions" name="mergetablepartitions"></span>
...
<span id="partitionorkeyvalue" name="partitionorkeyvalue"></span>
...
You can specify whether to update indexes when merging partitions. This can be omitted; if omitted, it means the indexes will not be updated during the merge.

- Specifying UPDATE INDEXES means to update both local partition indexes and global indexes when merging partitions.

- Specifying UPDATE GLOBAL INDEXES means to update global indexes when merging partitions to keep them valid. If not specified, global partitions may become invalid:

    - If the original partition has data, the global index will become invalid after merging partitions and needs to be manually [rebuilt](./ALTER INDEX.md#rebuildclause) to restore it.

    - If the original partition has no data, merging partitions will not change the status of the global index.
...
<span id="mergetablesubpartitions" name="mergetablesubpartitions"></span>
...
<span id="subpartitionorkeyvalue" name="subpartitionorkeyvalue"></span>
...
<span id="exchangeclause" name="exchangeclause"></span>
...
<span id="altertableconstraint" name="altertableconstraint"></span>

### alter\_table\_constraint
...
<span id="addconstraint" name="addconstraint"></span>
...
Constraint item, and for a detailed description of the constraint item, please refer to the general SQL syntax [constraint](../General SQL Syntax/constraint).
...
<span id="dropconstraint" name="dropconstraint"></span>
...
<span id="modifyconstraint" name="modifyconstrain"></span>
...
Enable or disable the specified constraint (see the description of ENABLE|DISABLE in [constraint](../General SQL Syntax/constraint)).
...
Enable or disable constraint checks (see the description of VALIDATE|NOVALIDATE in [constraint](../General SQL Syntax/constraint)).
...
<span id="enabledisableconstraint" name="enabledisableconstraint"></span>

#### enable\_disable\_constraint
...
When enabling primary key/unique constraints, this syntax can be used to manually specify the index that was automatically reused or created by the system. For detailed syntax definitions and descriptions, see [constraint](../General SQL Syntax/constraint) in using_index_clause.
...
<span id="reclaimtableclause" name="reclaimtableclause"></span>

### reclaim\_table_clause
...
<span id="altertableidentity" name="altertableidentity"></span>

### alter\_table\_identity
