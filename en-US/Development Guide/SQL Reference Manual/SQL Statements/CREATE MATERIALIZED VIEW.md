...
A materialized view can only be created based on HEAP tables in Standalone Deployment or YAC/Distributed Cluster Deployment; there is no materialized view related functionality in ISC Distributed Cluster Deployment.
...
This statement is used to specify the name of the materialized view to be created; it cannot be omitted and must comply with YashanDB's [object naming conventions](../Basic SQL Elements/Identifiers).
...
This statement is used to specify aliases for the columns of the materialized view; it can be omitted, does not specify data types, and must comply with YashanDB's [object naming conventions](../Basic SQL Elements/Identifiers).
...
- ON DEMAND: manual refresh, i.e., using the [DBMS_MVIEW](../../PL Reference Manual/Built-in Advanced PL Packages/DBMS_MVIEW) advanced package for refresh
