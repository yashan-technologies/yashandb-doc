...
In YAC/Distributed Cluster Deployment, by specifying the [YFS](../../../Database Administration/Storage Management/YFS Management/00YFS Management) path, data can be backed up to shared storage; when specified as a regular disk path, the data is backed up to the server where the instance is located.
