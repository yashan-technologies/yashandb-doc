...
After the audit policy is successfully created, its definition can be viewed through the [AUDIT_UNIFIED_POLICIES](../../../Reference Manual/System Views/DBA Views/AUDIT_UNIFIED_POLICIES) view.
...
```ebnf
= CREATE AUDIT POLICY policy_name [privilege_audit_clause] [action_audit_clause] [role_audit_clause] [when_clause] [toplevel_clause].
```
...
```ebnf
= PRIVILEGES system_privileges_clause {',' system_privileges_clause}.
```

**[action\_audit\_clause](#actionauditclause)::=**

```ebnf
= ACTIONS (system_action_clause|object_action_clause) {',' (system_action_clause|object_action_clause)}.
```
...
```ebnf
= (object_action|ALL) ON [schema '.'] object_name.
```
...
<span id="privilegeauditclause" name="privilegeauditclause"></span>
...
<span id="actionauditclause" name="actionauditclause"></span>
...
<span id="systemactionclause" name="systemactionclause"></span>
...
Global audit items, and specific items can be viewed through the [V$AUDITABLE_SYSTEM_ACTIONS](../../../Reference Manual/System Views/Dynamic Performance Views/V$AUDITABLE_SYSTEM_ACTIONS) view.

##### ALL

When the global audit item is specified as ALL, it means that all items listed in V$AUDITABLE_SYSTEM_ACTIONS will be included in the audit policy.

<span id="objectactionclause" name="objectactionclause"></span>
...
Audit items for objects, and specific items can be viewed through the [V$AUDITABLE_OBJECT_ACTIONS](../../../Reference Manual/System Views/Dynamic Performance Views/V$AUDITABLE_OBJECT_ACTIONS) view.
...
<span id="roleauditclause" name="roleauditclause"></span>
...
<span id="toplevelclause" name="toplevelclause"></span>
