...
> **Note**:
>
> In the presence of a cascade standby environment, the upper-level standby database of the cascade standby database corresponds to the primary database mentioned below, while the cascade standby database corresponds to the standby database mentioned below, having the same syntax, functionality, and constraints as the main standby database.
...
- Directly connecting to PDB and executing BUILD DATABASE: Only construct the corresponding standby PDB
  ...
  Additionally, standby PDB(s) construction can also be performed by connecting to the CDB root and executing [BUILD PLUGGABLE DATABASE](./BUILD PLUGGABLE DATABASE) for global operations.
...
In YAC/Distributed Cluster Deployment, only one standby cluster can be specified when executing this operation.
...
```sql
-- Execute BUILD for the specified corresponding standby cluster DB_UNIQUE_NAME.
