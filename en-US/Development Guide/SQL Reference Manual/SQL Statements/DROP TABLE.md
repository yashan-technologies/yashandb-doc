...
When the [recycle bin is enabled](../../../Database Administration/Fault Handling/Flashback/Recycle Bin Object Flashback/00Recycle Bin Object Flashback) (configuration parameter RECYCLEBIN_ENABLED is ON), executing a DROP statement to delete a HEAP table without specifying the PURGE keyword will move the table and some of its sub-objects into the recycle bin (the table name is modified to a system-generated string starting with BIN).; deleting TAC tables, LSC tables, or external tables will always result in permanent deletion.
