...
A maximum of 255 indexes can be created on each table.
...
```ebnf
= CREATE [UNIQUE|COLUMNAR|RTREE|SEARCH|BITMAP] INDEX [schema"."] index_name ON table_name "(" index_expr [DESC|ASC]{ "," index_expr [DESC|ASC]} ")" [index_attr_clause].
```
...
```ebnf
= STORE IN "(" tablespace {"," tablespace} ")" | "(" SUBPARTITION [subpartition] [TABLESPACE tablespace] [USABLE|UNUSABLE] {"," SUBPARTITION [subpartition] [TABLESPACE tablespace] [USABLE|UNUSABLE]} ")".
```

<span id="UNIQUE" name="UNIQUE"></span>
...
Unique indexes have the following restrictions:

-  When specified as a unique index, it cannot also be specified as a [BITMAP index](#BITMAP).

- When creating an index on an LSC table, this keyword must be specified.

- In ISC Distributed Cluster Deployment, the partition column must be a subset of the unique index key columns, otherwise an error will occur.
...
<span id="COLUMNAR" name="COLUMNAR"></span>
...
- RTree indexes can only be created for columns of [ST_Geometry type](../Data Types/ST_GEOMETRY Type), and each RTree index can only specify one column.
...
This statement is used to specify the creation of a BITMAP index.

BITMAP indexes have the following restrictions:

- The BITMAP index is not applicable to ISC Distributed Cluster Deployment.

- The BITMAP index is only applicable to HEAP tables.

-  When specified as a BITMAP index, it cannot also be specified as a [UNIQUE index](#UNIQUE), [function index](#columnexpression), or [REVERSE index](#REVERSE), and it cannot be created (CREATE) or rebuilt (ALTER INDEX) online (ONLINE).

***Example*** for Heap tables

```sql
CREATE BITMAP INDEX idx_bitmap ON employees (sex);
```

### index\_name

This statement specifies the name of the index to be created. It cannot be omitted and must comply with YashanDB's [object naming conventions](../Basic SQL Elements/Identifiers).
...
- Columns of type [LOB](../Data Types/Large Object Types) or [UDT](../Data Types/User-Defined Types) cannot be used as index columns.

- Columns of type [RAW](../Data Types/RAW Type) cannot be used as columnar index columns.


- Only RTree indexes can be created on columns of [ST_Geometry type](../Data Types/ST_GEOMETRY Type), and each RTree index can only specify one column.


- Each full-text index can specify only one column, and it must be of type CHAR, VARCHAR, or CLOB.

- Each bitmap index can specify up to 30 columns, and cannot be applied to columns with [constraint](../General SQL Syntax/constraint).

- Except that RTree index / full-text index can only have 1 column, and BITMAP index can have a maximum of 30 columns, other indexes can specify a maximum of 32 columns. The length of the index key value (i.e., the total length of the specified column name set) must not exceed the following limits:

    - When the data block size `DB_BLOCK_SIZE` is 8K, the maximum index key length is 6,400 bytes.

    - When the data block size `DB_BLOCK_SIZE` is 16K, the maximum index key length is 12,960 bytes.
    
    - When the data block size `DB_BLOCK_SIZE` is 32K, the maximum index key length is 26,510 bytes.

    > **Note**:
    >
    > For VARCHAR, NVARCHAR, or RAW columns, if the column currently contains no data, its length is counted as 1 byte for validation. When inserting or updating the value of a variable-length column, the actual data length will be used for validation.
...
- When specified as an [RTREE INDEX](#RTREE) or [BITMAP INDEX](#BITMAP), it cannot be simultaneously specified as a FUNCTION index.
...
- If there is a UDF in column_expression, the following additional constraints apply:

    - The owner of the index must have execution permissions for the corresponding UDF.  

    - The UDF must be deterministic.

    - The UDF that the index depends on must be in valid status (STATUS=VALID in DBA_OBJECTS view). An invalid UDF will cause errors when executing operations related to that index (except DROP INDEX). For example, if tables or UDT objects used in the UDF are deleted, the UDF will become invalid, causing the corresponding index to be unusable. In this case, the index must first be deleted, the UDF updated, and then a new index created to resume normal usage.

    - The UDF and its procedure body that the index depends on are not allowed to be deleted or replaced. If deletion or replacement is needed, the corresponding index must first be deleted.
...
CREATE OR REPLACE FUNCTION calc_finance_total(revenue_total number, cost_total number, fee_total number) RETURN number DETERMINISTIC IS
    BEGIN
        RETURN revenue_total+cost_total+fee_total;
    END;
/

CREATE INDEX idx_udf ON finance_info_idx (calc_finance_total(revenue_total,cost_total,fee_total));
INSERT INTO finance_info_idx VALUES ('2001','01','0201',2888,2000,300);
INSERT INTO finance_info_idx VALUES ('2021','01','0201',28888,24000,3000);
INSERT INTO finance_info_idx VALUES ('2021','01','0101',38888,34000,4000);
INSERT INTO finance_info_idx VALUES ('2021','02','0101',37778,33000,6000);

select year,month,branch,revenue_total,cost_total,fee_total 
from finance_info_idx 
where calc_finance_total(revenue_total,cost_total,fee_total) > 75000;
...
When creating a [BITMAP index](#BITMAP), the index is always sorted in ascending order regardless of whether DESC, ASC, or neither is specified.
...
<span id="indexattrclause" name="indexattrclause"></span>
...
This statement is used to specify the storage properties of the index. If omitted, INITRANS/PCTFREE defaults to 2/8; for other storage properties, please refer to the general SQL syntax [storage](../General SQL Syntax/storage_clause) description.
...
This statement is used to create a global index.

Global indexes have the following restrictions:

- When creating a [BITMAP index](#BITMAP), this keyword cannot be specified.

- When creating an index on a sharded table, this keyword cannot be specified, otherwise an error will be reported.

- In other scenarios, if this keyword is not specified, YashanDB will also default to creating a global index.
...
- Online index creation is not allowed for [COLUMNAR INDEX](#COLUMNAR), [RTREE INDEX](#RTREE) or [BITMAP INDEX](#BITMAP).
- In YAC/Distributed Cluster Deployment, ongoing create/rebuild index online operations will fail upon experiencing instance start and stop events.

<span id="parallel" name="parallel"></span>
...
<span id="REVERSE" name="REVERSE"></span>
...
<span id="localindexclause" name="localindexclause"></span>

#### local_index_clause

This statement is used to create a partition index (Local Partitioned Index). 

The usage rules of this statement are as follows:

- Partition indexes can only be created on partition tables, and the number of index partitions and partition boundaries must be the same as that of the table.

- When creating a partitioned unique index, the union of the primary partition key and secondary partition key must be a subset of the index key.
...
<span id="indexhashpartitionclause" name="indexhashpartitionclause"></span>
...
<span id="indexpartitionclause" name="indexpartitionclause"></span>
...
<span id="indexpartitionattrclause" name="indexpartitionattrclause"></span>
...
<span id="indexcomppartitionclause" name="indexcomppartitionclause"></span>
...
<span id="indexsubpartitionclause" name="indexsubpartitionclause"></span>
...
***Example*** for Standalone/YAC/Distributed Cluster Deployment and Heap tables and TAC tables
