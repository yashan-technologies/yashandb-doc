...
```ebnf
=[store_in_clause] "(" PARTITION [partition] [index_partition_attr_clause] [index_subpartition_clause] {"," PARTITION [partition] [index_partition_attr_clause] [index_subpartition_clause]} ")".
```
...
- Each index can specify up to 32 columns, and the total length of all column values (i.e., the index key length) must not exceed the following limits:
