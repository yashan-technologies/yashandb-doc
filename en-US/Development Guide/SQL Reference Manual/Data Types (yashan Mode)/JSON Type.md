...
> **Note**: 
>
> 1. Some internal metadata is generated during storage; the byte limit for heap tables (HEAP tables) includes the space occupied by this metadata, while the byte limit for LSC tables (TAC and LSC tables) does not include the space occupied by metadata.
> 2. Taking DB_BLOCK_SIZE = 8K as an example, the maximum byte length for JSON type data in heap tables is `4G*8K=32T`.
