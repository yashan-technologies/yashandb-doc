...
This statement is used to specify the object to be updated, which can be a table name (including tables from local databases or [remote tables](../General SQL Syntax/dblink)), partition names of tables, view names, or subqueries, and an alias can be assigned to it.
...
For sharded tables in a ISC Distributed Cluster Deployment, modifying the values of its partition key fields is not allowed.
