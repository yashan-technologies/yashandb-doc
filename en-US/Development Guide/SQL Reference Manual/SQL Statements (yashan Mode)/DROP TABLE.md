...
When the recycle bin is enabled (the configuration parameter RECYCLEBIN_ENABLED is set to ON), executing the DROP statement to delete a table will cause the target table and its indexes, constraints, triggers, LOB columns, etc., to enter the recycle bin.
...
The recycle bin functionality applies only to HEAP tables. Whether to use the recycle bin is controlled by the configuration parameter RECYCLYBIN_ENABLED (default value is ON). When set to ON, performing a DROP operation on HEAP tables will cause the table object and data to be placed in the recycle bin (the table name is changed to a system-generated string starting with BIN) to facilitate recovery after accidental operations. The DROP operation on TAC tables, LSC tables, and external tables will always result in permanent deletion.

If the PURGE statement is explicitly specified, the dropped table object and data will not enter the recycle bin but will be permanently deleted and cannot be recovered.
