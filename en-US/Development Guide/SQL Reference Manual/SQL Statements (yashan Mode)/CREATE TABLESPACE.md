...
- Storage paths for temporary files:
  ...
  - In YAC Deployment, the temporary file path must be a YFS path.
...
The local temporary tablespace is only applicable to YAC Deployment. When created, independent temporary files will be created for each instance within the cluster, and these files are not shared among instances.
...
The local SWAP tablespace is only applicable to YAC Deployment. When created, independent temporary files will be created for each instance within the cluster, and these files are not shared among instances.
...
Data file paths in a YAC must be of [YFS](../../../YashanDB for Cluster/Yashan File System/00Yashan File System) type.
