...
In YAC Deployment, by specifying the [YFS](../../../YashanDB for Cluster/Yashan File System/00Yashan File System) path, data can be backed up to shared storage; when specified as a regular disk path, the data is backed up to the server where the instance is located.
