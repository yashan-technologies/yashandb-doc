...
`ANALYZE SCHEMA` is used to collect statistics for all objects (tables, ACs, columns, indexes) under the specified user. This corresponds to the `GATHER_SCHEMA_STATS` procedure in the advanced package [DBMS_STATS](../../PL Reference Manual/Built-in Advanced PL Packages/DBMS_STATS).
