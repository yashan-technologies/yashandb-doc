...
This statement is used to specify the tablespace for the AC. If omitted, it defaults to the tablespace of the source table. In a ISC Distributed Cluster Deployment, users cannot specify a tablespace for an AC created on a sharded table; the tablespace of the table will be used as the tablespace for the AC.
