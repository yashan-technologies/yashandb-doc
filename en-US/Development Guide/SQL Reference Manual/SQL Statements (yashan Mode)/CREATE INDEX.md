...
When creating an index for an encrypted column, only RTREE indexes are supported.
...
In a ISC Distributed Cluster Deployment, the partition column must be a subset of the unique index key columns, otherwise an error will occur.
...
- Only one [ST_Geometry type](../Data Types (yashan Mode)/ST_GEOMETRY Type) is allowed in the specified column at a time. Other types are not restricted. If you need to create RTREE indexes for multiple ST_Geometry type columns in the same table, you must execute the statement again and specify different columns.
- RTREE indexes are only applicable to HEAP tables in Standalone Deployment and YAC Deployment.
- RTREE indexes cannot be created for temporary tables.
...
LOB large object columns cannot be included in any of the index columns or expressions.
