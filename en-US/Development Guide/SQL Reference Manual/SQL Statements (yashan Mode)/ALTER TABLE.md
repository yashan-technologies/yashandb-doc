...
**[inline_constraint](../General SQL Syntax/constraint (yashan Mode))** 
...
**[out_of_line_constraint](../General SQL Syntax/constraint (yashan Mode))** 
...
To use the [Ystream service](../../PL Reference Manual/Built-in Advanced PL Packages/DBMS_YSTREAM_ADM), supplemental logging (either at the database level or table level) must be enabled before creating the Ystream service. If the Ystream service is created first and then supplemental logging is enabled, an error will occur.
...
The current enabled supplemental logging information can be viewed through the views [DBA_LOG_GROUPS](../../../参考手册/系统视图/DBA视图/DBA_LOG_GROUPS), [ALL_LOG_GROUPS](../../../参考手册/系统视图/ALL视图/ALL_视图), and [USER_LOG_GROUPS](../../../参考手册/系统视图/USER视图).
...
- In YAC Deployment, if instance startups or joins occur, or YAC faults happen, all nologging tables will be marked as corrupted.
- In YAC Deployment, setting table properties to nologging will bind the instance (the bound instance ID can be viewed via the GV$DICT_CACHE view with the nologging_instance_id field). The following operations can only be executed on the bound instance:
...
This specifies the data type of the column field. Refer to YashanDB's [data type descriptions](../Data Types (yashan Mode)/00Data Types (yashan Mode)).
...
This clause is used to define inline constraints when adding a column field. Refer to general SQL syntax [constraints](../General SQL Syntax/constraint (yashan Mode)) for detailed descriptions.
...
This modifies the data type of the column field to one recognized by YashanDB [data types](../Data Types (yashan Mode)/00Data Types (yashan Mode)).
...
- **Columns with indexes**:
    ...
    - If the table is empty (i.e., contains no data), it is allowed to modify its type to one other than LOB/JSON/UDT.
...
- **Other non-empty columns with original data types of character types**: Modifications to different character types are allowed, but still must follow these rules:
    ...
    - HEAP tables do not allow modification of data types across storage modes. For example, in HEAP tables, it is not allowed to modify the column data type from VARCHAR(3200) (with standard string storage) to VARCHAR(8004) (which will automatically convert to LOB storage). TAC tables, on the other hand, allow such operations. For an introduction to storage methods, refer to [character types](../Data Types (yashan Mode)/Character Types).
...
This modifies the constraints of a column field, following the syntax rules defined in general [constraints](../General SQL Syntax/constraint (yashan Mode)).
...
- Cannot execute modify PRIMARY KEY on column fields that do not meet the requirements for creating primary keys. For the requirements for creating primary keys, refer to [constraints](../General SQL Syntax/constraint (yashan Mode)).
...
- Cannot execute modify FOREIGN KEY on column fields that do not meet the requirements for creating foreign keys; for the requirements for creating foreign keys, refer to [constraints](../General SQL Syntax/constraint (yashan Mode)).
...
Constraint item, and for a detailed description of the constraint item, please refer to the general SQL syntax [constraint](../General SQL Syntax/constraint (yashan Mode)).
...
Enable or disable the specified constraint (see the description of ENABLE|DISABLE in [constraint](../General SQL Syntax/constraint (yashan Mode))).
...
Enable or disable constraint checks (see the description of VALIDATE|NOVALIDATE in [constraint](../General SQL Syntax/constraint (yashan Mode))).
...
When enabling primary key/unique constraints, this syntax can be used to manually specify the index that was automatically reused or created by the system. For detailed syntax definitions and descriptions, see [constraint](../General SQL Syntax/constraint (yashan Mode)) in using_index_clause.
