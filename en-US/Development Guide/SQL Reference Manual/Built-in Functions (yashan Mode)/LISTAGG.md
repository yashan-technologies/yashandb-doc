...
When the OVER keyword is specified, LISTAGG acts as a [window function](00Built-in Functions (yashan Mode).html#WindowFunction), concatenating rows within each group window, where the concatenation result of each row in the group window is the same.
