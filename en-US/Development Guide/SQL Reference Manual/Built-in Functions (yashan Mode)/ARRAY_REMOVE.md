...
The ARRAY_REMOVE function removes all members from the array ([array variable](../../PL Reference Manual/PL Language Fundamentals/Variables/Collection Variables), [array object](../../PL Reference Manual/PL Objects/User-Defined Types), [array type](../Data Types (yashan Mode)/User-Defined Types)) array_var that are the same as compare_member and returns the array after deletion.
...
- An existing array where the members must be of [ordinary scalar data types](../Data Types (yashan Mode)/00Data Types (yashan Mode)).
- NULL, in which case the function returns NULL.

**compare_member**

A [general expression](../General SQL Syntax/expr) whose value type must be comparable to the type of array_var's array members. For specific rules, refer to YashanDB's [comparison operation rules](../Operators/Comparison Operators).
