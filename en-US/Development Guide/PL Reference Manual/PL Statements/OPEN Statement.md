The OPEN statement is used to open a [cursor](../PL Language Fundamentals/Variables/Cursors). Opening a cursor means executing the SQL statement to which the cursor is bound (either static binding or dynamic binding) and obtaining the result set.
