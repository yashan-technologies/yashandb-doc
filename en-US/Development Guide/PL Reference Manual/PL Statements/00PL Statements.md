...
The following SQL statements can be written in PL, with the same syntax as ordinary SQL statements outside of procedure bodies:

- [DML](DML Statement): INSERT/UPDATE/SELECT/MERGE/DELETE, etc. A SELECT statement must have an INTO clause, and the number of items in the select_list must correspond to the variables or RECORD members in the INTO clause.
- [DCL](DCL Statement): COMMIT/ROLLBACK/SAVEPOINT/LOCK TABLE, etc.
