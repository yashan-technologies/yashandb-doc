...
In a ISC Distributed Cluster Deployment, users with the dba role or users with the execute any procedure privilege can perform cleanup of audit logs on the CN node. On the DN and MN nodes, operations must be performed using the sys user connected directly to the node.
