...
> - In ISC Distributed Cluster Deployment, stored procedures/functions in this advanced package are only allowed to be called one by one on CN nodes.
...
- The memory upper and lower limits are calculated by multiplying CDB_MAX_MEMORY_SIZE by the corresponding percentage. PDB usage is only based on memory-related parameter configuration values (not current actual usage), including DATA_BUFFER_SIZE, SHARE_POOL_SIZE, VM_BUFFER_SIZE, etc.

    - When creating a new PDB or starting an existing PDB, the PDB's required memory must be within the range [CDB_MAX_MEMORY_SIZE * MEMORY_MIN, CDB_MAX_MEMORY_SIZE * MEMORY_LIMIT] and the CDB memory must be sufficient for normal execution.

    - If, during PDB runtime, MEMORY_LIMIT is adjusted to be smaller than the PDB's configured memory usage, the system will automatically generate a recommended memory parameter value for the PDB, which does not affect the PDB's current runtime, but online expansion of the PDB's memory configuration will be disabled during this runtime. If the new MEMORY_LIMIT is set reasonably, after restarting the PDB, the new recommended memory parameter value will take effect, and the new upper limit will be calculated based on the new MEMORY_LIMIT value. If the new MEMORY_LIMIT is set too small, it may cause the recommended memory parameter value generation to fail (a WARN log will be recorded in the runtime log), which may prevent the PDB from restarting. In this case, you must re-increase the MEMORY_LIMIT value.
    
    - If, during PDB runtime, the lower limit is adjusted to be larger than the PDB's configured memory usage, the system will automatically generate a recommended memory parameter value for the PDB, which does not affect the PDB's current runtime. After restarting the PDB, the new recommended memory parameter value will take effect, and it will adopt the latest lower limit configuration.
