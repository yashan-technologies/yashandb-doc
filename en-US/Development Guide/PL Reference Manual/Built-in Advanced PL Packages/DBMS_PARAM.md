...
> **Caution**:
>
> - For environments that need to apply recommended parameters, it is recommended to execute this stored procedure on the basis of default database configuration to avoid extra configuration items affecting the validity of recommended parameters.
>
> - During the process of generating recommended parameters, the system will perform performance testing on the disks where datafiles and redofiles are located. It is recommended to execute during no business running to ensure the parameter recommendation results are accurate and reliable.
> 
> - In multi-node environments (e.g., Standalone Primary-Standby Deployment, YAC Deployment), to ensure global consistency of parameter configuration, pay attention to the following matters:
>
> 	- The server hardware resource configuration for each node should remain unified.
>
>	- Parameter configuration changes usually do not support automatic synchronization, and this stored procedure needs to be executed on each node separately.
>
> - **Not recommended** to use TPCC tuning scenario (i.e., scene=TPCC) in production environment. If needed in test environment, be sure to back up the [configuration file](../../../Database Administration/Storage Management/Database File Management/Configuration Parameter File and Password File Management) in advance to prevent parameters from being overwritten and affecting normal system operation.
...
| os_memory_limit | The upper limit of the percentage of server memory available for the database to use, defaults to 100, which allocates all memory. **It is recommended to set to 80**. |
...
| scene           | The scenario for parameter tuning, default is NORMAL<br/>* In Standalone Deployment, YAC Deployment or Distributed Cluster Deployment, optional values are [NORMAL,BATCH,TPCC]<br />* In ISC Distributed Cluster Deployment, optional values are [NORMAL,TPCH]<br /><br />The applicable scenarios for each parameter value are as follows:<br />* NORMAL: General mixed workload scenario, applicable to all table types. <br/>* BATCH: Batch processing scenario, can only be set to BATCH when the table type is HEAP. When the business mainly uses HEAP tables for OLTP and experiences poor performance during periodic batch scenarios, consider using the BATCH scenario for tuning<br />* TPCC: TPCC: High-concurrency online transaction processing scenario, can be set to TPCC only when the table type is HEAP table. When using TPCC scenario for tuning recommendation, scale_factor represents the number of test data warehouses. To ensure the effectiveness of recommended parameters, TPCC tuning has certain requirements for server hardware configuration (100 or 200 warehouses of data: at least 8-core CPU + 32G memory; 500 warehouses of data: at least 32-core CPU + 128G memory; 1000 warehouses of data: at least 96-core CPU + 384G memory)<br />* TPCH: Complex analytical query scenario, can only be set to TPCH when the table type is LSC. When using TPCH scenario for tuning recommendation, ensure all nodes execute the parameter recommendation; otherwise, restart may fail due to mismatched connection parameters |
| scale_factor    | * Data volume for test scenarios such as TPCH (unit: GB) <br/> * Number of warehouses for TPCC test scenario. <br/>Value range is [1, 100000], defaults to 100<br />Only used to assist in recommending certain parameter values when memory is sufficient. When memory is insufficient, priority is given to ensuring no over-provisioning, and this parameter will automatically degrade. |
