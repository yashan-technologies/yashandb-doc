...
- It is recommended to enable supplemental logging ([database-level](../../SQL Reference Manual/SQL Statements (yashan Mode)/ALTER DATABASE.html#supplementallogclauses) or [table-level](../../SQL Reference Manual/SQL Statements (yashan Mode)/ALTER DATABASE.html#addsupplementalloggingclause)) before creating the YStream server. If not enabled, only supplemental information from existing logs can be parsed, and enabling supplemental logging after an existing YStream server will result in an error.
...
The START function is used to start an existing YStream server. Only after the YStream server is started will the [YStream client](../../YStream参考手册/00YStream参考手册) be allowed to connect.
