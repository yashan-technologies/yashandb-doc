YStream is a logical log parsing interface provided by YashanDB. Its server-side API (also referred to as the "YStream server") is managed by built-in functions provided by the DBMS_YSTREAM_ADM package, including creating, adjusting configurations, starting/stopping, and deleting. For the client API usage guide, refer to the [YStream Reference Manual](../../YStream Reference Manual/00YStream Reference Manual).
...
| STARTED | Indicates the YStream server has been started, waiting for the [YStream client](../../YStream Reference Manual/00YStream Reference Manual) to connect | The instance that starts the YStream server allows:<br/>* [ADD_TABLES](#add_tables): Add parsing table names and schemas<br/>* [STOP](#stop): Stop the YStream server<br/><br/> When the instance that starts the YStream server is offline, other instances in the cluster allow:<br/>* [STOP](#stop): Stop the YStream server<br/>* [TEST](#test): Test if the start_scn of the YStream server is appropriate |
| RUNNING | * Indicates that the [YStream client](../../YStream Reference Manual/00YStream Reference Manual) has connected, and the YStream server is running on the current instance<br/>* Indicates that the current instance is executing the [TEST](#test) function | The instance that starts the YStream server allows:<br/>* [ADD_TABLES](#add_tables): Add parsing table names and schemas.However, it is not possible to narrow the scope of parsing objects for a running YStream server whose parsing objects are all tables. (You need to stop the server first and then modify the parsing scope)<br/>* [STOP](#stop): Stop the YStream server<br/><br/> When the instance that starts the YStream server is offline, other instances in the cluster allow:<br/>* [STOP](#stop): Stop the YStream server |
...
<span id="add_tables" name="add_tables"></span>
...
ADD_TABLES function is used to specify the parsing scope (schema or table) for an existing YStream server.

After creating YStream server, the default parsing scope is all tables in the database (including newly created tables in the future). If ADD_TABLES is not executed or both table_names and schemas parameters are set to empty strings when executing ADD_TABLES, the default parsing scope is maintained.

Function usage rules are as follows:

- For the status requirements of the target YStream server for this operation, please refer to [Status Introduction](#STATUS).

  - If the parsing scope of a server is all tables, it is not possible to specify specific parsing objects (i.e., narrow the parsing scope) through ADD_TABLES when the server is in the STARTED or RUNNING state. You need to stop the server first and then execute ADD_TABLES.

  - If you need to add existing tables to the parsing scope of a Ystream server, it is not recommended to perform the operation when the server is in the STARTED or RUNNING state. Otherwise, you must ensure that the [table-level supplemental logging](../../SQL Reference Manual/SQL Statements/ALTER TABLE.md#addsupplementalloggingclause) of the target tables are not enabled before executing ADD_TABLES. And after executing ADD_TABLES, you need to manually enable the table-level supplemental logging of these tables so that the Ystream server can parse this table.
...
The START function is used to start an existing YStream server. Only after the YStream server has been started can the [YStream client](../../YStream Reference Manual/00YStream Reference Manual) establish a connection.
...
<span id="drop_tables" name="drop_tables"></span>
...
<span id="set_parameter" name="set_parameter"></span>
