...
```ebnf
= PROCEDURE procedure_name ["(" argument_define {"," argument_define} ")"] (IS|AS) procedure_body_clause ";".
```
...
```ebnf
= FUNCTION function_name ["("  argument_define {"," argument_define}  ")"]  RETURN return_datatype (IS|AS) function_body_clause ";".
```
