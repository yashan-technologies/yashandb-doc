...
The number of cursors that can be opened per session in YashanDB is limited by the configuration parameter [OPEN_CURSORS](../../../../参考手册/配置参数), with a default value of 310.

- When the SQL bound to the cursor contains a FOR UPDATE statement, dblink is not supported. Other constraints are the same as those in [Limitations at the SQL Level](../../../SQL参考手册/SQL语句（yashan模式）/SELECT.html#forupdateclause), for example, in a multi-table join query, only the table corresponding to the column specified by OF column_name is updated. 
...
Explicit cursors are declared using cursor definition statements and can then perform operations such as [opening a cursor](../../PL语句/OPEN Statement), [fetching from a cursor](../../PL语句/FETCH Statement), and [closing a cursor](../../PL语句/CLOSE Statement) within the procedure body.
...
A [SELECT](../../../SQL参考手册/SQL语句（yashan模式）/SELECT) statement that does not allow specifying placeholders (bind parameters). This section can utilize the INTO clause for compatibility purposes, but it has no actual effect.
...
```ebnf+diagram
syntax::= cursor_variable type [[NOT] NULL]";"
```
...
<span id="hidecursor" name="hidecursor" class="yaslink"></span>
