...
- In Standalone Primary-Standby or Primary-Standby Cluster Deployment, UDTs can only be created, managed, and used on the primary database/primary cluster.
...
```ebnf
=  CREATE [OR REPLACE] [EDITIONABLE|NONEDITIONABLE] TYPE [schema '.'] type_name
...
```ebnf
= OBJECT  [ '(' (attribute datatype [NULL]) { ','  (attribute datatype [NULL])  }
...
```ebnf
= ( constructor_spec | subprogram_spec | map_order_function_spec ).
```

**[constructor\_spec](#constructorspec)::=**

```ebnf
= CONSTRUCTOR FUNCTION type_name [ '(' [SELF IN OUT datatype ','] (parameter datatype) { ',' (parameter datatype) }  ')' ] RETURN SELF AS RESULT.
```
...
```ebnf
= UNDER [ schema '.' ] supertype [ '(' (attribute datatype) { ','  (attribute datatype)  } [ ',' element_spec ] ')' ] [ [NOT]  FINAL ] .
```

<span id="createtype" name="createtype"></span>
...
<span id="invokerrightsclause" name="invokerrightsclause"></span>

### invoker_rights_clause

For details, please refer to the relevant description of [invoker_rights_clause](00PL Objects.md#invoker).

<span id="objectbasetypedef" name="objectbasetypedef"></span>

### object\_base\_type\_def

Creating a base UDT.

<span id="objectsubtypedef" name="objectsubtypedef"></span>
...
<span id="elementspec" name="elementspec"></span>
...
<span id="constructorspec" name="constructorspec"></span>
...
<span id="subprogramspec" name="subprogramspec"></span>
...
<span id="maporderfunctionspec" name="maporderfunctionspec"></span>
...
<span id="varraytypespec" name="varraytypespec"></span>
...
<span id="nestedtabletypedef" name="nestedtabletypedef"></span>
...
<span id="objecttypedef" name="objecttypedef"></span>
...
When creating a UDT type body, if element_spec is a function, it supports the syntax of [PARALLEL_ENABLE](User-Defined Functions.md#parallel_enable) and [DETERMINISTIC](User-Defined Functions.md#deterministic), but PARALLEL_ENABLE and DETERMINISTIC have no actual effect and are only for syntax compatibility.

**element_spec_body::=**

```ebnf
= (IS|AS) [variable_declare] BEGIN pl_statements END ';'.
```
...
Within an Object type UDT, it is allowed to define multiple methods with the same name but different [parameter](../Parameters/Formal and Actual Parameters) names, quantities, orders, or [data types](../../SQL Reference Manual/Data Types/00Data Types). PL can use overloading to filter and call such methods with the same name, following the same rules as [package overloading calls to subprocedures](./Packages.md#overload).
