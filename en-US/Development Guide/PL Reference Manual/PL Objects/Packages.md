...
You can create the PACKAGE HEAD or PACKAGE BODY using the [CREATE PACKAGE](../../SQL Reference Manual/SQL Statements (yashan Mode)/CREATE PACKAGE) statement, with the following syntax definition:
...
The name of the package, which cannot be omitted and must conform to YashanDB's [object naming conventions](../../SQL参考手册/基本SQL元素/标识符).
...
In YashanDB, the variables of the package are managed in binding with the package, meaning that if the HEAD or BODY of the package changes, all variables of that package will be reset. 
...
```plsql
-- Creating a package without HEAD
DROP PACKAGE IF EXISTS calc_fee;
...
Advanced packages allow defining multiple subprograms with the same name but different [parameter](../Parameters/Formal and Actual Parameters) names, counts, orders, or [data types](../../SQL Reference Manual/Data Types (yashan Mode)/00Data Types (yashan Mode)) to be used. PL can filter calls to such identically named subprograms through the overloading functionality.
...
- If only types are inconsistent and all are [numeric types](../../SQL Reference Manual/Data Types (yashan Mode)/Numeric Types), it will match according to the closest type and precision first, selecting the first matched subprogram for the call.

- If only types are inconsistent, YashanDB will first attempt to implicitly convert the actual parameters' data types [to match](../../SQL Reference Manual/Data Types (yashan Mode)/Data Type Conversion) the parameter types.
...
- Use type conversion functions to convert actual parameters to match the data types of the parameters in the target subprogram, such as using the [CAST](../../SQL Reference Manual/Built-in Functions (yashan Mode)/CAST) function or [explicit conversions](../../SQL Reference Manual/Data Types (yashan Mode)/Data Type Conversion).
...
Use the [DROP PACKAGE](../../SQL Reference Manual/SQL Statements (yashan Mode)/DROP PACKAGE) statement to delete the package.
...
Use the [ALTER PACKAGE](../../SQL Reference Manual/SQL Statements (yashan Mode)/ALTER PACKAGE) statement to modify the package.