...
After the first execution of the dump operation, a trace file corresponding to the session ID (sid) will be generated, with the default filename format {dbname}yas{sid}.trc. Subsequent dump operations will only append new data to the existing file corresponding to the same session ID.
