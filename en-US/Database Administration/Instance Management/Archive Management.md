...
<span id="EnableArchiving" name="EnableArchiving"></span>
...
After the archiving mode is enabled, database administrators can also adjust automatic cleanup strategies, archive paths, and other related configurations or perform [archive log files management](../Storage Management/Database File Management/Archive Log File Management) based on actual business needs and disk planning.
...
<span id="archive_local_dest" name="archive_local_dest"></span>
...
If the path is adjusted after enabling the archiving mode, the archive log files that already exist in the original path will remain unchanged. They can be automatically or manually cleaned based on the cleanup rules. For cleanup operations, please refer to [archive log files management](../Storage Management/Database File Management/Archive Log File Management.md#cleanup).
...
<span id="auto_cleanup_config" name="auto_cleanup_config"></span>
...
Manual archiving ([ALTER SYSTEM ARCHIVE LOG CURRENT](../../Development Guide/SQL Reference Manual/SQL Statements/ALTER SYSTEM.md#archive_logfile)) can only archive redo log files that are in the CURRENT state. During the archiving process, the redo log files are also switched. For specific operations, refer to [switching and archiving redo log files](../Storage Management/Database File Management/Redo Log File Management.md#SWITCH).

The archive log files generated through manual archiving are managed the same way as those generated through automatic archiving, and are subject to the same automatic cleanup policy. They can be managed as needed, following [file management](../Storage Management/Database File Management/Archive Log File Management).
...
- Need to use [YStream](../../Development Guide/YStream Reference Manual/00YStream Reference Manual).

- Need to use [database flashback](../Fault Handling/Flashback/00Flashback).
