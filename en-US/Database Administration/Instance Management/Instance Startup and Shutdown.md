...
*   **NOMOUNT**: Start the database instance, read the [configuration parameter file](../Storage Management/Database File Management/Configuration Parameter File and Password File Management), but do not load the database; only the sys user is allowed to log in.

*   **MOUNT**: Start the database instance, read the [control file](../Storage Management/Database File Management/Control File Management), load the database, but the database is in a closed state; only the sys user is allowed to log in.
...
- **RESETLOGS**: When a PITR (Point In Time Recovery), database flashback, or logical standby database configuration has occurred, and if a complete recovery cannot be performed, the database must be opened in RESETLOGS mode. This mode will reset the redo log sequence number.
  ...
  > **Caution**: 
  >  
  > - In practical use, it is recommended to open the physical standby database in READONLY mode for primary/standby Standalone deployments; this mode is not applicable for distributed, YAC/Distributed Cluster Deployment.  
...
When starting a database instance, its startup phases can be specified sequentially or skipped from NOMOUNT -> MOUNT -> OPEN, but direct rollback is not possible.
...
YashanDB supports closing the database in the following three modes:
