...
>
> This chapter only describes the operations and maintenance of data DiskGroups. For operations and maintenance of the system DiskGroup, please refer to [System DiskGroup Management](./System DiskGroup Management).

## Overview

Data DiskGroups are the core storage component of YFS. Compared with system DiskGroups, data DiskGroups have the following characteristics:

- Used to store business data.

- Supports operations and maintenance such as creation, expansion, and faulty disk replacement.

- Supports redundancy mechanisms to ensure high data availability.

## Data DiskGroup High Availability Mechanism
...
The [redundancy](../../../Product Concepts/YAC Infrastructure/Yashan File System.md#redundancy) level of the data DiskGroup determines the number of data replicas, directly affecting the level of data high availability and reliability. 
...
DiskGroups with default names starting with DG* are data DiskGroups, used for storing business data. The name of this DiskGroup can be configured by users during installation. If not configured, the default value starts from DG0.
...
Data DiskGroup information includes:
...
- type: The type of the DiskGroup, where USER indicates a data DiskGroup.
- redundancy: The redundancy level of the DiskGroup. The redundancy level of each DiskGroup is independent.
  ...
  - NORMAL: Under this configuration, the data DiskGroup provides [2,3] YFS metadata replicas (the exact number depends on the number of FailureGroups) + 2 user data replicas.
  - HIGH: Under this configuration, the data DiskGroup provides [3,5] YFS metadata replicas (the exact number depends on the number of FailureGroups) + 3 user data replicas.
...
The handling options for disk failures in data DiskGroups are as follows:

- If both the disk path and name can be ensured to be consistent with the original disk when repairing hardware issues, the repair can be completed through the [ONLINE operation](#online).

- If the disk path or name cannot be ensured to be consistent with the original disk when repairing hardware issues, the repair can only be completed through the [REPLACE operation](#replace).
...
A disk in normal status that was taken offline due to misoperation can be re-onlined. After hardware failure, the disk can also be re-onlined after being processed at the original location. The specific operation rules for online are as follows:
...
- If the original disk is offline due to hardware failure, it is recommended to use the [REPLACE operation](#replace) for repair. If using the ONLINE operation for repair, you must first fix the hardware issue and ensure **the new disk's path and disk name are exactly the same as the old disk** before completing the repair through the online operation.
