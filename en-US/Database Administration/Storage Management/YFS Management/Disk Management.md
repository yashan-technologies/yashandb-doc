...
Before and after expansion, the DiskGroup must meet the following configuration requirements:

 

- If the redundancy level of the DiskGroup is planned to be Normal or High, both the available **capacity** of all disks and the **number** of disks in each FailureGroup in the same DiskGroup need to be consistent.
...
| ---------- | ------ | ----------- |
| Add disks to the existing DiskGroup(s)   |  If the redundancy of the DiskGroup is Normal or High, the same number of disks should be added to each FailureGroup of the DiskGroup.   | Pros: No adjustment is required for the storage locations of existing database files (i.e., business "unaware").     |
