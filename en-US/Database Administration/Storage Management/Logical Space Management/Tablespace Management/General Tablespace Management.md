...
Shrinking a tablespace can reclaim its free space and reduce waste.



The usage rules for shrinking a tablespace are as follows:

- The UNDO tablespace cannot be shrunk.

- In primary-standby HA deployment, shrinkage operations of SWAP tablespaces or temporary tablespaces performed on the primary database will not be synchronized to the standby database.

- In YAC/Distributed Cluster Deployment, shrinkage operations of local SWAP tablespaces or local temporary tablespaces will not be synchronized to other instances.

- When shrinking a tablespace, shadow files will be automatically skipped and left untouched.
