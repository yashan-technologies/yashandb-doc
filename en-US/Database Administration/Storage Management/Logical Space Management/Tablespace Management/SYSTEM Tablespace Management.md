...
> **Note**: 
>
> When specifying the data file path, for Standalone Deployment, both absolute and relative paths can be specified. For YAC/Distributed Cluster Deployment, only absolute paths can be specified, and they must be YFS paths. For ISC Distributed Cluster Deployment, only relative paths are allowed.
...
```sql
ALTER DATABASE DATAFILE '+DG0/dbfiles/system01' RESIZE 20G;
```
