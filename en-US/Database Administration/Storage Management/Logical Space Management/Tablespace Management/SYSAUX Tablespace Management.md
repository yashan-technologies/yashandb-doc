...
> **Note**: 
>
> When specifying the data file path, it can be specified as an absolute or relative path in Standalone Deployment, as an absolute path for YAC/Distributed Cluster Deployment (only YFS paths), and only as a relative path in ISC Distributed Cluster Deployment.
