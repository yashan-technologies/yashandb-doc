...
- In YAC/Distributed Cluster Deployment, you can use the built-in SWAP tablespace (named SWAP), or create a custom SWAP tablespace. The custom-created SWAP tablespace can be further divided into a SWAP tablespace shared by all instances and a local SWAP tablespace exclusive to each instance.
...
> **Note**: 
>
> In YAC/Distributed Cluster Deployment, the SWAP tablespace only supports adding files with YFS paths; the local SWAP tablespace supports adding files with either local disk paths or YFS paths, but a single tablespace must not contain both YFS paths and local disk paths simultaneously.
...
In YAC/Distributed Cluster Deployment, local SWAP tablespace data file resizing cannot be performed across instances, i.e., only resize local files on the server hosting the current instance.
...
> **Note**: 
>
> In YAC/Distributed Cluster Deployment, all instances must be online to delete files.
...
> **Note**: 
>
> In YAC/Distributed Cluster Deployment, all instances must be online to delete the SWAP tablespace.
