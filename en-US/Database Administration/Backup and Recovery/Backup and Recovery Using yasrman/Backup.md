...
- Full Database Backup: Applicable to all deployment types; the commands and usage for Standalone Deployment and YAC Deployment are identical.
...
- Backup Archive Log Files: Applicable to both Standalone Deployment and YAC Deployment, with identical commands and usage.
...
- In a ISC Distributed Cluster Deployment, if any node experiences a backup exception during the backup process, the backups of all nodes will fail.
...
Backing up archive log files is applicable to both Standalone Deployment and YAC Deployment.
