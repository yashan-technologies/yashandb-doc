...
- **Restore Archive Log Files**: This applies to both Standalone Deployment and YAC Deployment, with completely consistent commands/usage. For more details, see [Archive Recovery](#rmanArchiveFileRestore).
...
| [Restore Archive Log Files](#rmanArchiveFileRestore) | Use in the PITR process which involves restoring archives in Standalone Deployment or YAC Deployment, automatically executed by *yasrman*, manual restoration of archive log files is not necessary | Archive log files backup set                           |
...
- In YAC Deployment, database recovery operations can only be performed on the primary instance (the instance whose INSTANCE_ROLE field in the V$INSTANCE view is MASTER_ROLE).
...
In a YAC Deployment, the order of archive generation across instances is not consistent, meaning that the ASN for archives at the same time across different instances may vary. When using archive recovery operations, it is advisable to specify SCN or TIME range to limit recovery to avoid missing logs.
...
- In YAC Deployment, archive recovery operations can only be performed on the primary instance (the instance whose INSTANCE_ROLE field in the V$INSTANCE view is MASTER_ROLE).
