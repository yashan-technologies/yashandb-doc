Executing recovery operations via SQL statement is applicable to databases in Standalone/YAC/Distributed Cluster Deployment.

Executing recovery operations require the target database and the source database of the backup set to be compatible versions (i.e., the first three components of the version number must be identical).
