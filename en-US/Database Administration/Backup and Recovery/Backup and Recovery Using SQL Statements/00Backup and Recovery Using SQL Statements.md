...
Backup and recovery operations executed via SQL statements are suitable for databases in Standalone Deployment or YAC Deployment.
...
1. Check and enable the database's archiving mode.
      ...
      LOG_MODE being ARCHIVELOG indicates archiving mode, and you can directly execute the subsequent operations. If LOG_MODE is NOARCHIVELOG, it indicates non-archiving mode, and you need to first [switch the database to archiving mode](../../Basic Database Management/Archive Management).
...
Backup and recovery operations executed via SQL statements for archive log files are suitable for databases in Standalone Deployment or YAC Deployment.
...
Besides specifying a SEQUENCE range, it is also possible to specify time or SCN ranges. For details, please refer to [BACKUP ARCHIVELOG](../../../Development Guide/SQL Reference Manual/SQL Statements (yashan Mode)/BACKUP ARCHIVELOG).
