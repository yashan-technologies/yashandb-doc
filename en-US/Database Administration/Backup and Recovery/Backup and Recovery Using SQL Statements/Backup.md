...
During the backup process, the V$BACKUP_PROGRESS view can be used to view the real-time backup progress, and the $YASDB\_DATA/log/run/run.log runtime log can be consulted for detailed logs of the backup process.
...
- In YAC/Distributed Cluster Deployment, only one instance can perform backup operations at any one time.
