...
- When starting a PDB, the optional `-node` parameter of the `ycsctl start pdb` command can be used to specify which subset or all servers the PDB should run on.

- During peak load periods, you can optionally temporarily shut down some PDB instances on high-load servers (using the `ycsctl stop pdb` command), for example, allowing certain low-peak PDBs to run only on a single instance.
...
For the CDB in YAC/Distributed Cluster Deployment, can only start the target PDB from the closed state to the MOUNT stage.
...
The target PDB can only be started from closed state or NOMOUNT stage to MOUNT stage.
...
Only applicable to the CDB in YAC/Distributed Cluster Deployment and can only start the target PDB from the closed state to the MOUNT stage.
...
Only applicable to the CDB in YAC/Distributed Cluster Deployment and can only start the target PDB from the closed state to the OPEN stage.

```shell
# If the -node parameter is not specified, the PDB will run on all servers in the cluster by default.
