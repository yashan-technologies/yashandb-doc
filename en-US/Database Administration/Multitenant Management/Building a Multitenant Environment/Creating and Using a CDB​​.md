...
- The currently installed YashanDB version is v23.5 or above. For Primary-Standby Cluster Deployment, the version should be v23.5.2 or above.

- YashanDB is deployed in Standalone Deployment, YAC Deployment or Distributed Cluster Deployment.

- The DROP DATABASE statement has been executed to [delete the current database](../../Instance Management/Database Dropping).
...
3. If `instance_status=close`, the database needs to be [started](../../Instance Management/Instance Startup and Shutdown) to NOMOUNT state.
...
In Standalone Deployment, CDB startup and shutdown is equivalent to [instance startup and shutdown](../../Instance Management/Instance Startup and Shutdown), which will simultaneously start or stop the CDB root and all PDBs.

In YAC/Distributed Cluster Deployment, CDB startup and shutdown is equivalent to [cluster startup and shutdown](../../Cluster Management/Cluster Startup and Shutdown), which will simultaneously start or stop the CDB root and all PDBs.
...
In subsequent daily usage, container information and status can be viewed through [V$CONTAINERS](../../../Reference Manual/System Views/Dynamic Performance Views/V$CONTAINERS)/[GV$CONTAINERS](../../../Reference Manual/System Views/Dynamic Performance Views/GV$CONTAINERS) and [V$PDBS](../../../Reference Manual/System Views/Dynamic Performance Views/V$PDBS)/[GV$PDBS](../../../Reference Manual/System Views/Dynamic Performance Views/GV$PDBS).
