...
2. Execute the [ycsctl add pdb](../../../Tools Guide/ycsctl/Cluster Management Commands.md#addpdb) command to add a PDB to the CDB cluster.
   ...
   - The optional `-policy` parameter can be used to specify the PDB's startup strategy — whether it follows the CDB root's startup.

   - The optional `-startoption` parameter can be used to specify the default startup stage option for the PDB; if no startup option is specified when starting the PDB subsequently, the value specified here is directly reused as the default.
