YashanDB resource management provides the ability to configure physical resources using the built-in advanced package [DBMS_RESOURCE_MANAGER](../../Development Guide/PL Reference Manual/Built-in Advanced PL Packages/DBMS_RESOURCE_MANAGER) and related configuration parameters.
...
If the corresponding user does not exist, please create the user first. Refer to [Create User](../../Product Security/Identity Identification and Authentication/User/Managing Users (yashan Mode)) for specific operations.
...
In a ISC Distributed Cluster Deployment scenario, execution retry conditions can be monitored using the following statement.
...
>**Note**:
>
>- When [deleting users](../../Product Security/Identity Identification and Authentication/User/Managing Users (yashan Mode)), the user's mapping relationships will be deleted.
