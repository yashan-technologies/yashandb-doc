...
Archiving can extend the retention time of the database's redo history operation records, which is used for [backup and recovery](../Backup and Recovery/00Backup and Recovery) to ensure data security and primary/standby synchronization during high availability deployment.
...
After the archiving mode is enabled, database administrators can also adjust automatic cleanup strategies, archive paths, and other related configurations or perform [archive log files management](../File Management/Archive Log File Management) based on actual business needs and disk planning.

### Prerequisites

- To enable archiving mode, the database needs to be started/restarted to the MOUNT state; please schedule the operation time reasonably.

- The current archiving mode must be checked [here](#check) to ensure it is not enabled (i.e., LOG_MODE=NOARCHIVELOG in the V$DATABASE view).
...
If the path is adjusted after enabling the archiving mode, the archive log files that already exist in the original path will remain unchanged. They can be automatically or manually cleaned based on the cleanup rules. For cleanup operations, please refer to [archive log files management](../File Management/Archive Log File Management.html#cleanup).
...
4. Restart the database (or instance) to the MOUNT state.
...
In YAC Deployment, to disable the archiving mode, only one instance needs to be in the MOUNT state while all other instances should be in the OFF state.
...
5. Connect and log in to the database as a DBA user.
    ```shell
    $ yasql sales/********@192.168.1.2:1688
    YashanDB SQL Enterprise Edition Release {version} x86_64
...
7. View the current archive path configuration.
...
If YashanDB's default automatic cleanup policy does not meet actual needs, database administrators can adjust the corresponding parameters as needed or temporarily perform [manual cleanup](../File Management/Archive Log File Management.html#cleanup_manual).
...
- In Standalone Deployment, if a [permanent restore point](../../Development Guide/SQL Reference Manual/SQL Statements (yashan Mode)/CREATE RESTORE POINT) (CREATE RESTORE POINT … GUARANTEE FLASHBACK DATABASE) has been created, it must also be earlier than the permanent restore point: the NEXT_CHANGE# field value of the corresponding archive log files in the [V$ARCHIVED_LOG](../../Reference Manual/System Views/Dynamic Performance Views/V$ARCHIVED_LOG) view must be less than or equal to the SCN field of the restore point in the [V$RESTORE_POINT](../../Reference Manual/System Views/Dynamic Performance Views/V$RESTORE_POINT) view where IS_GUARANTEE is 1.
...
Manual archiving ([ALTER SYSTEM ARCHIVE LOG CURRENT](../../Development Guide/SQL Reference Manual/SQL Statements (yashan Mode)/ALTER SYSTEM.html#archive_logfile)) can only archive redo log files that are in the CURRENT state. During the archiving process, the redo log files are also switched. For specific operations, refer to [switching and archiving redo log files](../File Management/Redo Log File Management.html#SWITCH).

The archive log files generated through manual archiving are managed the same way as those generated through automatic archiving, and are subject to the same automatic cleanup policy. They can be managed as needed, following [file management](../File Management/Archive Log File Management).
...
- High availability deployments, such as primary/standby standalone deployment or primary/standby cluster deployments.

- Use of [YStream](../../Development Guide/YStream Reference Manual/00YStream Reference Manual).

### Prerequisites

- To disable archiving mode, the database needs to be started/restarted to the MOUNT state; please schedule the operation time reasonably.
...
3. Adjust the database archiving mode.
    
    ```sql
    ALTER DATABASE NOARCHIVELOG;
    ```

4. Check the archiving mode again to confirm the modification has taken effect.
