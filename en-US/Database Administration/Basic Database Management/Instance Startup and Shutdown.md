...
This document mainly introduces the instances of starting Standalone Deployment and ISC Distributed Cluster Deployment. For information on starting YAC instances, please refer to [YAC Startup and Shutdown](../../YashanDB for Cluster/Yashan Cluster Service/YAC Startup and Shutdown).
...
*   **MOUNT**: Start the database instance, read the control file, load the database, but the database is in a closed state.
...
- **RESETLOGS**: When a PITR (Point In Time Recovery), database flashback, or logical standby database configuration has occurred, and if a complete recovery cannot be performed, the database must be opened in RESETLOGS mode. This mode will reset the redo log sequence number.
  ...
  > **Caution**: 
  >  
  > - In practical use, it is recommended to open the physical standby database in READONLY mode for primary/standby standalone deployments; this mode is not applicable for distributed, YAC Deployment.  
...
- SQL statement: Connect to the database with the sys user and use the ALTER DATABASE statement to adjust the database instance from the NOMOUNT stage to the OPEN stage, or a user with dba privilege can use the ALTER DATABASE statement to adjust the database instance from the MOUNT stage to the OPEN stage.
...
```shell
# Here, an example is shown with the dbaPrivilege user sales to start the database from the MOUNT stage to the OPEN stage:
$ yasql sales/********@192.168.1.2:1688
...
This document mainly introduces the closing operations of Standalone Deployment and ISC Distributed Cluster Deployment instances. For information about closing YAC instances, refer to [YAC Startup and Shutdown](../../YashanDB for Cluster/Yashan Cluster Service/YAC Startup and Shutdown).
...
1. Create and edit the service startup script.
   ...
   YASDB_HOME=/data/yashan/yasdb_home/{version}   # YashanDB HOME directory
...
## Handling Failed Startup Scenarios
...
- **Control file actual path does not match the CONTROL_FILES parameter configured path**
  ...
  In such cases, the solutions are as follows:

  - Method 1: Modify the control file path.
...
- **Database version in the control file is inconsistent with the instance version**
  ...
  In this case, starting the instance will prompt an error *database version x.x.x incompatible with YashanDB version x.x.x*. Choose whether to perform a version upgrade based on the actual situation.

- **Database ID of control file group members is inconsistent**

  This situation usually occurs when multiple database instances exist on a single server, and the CONTROL_FILES parameter is configured with control files of different databases. Modify it to the correct control file path.
...
- **Built-in tablespace data file missing or corrupted**
  ...
  In this case, the latest available backup set can usually be restored through [recovery](../Backup and Recovery/00Backup and Recovery); if no backup set exists, recovery can be achieved through file recovery or physical recovery, etc.

- **User data file missing or corrupted**

  When the data file mounted for the user tablespace is missing or corrupted, it can usually be resolved by restoring the latest available backup set through [recovery](../Backup and Recovery/00Backup and Recovery). If no backup set exists, emergency handling can be done using the emergency channel provided by YashanDB to ensure the database can start, open, and run normally. For specific operations, please refer to [Emergency Handling After Data File Corruption](../File Management/Data File Management.html#EH).
...
In this case, the latest available backup set can usually be restored through [recovery](../Backup and Recovery/00Backup and Recovery).
