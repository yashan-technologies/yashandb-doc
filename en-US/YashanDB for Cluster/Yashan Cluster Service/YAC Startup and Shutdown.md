...
The startup and shutdown of YAC include two modes:

- Starting or stopping a specific server through the *ycsctl* command.
- Starting or stopping the entire cluster through the *yasboot* command.

When using the *ycsctl* command to start or stop a server, the following environment variable must be correctly configured in the system:

- $YASCS_HOME: The YCS HOME directory corresponding to the server, which contains shell scripts for managing external resources.

## Process System

A YAC server needs to run cluster service processes and database processes to provide cluster database services. The cluster service processes include yascsm (YCSM), yascs (YCS), and *ycsrootagent* (YCSRA).
...
YCS controls the management of the operation of the entire YAC. The startup and shutdown status is displayed according to the resources it manages, including:
...
YCSRA does not participate in the above startup and shutdown process. When the cluster is configured with services such as I/O Fencing, which must interact with the OS and hardware devices under root privilege, the user needs to manually start YCSRA using sudo during cluster deployment and server boot; if the YCSRA process is abnormal, the user also needs to manually restart it using sudo.
...
In addition, when AUTO_START is configured to ALWAYS (i.e., simultaneously starting the database instance on this server), the -m parameter can be used to specify the database instance's startup mode.
...
The command `ycsctl start instance <-m mode>` can be used to start the database instance on the server individually. The -m parameter can be used to specify the database instance startup status; if not specified, it defaults to starting in OPEN status.
...
The *yasboot* tool can be used to start and stop the entire YAC, which is equivalent to performing the "Start YCS" and "Stop YCS" operations on each server in the cluster.
...
When using the *yasboot* tool to start or restart YAC, starting in mount status is currently not supported.
