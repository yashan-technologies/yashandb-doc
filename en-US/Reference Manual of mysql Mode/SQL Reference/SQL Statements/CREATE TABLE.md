...
- Support BTree index; and HASH index is used for compatibility and have no practical meaning;
...
When configuring the CHARACTER SET and COLLATE in the configuration table, the configured values need to be compatible with the database server's configuration parameters at deployment time; otherwise, the table creation statement will result in an error.
