...
```ebnf
= change_column_clause|modify_column_clause|add_column_clause|rename_column_clause|drop_column_clause.
```
...
```ebnf
= ADD PARTITION (((PARTITION [partname] add_range_partition_clause)
...
```ebnf
= range_values_clause [partition_storage_clause]
[("(" (
...
```ebnf
= [partition_storage_clause] "(" partition part_name ")" .
```

**[partition\_storage\_clause](#partitionstorageclause)::=**

```ebnf
= ((TABLESPACE tablespace_name) | ([STORAGE] ENGINE ["="] engine_name) | (COMMENT ["="] comment) | (DATA DIRECTORY ["="] data_directory) | (INDEX DIRECTORY ["="] index_directory) | (MAX_ROWS ["="] max_rows) | (MIN_ROWS ["="] min_rows)) .
```
...
```ebnf
= drop_index_clause|add_index_clause.
```

**[add\_index\_clause](#addindexclause)::=**

```ebnf
= ADD ((UNIQUE [KEY])|(PRIMARY KEY)|INDEX|KEY) index_name"(" index_expr [DESC|ASC]{ "," index_expr [DESC|ASC]} ")" [index_type].
```
The meanings and usage of index_expr and index_type are introduced in [index_expr](CREATE INDEX.md#indexexpr) and [index_type](CREATE INDEX.md#indextype) in CREATE INDEX.
...
```ebnf
= add_constraint_clause|drop_constraint_clause.
```

**[add\_constraint\_clause](#addconstraintclause)::=**

```ebnf
= ADD CONSTRAINT  constraint_name ((UNIQUE "(" (column_name {"," column_name}) ")")|(PRIMARY KEY "(" (column_name {"," column_name}) ")")|(FOREIGN KEY "(" (column_name {"," column_name}) ")" references_clause)) .
```

**[drop\_constraint\_clause](#dropconstraintclause)::=**

```ebnf
= DROP ((PRIMARY KEY)|(FOREIGN KEY forgien_constraint_name)|(CONSTRAINT constraint_name)) .
```
...
This statement is used to rename the table.

Renaming a table requires ALTER, CREATE, DROP, and INSERT privileges.

Renaming multiple tables simultaneously is not supported.

Renaming a table to another schema is not supported.

`ALTER TABLE RANAME old_table_name [TO | AS] new_table_name` and `RENAME TABLE old_table_name TO new_table_name` have the same meanings.
...
<span id="changecolumnclause" name="changecolumnclause"></span>
...
<span id="conDefdesc" name="conDefdesc"></span>

#### column\_definition

This statement is used to define the columns of the table, including data types, default values,  [inline_constraint](../General SQL Syntax/constraint), and various column attributes such as CHARACTER SET, COLLATE, COMMENT, and AUTO_INCREMENT.



**ZEROFILL**

For numeric types, the ZEROFILL keyword can be specified. It is used for compatibility and have no practical meaning.
...
The BINARY keyword allows specifying a collation for columns of type CHAR, VARCHAR, and TEXT.

- When the `BINARY` keyword is specified without an explicit `COLLATE collation_name`, the column's collation is set to the _BIN collation corresponding to its character set.
- When both `BINARY` and `COLLATE collation_name` are specified, the column's collation is set to the _BIN collation of the character set associated with the specified collation_name.

Syntax constraints for the BINARY keyword are as follows:

- The BINARY keyword cannot be repeated.
- BINARY cannot appear after a constraint clause.
- BINARY cannot appear after the COLLATE clause.
- BINARY cannot appear after the DEFAULT clause.
- BINARY cannot appear after the COMMENT clause.
- BINARY cannot coexist with AUTO_INCREMENT.
...
- An auto-increment column must be of a numeric type. The INT, DOUBLE, and FLOAT types are supported for configuring auto-increment columns.
...
- The auto-increment step is fixed at 1.
...
- An auto-increment column cannot be configured with a default value.

**ON UPDATE CURRENT_TIMESTAMP**

This attribute can be configured to define an automatic update mechanism for the corresponding column values. Once configured, the column value is automatically updated to the current time whenever data in other columns of the same row is modified.

When the system variable `EXPLICIT_DEFAULTS_FOR_TIMESTAMP` set to FALSE (the default value), even if the attribute is not explicitly specified in the table definition, it is implicitly added to the first TIMESTAMP columns in the table. After setting the system variable to TRUE, the ON UPDATE CURRENT_TIMESTAMP attribute must be explicitly specified.

The usage rules for this parameter are as follows:

- When the column data type is TIMESTAMP or DATETIME, this attribute can be configured to define an automatic update mechanism for the corresponding column values: once configured, the column value is automatically updated to NOW() whenever data in other columns of the same row is modified.

- In statements, CURRENT_TIMESTAMP can be replaced with its synonyms: CURRENT_TIMESTAMP(), NOW(), LOCALTIME, LOCALTIME(), LOCALTIMESTAMP, or LOCALTIMESTAMP().



##### dataType

Config the data type of the column field to one recognized by YashanDB [data types](../Data Types/Data Types).
...
<span id="alterindexclause" name="alterindexclause"></span>

### alter\_index\_clause

This statement is used to add or drop indexes for the table.

<span id="addindexclause" name="addindexclause"></span>
...
<span id="dropindexclause" name="dropindexclause"></span>

#### drop\_index\_clause

This statement is used to drop indexes from the table.

<span id="alterconstraintclause" name="alterconstraintclause"></span>

### alter\_constraint\_clause

This statement is used to add or drop constraints for the table.

<span id="addconstraintclause" name="addconstraintclause"></span>

#### add\_constraint\_clause

This statement is used to add constraints to the table.

<span id="dropconstraintclause" name="dropconstraintclause"></span>
