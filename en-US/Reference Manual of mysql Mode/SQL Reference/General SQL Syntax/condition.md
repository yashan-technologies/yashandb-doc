...
The RLIKE syntax uses a regular expression matching algorithm to match the character expression expr with the regular expression regexp, returning TRUE if matched successfully, or FALSE if not. Its complete syntax form is:
