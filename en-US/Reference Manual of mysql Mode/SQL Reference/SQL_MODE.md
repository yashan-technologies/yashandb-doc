...
|NO_BACKSLASH_ESCAPES        |Controls whether the backslash character `\` is treated as an escape character in strings and identifiers. When enabled: Backslashes become ordinary characters (like other letters/numbers), and the default escape sequence in LIKE expressions changes (no longer uses `\`).  |NO      |
...
|ONLY_FULL_GROUP_BY          |This option requires enabling strict compliance with SQL standard GROUP BY syntax, demanding that all columns appearing in the SELECT clause either be aggregate functions or must appear in the GROUP BY clause. After disabling this option, it supports allowing attribute columns not in the GROUP BY list to appear in the query columns, HAVING list and ORDER BY columns.  |YES      |
...
|STRICT_ALL_TABLES           |The strict mode for configuring column consistency requires that after configuration, it is not allowed to insert NULL into numerical type data columns.  |YES      |
|STRICT_TRANS_TABLES         |The strict mode for configuring column consistency requires that after configuration, it is not allowed to insert NULL into numerical type data columns.    |YES      |
...
2. Create/edit the my.ini file, save, and exit.
    ...
    SQL_MODE = ANSI_QUOTES,ERROR_FOR_DIVISION_BY_ZERO,IGNORE_SPACE,NO_AUTO_CREATE_USER,NO_AUTO_VALUE_ON_ZERO,NO_ENGINE_SUBSTITUTION,NO_UNSIGNED_SUBTRACTION,NO_ZERO_DATE,NO_ZERO_IN_DATE,ONLY_FULL_GROUP_BY,STRICT_ALL_TABLES,STRICT_TRANS_TABLES,TIME_TRUNCATE_FRACTIONAL
    ```
...
4. Establish a new database connection using MySQL Client and check the sql_mode parameter value.
    ...
    | @@global.sql_mode                                                                                                                                                                                                                                                                |
    +----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
    | ANSI_QUOTES,ERROR_FOR_DIVISION_BY_ZERO,IGNORE_SPACE,NO_AUTO_CREATE_USER,NO_AUTO_VALUE_ON_ZERO,NO_ENGINE_SUBSTITUTION,NO_UNSIGNED_SUBTRACTION,NO_ZERO_DATE,NO_ZERO_IN_DATE,ONLY_FULL_GROUP_BY,STRICT_ALL_TABLES,STRICT_TRANS_TABLES,TIME_TRUNCATE_FRACTIONAL |
    +----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
    1 row in set (0.00 sec)
