...
Description: Wait for response events during interaction between executing across servers in a YAC/Distributed Cluster.

Trigger Condition: Occurs when executing cross-server queries in YAC/Distributed Cluster, such as GV views, while control messages are sent between servers.
...
Trigger Condition: Wait occurs when YAC/Distributed Cluster fails to obtain a lock during the invalidation of hot pages in the index buffer.
...
<span id="cwait" name="cwait"></span>

### check xa end

Description: Waiting for the second phase commit of Distributed Cluster transactions.
...
Description: Waiting during internal remote procedure calls.

Trigger Condition: Waiting for the remote instance to respond during internal remote procedure calls executed in a multi-instance environment.

### dstb alter session

Description: CN is waiting for other nodes to complete alter session operations.

Trigger Condition: Waiting occurs during execution of Distributed Cluster alter session.

### dstb alter system

Description: CN is waiting for other nodes to complete alter system operations.

Trigger Condition: Waiting occurs during execution of Distributed Cluster alter system.
...
Trigger Condition: Waiting occurs during execution of Distributed Cluster DDL.

### dstb execute ddl

Description: CN is waiting for other nodes to execute DDL commands.

Trigger Condition: Waiting occurs during the execution of Distributed Cluster DDL.

### dstb lob data request

Description: Distributed Cluster nodes are waiting for the requested LOB data to be sent.
...
Trigger Condition: Waiting occurs during global resource release after failure to request parallel execution resources in a Distributed Cluster environment.
...
Trigger Condition: This wait is triggered during various rollback operations in Distributed Cluster environments.
...
### execute rsrc mgr ddl

Description: Waiting for other instances to complete modifications to resource management-related system tables.

Trigger Condition: Cluster instance mutual exclusion, waiting for other instances to complete modifications to resource management system tables.
...
<span id="fwait" name="fwait"></span>

### flashback db cluster lock

Description: Waiting during database flashback operations in a YAC/Distributed Cluster when acquiring locks.

Trigger Condition: Triggering database flashback-related operations in a YAC/Distributed Cluster.

### flashback db free space

Description: Waiting during the resource cleanup for database flashback.

Trigger Condition: This wait is triggered when resource usage exceeds a threshold during database flashback.

### flashback db read data

Description: Waiting for reading flashback logs during database flashback.

Trigger Condition: Waiting occurs while scanning flashback logs during database flashback.

### flashback db wait fb log flush

Description: Waiting for flashback logs to be written to disk during page transmission in a YAC/Distributed Cluster.

Trigger Condition: Page flow activation after enabling flashback in a YAC/Distributed Cluster.
...
Description: Waiting for writing flashback logs during database flashback.

Trigger Condition: This wait is triggered when the business operations generate flashback logs after enabling database flashback functionality and writing to flashback log files.
...
<span id="gwait" name="gwait"></span>

### gc boc wait

Description: A YAC/Distributed Cluster waiting event for SCN synchronization.

Trigger Condition: Waiting for SCN transmission.

### gc buffer busy acquire

Description: A YAC/Distributed Cluster waiting event where the requested buffer is in a globally busy state, and the buffer is currently being requested by other sessions of this instance from remote instances.

Trigger Condition: The buffer is currently being requested from remote instances by other sessions of this instance.

### gc buffer busy release

Description: A YAC/Distributed Cluster waiting event where the requested buffer is in a globally busy state, and the buffer is held by other sessions with shared or exclusive locks.

Trigger Condition: The buffer is held by other sessions with shared or exclusive locks.

### gc cr block 2 way

Description: A YAC/Distributed Cluster waiting event where the requested consistent read page is transmitted from another instance in YAC/Distributed Cluster, involving 2 network hops.

Trigger Condition: Requests involving 2 network hops.

### gc cr block 3 way

Description: A YAC/Distributed Cluster waiting event where the requested consistent read page is transmitted from another instance in YAC/Distributed Cluster, involving 3 network hops.

Trigger Condition: Requests involving 3 network hops.

### gc cr block grant

Description: A YAC/Distributed Cluster waiting event where the requested consistent read page is authorized by this instance to load from disk.

Trigger Condition: Authorization from this instance to load from disk.

### gc cr block grant 2 way

Description: A YAC/Distributed Cluster waiting event for the requested consistent read page, authorized by other instances to load from disk.

Trigger Condition: Authorization from other instances to load from disk.

### gc cr request

Description: A YAC/Distributed Cluster waiting event for requesting consistent read pages.

Trigger Condition: Request for consistent read pages.

### gc current block 2 way

Description: A YAC/Distributed Cluster waiting event where the requested latest page is transmitted from another instance in YAC/Distributed Cluster, involving 2 network hops.

Trigger Condition: Requests involving 2 network hops.

### gc current block 3 way

Description: A YAC/Distributed Cluster waiting event where the requested latest page is transmitted from another instance in YAC/Distributed Cluster, involving 3 network hops.

Trigger Condition: Requests involving 3 network hops.

### gc current block busy

Description: A YAC/Distributed Cluster waiting event where the instance is accessing a page that is held by other sessions of the same instance.

Trigger Condition: Accessing a page that is held by other sessions.

### gc current block request

Description: A YAC/Distributed Cluster waiting event for requesting the latest page.

Trigger Condition: Request for the latest page.

### gc current grant

Description: A YAC/Distributed Cluster waiting event where the requested latest page is authorized by this instance to load from disk.

Trigger Condition: Authorization from this instance to load from disk.

### gc current grant 2 way

Description: A YAC/Distributed Cluster waiting event where the requested latest page is authorized by other instances to load from disk.

Trigger Condition: Authorization from other instances to load from disk.

### gc flush block

Description: A YAC/Distributed Cluster waiting event where the owner instance is flushing data while sending the latest page.

Trigger Condition: Waiting occurs when the current redo size exceeds the expected recovery size while sending the page.

### gc invalid block

Description: A YAC/Distributed Cluster waiting event where an invalid block is held by an active page of an instance.

Trigger Condition: An invalid block is held by an active page.

### gc lwlock S

Description: A YAC/Distributed Cluster waiting event requesting a gls shared lock.

Trigger Condition: Request for a gls shared lock.

### gc lwlock X

Description: A YAC/Distributed Cluster waiting event requesting a gls exclusive lock.

Trigger Condition: Request for a gls exclusive lock.

### gc mutex

Description: A YAC/Distributed Cluster waiting event requesting a mutex lock.

Trigger Condition: Request for a mutex lock.

### gc recycle block

Description: YAC/Distributed Cluster wait event, gcs lock memory reclamation.

Trigger Condition: When data block cache is evicted, the corresponding GCS lock permissions need to be immediately evicted, or generate orphaned GCS locks that are evicted by background threads at some point.

### gc recycle lock

Description: A YAC/Distributed Cluster waiting event for memory reclamation with gls locks.

Trigger Condition: Request for memory allocation when local lock memory reaches its limit.

### gc send block

Description: A YAC/Distributed Cluster waiting event where the owner instance sends the latest page.

Trigger Condition: Owner instance sending the latest page.

### gc upgrade block 2 way

Description: A YAC/Distributed Cluster waiting event requesting a page lock upgrade, requiring other instances to invalidate the page, involving 2 network hops.

Trigger Condition: Requests involving 2 network hops.

### gc upgrade block 3 way

Description: A YAC/Distributed Cluster waiting event requesting a page lock upgrade, requiring other instances to invalidate the page, involving 3 network hops.

Trigger Condition: Requests involving 3 network hops.

### gc upgrade grant

Description: A YAC/Distributed Cluster waiting event with no other instances holding valid pages.

Trigger Condition: Request for page lock upgrade.

### gc upgrade request

Description: A YAC/Distributed Cluster waiting event requesting page lock upgrade.

Trigger Condition: Request for page lock upgrade.

### gcs lock busy wait

Description: YAC/Distributed Cluster wait event, GCS lock busy waiting.

Trigger Condition: Requesting page lock escalation.

### gcs lock free wait

Description: YAC/Distributed Cluster wait event, proactive elimination and release of GCS lock.

Trigger Condition: Requesting page lock escalation.

### get remote xact info

Description: A YAC/Distributed Cluster waiting event to acquire remote transaction information.

Trigger Condition: Querying transaction information.

### get remote xact status

Description: A YAC/Distributed Cluster waiting event to acquire remote transaction status.

Trigger Condition: Querying transaction status.

<span id="hwait" name="hwait"></span>
...
Description: A YAC/Distributed Cluster waiting event for concurrent table partition extension.

Trigger Condition: Triggered when concurrent interval partition extensions occur on the same table in YAC/Distributed Cluster.

<span id="kwait" name="kwait"></span>
...
<span id="lwait" name="lwait"></span>

### lbac policy

Description: A YAC/Distributed Cluster waiting event for constrained LBAC concurrency control.

Trigger Condition: Executing actions such as opening/closing LBAC switches, creating/deleting LBAC policies, creating/deleting LBAC tags, creating/deleting LBAC policy association tables, creating/deleting LBAC associated users in a YAC/Distributed Cluster.
...
<span id="pwait" name="pwait"></span>

### profile create

Description: A YAC/Distributed Cluster waiting event for creating a profile.

Trigger Condition: Concurrently creating profiles.

<span id="qwait" name="qwait"></span>
...
Trigger Condition: In YAC/Distributed Cluster mode, the standby database receives redo from multiple instances, and if the dependent redo has not arrived, it will wait.
...
### redo traffic control

Description: Wait caused by redo traffic control.

Trigger Condition: When redo traffic control is enabled and the redo volume reaches a certain threshold.
...
Description: A YAC/Distributed Cluster waiting event for row lock conflicts between two instances.
...
Description: When cross-instance transaction conflicts occur, after the original instance's transaction commits, the instance requesting to acquire locks on the data will trigger a consistency read check wait event for the transaction management unit from the original instance.

Trigger Condition: When instance 1's transaction A acquires a row lock without committing, and instance 2's transaction B waits for the same row lock, transaction B triggers the check after transaction A commits in instance 1.

### role create

Description: A YAC/Distributed Cluster waiting event for creating roles.
...
Description: A YAC/Distributed Cluster waiting event for segment creation.
...
Description: Waiting for obtaining global mutex lock while setting global statistics options in a YAC/Distributed Cluster.
...
Description: A YAC/Distributed Cluster waiting event, waiting for extent allocation.
...
Description: In Distributed Cluster environments, CN receives statistics from DN via channel.

Trigger Condition: Triggered during statistics collection in a Distributed Cluster environment.
...
Description: A YAC/Distributed Cluster waiting event for creating users.

Trigger Condition: Concurrently creating users.

<span id="wwait" name="wwait"></span>

### wait distributed result

Description: CN waiting for response times from various nodes.

Trigger Condition: This time will be recorded for any Distributed Cluster execution involved.
...
<span id="xwait" name="xwait"></span>

### xa commit

Description: CN waits for all nodes to complete the commit phase of the two-phase commit.

Trigger Condition: Triggered during transaction commits involving multiple DNs in Distributed Cluster processing.
