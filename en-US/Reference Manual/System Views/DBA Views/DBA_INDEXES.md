...
| INDEX_TYPE         | VARCHAR(25)   | Index type<br>*   NORMAL: BTree index<br>*   NORMAL/REV: BTree Reverse index<br>*   FUNCTION-BASED NORMAL: Function BTree index<br>*   FUNCTION-BASED NORMAL/REV: Function BTree Reverse index<br>*   LOB: Lob index<br>*   COLUMNAR: Columnar index<br>*   RTREE: RTree index<br>*  BITMAP: BITMAP index<br>*   VECTOR: Vector index<br>*   SEARCH: Full-text index |
...
| BLEVEL             | INTEGER       | Height of the BTree<br/>It has a value only after the statistics of the object have been collected |
| LEAF_BLOCKS        | BIGINT        | Number of leaf blocks<br/>It has a value only after the statistics of the object have been collected |
| DISTINCT_KEYS      | BIGINT        | Number of unique key values<br/>It has a value only after the statistics of the object have been collected |
| AVG_LEAF_BLOCKS_PER_KEY | BIGINT   | Average number of leaf blocks per key value<br/>It has a value only after the statistics of the object have been collected |
| AVG_DATA_BLOCKS_PER_KEY | BIGINT   | Average number of blocks per key value<br/>It has a value only after the statistics of the object have been collected |
| STATUS             | VARCHAR(8)    | Status<br>*   UNUSABLE: Unusable<br>*   VALID: Usable<br>*   N/A: Partition index, check status in DBA_IND_PARTITIONS |
| NUM_ROWS           | BIGINT        | Number of rows in the index<br/>It has a value only after the statistics of the object have been collected |
| SAMPLE_SIZE        | BIGINT        | Number of rows sampled for statistics<br/>It has a value only after the statistics of the object have been collected |
| LAST_ANALYZED      | DATE          | Time when statistics were last collected<br/>It has a value only after the statistics of the object have been collected |
...
| DATABASE_MAINTAINED| VARCHAR(1)    | Whether it is DATABASE MAINTAINED<br>*   Y: Index created by the database system<br>*   N: Index created by the user |
...
| ANSI_NULL | VARCHAR(1)  | Whether the handling of NULL values by indexes comply with the ANSI standard<br>*   Y: compliant<br>*   N: non-compliant |
| NULL_FIRST | VARCHAR(1)  | Whether the handling of NULL values by indexes place NULL values at the top<br/>\*   Y：Yes<br/>\*   N：No |
