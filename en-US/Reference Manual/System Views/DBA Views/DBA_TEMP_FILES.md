...
| --- |--------------| --- |
| FILE\_ID        | INTEGER        | Temporary File ID |
| FILE\_NAME      | VARCHAR(255)   | Temporary File Name |
...
| RELATIVE\_FNO   | INTEGER        | Internal number within the tablespace |
...
| TABLESPACE\_NAME| VARCHAR(64)    | Name of the tablespace to which the temporary file belongs |
| MAXBYTES        | BIGINT         | Maximum file size (Unit: Bytes) |
| MAXBLOCKS       | NUMBER         | Maximum number of blocks the file can contain |
| INCREMENT\_BY   | NUMBER         | Default increment for automatic extension (Unit is 1 block) |
| USER\_BYTES     | BIGINT         | Currently available free data size in the temporary file (Unit: Bytes) |
| USER\_BLOCKS    | INTEGER        | Current number of free available blocks in the temporary file |
...
| INST\_ID        | VARCHAR(44)    | Instance ID corresponding to the temporary file in the local temporary tablespace |