...
| AVG\_SPACE | INTEGER | Average free space per block in the table (Unit: bytes)<br/>It has a value only after the statistics of the object have been collected |
| CHAIN\_CNT | BIGINT | Number of row links in the table<br/>It has a value only after the statistics of the object have been collected |
| AVG\_ROW\_LEN | INTEGER | Average row length of the table (Unit: bytes)<br/>It has a value only after the statistics of the object have been collected |
...
| DATABASE_MAINTAINED  | VARCHAR(1)    | Whether it is DATABASE MAINTAINED<br>* Y: table created by the database system<br>* N: table created by the user |
...
| INITIAL_EXTENT       | NUMBER(29)        | The size of the first extent of the segment, in bytes                                          |
| NEXT_EXTENT          | NUMBER(29)        | Compatibility field, no actual meaning, fixed to be the same as INITIAL_EXTENT                |
