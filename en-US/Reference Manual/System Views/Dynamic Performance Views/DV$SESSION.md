...
| GLOBAL\_SESSION\_ID     | INTEGER      | Global session ID in a distributed environment              |
...
| WAIT\_EVENT             | VARCHAR(32)  | Name of the wait event, the field value refers to the description in the [Wait Events](../../Wait Events) document.                       |
...
| CLI\_PROGRAM            | VARCHAR(256) | Client program name (e.g., *yasql*, JDBC Driver, C Driver, Go Driver...)         |
...
| SQL\_HASH\_VALUE        | BIGINT       | SQL hash value of the statement currently being executed by the session (calculated from SQL text)        |
| SQL\_ID                 | VARCHAR(13)  | SQL ID of the statement currently being executed by the session (hash/encryption result of SQL text)      |
...
| IP\_ADDRESS             | VARCHAR(45)  | Client IP address                            |
| IP_PORT                 | INTEGER      | Client port number                      |
| EXEC\_START\_TIME       | TIMESTAMP(6)    | Execution time of the current SQL statement             |
