...
| RUNTIME\_MEM | INTEGER | Memory requested by this SQL during execution (in bytes) (reserved field)|
...
| PX\_SERVERS\_EXECUTIONS | BIGINT | Total number of executions in parallel (reserved field) |
...
| DIRECT\_WRITES | BIGINT | Number of direct writes by the cursor (reserved field) |
| DIRECT\_READS | BIGINT | Number of direct reads by the cursor (reserved field) |
...
| ACTION                   | VARCHAR(64) |   The name of the operation currently executing when the SQL statement was first parsed.                                           |
