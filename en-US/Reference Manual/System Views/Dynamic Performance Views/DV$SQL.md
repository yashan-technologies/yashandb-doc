This view displays the SQL execution statistics for all nodes in a distributed cluster. The same CN executing the same SQL shows one entry, while different CNs executing the same SQL show multiple entries.
