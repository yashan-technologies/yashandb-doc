This view displays the current node's real-time election status when leader election is enabled in the HA architecture of a distributed cluster. When leader election is disabled, this view has no data.
...
| LAST_HEARTBEAT_TIME   | TIMESTAMP | The time when the current node last received a heartbeat |
