This view shows the statistics of each SQL in the shared SQL area across each node in the distributed cluster, including memory consumption, parsing, optimization, and execution information for the SQL in the statement. The same CN executing the same SQL shows one entry, while different CNs executing the same SQL show multiple entries.
...
| ELAPSED_TIME            | BIGINT   | Time spent on parsing, executing, and fetching data; if in a distributed cluster, includes execution time on the DN side (unit: microseconds) |
