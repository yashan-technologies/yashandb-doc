This view displays the current child cursor execution statistics (one for each child cursor).
...
| END\_OF\_FETCH\_COUNT | BIGINT | The number of times the child cursor has completely executed after being brought to the buffer |
...
| DIRECT\_WRITES | BIGINT | The number of direct writes by the child cursor (reserved field) |
| DIRECT\_READS | BIGINT | The number of direct reads by the child cursor (reserved field) |
...
| CPU\_TIME | BIGINT | The CPU usage time for parsing/executing/fetching (unit: microseconds) |
| ELAPSED\_TIME | BIGINT | The elapsed time for parsing/executing/fetching (unit: microseconds) |
...
| IS\_OBSOLETE | VARCHAR(1) | Indicates whether the child cursor is obsolete (Y/N) when the number of child cursors is too many |
