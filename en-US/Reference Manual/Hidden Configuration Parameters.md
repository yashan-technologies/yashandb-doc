...
| _PROCEDURE_LOCK_TIMEOUT                      | The timeout duration for waiting during stored procedure compilation (units: seconds); equal to high value indicates infinite waiting. |
...
| ENABLE_TRAFFIC_CONTROL                       | Whether to enable the DATA BUFFER flow control mechanism. If enabled, it will limit the DATA BUFFER to prevent dirty pages from being produced too quickly. |
...
| _OPTIMIZER_ADAPTIVE_CURSOR_SHARING | Controls whether different parameter value ranges affect plan generation. |
| _HASH_CACHE_THRESHOLD                      | The maximum cache size for the hash table optimized for batch execution of deterministic UDFs. The actual maximum cache size depends on the data distribution and ranges between [parameter_value, parameter_value + 1024]. |