...
1. Create the database user ystream_user and grant it the YSTREAM_CAPTURE privilege.

2. Enable supplemental logging on the YashanDB primary database. There are two ways to enable supplemental logging:
    
    Table-level supplemental logging:
    ```sql
    ALTER TABLE tablename ADD SUPPLEMENTAL LOG DATA (PRIMARY KEY OR ALL) COLUMNS;
    ```
...
3. Create the YStream Server, using the current SCN of the database.
    ```sql
    EXEC DBMS_YSTREAM_ADM.CREATE('server_1', 'ystream_user', null);
    ```

4. Specify the table names to be parsed, indicating 2 tables in this case.
    ```sql
    EXEC DBMS_YSTREAM_ADM.ADD_TABLES('server_1', '"sales"."employees","sales"."department"', null);
    ```

5. Start the YStream Server.
    ```sql
    EXEC DBMS_YSTREAM_ADM.START('server_1');
    ```

6. Configure client parameters to start YStream parsing, the client example is as follows.
