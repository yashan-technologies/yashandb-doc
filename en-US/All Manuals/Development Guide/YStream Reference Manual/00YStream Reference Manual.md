...
YStream only supports parsing redo and archive log files generated under archiving mode. When using YStream, the database [archiving feature](../../数据库管理/基本数据库管理/归档管理) must be enabled; otherwise, an error will occur when creating the YStream server.
...
To create, configure, and start the YStream server, refer to the [YStream Usage Example](../PL参考手册/内置高级包/DBMS_YSTREAM_ADM) for specific operations.
