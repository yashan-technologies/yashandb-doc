...
YStream only supports parsing redo and archive log files generated under archiving mode. When using YStream, the database [archiving feature](../../数据库管理/实例管理/归档管理) must be enabled; otherwise, an error will occur when creating the YStream server.
