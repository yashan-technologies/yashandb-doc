...
- In Standalone and YAC Deployment, the return value includes timezone information (the timezone set by the operating system), and the return type is TIMESTAMP WITH TIME ZONE, consistent with the format specified by the TIMESTAMP_TZ_FORMAT parameter.
