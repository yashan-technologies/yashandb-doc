...
The ARRAY_APPEND function adds new_member to the end of the array (see [array variable](../../PL Reference Manual/PL Language Fundamentals/Variables/Collection Variables), [array object](../../PL Reference Manual/PL Objects/User-Defined Types), [array type](../Data Types (yashan Mode)/User-Defined Types)) array_var and returns the resulting array after the addition.
...
- An existing array, with members that must be of [ordinary scalar data types](../Data Types (yashan Mode)/00Data Types (yashan Mode)).
...
A [general expression](../General SQL Syntax/expr) whose type must be the same as the members of the array_var or a data type that can be implicitly converted to it.
