...
The string can be a general expression [expr](../General SQL Syntax/expr) or a subquery returning a single row of data from a single column. The value of string must be CHAR, VARCHAR, or other types that can be converted to CHAR or VARCHAR, except for JSON, NCLOB, and timezone types.
...
When the OVER keyword is specified, WM_CONCAT will act as a [window function](00Built-in Functions (yashan Mode).html#WindowFunction), returning the sum of multiple rows' values, with the return type as CLOB.
