...
The ARRAY_REPLACE function replaces all members in the array (array variable [array_var](../../PL Reference Manual/PL Language Fundamentals/Variables/Collection Variables), [array object](../../PL Reference Manual/PL Objects/User-Defined Types), [array type](../Data Types (yashan Mode)/User-Defined Types)) that are equal to compare_member with replace_member, and returns the modified array.
...
- An existing array, where the array members must be of [basic scalar data types](../Data Types (yashan Mode)/00Data Types (yashan Mode)).
- NULL, in which case the function returns NULL.

**compare_member**

[General expression](../General SQL Syntax/expr), whose value type must be comparable with the member types of the array_var array. For specifics, please refer to YashanDB's [comparison operation rules](../Operators/Comparison Operators).

**replace_member**

[General expression](../General SQL Syntax/expr), whose type must be the same as the array_var members or a data type that can be implicitly converted.
