...
This example uses the YashanDB JDBC driver client program to insert a record containing an image into customer_img. The example refers to the getConnection method in the [YashanDB JDBC driver usage examples](../../JDBC驱动/JDBC驱动使用介绍/基于JDBC的应用开发示例).
...
Comparisons between CLOB and CLOB/CHAR/VARCHAR type data can be achieved using [DBMS_LOB](../../PL参考手册/内置高级包/DBMS_LOB).COMPARE. This advanced package also offers some operational methods for the CLOB type.
