...
The value of [expr](../General SQL Syntax/expr) must be numeric or a character type that can be converted to NUMBER type (conversion failure returns Invalid number error). For other types, the function returns that the type is not supported.
