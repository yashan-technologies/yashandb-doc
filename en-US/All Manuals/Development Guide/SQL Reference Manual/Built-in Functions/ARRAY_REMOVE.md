...
The ARRAY_REMOVE function removes all members from the array ([array variable](../../PL Reference Manual/PL Language Fundamentals/Variables/Collection Variables), [array object](../../PL Reference Manual/PL Objects/User-Defined Types), [array type](../数据类型/用户自定义类型) array_var that are the same as compare_member and returns the array after deletion.
...
- An existing array where the members must be of [ordinary scalar data types](../数据类型/00数据类型).
