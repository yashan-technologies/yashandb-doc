...
A value recognized by YashanDB as a [universal expression](../General SQL Syntax/expr), the value of timestamp_expr must be DATE, TIMESTAMP, TIME, or character data. If it is character data, it must ensure that the string conforms to the current format requirements of the TIMESTAMP type.

When timestamp_expr is NULL, the function returns NULL.

**time_expr**

A value recognized by YashanDB as a [universal expression](../General SQL Syntax/expr), the value of time_expr must be DATE, TIMESTAMP, TIME, or character data. If it is character data, it must ensure that the string conforms to the current format requirements of the TIME type.
