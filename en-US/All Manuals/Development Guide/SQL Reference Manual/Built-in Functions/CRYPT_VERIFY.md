...
The CRYPT_VERIFY function verifies the data from [expr1](../General SQL Syntax/expr) against the signature from [expr2](../General SQL Syntax/expr) using the public key provided in [expr3](../General SQL Syntax/expr), the signature public key algorithm type in [expr4](../General SQL Syntax/expr), and the signer's signature ID in [expr5](../General SQL Syntax/expr), returning a verification result of TRUE or FALSE.

A return result of TRUE indicates that the verification passed, while FALSE indicates that the verification failed.

> **Note**:
>
> If you need to use this function and use the OpenSSL tool, please first refer to [Dependency Preparation](../../../Installation and Upgrade/Installation and Deployment/Pre-Installation Preparation/Preparing Dependencies) to check and ensure that the required tools are installed on the server system.
