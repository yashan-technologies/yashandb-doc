...
The ARRAY_POSITION function searches for the first occurrence of compare_member in the array ([array variable](../../PL Reference Manual/PL Language Fundamentals/Variables/Collection Variables), [array object](../../PL Reference Manual/PL Objects/User-Defined Types), [array type](../数据类型/用户自定义类型) array_var, starting from start_locate, and returns the position as an INT type.
...
- An existing array, where the array members must be [ordinary scalar data types](../数据类型/00数据类型).
