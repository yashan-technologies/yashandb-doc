...
The CRYPT_HASH function computes a hash for the data in [expr1](../General SQL Syntax/expr) using the hash algorithm type specified in [expr2](../General SQL Syntax/expr), generating the corresponding digest and returning a hash digest in HEX format as a VARCHAR type.

> **Note**:
>
> If you want to use this function and the OpenSSL tool, please first refer to [Dependency Preparation](../../../Installation and Upgrade/Installation and Deployment/Pre-Installation Preparation/Preparing Dependencies) to check and ensure that the required tools are installed on the server system.
