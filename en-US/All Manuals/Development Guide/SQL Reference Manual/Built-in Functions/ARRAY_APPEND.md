...
The ARRAY_APPEND function adds new_member to the end of the array (see [array variable](../../PL Reference Manual/PL Language Fundamentals/Variables/Collection Variables), [array object](../../PL Reference Manual/PL Objects/User-Defined Types), [array type](../数据类型/用户自定义类型) array_var and returns the resulting array after the addition.
...
- An existing array, with members that must be of [ordinary scalar data types](../数据类型/00数据类型).
