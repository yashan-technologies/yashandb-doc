...
* For LOB type fields in LSC tables, if any row of data is stored out-of-line, this function cannot be used for conversion.
...
```sql
SELECT CONVERT('abcd' ,CHAR(6));

convert('abcd' ,char(6))  
------------------------- 
abcd                     

SELECT HEX(CONVERT('——' ,CHAR(9) ascii));
hex(convert('——' ,char(9) ascii))
----------------------------------
2D2D2D2D                                   
...
SELECT HEX(CONVERT('——' ,CHAR(9) CHARACTER SET gbk));
hex(convert('——' ,char(9) character set gbk))
----------------------------------------------
A1AAA1AA         
```
