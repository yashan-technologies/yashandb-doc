...
json_value is a binary [json](../通用SQL语法/json) data, which can be obtained through the [JSON function](../内置函数（yashan模式）/JSON JSON_PARSE).

When json_value is NULL, the function returns NULL.

**json_path**

Path expression, which is a constant string, and its format definition can be referenced in the [json](../通用SQL语法/json) documentation.
