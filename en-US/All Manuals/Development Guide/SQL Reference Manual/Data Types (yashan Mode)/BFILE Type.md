...
- When using the BFILE type, a file path alias must be created; specifications can refer to [CREATE DIRECTORY](../SQL Statements (yashan Mode)/CREATE DIRECTORY).

- BFILE type usage restrictions can be referred to in [LOB Usage Restrictions](Large Object Types.html#lob_use_restrictions).
...
The BFILE type defines a BFILE locator through the built-in function [BFILENAME](../Built-in Functions (yashan Mode)/BFILENAME) for inserting operations, and file data operations can be performed using related advanced package functionality [DBMS_LOB](../../PL Reference Manual/Built-in Advanced PL Packages/DBMS_LOB.html#dbms_lob_bfile).
