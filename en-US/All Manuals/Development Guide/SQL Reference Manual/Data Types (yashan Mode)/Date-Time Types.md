...
> - The system can accept microsecond inputs in the range of 0-9 digits but will pad or round to 6 digits.
...
-- Maximum precision of 9 is defined for microseconds, but the actual storage precision is 6
...
- Specify the time zone for TIMESTAMP, TIMESTAMP LTZ type data (will convert to TZ type, similar in effect to [FROM_TZ](../Built-in Functions (yashan Mode)/FROM_TZ) and [TO_TIMESTAMP_TZ](../Built-in Functions (yashan Mode)/TO_TIMESTAMP_TZ) function).
