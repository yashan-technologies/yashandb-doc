...
If the database service process reports an insufficient memory error during restart, you can either reduce the values of the parameter `DBLINK_CURSOR_COUNT` by adjusting configurations in Configuration File Adjustment, or expand memory allocation, then attempt to restart the database service after implementing these changes.
...
If it is confirmed that a DBLink is temporarily not needed, it can be closed (by executing [ALTER SESSION](../../SQL语句/ALTER SESSION.html#closedblink) statement or [DBMS_SESSION.CLOSE_DATABASE_LINK](../../../PL参考手册/内置高级包/DBMS_SESSION.html#closedblink) stored procedure) after committing/rolling back its corresponding transactions to release the corresponding resources.
