...
*   IS EMPTY|IS NOT EMPTY (expr must be of a nested table type)
...
***Example*** for LSC tables and TAC tables
...
The RLIKE syntax implements the same functionality as the [REGEXP_LIKE](../Built-in Functions/REGEXP_LIKE) function. It uses a regular expression matching algorithm to match the character expression expr with the regular expression regexp, returning TRUE if matched successfully, or FALSE if not. Its complete syntax form is:
...
ROWNUM is a system-assigned sequential number for the returned rows of a query, where the first row is assigned 1, the second row is 2, and so on. This statement limits the total number of returned rows using the ROWNUM pseudo-column. Specific usage rules can be found in [ROWNUM pseudo-column](../Basic SQL Elements/Pseudo Columns).

***Example*** for Heap tables

```sql
SELECT * FROM branches WHERE ROWNUM != 3; -- Returns records WITH ROWNUM 1 AND 2, total 2 records
SELECT * FROM branches WHERE ROWNUM != 1; -- No records returned
SELECT * FROM branches WHERE ROWNUM >= 1; -- Returns ALL records
SELECT * FROM branches WHERE ROWNUM > 1; -- No records returned
SELECT * FROM branches WHERE ROWNUM < 5; -- Returns records WITH ROWNUM 1,2,3,4, total 4 records
SELECT * FROM branches WHERE ROWNUM <= 5; -- Returns records WITH ROWNUM 1,2,3,4,5, total 5 records
SELECT * FROM branches WHERE ROWNUM = 1; -- Returns records WITH ROWNUM 1, total 1 record
SELECT * FROM branches WHERE ROWNUM = 2; -- No records returned
SELECT * FROM branches WHERE ROWNUM BETWEEN 1 AND 8; -- Returns records WITH ROWNUM 1,2..8, total 8 records
SELECT * FROM branches WHERE ROWNUM BETWEEN 2 AND 8; -- No records returned
 
-- USING ROWNUM IN a subquery TO get the top 5 records
