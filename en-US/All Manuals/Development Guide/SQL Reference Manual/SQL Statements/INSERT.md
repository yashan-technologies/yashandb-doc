...
This statement is used to specify the table (including tables in the local database or [remote tables](../通用SQL语法/dblink/dblink语法说明)) to insert data, as well as the column fields, and may define an alias for the table.
...
This statement is used to specify the object into which data is to be inserted, which can be a table name (including both local database tables or [remote tables](../通用SQL语法/dblink/dblink语法说明)), a table partition name, or a view name.
...
This statement indicates that the data to be inserted is a remote table; for detailed descriptions, see [dblink](../通用SQL语法/dblink/dblink语法说明).
...
- For UDT column fields, values are assigned through object initialization methods; see [User UDT](../数据类型/用户自定义类型) for descriptions.
...
- The data type of column_name_1 must support [implicit conversion](../数据类型/数据类型转换) to the data type of column_name.
...
- For UDT column fields, values are assigned through object initialization methods, as described in [User UDT](../数据类型/用户自定义类型).
