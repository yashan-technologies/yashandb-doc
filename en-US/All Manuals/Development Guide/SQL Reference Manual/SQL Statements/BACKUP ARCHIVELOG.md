...
For detailed operational descriptions on backup recovery, please refer to [Backup and Recovery](../../../数据库管理/备份与恢复/00备份与恢复).
...
In YAC/Distributed Cluster Deployment, by specifying the [YFS](../../../数据库管理/存储管理/集群文件系统管理/00集群文件系统管理) path, data can be backed up to shared storage; when specified as a regular disk path, the data is backed up to the server where the instance is located.
