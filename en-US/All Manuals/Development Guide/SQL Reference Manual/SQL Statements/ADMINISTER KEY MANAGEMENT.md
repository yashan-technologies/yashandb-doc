...
- [Third-party key management](#kms): Integrate with a key management service provided by a third party, which protects the master key. Currently, only Tencent Cloud KMS is supported. To use third-party key management, you must first modify the configuration parameter [TDE_CONFIGURATION](../../../Reference Manual/Configuration Parameters.html#TDE_CONFIGURATION) to `KEYSTORE_CONFIGURATION=FILE;KMS=TRUE`.
