...
- RTree indexes can only be created for columns of [ST_Geometry type](../数据类型/ST_GEOMETRY), and each RTree index can only specify one column.
...
- Columns of type [LOB](../数据类型/大对象型) or [UDT](../数据类型/用户自定义类型) cannot be used as index columns.

- Columns of type [RAW](../数据类型/RAW) cannot be used as columnar index columns.

- Only RTree indexes can be created on columns of [ST_Geometry type](../数据类型/ST_GEOMETRY), and each RTree index can only specify one column.
