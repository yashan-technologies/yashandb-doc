...
* CASE statement
* `udt_expr`: For Object UDT columns, the format of `udt_expr` is `table_alias.attribute_name`; for Varray UDT and Nested Table UDT columns, the format is `table_alias.*`. For detailed usage methods and examples, see [User UDT](../数据类型/用户自定义类型).
...
* [dblink](../通用SQL语法/dblink/dblink语法说明): Remote tables.
...
Can be used to query Varray UDT or Nested Table UDT data, for details refer to [User UDT](../数据类型/用户自定义类型). 
...
The table() can be used for queries, for specific descriptions of table functions, refer to  [Built-in Table Functions](../内置函数/00内置函数.html#tableFunc).
