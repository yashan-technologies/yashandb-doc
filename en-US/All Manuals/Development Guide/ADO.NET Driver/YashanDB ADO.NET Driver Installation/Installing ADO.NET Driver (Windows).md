...
1. Refer to the [YashanDB Software Package List](../../../安装和升级/安装部署/安装前准备/下载软件包) to obtain the YashanDB ADO.NET driver installation package.

2. Download and extract the zip file to a local path, for example, `/path/Yashandb.Data.YashandbClient`.

3. Edit the application project to reference the `Yashandb.Data.YashandbClient` project in that project. The project file example used in this manual is `Examples`. For specific content, refer to the section [YashanDB ADO.NET Driver Usage Examples](../ADO.NET驱动使用示例).
