...
- Transparent application failover functionality, allowing the client to automatically reconnect to other configured nodes after a failure occurs in the current connection node; see the description in [JDBC Transparent Application Failover](./JDBC驱动使用介绍/透明应用故障转移).
