...
|Parameter |Description |
| -------- | ------------------------------------------------------------ |
| url       | Database connection descriptor, formatted as jdbc:yasdb://host:port/database_name<br>\*   `host:port`: The database's listening address, where host is the server IP address or domain name. If host is omitted, it means connecting to localhost; port is the database service port, for example 1688. <br>\*   `database_name`: Database name, a required parameter. In a non-CDB environment, this parameter is only used for compatibility purposes, has no actual business meaning, and its value is not validated by the system. To connect to YashanDB deployed as a CDB, you must use the JDBC driver of version v1.10.2 or later and specify the [correct target container name](../../Database Administration/​​Multitenant Management/​​Managing PDBs​​/​View PDBs). |
...
public static Connection getConnection(String username, String passwd) {
        ...
        String sourceURL = "jdbc:yasdb://10.10.10.2:1688/pdb1";
...
jdbc:yasdb://192.168.1.2:1690/schema_name?compatVector=mysql
```
...
Control the connection mode through the serverMode parameter; defaults to shared mode if not configured.

|Connection Mode |Description |
| --------- | ---------------------- |
| dedicated        | Indicates dedicated server mode, meaning each client connection request must allocate a separate thread (not affected by relevant YashanDB server configurations). |
| shared           | Indicates shared server mode, allowing client connections to use a shared thread pool (actual use of a shared thread pool depends on relevant YashanDB server configuration parameter [MAX_REACTOR_CHANNELS](../../Reference Manual/Configuration Parameters.html#max_reactor_channels)). |
