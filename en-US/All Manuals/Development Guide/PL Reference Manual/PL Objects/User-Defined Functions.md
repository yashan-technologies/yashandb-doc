...
Use the [CREATE FUNCTION](../../SQL Reference Manual/SQL Statements (yashan Mode)/CREATE FUNCTION) statement to create a UDF, with the following syntax definition:
...
To specify a pipeline function created with PIPELINED as an independent function, use the CREATE FUNCTION statement in the function head. Pipeline functions can also be specified in the function declarations and definitions when creating [Packages](./自定义高级包).
...
User-defined aggregate functions can be used in the [select_list](../../SQL Reference Manual/SQL Statements (yashan Mode)/SELECT.html#select_list), HAVING clause, and ORDER BY clause as [aggregate functions](../../SQL Reference Manual/Built-in Functions (yashan Mode)/00Built-in Functions (yashan Mode).html#aggregate-function), returning single-row data or grouped data according to GROUP, or can be used as [window functions](../../SQL Reference Manual/Built-in Functions (yashan Mode)/00Built-in Functions (yashan Mode).html#window-function) using the OVER syntax.
...
The method of calling is the same as that for [built-in functions](../../SQL Reference Manual/Built-in Functions (yashan Mode)/00Built-in Functions (yashan Mode)), used as SQL expressions, for example:
...
The method of calling is the same as that for [built-in functions](../../SQL Reference Manual/Built-in Functions (yashan Mode)/00Built-in Functions (yashan Mode)), used as expressions in PL statements or for assigning values to variables, for example:
...
Use the [DROP FUNCTION](../../SQL Reference Manual/SQL Statements (yashan Mode)/DROP FUNCTION) statement to delete a UDF.

#### Modifying UDF

Use the [ALTER FUNCTION](../../SQL Reference Manual/SQL Statements (yashan Mode)/ALTER FUNCTION) statement to modify a UDF.
...
Once it is confirmed that the user-defined library used by the external UDF is no longer needed, it can be deleted using the [DROP_LIBRARY](../../SQL Reference Manual/SQL Statements (yashan Mode)/DROP LIBRARY) statement.
