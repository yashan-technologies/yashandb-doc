...
Used to identify deterministic functions, which are functions that produce fixed outputs when the same inputs are provided. Such functions typically meet the following conditions:
...
If the target UDF is used to create a [function index](../../SQL参考手册/SQL语句/CREATE INDEX.html#columnexpression), the UDF and its procedure body will not be allowed to be directly deleted, modified, recompiled, or replaced. The corresponding index must first be deleted before operating on the target UDF. The DBA_DEPENDENCIES view can be queried to check if the target UDF name exists in REFERENCED_NAME. If it exists, it indicates that the UDF is dependent on other objects, and its operation needs to consider the associations.
...
Method Two: Specify the library during function creation as the path of the user-defined library loaded via the [DBMS_STANDARD](../内置高级包/DBMS_STANDARD).LOADJAVA method (this method is not recommended).
