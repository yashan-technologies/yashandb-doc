...
- For the status requirements of the target YStream server for this operation, please refer to [Status Introduction](#STATUS).
  ...
  - If you need to add existing tables to the parsing scope of a Ystream service, it is not recommended to perform the operation when the service is in the STARTED or RUNNING state. Otherwise, you must ensure that the [table-level supplemental logging](../../SQL参考手册/SQL语句/ALTER TABLE.html#addsupplementalloggingclause) of the target tables are not enabled before executing ADD_TABLES. And after executing ADD_TABLES, you need to manually enable the table-level supplemental logging of these tables so that the Ystream service can parse this table.
