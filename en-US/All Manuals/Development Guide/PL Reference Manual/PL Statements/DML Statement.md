...
For SELECT statements where a single result cannot be guaranteed, a [cursor](../PL Language Fundamentals/Variables/Cursors) must be used for result set assignment.
...
The result set can contain one or more columns of data, each column being a [YashanDB general expression](../../SQL Reference Manual/General SQL Syntax/expr), but cannot include sequence, pseudocolumns, aggregate functions, window functions, or subqueries.

Under this usage, the INSERT statement can only use the [insert_values_clause](../../SQL Reference Manual/SQL Statements (yashan Mode)/INSERT) syntax, not for multi-row or subquery inserts.

In this usage, the [returning_clause](../../SQL Reference Manual/SQL Statements (yashan Mode)/INSERT) syntax cannot return variables.
...
The result set can contain one or more columns of data, each column being a [YashanDB general expression](../../SQL Reference Manual/General SQL Syntax/expr), but cannot include sequence, pseudocolumns, aggregate functions, window functions, variables, or subqueries.

In this usage, the [returning_clause](../../SQL Reference Manual/SQL Statements (yashan Mode)/UPDATE.html#returningclause) syntax cannot return variables.
