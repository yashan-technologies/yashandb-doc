...
Where, label_name is optional and identifies the current or outer loop to exit; WHEN condition is optional, with condition being an expression that produces a boolean result.
...
* label_name indicates the current or outer loop to exit. If the label_name does not correspond to the current or outer loop, or if it does not exist or is in other illegal situations, an error will be raised. label_name can be omitted, in which case the EXIT statement will exit the current loop when executed.
