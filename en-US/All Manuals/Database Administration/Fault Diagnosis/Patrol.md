...
YashanDB's high availability architecture supports three types of [protection modes](../../高可用/YashanDB高可用概述.html#protection_mode). In maximize protection mode, if the synchronous standby database is disconnected from the primary database or is unavailable, business submissions from primary database users will be stalled.
