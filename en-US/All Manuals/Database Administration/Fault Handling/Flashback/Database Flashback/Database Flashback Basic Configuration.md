...
- In YAC/Distributed Cluster Deployment, flashback operations must be executed on the master instance (`INSTANCE_ROLE = 'MASTER_ROLE'` in [GV$INSTANCE](../../../../Reference Manual/System Views/Dynamic Performance Views/GV$INSTANCE)).
...
2. The FLASHBACK_ON column in the [V$DATABASE](../../../../Reference Manual/System Views/Dynamic Performance Views/V$DATABASE) view indicates whether the global flashback feature is enabled.
...
- The FLASHBACK_ON column in the [V$DATABASE](../../../../Reference Manual/System Views/Dynamic Performance Views/V$DATABASE) view indicates whether the global flashback feature is enabled.
...
|--------------------|-------------------------------|-------|
| DB_FLASHBACK_FILE_DEST_SIZE  | Maximum allowed space for flashback logs (in bytes).<br/>Triggers auto-purge tasks when exceeded. | 8G |
| DB_FLASHBACK_RETENTION_TARGET  | Maximum retention period for flashback logs (in minutes).<br/>Determines if a log file can be purged. | 1440 |
...
The default process for the automatic cleanup of flashback log files is as follows (the following descriptions use default parameter values as examples):

1. The system periodically checks if total flashback log size exceeds DB_FLASHBACK_FILE_DEST_SIZE (8GB).

2. When total size ≥ 8GB, the auto-purge task is triggered.

3. The system checks if the oldest log file (by sequence) meets purge criteria:

    - If a file meets both conditions: being earlier than permanent restore points and exceeding the maximum retention duration requirement (DB_FLASHBACK_RETENTION_TARGET parameter), the cleanup operation will be executed.

    - Otherwise, a warning is logged, and manual adjustment of DB_FLASHBACK_FILE_DEST_SIZE is required to avoid service disruption.

4. After processing a file, the system rechecks total log size against DB_FLASHBACK_FILE_DEST_SIZE.

    If total size still exceeds 8GB, the next oldest file is evaluated, and the process repeats.

5. The task stops when total size ≤ 8GB.
