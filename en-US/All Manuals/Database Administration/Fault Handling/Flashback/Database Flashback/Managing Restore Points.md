...
| Deletion Mechanism | Manual deletion only | * Manual deletion allowed<br/>* All normal restore points are automatically deleted when Flashback is disabled |
| Recovery Mechanism | * For [backup and recovery database](../../../Backup and Recovery/00Backup and Recovery): All restore points will always be deleted after recovery<br/>* For [flashback database](./Performing Flashback Database): Depends on the chronological relationship between the recovery time point and the target restore point creation time point | Same as permanent restore point |
| Additional Notes | * Mandatory retention of [archive log files](../../../Instance Management/Archive Management.html#auto_cleanup) <br/>* Normally cannot [disable database flashback](./Database Flashback Basic Configuration.html#flashback_off) when permanent restore points exist, but flashback on old primary is forcibly and automatically disabled during SWITCHOVER (even if permanent restore points exist) | - |
...
- [Database flashback](./Database Flashback Basic Configuration) must be enabled.
...
2. Create permanent restore points using [CREATE RESTORE POINT](../../../../Development Guide/SQL Reference Manual/SQL Statements/CREATE RESTORE POINT):
...
2. Create normal restore points using [CREATE RESTORE POINT](../../../../Development Guide/SQL Reference Manual/SQL Statements/CREATE RESTORE POINT):
...
The [GV$RESTORE_POINT](../../../../Reference Manual/System Views/Dynamic Performance Views/GV$RESTORE_POINT) and [V$RESTORE_POINT](../../../../Reference Manual/System Views/Dynamic Performance Views/V$RESTORE_POINT) views contain information about restore points. Before performing a full database Flashback, you can first query these views to identify the target time point for recovery.
