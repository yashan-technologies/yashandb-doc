Table flashback essentially restores tables/table partitions and their dependent objects from the [Recycle Bin](./回收站管理). This feature is therefore subject to Recycle Bin constraints (e.g., the Recycle Bin must be enabled) and requires the target objects to remain in the Recycle Bin.  
