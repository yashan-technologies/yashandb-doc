...
The [ycsctl get](../../工具手册/ycsctl/节点管理命令) command can be used to view the configuration parameter values on the current node.

```shell
$ ycsctl get AUTO_START
```

The [ycsctl set](../../工具手册/ycsctl/节点管理命令) command can be used to modify the configuration parameter values on the current node (except for read-only parameters).
...
3. Execute the [ycsctl set_ycr](../../工具手册/ycsctl/集群管理命令) command to modify cluster configuration parameters.
