Data synchronization, also known as replication, refers to the real-time synchronization of data from the primary database to the standby database.
...
- Physical Replication: After the standby database receives the redo logs, it directly applies the redo logs to update the data pages of the standby database, making the data of the standby database completely consistent with that of the primary database. This method is called physical replication, and the standby database can be referred to as a physical standby database. In a [High Availability](../../High Availability/Overview of YashanDB High Availability) deployment, physical replication between primary and standby databases is fully automated and does not require manual configuration.
...
The logical replication-related functionality is applicable only to Standalone Deployment.

<span id="scope" name="scope" class="yaslink"></span>

## Scope of Logical Replication

The primary objects that can be synchronized through logical replication are HEAP tables in the primary database under non-SYS users, where the data types do not include JSON or UDT. The contents that will not be synchronized include:
...
- LSC tables, TAC tables, external tables.

<span id="requirement" name="requirement" class="yaslink"></span>
...
- If the logical standby database needs to execute failover, all other primary standby databases in the database cluster need to be rebuilt after completion.
