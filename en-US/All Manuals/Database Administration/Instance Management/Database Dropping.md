...
- In a CDB (with configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), if only a PDB needs to be deleted, please refer to [Delete PDB](../​​Multitenant Management/​​Managing PDBs​​/​Dropping a PDB​). To delete the CDB root, DROP PLUGGABLE DATABASE ALL must first be executed to delete all PDBs (including PDB seed).  
