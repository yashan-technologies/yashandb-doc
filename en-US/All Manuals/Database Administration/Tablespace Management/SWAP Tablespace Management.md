...
> **Note**: 
>
> In YAC Deployment, the SWAP tablespace only supports adding files with YFS paths; the local SWAP tablespace supports adding files with either local disk paths or YFS paths, but a single tablespace must not contain both YFS paths and local disk paths simultaneously.
...
In YAC Deployment, local SWAP tablespace data file resizing cannot be performed across instances, i.e., only resize local files on the server hosting the current instance.
