When YashanDB is deployed as a CDB (configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), it provides dual backup and recovery capabilities at both CDB and PDB levels through [*yasrman*](../../../Tools Guide/yasrman/00yasrman). The hierarchical backup strategy supports both unified protection of the entire CDB and independent backup and recovery operations for individual PDBs, providing a flexible and efficient data security mechanism.
...
PDB-level backup and recovery mainly refers to operations performed by directly connecting to the target PDB. Its [operation methods](../Backup and Recovery Using yasrman/00Backup and Recovery Using yasrman) are consistent with non-CDBs. The specific command keywords are as follows:
