...
> **Note**: 
>
> When specifying the data file path, absolute or relative paths can be used in Standalone Deployment, absolute paths must be specified for YAC/Distributed Cluster Deployment and can only be YFS paths, and only relative paths are permitted in ISC Distributed Cluster Deployment. 
