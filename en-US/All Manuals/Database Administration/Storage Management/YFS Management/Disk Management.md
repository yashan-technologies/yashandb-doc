...
<span id="Viewing" name="Viewing" class="yaslink"></span>

## Viewing DiskGroup Information

You can view YFS disk configuration, usage, and other related information using commands from the YFS management tool [*yfscmd*](../../../工具手册/yfscmd/00yfscmd), or via the corresponding dynamic views.
...
The DiskGroup, FailureGroup, and disk information can be queried separately via the dynamic views [GV$YFS_DISKGROUP](../../../参考手册/系统视图/动态视图/GV$YFS_DISKGROUP)/[V$YFS_DISKGROUP](../../../参考手册/系统视图/动态视图/V$YFS_DISKGROUP), [GV$YFS_FAILGROUP](../../../参考手册/系统视图/动态视图/GV$YFS_FAILGROUP)/[V$YFS_FAILGROUP](../../../参考手册/系统视图/动态视图/V$YFS_FAILGROUP), and [GV$YFS_DISK](../../../参考手册/系统视图/动态视图/GV$YFS_DISK)/[V$YFS_DISK](../../../参考手册/系统视图/动态视图/V$YFS_DISK).
...
- Each FailureGroup should, as much as possible, achieve [fault isolation](../../../概念手册/集群基础设施/崖山存储管理.html#FailureGroup).
...
The prerequisites for disk replacement (i.e., preparations for the new disk) are as follows:
    ...
    - The storage device has been [prepared](../../../安装和升级/安装部署/安装前准备/服务器准备.html#Storage) according to requirements, and the hardware devices planned for each FailureGroup must meet the [fault isolation standard](../../../概念手册/集群基础设施/崖山存储管理.html#FailureGroup).

    - The storage device has been [configured](../../../安装和升级/安装部署/安装前准备/配置存储设备) according to requirements (e.g., partitioning, mounting, etc.), and the disks must be bound under the same parent directory as the existing disk (e.g., `/dev/yfs`).

    - If [SCSI I/O Fencing](../../../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing) is used, it is recommended to first run the [fenceScsiCheck](../../../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing.html#fenceScsiCheck_usage) script to verify that the disks meet the requirements before creating the DiskGroup.
...
4. Execute the [ALTER DISKGROUP](../../../工具手册/yfscmd/磁盘管理命令.html#ALTERDG) command to add disks to the data DiskGroup.
...
4. Execute the [CREATE DISKGROUP](../../../工具手册/yfscmd/磁盘管理命令) command to add a new data DiskGroup.
    ```shell
...
6. Plan storage paths for new business data reasonably; for details, please refer to [Logical Space Management](../逻辑空间管理/00逻辑空间管理).
...
In YFS, monitoring disk status can be done by examining the `status` field in the output of the `show disk` command or the `MOUNT_STATUS` field in [GV$YFS_DISK](../../../参考手册/系统视图/动态视图/GV$YFS_DISK)/[V$YFS_DISK](../../../参考手册/系统视图/动态视图/V$YFS_DISK) view. For specific viewing operations, please refer to [Viewing Disk Information](#Viewing).
...
4. Execute the [ALTER DISKGROUP](../../../工具手册/yfscmd/磁盘管理命令.html#ALTERDG) command to replace the faulty disk with a new disk.
...
3. Execute the [ALTER DISKGROUP](../../../工具手册/yfscmd/磁盘管理命令.html#ALTERDG) command to re-online the target disk.
