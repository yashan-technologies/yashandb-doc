...
The default filename format for archive log files is `arch_{resetid}_{sequence}.ARC`, which in YAC/Distributed Cluster Deployment may also be `arch{instanceId}_{resetid}_{sequence}.ARC`. The specific field meanings are as follows:
...
- instanceId: A unique field in YAC/Distributed Cluster Deployment used to mark the instance ID to which the archive log files belong. The archive log files for instance 1 will omit the instanceId field.

### Viewing through Views

You can view information about the archive log files, such as the sequence number, filename, and archive time, through the [V$ARCHIVED_LOG](../../../参考手册/系统视图/动态视图/V$ARCHIVED_LOG) view.
...
4. To avoid accidental deletion of archive log files, it is recommended to back them up first.
    ...
    In Standalone/YAC/Distributed Cluster Deployment, you can back up archive log files individually. For specific operation steps, please refer to [Backup archive log files](../../备份与恢复/SQL语句备份恢复/备份.html#backup_arch).
...
```
    ...
    In ISC Distributed Cluster Deployment, archive log files can only be backed up by backing up the entire database. For specific operation steps, please refer to [Backup entire database](../../备份与恢复/yasrman工具备份恢复/备份).
...
6. Manually clean up archive log files; for specific syntax, please refer to [ALTER DATABASE](../../../开发手册/SQL参考手册/SQL语句/ALTER DATABASE.html#deletearchivelogclauses).
