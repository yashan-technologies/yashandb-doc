...
- **Primary Database**
    ...
    In the primary/standby cluster deployment, the primary database expands to the concept of a main cluster, where multiple instances in the main cluster simultaneously provide online database services, all in read-write mode.
...
- **Standby Database**
    ...
    In primary/standby cluster deployments, the standby database expands to the concept of a backup cluster, but only the primary instance in the backup cluster is needed to receive and apply logs.
