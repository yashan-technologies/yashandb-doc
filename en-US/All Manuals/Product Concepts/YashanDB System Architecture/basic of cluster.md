...
YCS is responsible for managing the YAC database, including cluster server configuration management, cluster resource configuration management, starting/stopping and monitoring servers and resources, and providing query capabilities for the server resource topology status. It is responsible for voting arbitration and restructuring the cluster during various failures.
...
- The parallel file system synchronizes metadata modifications in real-time across all instances in the YAC, ensuring that all database instances can access consistent directory file metadata information.
...
- The core components of the YAC mainly include Yashan Cluster Kernel (YCK), Yashan Cluster Service (YCS), and Yashan File System (YFS).
...
- The compute cluster constitutes a multi-instance active-active cluster composed of multiple compute nodes. Beyond inheriting the core infrastructure capabilities of YAC, it possesses the following distributed characteristics:
  - The compute cluster's [distributed transaction management](./存储引擎.html#DstbTm) employs decentralized architecture with globally synchronized high-precision logical clocks, delivering real-time strong data consistency across multiple instances while eliminating scalability bottlenecks inherent in centralized transaction coordination.
  - The compute cluster employs [distributed parallel execution scheduling](./SQL引擎.html#DstSqlExec) to coordinate multiple instances in collaboratively processing data computation tasks. This architecture maximizes utilization of distributed multi-instance resources, significantly accelerating massive-scale data analytics.
- The storage cluster constitutes a distributed intelligent storage service cluster composed of multiple storage nodes, delivering unified distributed storage service capabilities to the compute cluster.
  - [The intelligent storage service](./集群基础.html#SmartStor) is responsible for storage device management and monitoring, end-to-end data lifecycle management, and near-storage compute capabilities, delivering elastic scalability and high-availability services.