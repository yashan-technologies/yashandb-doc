...
YashanDB supports a data dictionary buffer that caches critical metadata in memory using specific data structures. Based on usage frequency, quantity, and memory footprint, the data dictionary buffer can be categorized into resident buffers and dynamic buffers.
