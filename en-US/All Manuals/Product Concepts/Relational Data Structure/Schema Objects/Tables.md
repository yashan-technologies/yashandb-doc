...
- **Hidden Column**: Its data occupies storage space, but users cannot directly query the data of the hidden column.
   ...
   For example, if a user creates a table containing a custom object type, the object type may contain several attributes, each of which is stored in the table as at least one hidden column. Users cannot directly query these hidden columns but can access them by querying the object column and then accessing the attributes of the object type.
