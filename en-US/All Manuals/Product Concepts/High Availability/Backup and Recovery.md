...
Full database backup of an ISC distributed database requires backing up the primary databases of MN, all CNs, and all DN groups. The backup is divided into two stages: the first stage involves backing up control files and data files at all target nodes. In the second stage, a consistent redo log point is obtained on all target nodes, and then the archive log files prior to the consistent redo log point are backed up. When restoring a distributed backup set, all nodes only apply redo to the consistency point recorded at the time of backup, ensuring the consistency of distributed transactions after restoration.
