...
| TABLE ACCESS BY USER ROWID          | Specify a single ROWID (`ROWID = ?` or `ROWID IN ('9912:0:0:5108:4', '9912:0:0:5108:8')`) to access data in the table. For the ROWID data format, please refer to [ROWID Data Format](../../../开发手册/SQL参考手册/数据类型/ROWID UROWID.html#Format).          | All                     | ×        |
| TABLE ACCESS BY ROWID RANGE   | Specify a range of ROWIDs (`ROWID BETWEEN ... AND ...` or `ROWID > ?`) to access multiple rows of data in a table. For the ROWID data format, please refer to [ROWID Data Format](../../../开发手册/SQL参考手册/数据类型/ROWID UROWID.html#Format).                    | Row                     | √     |
...
| PART SCAN COMBINED ITERATOR         | Scans specified secondary partitions.  | All                     | √                    |
