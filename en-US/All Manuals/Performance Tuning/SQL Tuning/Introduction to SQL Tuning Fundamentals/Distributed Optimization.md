...
1. An increase in the PX operator, which is responsible for data exchange across networks. PX has the following types:
    ...
    - REMOTE: ISC Distributed Cluster Deployment.

    - LOCAL: Standalone Deployment parallel environment.
...
2. To fully utilize the computing resources of each node, the YashanDB optimizer attempts to push down computations to the DN (two-phase computation). The operators that can perform two-phase computations include:
    - Two-phase ORDER BY
...
3. By creating small tables as duplicated tables, cross-node joins are minimized.