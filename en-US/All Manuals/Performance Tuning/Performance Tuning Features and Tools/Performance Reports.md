...
By default, snapshots are created by the system at regular intervals (default is every hour; this value can be modified using [DBMS_AWR](../../开发手册/PL参考手册/内置高级包/DBMS_AWR).MODIFY_SNAPSHOT_SETTINGS).

To diagnose performance bottlenecks, users may need to create a snapshot instantaneously at a specified point in time (e.g., before and after executing a SQL statement) to generate a performance report. At this point, a snapshot can be manually created by calling the [DBMS_AWR](../../开发手册/PL参考手册/内置高级包/DBMS_AWR).CREATE_SNAPSHOT procedure.

Snapshot data will be automatically cleaned by the system after a certain period (default is 8 days; this value can also be modified using [DBMS_AWR](../../开发手册/PL参考手册/内置高级包/DBMS_AWR).MODIFY_SNAPSHOT_SETTINGS), or manually cleaned via [DBMS_AWR](../../开发手册/PL参考手册/内置高级包/DBMS_AWR).DROP_SNAPSHOT_RANGE.
...
Generating a report involves comparing two snapshots to create a performance report according to different dimensions. The calculation of the report is performed by the stored procedure ([DBMS_AWR](../../开发手册/PL参考手册/内置高级包/DBMS_AWR).AWR_REPORT), which the user actively calls when a report is needed.
...
3. Retrieve the performance report (HTML file) using the following methods; please specify the corresponding file path as needed. This document uses the current path when executing the *yasql* command as an example.
    ...
    The file extension should be specified as .html; for file naming rules, please refer to the [yasql usage guide](../../Tools Guide/yasql/User Guide for yasql.html#spool).
