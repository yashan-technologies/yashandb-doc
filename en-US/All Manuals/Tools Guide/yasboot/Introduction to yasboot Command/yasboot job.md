...
| *-p, --password*          | Password corresponding to the cluster username       |
...
This command is used to add a job and save it to `YASOM` (currently does not support YACs).
...
This command is used to apply a job to a database node (currently does not support YACs).
...
This command is used to delete job strategies (currently does not support YACs).
