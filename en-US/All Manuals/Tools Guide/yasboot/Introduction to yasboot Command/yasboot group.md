...
| *-gt, --group-type*  | Group type name: db: Standalone Deployment, ce: YAC, mn: Distributed MN Group, cn: Distributed CN Node, dn: Distributed DN Group |
...
| *-c, --cluster*      | The cluster name of YashanDB (mandatory parameter)  |
| *-g, --group-id*     | Group ID (can be viewed through the cluster status command; the value before the hyphen in nodeid is the group-id, for example, the group ID for `1-1:1` is 1) (mandatory parameter) |
| *-m, --start-mode*   | Start mode, optional [mount|nomount|open], default is open (YAC Deployment does not support starting in mount mode) |
| *-w, --nowait*       | Do not wait for the execution result after the command runs |
| *-d, --child*        | Display task and sub-task information                |
| *--disable*          | Suppress the display of running progress information  |
| *-u, --username*  | Specify the database user. If not specified, the default user sys is used |
...
| *-d, --child*        | Display task and sub-task information                |
| *--disable*          | Suppress the display of running progress information  |
| *-s, --stop-mode*    | Shutdown mode, optional [normal|immediate|abort], default is immediate (not currently available for YAC Deployment) |
| *-u, --username*  | Specify the database user. If not specified, the default user sys is used |
| *-p, --password*  | Password for the database user <br/>If the `sys` user is used and [OS authentication](../../../产品安全/身份标识与鉴别/操作系统认证/00操作系统认证) (enabled by default after installation) is activated, no password needs to be specified   |
| *--wait-timeout*     | Command execution timeout (hidden parameter)        |
...
| *-t, --toml*      | Configuration file for the expanded node          |
| *-d, --child*     | Display task and sub-task information              |
| *--disable*       | Suppress the display of running progress information |
