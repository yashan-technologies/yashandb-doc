...
This command is used to shrink the standby database in a standalone primary/standby deployment, CN nodes in ISC Distributed Cluster Deployment, DN standby nodes or MN standby nodes. In YAC/Distributed Cluster Deployment, it can only clear nodes with expansion failures temporarily.
...
| *-d, --child*     | Display task and subtasks information   |
| *--disable*       | Disable the display of running progress  |
| *--node-ids*      | Node IDs to be deleted, supports multiple, separated by commas |
...
| *--with-unconnected-host* | If the server of the deleted node is no longer connected, use this parameter to skip some tasks and forcibly delete the node. This option is only applicable to Standalone Deployment and does not apply when removed nodes and retained nodes are on the same server. |
...
This command is used to expand the standby database in a standalone primary/standby deployment, instances in YAC/Distributed Cluster Deployment (main cluster), or CN nodes in ISC Distributed Cluster Deployment.
