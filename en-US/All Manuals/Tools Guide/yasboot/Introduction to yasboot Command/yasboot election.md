...
> **Caution**: 
>
> - The [OS authentication](../../../数据库管理/基本数据库管理/操作系统身份认证配置) must be enabled (default enabled during standard installation) for proper use of yasom election functionality.
> - *yasom* election and yasom self-repair are mutually exclusive. When the yasboot process is set to yasom repair on, yasom election cannot be used.
