...
| *--cpu-limit*           | Maximum percentage limit of CPU usable by the server, defaults to 100. Must be used with --recommend-param           |
| *--scale-factor*        | Data scale for test scenarios such as TPCH, defaults to 100, indicating 100G test data. Must be used with --recommend-param    |
| *--scene*               | Scenario for parameter tuning, optional [NORMAL,TPCH], defaults to NORMAL. <br>When performing TPC-H performance testing with distributed LSC table, consider using TPCH scene parameter for tuning, and ensure that all nodes perform parameter recommendation, otherwise it may fail due to connection parameter mismatch during reboot. <br>When business uses distributed in-memory databases, it can be configured for AIM tuning recommendationsMust be used with --recommend-param   |
| *--mms_limit*         | On memory DB, the size of the memory tablespace as a percentage of the process memory is only valid when the scene mode is set to AIM. The valid range is [0, 100], with a default value of 80.         |
| *--create-cgroup*       | Whether to create the resource management cgroup directory, not created by default. Creation requires sudo permissions and at least one of -u, -p, -su, or -sp must be specified    |
| *--cgroup-path*         | Customize the resource management cgroup directory. When not specified, it defaults to `/sys/fs/cgroup` |
| **Other Parameters** |  |
| *-h,--help*          | View help information for the current command    |
| *--ssl-protocol*        | SSL protocol, only supports SSL                     |
