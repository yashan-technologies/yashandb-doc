...
```shell
$ yasql [-S] username/password@host:port[/pdb_name] [-c "SQL" | -f [-e]  filename[.ext] | @filename[.ext]] 
...
When logging in via operating system authentication, the username and password can be omitted, but `/` cannot be omitted. Operating system authentication login requires configuration before use; please refer to the [OS authentication](../../Product Security/Identity Identification and Authentication/OS Authentication/00OS Authentication).

**@host:port[/pdb_name]**

Specify the target database URL for login or connection, where `/pdb_name` is used in CDBs to connect to a specific PDB (if omitted, defaults to connecting to the CDB root).  

For details, please refer to [login and connection](#login).
...
| OS authentication login|  Command format: `yasql / as sysdba` or `yasql username/password as sysdba` <br/>Using this method does not require a configured connection, and can only log in to the database instance corresponding to the local YASDB_DATA environment variable. |
...
In the *yasql* command-line interface, you can execute operating system-level shell commands with `!`.
...
serveroutput OFF
```
<span id="show_pdbs" name="show_pdbs" class="yaslink"></span>
