...
If the username or password contains special symbols (except for underscores), such as `@`, `/`, `.`, `!`, `$`, `'`, etc., the corresponding values must be enclosed in double quotes during [creation](../../Development Guide/SQL Reference Manual/SQL Statements/CREATE USER)/[management](../../Development Guide/SQL Reference Manual/SQL Statements/ALTER USER). To ensure that *yasql* can correctly pass the username and password values to the database server, it is recommended to use [step-by-step login](#nolog) when there are special characters; in this case, only double quotes are needed. If logging in directly, the characters must be rewritten accordingly:
...
When logging in via operating system authentication, the username and password can be omitted, but `/` cannot be omitted. Operating system authentication login requires configuration before use; please refer to the [OS authentication](../../产品安全/身份标识与鉴别/操作系统认证/00操作系统认证).
...
Identifier for logging in via operating system authentication.
...
| [set SERVEROUTPUT](#serveroutput) | Set whether to print information from the server-side buffer configured by [DBMS_OUTPUT](../../开发手册/PL参考手册/内置高级包/DBMS_OUTPUT).PUT_LINE |
...
- Method 3: Operating system authentication login, command format: `yasql / as sysdba` or `yasql username/password as sysdba`. Using this method does not require a configured connection, and can only log in to the database instance corresponding to the local YASDB_DATA environment variable.
...
<span id="shell" name="shell" class="yaslink"></span>
...
<span id="show" name="show" class="yaslink"></span>
...
<span id="desc" name="desc" class="yaslink"></span>
...
<span id="directexecute" name="directexecute" class="yaslink"></span>
...
<span id="feedback" name="feedback" class="yaslink"></span>
...
<span id="serveroutput" name="serveroutput" class="yaslink"></span>

### Set Print Cache Information

You can set whether to print information from the server-side buffer configured by [DBMS_OUTPUT](../../开发手册/PL参考手册/内置高级包/DBMS_OUTPUT).PUT_LINE with `set SERVEROUT[PUT] on|off`, with the default being OFF.
...
<span id="nchar" name="nchar" class="yaslink"></span>
...
<span id="set_def" name="set_def" class="yaslink"></span>
...
<span id="DEFINE" name="DEFINE" class="yaslink"></span>
...
<span id="heading" name="heading" class="yaslink"></span>
...
<span id="COLUMN" name="COLUMN" class="yaslink"></span>
...
<span id="EXIT" name="EXIT" class="yaslink"></span>
