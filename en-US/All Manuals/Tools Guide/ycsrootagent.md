...
In scenarios where automatic detection of whether storage devices support SCSI I/O Fencing is needed, *yasboot* will automatically call the fenceScsiCheck script to invoke relevant subcommands for this command. For manual detection, please directly execute the [fenceScsiCheck script](../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing.html#fenceScsiCheck_usage), **it is not recommended for users to execute this command directly**.
...
In scenarios where automatic detection of whether storage devices support SCSI I/O Fencing is needed, *yasboot* will automatically call the fenceScsiCheck script to invoke this command. For manual detection, please directly execute the [fenceScsiCheck script](../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing.html#fenceScsiCheck_usage), **it is not recommended for users to execute this command directly**.
