...
Specify the [wallet password](../../Product Security/Encryption/Storage Encryption/Key Management) for key management of the database instance. When there are existing encrypted objects in a YAC/Distributed Cluster and you need to start the database directly to MOUNT or OPEN mode, it must be specified; otherwise, the database instance will fail to start.
...
> - The contents of the script to stop the database instance usually are: *yasql* logs into the database instance and executes the "shutdown immediate" command. To enhance security, it is recommended to complete the [OS authentication configuration](../../Product Security/Identity Identification and Authentication/OS Authentication/Configuring OS Authentication) and use yasql for [password-free login](../yasql/User Guide for yasql).
> - When deploying a YAC/Distributed Cluster using *yasboot*, the *yasboot* tool automatically completes the above configuration and generates a script for password-free login.

### ycsctl start pdb

This command is only applicable to a YAC/Distributed Cluster Deployment CDB, used to start an instance of the target PDB on the current server. The command format is as follows:
...
This command is only applicable to a YAC/Distributed Cluster Deployment CDB, used to stop the instance of the target PDB on the current server. The command format is as follows:
...
Parameter name, derived from the allowed [configuration parameters](../../Database Administration/Cluster Management/Cluster Parameter Configuration) of YCS. The following parameters can be specified for querying:
...
Parameter name, derived from the allowed [configuration parameters](../../Database Administration/Cluster Management/Cluster Parameter Configuration) of YCS. The following parameters can be specified for setting:
...
The target parameter value, which must be within the allowed range as specified by YCS [configuration parameters](../../Database Administration/Cluster Management/Cluster Parameter Configuration).
...
Whether it is a primary database.
...
This command is used to view information about the target PDB. The command format is as follows:
...
For detailed descriptions of the above parameters, see the [YAC parameter configuration](../../Database Administration/Cluster Management/Cluster Parameter Configuration) section.
