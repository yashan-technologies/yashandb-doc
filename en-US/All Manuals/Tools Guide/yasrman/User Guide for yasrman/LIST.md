...
```ebnf+diagram
syntax::= LIST BACKUP [[(TAG tag_name)] [(DETAIL [FORMAT XML])|(NODE INFO TO "nodeinfo.txt")|(DATABASE CONFIGURE PARAMETER)] | (OF PLUGGABLE DATABASE ((pdb_name) {"," (pdb_name)}))]
```
...
This statement is used to list backup set information for specified PDBs. The full syntax is `LIST BACKUP OF PLUGGABLE DATABASE pdb_name [, pdb_name]`, and it cannot be used concurrently with other LIST commands.
