...
| *-k,--key*      | Token used for validation when connecting to yasom, needs to match the configuration in yasom. The token value is configured in the installation directory of ​​yasom​​, the value of `rpc_secret_key` in the file `${YASDB_HOME}/om/<Cluster_name>/conf/tls.toml`.              |
...
> -  YAC/Distributed Cluster Deployment: Select the first node for backup, and select the master role node for recovery.
