...
2. Check and confirm that each cluster node has configured service port number (service port).
```shell
...
$ ycsctl show config
    ...
    Node name: yas2, yascs/yasfs inter connect URL: 192.168.7.134:1788, Node ID: 2
        public service port: 1601           # The node yas2 has configured the service port number as 1601
        yasdb instance name:yasdb-1-2, yasdb instance id:1
```

3. If a node's service port number is not configured or needs to modify it, perform the following operations.
```shell
# Stop the cluster
