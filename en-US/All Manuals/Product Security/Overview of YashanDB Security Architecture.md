...
- [Connection Management](Connection Management/00Connection Management): Provides anti-intrusion capabilities through black/white listing of connection request IPs and connection monitoring.

- [Resource Control](Resource Control/00Resource Control): Profile is a set of restrictions controlling user behavior. By associating users and profiles, it can restrict the session resources available to users, including the maximum number of concurrent session connections for the same user and the maximum idle session time limit.
...
| ------------ | -------- | ------------------------------------- |
| User         |      | No ordinary users; the first ordinary user must be created and authorized by the sys super administrator |
| Identity Authentication | Password authentication | Enabled                            |
|               | OS authentication | Enabled                            |
| Data Access Management | Separation of powers | Disabled                            |
|               | Mandatory access control | Disabled                            |
| Security Audit |     | Disabled                                   |
| Encryption Support | Trusted channel | Disabled                            |
| Connection Management | Requester black/white list | Disabled                            |
|               | Connection monitoring | Enabled                            |
| Resource Control | Session resource control | Disabled                            |
