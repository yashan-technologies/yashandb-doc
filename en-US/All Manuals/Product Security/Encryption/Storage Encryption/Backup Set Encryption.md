...
Backup encryption can be used with any backup method provided by YashanDB. For details, please refer to [Backup and Recovery](../../../数据库管理/备份与恢复/00备份与恢复). Each backup set of incremental backups must maintain a consistent encryption strategy (either all encrypted or all unencrypted), and the keys of each backup set must remain the same.
