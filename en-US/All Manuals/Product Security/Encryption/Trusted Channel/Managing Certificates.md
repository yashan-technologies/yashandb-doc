...
The certificate file list for configuring SSL trusted channels ([Client\<->Server SSL Trusted Channel](./配置可信信道.html#ssl), [Primary-Standby Replication Network Trusted Channel](./配置可信信道.html#hassl), or [DIN Trusted Channel](./配置可信信道.html#dinssl)) is as follows. The corresponding filenames and paths need to be recorded.
...
- To generate SSL-related certificates, the OpenSSL tool must be used. Please refer to [Preparing Dependencies](../../../安装和升级/安装部署/安装前准备/依赖项准备) to check and ensure that the necessary tools are installed on the server system.
...
Certificate file list for configuring [Client \<-> Server TLCP Trusted Channel](./配置可信信道.html#tlcp) is as follows. The corresponding filenames and paths need to be recorded.
...
- To generate TLCP-related certificates, the GmSSL tool must be used. Please refer to [Preparing Dependencies](../../../安装和升级/安装部署/安装前准备/依赖项准备) to check and ensure that the required tools are installed on the server system.
