...
- Using SQL statements: The backup set must be stored on the database server.
    ...
    - Backup: [BACKUP DATABASE](../../Development Guide/SQL Reference Manual/SQL Statements/BACKUP DATABASE) statement		
	- Restore: [RESTORE DATABASE](../../Development Guide/SQL Reference Manual/SQL Statements/RESTORE DATABASE) and [RECOVER DATABASE](../../Development Guide/SQL Reference Manual/SQL Statements/RECOVER DATABASE) statements

- Using [yasrman](../../Tools Guide/yasrman/00yasrman) tool: Remote backup/recovery operations can be initiated, and the backup set can be optionally stored on the tool side or the database server side.

For detailed operational procedures regarding backup and recovery, please refer to [Backup and Recovery](../../Database Administration/Backup and Recovery/00Backup and Recovery).
In addition to directly performing hot backups and recovery, using the [exp](../../Tools Guide/exp/00exp) tool to export data forms file backups, and then using the [imp](../../Tools Guide/imp/00imp) tool to import data when needed can also achieve the effect of backup and recovery.
