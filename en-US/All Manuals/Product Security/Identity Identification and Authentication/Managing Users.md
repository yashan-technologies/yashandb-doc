...
In YashanDB, users can be classified into system users and regular users based on their creation method:
...
- Regular users: This is a user created using an SQL statement, and its identity can be specified by granting authorizations to the regular user.
...
- The first regular user can only be created by the sys user.
...
1. Log in to the database using a user with the corresponding privileges.
    ...
    In a CDB, creating common users requires connecting to the CDB root, while creating local users requires connecting to the target PDB.  
...
1. Log in to the database using a user with the corresponding privileges.
    ...
    In a CDB, it is recommended to connect to the CDB root when modifying common user attributes, while modifying local user attributes requires connecting to the target PDB.
...
1. Log in to the database using a user with the corresponding privileges.
    ...
    In a CDB, deleting common users requires connecting to the CDB root, while deleting local users requires connecting to the target PDB.  
