...
- A request must be made to the OS administrator to add the target OS user to the **YASDBA user group** (usually created during the [preparation work](../../../安装和升级/安装部署/安装前准备/创建用户和目录) of YashanDB installation) and the **same name user group of the database installation user** (for example, yashan).
