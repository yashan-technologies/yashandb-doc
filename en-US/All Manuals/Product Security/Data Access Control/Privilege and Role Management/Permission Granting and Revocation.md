...
- Be cautious when granting the DBA role to users. Users with the DBA role (when the separation of duties is disabled) are allowed to perform any operation, including damaging system tables, which may lead to severe database failures.

- Be cautious when granting privileges to the PUBLIC role, as all users will automatically inherit these privileges upon grant.  
...
2. Log in to the database using a user satisfying the grantor conditions.

    In a CDB (with configuration parameter ENABLE_PLUGGABLE_DATABASE=TRUE), it is recommended to connect to the CDB root when granting permissions to global users and global roles, while granting permissions to local users and local roles requires connecting to the target PDB.

3. Execute the [GRANT](../../../Development Guide/SQL Reference Manual/SQL Statements/GRANT) statement to initiate authorization to another user or role.
...
- Regular users can only revoke privileges they have granted; even if privilege transfer across users has been achieved through the WITH GRANT|ADMIN OPTION statement, they cannot revoke across users.
...
1. Log in to the database using a user satisfying the authorization revocation conditions.

2. Execute the [REVOKE](../../../Development Guide/SQL Reference Manual/SQL Statements/REVOKE) statement to initiate revocation on another user/role.
