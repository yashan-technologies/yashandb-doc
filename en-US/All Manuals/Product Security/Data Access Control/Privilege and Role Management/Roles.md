...
| -------------- | ------------------------------------------------------------ |
| DBA            | * When the separation of duties is not enabled: Has nearly all privileges (except SHUTDOWN).<br>* When the separation of duties is enabled: Permissions are restricted — cannot perform DML operations on unprivileged database objects (tables, indexes, etc.), and cannot perform GRANT or REVOKE operations for permission management. For specific permissions, please refer to [Separation of Duties](../三权分立). |
