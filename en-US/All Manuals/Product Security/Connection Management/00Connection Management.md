...
In certain scenarios, YashanDB will restrict users to log in to the database only via UDS local connections, such as:
    ...
    - After setting the connection listener to maintenance mode, only local connections initiated by sys users are accepted
...
- [Managing Connection Listener](./管理连接监听器): By adjusting the operating mode of the connection listener, whether to accept new connections can be controlled as needed, achieving refined connection management of database services, such as connection admission control, load regulation, etc.

- [Managing IP Blacklist and Whitelist](./管理IP黑白名单): Control access to login requests initiated by IPs on the list through whitelisting or blacklisting.

- [Listening Connection](./监听连接): Records all connection information to YashanDB, allowing timely detection of abnormal connections and implementation of measures.

- [Controlling Session Resources](./管控会话资源): Session resource controls, such as limiting the number of parallel connections and session idle time, can effectively reduce the security risks posed by idle connections, prevent malicious attacks and potential data leaks, avoid resource abuse, and improve the performance and stability of the database.

<span id="Reservation" name="Reservation" class="yaslink"></span>
