This view displays summary information of the session worker pool when opened in shared mode.
...
| LAST_TASK_START_TIME | TIMESTAMP | Time when the worker started executing the last registered task |
| LAST_TASK_END_TIME | TIMESTAMP | Time when the worker finished executing the last registered task; NULL if currently not finished |
