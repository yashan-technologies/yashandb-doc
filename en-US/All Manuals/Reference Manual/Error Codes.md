...
**Action**: Current type conversion is not supported, please refer to [Data Type Conversion for yashan mode](../开发手册/SQL参考手册/数据类型/数据类型转换) for conversion rules. If this problem occurs while using [unsigned numeric types](../mysql模式参考手册/SQL参考手册/数据类型/00数据类型) in mysql mode, it may be due to the *yasql* client version being too low, please check and upgrade the client.
...
- For insufficient driver memory pool, you can adjust the parameter YDBC_BUFFER_SIZE in the yex_server.ini configuration file; for specific operations, please refer to [Adjusting Memory Pool Size](../开发手册/SQL参考手册/通用SQL语法/dblink/yex_server沙箱进程管理).
...
- From the perspective of unreasonable configuration; whether the resource configuration can meet actual concurrency, please refer to [Session Management](../数据库管理/运行监控/会话管理) for managing concurrent tasks.
...
- If the database has not entered the OPEN state, you can try executing ALTER DATABASE OPEN to start the database. If it cannot be OPEN due to a corrupted log file, you need to restore the database from a backup set; please refer to [Backup and Recovery](../数据库管理/备份与恢复/00备份与恢复).
...
**Action**: During the instance scaling process, distributed transactions, file - related operations, and database operations that change the instance status cannot be executed.

### YAS-02827
**Message**: failed to scale instance, reason: %s

**Action**: The instance scaling operation has failed. Please make corresponding repairs according to the specific reasons.
...
**Action**: A failure occurred during instance scaling of the cluster, and executing this operation is not allowed when the cluster is being restarted. Please execute ALTER DATABASE OPEN first and then restart to perform this operation.
...
**Action**: The input parameter is invalid, please refer to [Built-in Functions](../Development Guide/SQL Reference Manual/Built-in Functions/00Built-in Functions) or [Built-in Advanced PL Packages](../开发手册/PL参考手册/内置高级包/00内置高级包) to modify your parameters and re-execute.
...
**Action**: This instance is invalid in the cluster. Please scale up the instance again and try again.

### YAS-05437
**Message**: remove instance %u is forbidden, the reason is %s

**Action**: The scale down operation cannot be performed. Please troubleshoot based on the error message and try again.
...
**Action**: Please correct the AUTOTRACE related option to a valid value according to the command introduction of [SET AUTOTRACE](../工具手册/yasql/yasql使用指导.html#autotrace).
...
**Action**: Please delete the corresponding index and then retry.
