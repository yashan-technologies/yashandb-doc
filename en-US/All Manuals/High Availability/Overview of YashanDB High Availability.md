...
In Standalone Primary/Standby Deployment, CDB is divided into primary CDB and standby CDB, while each PDB's role within the CDB can be inconsistent with the CDB root's own primary-standby role.
...
In Standalone Primary/Standby Deployment, YashanDB provides complete operations management capabilities for PDBs:
...
When abnormal situations such as standby PDB failures occur, the standby PDB needs to be rebuilt. PDB construction can be performed by PDB administrators themselves or managed uniformly through the CDB root. For specific operations, please refer to [Standby PDB Abnormal Repair](./​​Standby Repair.html#buildpdb).
