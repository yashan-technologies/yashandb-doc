...
- **Initial Password for System User**
    ...
    To protect user privacy, YashanDB does not provide an initial password for the system user. Users must set it using the [yaspwd](../../工具手册/yaspwd) tool or the [yasboot](../../工具手册/yasboot/yasboot命令介绍/yasboot cluster) tool after product installation.

- **Password Modification for System User**

    For security reasons, it is recommended to regularly change the password for the system user. This can be done using the [yaspwd](../../工具手册/yaspwd) tool, [yasboot](../../工具手册/yasboot/yasboot命令介绍/yasboot cluster) tool, or the [ALTER USER](../SQL参考手册/SQL语句/ALTER USER) statement. 
...
- **Creating Normal Users**
    ...
    When creating normal users using the [CREATE USER](../SQL参考手册/SQL语句/CREATE USER) statement, passwords can be set in plaintext or encrypted formats.
