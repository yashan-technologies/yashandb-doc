...
4. If it is necessary to use YashanDB PL stored procedure capability in mysql mode, the YashanDB PL syntax must be followed. However, static and dynamic SQL statements inside the stored procedures will be parsed according to the compatibility mode configured by the session parameter `COMPAT_VECTOR`. If the session parameter `COMPAT_VECTOR` is not adjusted, it defaults to mysql mode. For YashanDB PL stored procedure capabilities, please refer to the [PL Reference Manual](../Development Guide/PL Reference Manual/00PL Reference Manual).
