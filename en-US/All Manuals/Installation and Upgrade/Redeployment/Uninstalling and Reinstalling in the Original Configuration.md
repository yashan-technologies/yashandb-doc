...
Before uninstalling YashanDB, it is recommended to perform [data backup](../../数据库管理/备份与恢复/00备份与恢复) to ensure data integrity and security, so that a complete data copy is available for restoration or migration if needed.
...
3. Uninstall yasom and yasagent.
    ...
    > **Note**:
    >
    > If you have enabled [Resource Management](../../数据库管理/资源管理/00资源管理) CPU management functionality, use the `-t` parameter in this command to clean the auto-start configuration of CPU resource management functionality.
