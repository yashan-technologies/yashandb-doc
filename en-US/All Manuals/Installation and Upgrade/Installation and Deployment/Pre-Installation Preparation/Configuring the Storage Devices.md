...
- **System Disk**
	...
	The number of planned system disks will affect the [redundancy](../../../概念手册/集群基础设施/崖山存储管理.html#redundancy) of the system disk group, with 1, 3, or 5 system disks corresponding to EXTERNAL, NORMAL, or HIGH redundancy.
...
| 2   | Step 4 (Optional): Modify Multi-path Configuration File | If using multipath binding and needing [IO Fencing based on SCSI persistent reservations](../../../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing) functionality, this functionality must be modified in the multipath configuration file on each server of YAC before use. |
...
When using multipath binding for storage devices, this optional step must be completed on each server in YAC before utilizing the [IO Fencing based on SCSI persistent reservations](../../../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing) functionality.
...
8. Add a subsystem port to the new subsystem and configure the port's IP protocol version `adrfam`, listening address `traddr`, transport type `trtype`, and transport port `trsvcid`. 
   ...
   The subsystem port ID must be a positive integer, and the listening address must be an IP address within the subnet of [Network Planning](./网络准备) (For example, 172.16.1.4 in the table above). The transport port trsvcid must be the port number opened according to the [Port Planning](./网络准备.html#openports).
