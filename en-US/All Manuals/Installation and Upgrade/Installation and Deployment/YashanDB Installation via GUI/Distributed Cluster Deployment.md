...
4. In the pop-up dialog, configure other server information and click [Confirm] to save the configuration.
   ...
   - Installation path: The database installation path, recommended to be configured as the planned [HOME directory](../安装前准备/创建用户和目录)/{version_number} (this does not check whether the /{version_number} subdirectory exists; it will be created automatically during installation), supports numbers, letters (case-sensitive), and certain symbols (`/`, `-`, `_`, `.`), e.g., /data/yashan/yasdb_home/{version_number}.

   - Log path: [Run log directory](../安装前准备/创建用户和目录), e.g., /data/yashan/log.
...
1. In the pop-up node scale configuration dialog, adjust the relevant configurations based on the [actual planning](../安装前准备/服务器准备) for the number of instances, and click [Confirm] to save the information.
...
On the [DB node parameters] page,  refer to [Configuration Parameters](../../../参考手册/配置参数) to add/delete/edit corresponding parameters for each database instance as needed, and after confirming the information is correct, click [Save and Go to the Next Step].
