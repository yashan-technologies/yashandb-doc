...
Connection modes (multi-IP scenarios) can only be configured when connecting to the YashanDB server via a driver, such as the [JDBC driver](../../../开发手册/JDBC驱动/JDBC驱动使用介绍/00JDBC驱动使用介绍), [C driver](../../../开发手册/C语言系驱动/C驱动/C驱动接口说明/连接、授权和初始化函数/yacConnect), [OCI driver](../../../开发手册/C语言系驱动/OCI驱动/OCI连接配置说明), and [ODBC driver](../../../开发手册/C语言系驱动/ODBC驱动/ODBC安装说明/00ODBC安装说明), and requires multiple nodes on the YashanDB server.
...
- Complete the [YashanDB Client Installation](../YashanDB客户端安装/00YashanDB客户端安装) and environment variable configuration. If connecting to the local database under the installation user on the server, there is no need to reinstall *yasql*.

- Ensure that the user has the [CREATE SESSION privilege](../../../产品安全/数据访问控制/特权与角色管理/系统特权) (or [CONNECT role](../../../产品安全/数据访问控制/特权与角色管理/角色)).

- Ensure that the IP of the device initiating the connection is not on the [blacklist](../../../产品安全/连接管理/管理IP黑白名单).
...
For details on driver configurations and connection parameters, ​please ​refer to [JDBC Driver](../../../mysql模式参考手册/快速入门/JDBC应用工程对接示例-MySQL驱动) and [C Driver](../../../开发手册/C语言系驱动/C驱动/00C驱动).
...
The connection mode (multi-IP scenario) can only be configured when using the driver to connect to the YashanDB server. The mysql mode supports only the [JDBC Driver](../../../开发手册/JDBC驱动/JDBC驱动使用介绍/00JDBC驱动使用介绍) and [C Driver](../../../开发手册/C语言系驱动/C驱动/C驱动接口说明/连接、授权和初始化函数/yacConnect) , and requires that the YashanDB server has multiple nodes.
...
- **Login**
    ...
    - **{database_name}**: Optional, specifies the default database for subsequent operations in the current session after connecting to YashanDB. If not specified, no database will be entered, and you can specify it using the [USE](../../../mysql模式参考手册/SQL参考手册/SQL语句/USE) statement after connecting.
