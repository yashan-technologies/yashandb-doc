...
Connection modes (multi-IP scenarios) can only be configured when connecting to the YashanDB server via a driver, such as the [JDBC driver](../../../开发手册/JDBC驱动/JDBC驱动使用介绍), [C driver](../../../开发手册/C语言系驱动/C驱动/C驱动接口说明/连接、授权和初始化函数/yacConnect), [OCI driver](../../../开发手册/C语言系驱动/OCI驱动/OCI连接配置说明), and [ODBC driver](../../../开发手册/C语言系驱动/ODBC驱动/ODBC安装说明/00ODBC安装说明), and requires multiple nodes on the YashanDB server.
...
- Ensure that the IP of the device initiating the connection is not on the [blacklist](../../../产品安全/连接管理/IP黑白名单管理).
...
The connection mode (multi-IP scenario) can only be configured when using the driver to connect to the YashanDB server. The mysql mode supports only the [JDBC Driver](../../../开发手册/JDBC驱动/JDBC驱动使用介绍) and [C Driver](../../../开发手册/C语言系驱动/C驱动/C驱动接口说明/连接、授权和初始化函数/yacConnect) , and requires that the YashanDB server has multiple nodes.
