YashanDB supports online expansion of CN nodes in Distributed Cluster Deployment through the [yasboot](../../../Tools Guide/yasboot/00yasboot) tool, and the expansion does not affect the usage of the entire cluster.

> **Note**:
>
> Each CN node will be hosted by a separate server (hereinafter referred to as "new server"). It is recommended that the new server's OS version be consistent with existing server(s).
...
3. Execute the [yasboot config node gen](../../../Tools Guide/yasboot/Introduction to yasboot Command/yasboot config) command to generate the configuration file for adding CN node(s).
...
6. On the server where the configuration file was generated, execute the following command to add CN node(s).
