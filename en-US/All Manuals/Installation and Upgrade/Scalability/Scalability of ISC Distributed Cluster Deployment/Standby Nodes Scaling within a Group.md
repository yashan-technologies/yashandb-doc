YashanDB supports scaling up or down the standby nodes in the DN group or MN group of ISC Distributed Cluster Deployment via the [yasboot](../../../Tools Guide/yasboot/00yasboot) tool. Please read the relevant [considerations](../Considerations for Scaling) carefully before performing specific operations.

> **Note**: 
>
> - Each operation can only scale up or down the standby nodes of 1 group at a time.
>
> - If there are faults with existing nodes during scaling, or if the scaling fails, please refer to the [scaling issues handling](../Scaling Issues Handling) for resolution.

## Online Adding Standby Nodes in the Group

1. If you need to add new server(s) and increase standby node(s) in MN or DN groups, first refer to [Pre-installation Preparation](../../安装部署/安装前准备/00安装前准备) to check and ensure that all environmental requirements of the new server system have been met.
...
6. (Optional) Backup the database.
    ...
    It is recommended to [backup](../../../数据库管理/备份与恢复/00备份与恢复) the database to ensure there is a baseline backup set available for recovery after scaling.

7. If expansion is performed by adding new servers, after completion, the [[host]] content in hosts_add.toml needs to be copied and pasted to the end of hosts.toml to avoid using old host information during the upgrade.


8. Optionally enable leader election functionality within the DN group nodes as needed to ensure business continuity:

    - One-primary/one-standby environment: You need to manually enable [yasom election](../../../高可用/自动选主配置/一主一备yasom仲裁选主).

    - One-primary/multi-standby environment: *yasboot* will automatically enable [leader election](../../../高可用/自动选主配置/一主多备自动选主).
...
3. Please check and turn off yasom election first:
    ...
    For a one-primary/one-standby environment, you need to perform current operations to check and turn off yasom election. For a one-primary/multi-standby environment, whether or not leader election is enabled does not affect online scaling down performed via *yasboot*.
...
If the server where the standby node to be deleted is located can no longer be connected, the `--with-unconnected-host` parameter must be specified for scaling down.
...
5. (Optional) Backup the database.
    ...
    It is recommended to [backup](../../../数据库管理/备份与恢复/00备份与恢复) the database to ensure there is a baseline backup set available for recovery after scaling down.

6. If the --with-host parameter was specified during scaling (i.e., empty servers were removed), you need to update the [[host]] content in the hosts.toml file after scaling to delete the corresponding server-related information to avoid using old host information during upgrades.
...
7. After deleting the standby node, depending on the database high availability configuration, you can restore/re-enable leader election functionality to ensure business continuity:
    ...
    - One-primary/one-standby environment: You can enable [yasom election](../../../高可用/自动选主配置/一主一备yasom仲裁选主).

    - One-primary/multi-standby environment: You can enable [leader election](../../../高可用/自动选主配置/一主多备自动选主).
