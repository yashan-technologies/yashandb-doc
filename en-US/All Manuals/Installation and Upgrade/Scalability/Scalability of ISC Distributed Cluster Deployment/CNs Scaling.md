...
1. If you need to add new server(s) and increase CN nodes, first refer to [Pre-installation Preparation](../../安装部署/安装前准备/00安装前准备) to check and ensure that all environmental requirements of the new server system have been met.
...
5. (Optional) Backup the database.
    ...
    It is recommended to [backup](../../../数据库管理/备份与恢复/00备份与恢复) the database to ensure that there is a recoverable baseline backup set after scaling.
...
4. (Optional) Backup the database.
    ...
    It is recommended to [backup](../../../数据库管理/备份与恢复/00备份与恢复) the database to ensure that there is a recoverable baseline backup set after scaling down.
