...
- After a successful upgrade, you need to determine whether any deprecated parameters are in use based on the running log warnings. Please refer to the parameter initialization section example in [Parameter Configuration](../../../数据库管理/实例管理/数据库参数配置). If deprecated parameters are in use, check the introduction of the corresponding parameters in [Configuration Parameters](../../../参考手册/配置参数) to determine if the parameter has become deprecated due to renaming and whether it is necessary to adjust the deprecated parameter to the corresponding new parameter. If the new parameter is not configured, the value of the deprecated parameter will be automatically converted to the new parameter value.

## Steps

The upgrade process requires the use of the *yasboot* tool, and all steps must be performed on the server where the *yasboot* tool is located (which has the hosts.toml file). For detailed introductions to *yasboot* commands, please refer to [yasboot](../../../工具手册/yasboot/00yasboot).
...
2. Check whether yasom and yasagent processes exist in the current environment and their versions.
    ...
    yashan 102280      1  0 16:24 ?        00:00:00 /data/yashan/yasdb_home/yashandb/{old_version_number}/bin/yasagent --init -c yashandb -l 192.168.1.2:1676 --host-id host0001 -k                  -d
    yashan 102323      1  0 16:24 ?        00:00:00 /data/yashan/yasdb_home/yashandb/{old_version_number}/bin/yasom --init -c yashandb -l 192.168.1.2:1675 -k                  -d
    
    # Get the bin directory of yasom and yasagent, and execute the -h command to obtain current version information
    $ {the bin directory of yasom and yasagent}/yasom -h
    Usages: yasom [<flags>] <command>

    yasom daemon process, version: Release {old_version_number}
    ```

    If yasom and yasagent processes exist, execute the following command or use the operating system's process termination command to stop them.

    ```shell
    $ {the bin directory of yasom and yasagent}/yasboot process yasom stop -c yashandb -t /home/yashan/install/hosts.toml && /data/yashan/yasdb_home/yashandb/{old_version_number}/bin/yasboot process yasagent stop -c yashandb -t /home/yashan/install/hosts.toml
...
4. Based on the current database environment information, execute the following command to generate the server configuration file (hosts.toml).
   ...
   ```shell
   $ ./bin/yasboot package se gen -c yashandb \
   ...
   |Parameter |Description |
   |--------------------|--------------------------------------|
   | -c, --cluster          | The cluster name of the current database |
...
9. Check the version of yasom and yasagent again.
    ...
    # The version is now {new_version_number}
    ```
...
2. Check the current versions of yasom and yasagent.
    ...
    yashan 102323      1  0 16:24 ?        00:00:00 /data/yashan/yasdb_home/yashandb/{old_version_number}/bin/yasom --init -c yashandb -l 192.168.1.2:1675 -k                  -d
    # Get the bin directory of yasom and yasagent, and execute the -h command to obtain current version information
    $ /data/yashan/yasdb_home/yashandb/{old_version_number}/bin/yasom -h
    Usages: yasom [<flags>] <command>
...
1. Check the current database version.
    ...
    ```shell
    $ yasql / as sysdba
    YashanDB SQL Enterprise Edition Release {old_version_number} x86_64

    Connected to:
    YashanDB Server Enterprise Edition Release {old_version_number} x86_64 - Linux
...
2. Execute the corresponding command based on the database deployment form to upgrade the database.
    ...
    - Standalone Deployment, YAC Deployment (upgrading between versions 23.4.x.x and above), ISC Distributed Cluster Deployment 
        ...
        If you need to use a [custom backup path](../升级前准备.html#space), the --backup-path parameter must also be specified.
    ...
    - YAC Deployment (from 23.2.x.x and earlier to 23.4.x.x and above)
        ...
        |Parameter |Description |
        |--------------------|--------------------------------------|
        | --disk-config       | Specify [YAC upgrade configuration file](../升级前准备.html#sys-data) |

3. Check the database version after upgrade and status.  

    ```shell
    $ ./bin/yasboot cluster status -c yashandb -d

    $ ./bin/yasql / as sysdba
    YashanDB SQL Enterprise Edition Release {new_version_number} x86_64

    Connected to:
    YashanDB Server Enterprise Edition Release {new_version_number} x86_64 - Linux
    SQL> SELECT version FROM V$INSTANCE;
...
4. If it is a YAC upgrade scenario, the cluster needs to be restarted after successful upgrade.

    ```shell 
    $ ./bin/yasboot cluster restart -c yashandb --disable
    task completed, status: SUCCESS
...
- Enable or Disable Certain Configurations
  ...
  If the functionality supported by the new version is disabled by default after the upgrade (for example, SCAN or VIP functionality for YAC), you can manually configure and enable the corresponding functionality according to the process (for example, [configure SCAN for YAC](../../../数据库管理/集群管理/SCAN管理.html#scan_configuration)).

  >**Note**:
  >
  > Starting from version 23.2.1.x, the yasom election functionality depends on enabled [OS Authentication](../../../产品安全/身份标识与鉴别/操作系统认证/00操作系统认证). If yasom election was enabled in versions below 23.2.1.x (which will be temporarily disabled during the pre-upgrade preparation), you must enable it again after upgrading to version 23.2.1.x or above. You should first check and enable the operating system authentication to ensure the proper functioning of the election functionality.