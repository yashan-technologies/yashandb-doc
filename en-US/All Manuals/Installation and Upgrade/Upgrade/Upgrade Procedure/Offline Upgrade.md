...
- After a successful upgrade, you need to determine whether any deprecated parameters are in use based on the running log warnings. Please refer to the parameter initialization section example in [Parameter Configuration](../../../数据库管理/基本数据库管理/参数配置). If deprecated parameters are in use, check the introduction of the corresponding parameters in [Configuration Parameters](../../../参考手册/配置参数) to determine if the parameter has become deprecated due to renaming and whether it is necessary to adjust the deprecated parameter to the corresponding new parameter. If the new parameter is not configured, the value of the deprecated parameter will be automatically converted to the new parameter value.
