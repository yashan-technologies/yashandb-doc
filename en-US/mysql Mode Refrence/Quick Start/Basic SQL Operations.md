...
2. Open Navicat and use the MySQL driver to connect to the database. Among them:
    ...
    - The **host** and **port number** are the parameter values of `mysql_addr` in the yashandb.toml file under the database installation path. If installed with default configuration, the host is the database server's IP address, and the default port is 1690 (Standalone (primary/standby) deployment) or 1691 (YAC deployment).
3. (Optional) When using mysql-client 5.7.42 to connect to YashanDB service, the connection string can refer to `mysql -h ip_addr -P port -u user_name -p passwd`, where **ip_addr** and **port** are the parameter values of `mysql_addr` in the yashandb.toml file under the database installation path. If installed with default configuration, it will be the database server's IP address and the default port is 1690 (Standalone (primary/standby) deployment) or 1691 (YAC deployment); **user_name** and **passwd** are the test users created in [Quick Start with YashanDB](Quick Start with YashanDB).
   ```sql
