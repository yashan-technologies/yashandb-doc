...
- System Users (Built-in Users):
    ...
    - System Administrator: When the separation of duties is not enabled, acts as the super administrator responsible for database system management, with all privileges; when the [separation of duties](../../All Manuals/Reference Manual of mysql Mode/数据访问控制/三权分立) is enabled, lacks audit and security management-related privileges. Username: SYS. The initial password is specified during installation, and subsequent password updates must be generated and maintained through the yaspwd tool.
...
2. Execute the [CREATE USER](../../All Manuals/Reference Manual of mysql Mode/SQL参考手册/SQL语句/CREATE USER) statement to create the user and specify the password for the new user if needed.
...
- Manage user privileges: New users created under the mysql mode are granted the database connection privilege by default, and [grant the corresponding additional privileges](../../All Manuals/Reference Manual of mysql Mode/产品安全/特权管理/权限授予与收回) before they can use database-related functionalities.
...
2. Execute the [ALTER USER](../../All Manuals/Reference Manual of mysql Mode/SQL参考手册/SQL语句/ALTER USER) statement to adjust the user's attributes, including changing the password.
...
- [Manage user privileges](../../All Manuals/产品安全/数据访问控制/特权与角色管理/权限授予与收回)
