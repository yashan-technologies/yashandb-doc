...
Users created under the mysql mode can be granted/revoked privileges that are the union of the privilege mechanisms of the yashan mode and mysql mode ([privileges related to mysql mode](00Privilege Management) and [privileges related to yashan mode](../../../All Manuals/产品安全/数据访问控制/特权与角色管理/00特权与角色管理), and privileges should be granted reasonably according to actual needs.

Privileges granted to users and revoked from users take effect immediately.

## Prerequisites

Execute the [CREATE USER](../../../All Manuals/Reference Manual of mysql Mode/SQL参考手册/SQL语句/CREATE USER) statement to create the user and specify the password for the new user if needed.
...
3. Execute the [GRANT](../../../All Manuals/Reference Manual of mysql Mode/SQL参考手册/SQL语句/GRANT) statement to initiate authorization for the target user. The optional privilege set can be found in [privileges related to mysql mode](../../../All Manuals/Reference Manual of mysql Mode/Product Security/Privilege Management/00特权管理).
...
2. Execute the [REVOKE](../../../All Manuals/Reference Manual of mysql Mode/SQL参考手册/SQL语句/REVOKE) statement to initiate the revocation operation for another user/role.
