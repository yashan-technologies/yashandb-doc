...
3. 开启附加日志，操作方式有以下两种：
  ...
  - [库级附加日志](../SQL参考手册/SQL语句/ALTER DATABASE.md#supplementallogclauses)：
  ...
  - [表级附加日志](../SQL参考手册/SQL语句/ALTER DATABASE.md#addsupplementalloggingclause)：
...
5. （可选）指定要解析的表名，以指定2张表为例。

    若待解析的范围是该数据库中所有表，则无需执行本操作。
