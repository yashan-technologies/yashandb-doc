...
- 创建YStream服务前，建议先开启附加日志（[库级](../../SQL参考手册/SQL语句（yashan模式）/ALTER DATABASE.html#supplementallogclauses)或[表级](../../SQL参考手册/SQL语句（yashan模式）/ALTER DATABASE.html#addsupplementalloggingclause)）。若不开启，则只能解析已有日志中的附加信息，且在已有YStream服务时再开启附加日志会报错。
