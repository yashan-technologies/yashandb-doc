DBMS_ROWID包提供了一组内置的函数，用于获取[ROWID](../../SQL参考手册/数据类型/ROWID UROWID.md)的数据块号blockId、相对文件号fileId和行号dir信息。
...
|ts_type_in| 保留字段，使用缺省值，默认值为'SMALLFILE'，不区分大小写，且仅接受SMALLFILE、BIGFILE。该参数的数据类型可以为VARCHAR类型和可以隐式转换为VARCHAR类型的其他数据类型，详情请查阅[数据类型转换](../../SQL参考手册/数据类型/数据类型转换.md)。 |
...
| :---- |:-----------------------------------------------------------------------------------------------------------------------|
|row_id| 待解释的ROWID。该参数的数据类型可为ROWID/UROWID/RAW/CHAR/VARCHAR/NCHAR/NVARCHAR类型。                                                                                                              |
|ts_type_in| 保留字段，使用缺省值，默认值为'SMALLFILE'，不区分大小写，且仅接受SMALLFILE、BIGFILE。该参数的数据类型可以为VARCHAR类型和可以隐式转换为VARCHAR类型的其他数据类型，详情参阅[数据类型转换](../../SQL参考手册/数据类型/数据类型转换.md)。|
