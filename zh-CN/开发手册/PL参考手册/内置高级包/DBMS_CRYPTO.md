...
|  | DBMS_CRYPTO.CHAIN_ECB | 执行流程如下：<br/>1. 将明文切分成若干小段<br/>2. 每个小段直接使用密钥进行独立加密<br/>3. 将生成的每段密文块拼接后输出 |
| 密码块填充模式 | DBMS_CRYPTO.PAD_NONE | 无填充，需确保src必须为分组加密的块大小的整数倍、key必须为分组加密的块大小，否则会返回错误 |
|  | DBMS_CRYPTO.PAD_PKCS7 | 公钥加密标准#7（PKCS#7）填充|
|  | DBMS_CRYPTO.PAD_PKCS5 | 公钥加密标准#5（PKCS#5）填充|
|  | DBMS_CRYPTO.PAD_ZERO | 零填充，用0x00字节填充至块大小整数倍|
...
|key|用于解密的密钥，必须为RAW类型或能隐式转换为RAW类型的其他类型，且不能为NULL。DES、3DES、3DES_2KEY算法对应密钥长度必须合法。|
...
|key|用于加密的密钥，必须为RAW类型或能隐式转换为RAW类型的其他类型，且不能为NULL。DES、3DES、3DES_2KEY算法对应密钥长度必须合法。|
|iv |可选参数，块密码的初始化向量，必须为RAW类型或能隐式转换为RAW类型的其他类型，默认为NULL。|
