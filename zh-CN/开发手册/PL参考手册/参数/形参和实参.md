...
如果实参的数据类型与形参不一致，系统会对其进行到形参数据类型的转换，转换规则参考[CAST](../../SQL参考手册/内置函数/CAST)函数描述，如转换失败将抛出错误。
...
该关键字用于减少聚合类型（VARRAY、NESTED TABLE或INDEX-BY TABLE）的参数在子程序调用时的内存复制开销，指定时参数会按引用传递，不指定则默认按值传递。

NOCOPY关键字的使用规则如下：

- 仅对存储过程、自定义函数、自定义高级包或自定义类型中的子过程、子函数或嵌套子过程的OUT和IN OUT参数生效，其他场景仅用于兼容。

- 不论是否指定NOCOPY关键字，绑定参数始终按值传递。
