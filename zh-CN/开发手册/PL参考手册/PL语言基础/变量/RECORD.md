...
PL中定义的[用户自定义类型](../../PL对象/自定义类型)和SQL中定义的[普通标量数据类型](../../../SQL参考手册/数据类型（yashan模式）/00数据类型（yashan模式）)。当数据类型为字符型时，须同时指定其Size属性，例如VARCHAR(10)，但输入CHAR默认等同于CHAR(1)；可指定的最大Size为65534。 
