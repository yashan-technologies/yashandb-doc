...
YashanDB在每个会话可打开的游标数目由配置参数[OPEN_CURSORS](../../../../参考手册/配置参数)限制，默认值为310。

- 游标所绑定的SQL中包含有FOR UPDATE语句时，不支持dblink，其他约束同[SQL层的限制](../../../SQL参考手册/SQL语句（yashan模式）/SELECT.html#forupdateclause)，例如多表连接查询时仅对通过OF column_name指定列对应的表进行更新。
...
[SELECT](../../../SQL参考手册/SQL语句（yashan模式）/SELECT)查询语句，不可指定占位符（绑定参数）。为兼容语法，此处可以使用INTO子句，无实际作用。
