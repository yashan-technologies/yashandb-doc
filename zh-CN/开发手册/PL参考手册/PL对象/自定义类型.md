...
通过[CREATE TYPE](../../SQL参考手册/SQL语句（yashan模式）/CREATE TYPE)语句创建一个UDT类型，包括UDT的定义和编译，如果UDT编译失败，依然会被创建，但在使用时将报错。
...
除datatype可以为[普通标量数据类型](../../SQL参考手册/数据类型（yashan模式）/00数据类型（yashan模式）)，及已定义的UDT外，数组类型UDT的定义、使用及操作方法均与在过程体内声明的[集合变量类型](../PL语言基础/变量/集合变量)中的数组类型一致。
...
除datatype可以为[普通标量数据类型](../../SQL参考手册/数据类型（yashan模式）/00数据类型（yashan模式）)及已定义的UDT外，嵌套表类型UDT的定义、使用及操作方法均与在过程体内声明的[集合变量类型](../PL语言基础/变量/集合变量)中的嵌套表类型一致。
...
通过[CREATE TYPE BODY](../../SQL参考手册/SQL语句（yashan模式）/CREATE TYPE BODY)语句创建一个UDT类型主体，类型主体的名称必须和已创建的UDT的名称一样，否则该类型主体是无效状态。
...
Object类型的UDT内允许定义使用多个具有相同名字，但[形参](../参数/形参和实参)的名称、数量、顺序或[数据类型](../../SQL参考手册/数据类型（yashan模式）/00数据类型（yashan模式）)不同的方法。PL可以通过重载对此类同名方法进行筛选调用，使用规则同[自定义高级包重载调用子过程](./自定义高级包.html#overload)。
...
通过[DROP TYPE BODY](../../SQL参考手册/SQL语句（yashan模式）/DROP TYPE BODY)可以删除一个UDT类型主体。

通过[DROP TYPE](../../SQL参考手册/SQL语句（yashan模式）/DROP TYPE)可以删除一个UDT类型，同时删除它存在的类型主体。

## 修改UDT

通过[ALTER TYPE](../../SQL参考手册/SQL语句（yashan模式）/ALTER TYPE)可以修改一个UDT类型。
