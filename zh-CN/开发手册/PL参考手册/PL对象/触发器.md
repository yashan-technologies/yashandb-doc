...
- 在单机主备、主备集群部署中，只能在主库/主集群上创建、管理以及使用触发器。
...
- 同一个dml\_event\_clause中可以使用多个OF column [, column ]，但要求所有OF column [, column ]必须全位于UPDATE之后。当存在多个OF column [, column ]时，指定的列字段以最后一个OF column [, column ]为准，但校验是否重复指定同名列字段时将校验所有OF column [, column ]。
...
condition的语法与通用SQL语法[condition](../../SQL参考手册/通用SQL语法/condition.md)一致，但不能包含子查询和自定义函数。其中，对于列字段的引用需要按OLD/NEW（或由[referencing\_clause](#referencingclause)定义的名称）区分新旧值，引用格式为OLD.column，NEW.column。
...
[PL变量描述](../PL语言基础/变量/00变量.md)。

##### PL\_statements

定义过程体中的执行语句，为[PL语句](../PL语句/00PL语句.md)中的一项或多项，但任何语句或由语句调用的子程序，均不可以包含DDL和DCL语句内容。

其中，对于触发对象的列字段的引用需要按OLD/NEW（或由[referencing\_clause](#referencingclause)定义的名称）区分新旧值，引用格式及取值定义见下表：
...
使用[ALTER TRIGGER](../../SQL参考手册/SQL语句/ALTER TRIGGER.md)语句可以启用或禁用一个已创建的触发器。

## 删除触发器

触发器的删除方法包括：

- 使用[DROP TRIGGER](../../SQL参考手册/SQL语句/DROP TRIGGER.md)语句可以删除一个已存在的触发器。

- 使用[DROP TABLE](../../SQL参考手册/SQL语句/DROP TABLE.md)删除某个触发器的的触发对象时，会同时删除该触发器。若回收站已开启（配置参数RECYCLEBIN_ENABLED = ON），触发器会随对应表一起进入回收站，若回收站未开启，则触发器随对应表被彻底删除。

## 闪回触发器

使用[FLASHBACK](../../SQL参考手册/SQL语句/FLASHBACK.md)语句闪回表时，其对应的触发器对象也会一并闪回，闪回后触发器的名称不会自动复原（将保持为进入回收站后的名称）。 
