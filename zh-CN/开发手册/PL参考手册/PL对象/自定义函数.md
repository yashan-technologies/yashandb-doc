...
使用[CREATE FUNCTION](../../SQL参考手册/SQL语句（yashan模式）/CREATE FUNCTION)语句创建自定义函数，其语法定义为：
...
用于指定创建确定性函数，确定性函数是指在输入相同的情况下输出结果固定的函数，此类函数通常满足以下条件：
...
自定义聚集函数可以在[select_list](../../SQL参考手册/SQL语句（yashan模式）/SELECT.html#select_list)、HAVING子句和ORDER BY子句中作为[聚集函数](../../SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）.html#聚集函数aggregate-function)使用，返回单行数据或按GROUP返回分组数据，也可以通过OVER语法作为[窗口函数](../../SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）.html#窗口函数window-function)使用。
...
与[内置函数](../../SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）)的调用方法相同，作为SQL表达式使用，例如：
...
与[内置函数](../../SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）)的调用方法相同，作为表达式在PL语句中使用，或者用于给变量赋值，例如：
...
使用[DROP FUNCTION](../../SQL参考手册/SQL语句（yashan模式）/DROP FUNCTION)语句删除自定义函数。

#### 修改自定义函数

使用[ALTER FUNCTION](../../SQL参考手册/SQL语句（yashan模式）/ALTER FUNCTION)语句修改自定义函数。
...
确认外置UDF使用的自定义库不再使用时，可对其进行删除，使用[DROP_LIBRARY](../../SQL参考手册/SQL语句（yashan模式）/DROP LIBRARY)语句删除自定义库。
