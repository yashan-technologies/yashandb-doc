...
- 在单机主备、主备集群部署中，只能在主库/主集群上创建、管理以及使用自定义函数。
...
自定义函数的参数（形参），可省略，此时function\_name()=function\_name。最多可以指定4095个参数。
...
- 管道函数支持作为查询数据源（FROM子句）及查询投影列使用。但当管道函数位于投影列位置时，系统将自动降级为普通函数执行，不创建管道线程，不触发流式处理逻辑。
- 管道函数单行数据最大长度始终以[行长度规格](../../../产品描述/产品规格/逻辑规格.md#table)中较小值为准（即单行数据最大长度为64512Bytes）。
- 管道函数参数只能为入参，不能为出参、出入参。
- 管道函数的RETURN语句不能指定返回值。
- 若管道函数不在自治事务中则不允许有DML/DDL语句。
- 管道函数不能被过程体调用。
- 管道函数中可调用其他管道函数，调用链需遵循以下限制：
    - 位于查询数据源的管道函数仅允许出现一个。

    - 位于查询投影列的管道函数支持递归调用（含递归调用自身），不限制调用层级。
- 管道函数只能返回集合类型（关联数组除外），PIPE ROW写入的行数据必须是SQL类型或全局UDT类型。
- 目前管道函数参数暂无法使用游标类型以及游标表达式。
- 管道函数可输出DBMS_OUTPUT信息，输出行为持续至查询中第一个位于查询数据源的管道函数执行前，后续管道函数执行过程中的DBMS_OUTPUT信息不对外输出。
...
用于标识确定性函数，确定性函数是指在输入相同的情况下输出结果固定的函数。

对于确定性函数，若多次调用确定性函数时入参相同，返回值应完全相同。
...
自定义聚集函数可以在[select_list](../../SQL参考手册/SQL语句/SELECT.md#select_list)、HAVING子句和ORDER BY子句中作为[聚集函数](../../SQL参考手册/内置函数/00内置函数.md#聚集函数aggregate-function)使用，返回单行数据或按GROUP返回分组数据，也可以通过OVER语法作为[窗口函数](../../SQL参考手册/内置函数/00内置函数.md#窗口函数window-function)使用。
...
arguments为与argument\_define一一对应的[实参](../参数/形参和实参)，且argument\_define不能为IN OUT或OUT[形参](../参数/形参和实参)。
...
arguments为与argument\_define一一对应的[实参](../参数/形参和实参)。
...
若目标UDF已被用于创建[函数索引](../../SQL参考手册/SQL语句/CREATE INDEX.md#columnexpression)，该UDF及其过程体将不允许直接删除、修改、重编译以及替换，需先删除对应索引再操作目标UDF。可通过查询DBA_DEPENDENCIES视图中的REFERENCED_NAME是否存在目标UDF名称，若有则表示该UDF被其他对象依赖，操作时需考虑关联性。
