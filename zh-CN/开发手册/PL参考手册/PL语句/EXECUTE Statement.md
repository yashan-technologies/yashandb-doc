...
其中，v\_sql为SQL语句形式的字符串变量或常量。在YashanDB里，v\_sql除了可以为普通SQL语句外，还可以为"BEGIN..END;"形式的匿名块。
...
EXECUTE Statement里的v\_sql为动态SQL。

## 常规用法

*EXECUTE IMMEDIATE v\_sql;*

执行v\_sql字符串里包含的SQL语句。
...
*EXECUTE IMMEDIATE v\_sql INTO v1,v2,...;*

'v1,v2...'为已声明的变量。

执行v\_sql字符串里包含的SQL语句（只能是SELECT语句），并将查询结果赋值给变量，'v1,v2...'与SELECT语句的查询项一一对应。
...
*EXECUTE IMMEDIATE v\_sql  USING \[IN|OUT|IN OUT\] a1,a2,...;*

a1,a2...为变量或常量形式的[绑定参数](../参数/绑定参数)。

> **Note**: 
> v\_sql是匿名块且使用`:name` 形式占位符，同名的占位符只需要绑定一次即可。

IN|OUT|IN OUT为绑定参数类型，默认为IN类型。规则如下：

*   IN参数可以为变量、变量表达式或常量；
*   OUT和IN OUT参数只能为已声明的变量；
*   当v\_sql里包含了对另一个过程体的调用时，对绑定参数的类型使用与该过程体定义的参数类型一致。
...
RETURING语法请查阅[UPDATE](../../SQL参考手册/SQL语句/UPDATE.md#returningclause)。

### RETURNING INTO 用法

*EXECUTE IMMEDIATE v\_sql RETURNING INTO v1,v2,...;*

`v1,v2...`为已声明的变量。

执行v\_sql字符串里包含的SQL语句（只能是UPDATE语句），并将v\_sql中returning指定的表达式的结果赋值给变量，`v1,v2...`与UPDATE语句的RETURNING项一一对应。

当USING和RETURNING同时使用时，USING语句应该位于RETURNING语句前面。
...
### RETURNING BULK COLLECT INTO 用法

*EXECUTE IMMEDIATE v\_sql RETURNING BULK COLLECT INTO v1,v2,...;*

`v1,v2...`为已声明的集合变量。

执行v\_sql字符串里包含的SQL语句（只能是UPDATE语句，UPDATE语句中不能使用"BULK COLLECT"），并将v\_sql中retruning 表达式的结果赋值给集合变量，`v1,v2...`与UPDATE语句的RETURNING 项一一对应。
