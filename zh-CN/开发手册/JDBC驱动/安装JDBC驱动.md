...
## 安装YashanDB JDBC驱动
### 方式一：通过Maven安装YashanDB JDBC驱动

在项目中使用Maven管理器配置yashandb-jdbc库。

yashandb-jdbc已发表在[The Maven](https://central.sonatype.com/search?q=yashandb-jdbc&smo=true)上，具有以下Group ID和artifactId：
...
> **Note**:
>
> x.y.z表示可用的jdbc版本号，例如1.9.27。


### 方式二：Gradle安装YashanDB JDBC驱动

在项目中使用Gradle管理器配置yashandb-jdbc库。
...
### 方式三：离线安装YashanDB JDBC驱动

1. 从[YashanDB官网下载中心](https://download.yashandb.com/download)下载最新JDBC安装包，或者联系我们的技术支持获取对应的软件包。

2. 将jar文件下载到本地路径，例如/path/yasdb-jdbc-{version_number}.jar。

3. 将YashanDB JDBC驱动加入到JDK环境中：
    ...
    # 编译Java源文件（假设源文件名为HelloWorld.java）
