...
FIRST_VALUE为[窗口函数](00内置函数（yashan模式）.html#WindowFunction)，并支持滑动窗口。本函数依据给定的窗口条件计算出窗口数据集合，并返回该集合的第一行记录所对应的[expr](../通用SQL语法/expr)的值。如果第一个值为NULL且未指定IGNORE NULLS，则函数返回 NULL。
