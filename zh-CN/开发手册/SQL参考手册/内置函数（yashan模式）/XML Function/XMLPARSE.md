...
* 使用DOCUMENT字段时，xml_string必须是一个完整的带有根节点的XML文档。
* 使用CONTENT字段时，xml_string可以不带根节点，但必须是一个合法的XML值。
* 当使用WELLFORMED字段时，则表明输入的xml是一个完整合法的XML数据，此时解析不会校验合法性，否则将校验xml_string的合法性。

**xml_string**

[通用表达式](../../通用SQL语法/expr)，待解析的XML字符串，遵循如下规则：
