...
ARRAY_POSITION函数以start_locate为起点查找数组（[数组变量](../../PL参考手册/PL语言基础/变量/集合变量)，[数组对象](../../PL参考手册/PL对象/自定义类型)，[数组类型](../数据类型/用户自定义类型)）array_var的成员中第一个出现compare_member的位置，并返回INT类型的该位置值。
...
- 一个存在的数组，数组成员必须为[普通标量数据类型](../数据类型/00数据类型)。
- NULL，此时函数返回NULL。
...
[通用表达式](../通用SQL语法/expr)，不可为NULL，其值类型须为INT，或可隐式转换为INT。
