...
ST_CONTAINSPROPERLY函数的功能是判断geometry1是否完全包含geometry2，完全包含时返回TRUE，否则返回FALSE。与[ST_CONTAINS](ST_CONTAINS)不同的是，ST_CONTAINS(A,A) = TRUE，ST_CONTAINSPROPERLY(A,A) = FALSE。
