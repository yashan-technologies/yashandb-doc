...
JSON\_ARRAY\_LENGTH函数返回一个JSON数组数据的长度。

**json_value**

json_value为一个二进制[json](../通用SQL语法/json)数据，可通过[JSON](./JSON JSON_PARSE)函数获取。

当json\_value为JSON Array类型时，函数返回数组的长度；当json\_value为JSON Object/String/Number/Boolean/Null/扩展格式类型时，函数返回NULL；当json\_value为NULL时，函数返回NULL。
