...
```ebnf
= MERGE [hint] INTO target_table_clause USING source_table_clause ON "(" condition ")" (merge_update_clause | merge_insert_clause) {" " (merge_update_clause | merge_insert_clause)}.
```

**[target\_table\_clause](#targettableclause)::=**

```ebnf
= ([schema "."] (table_name [partition_extension_clause] | view_name) | ( "(" (subquery) [subquery_restriction_clause] ")")) [t_alias].
```
...
<span id="targettableclause" name="targettableclause"></span>

### target\_table\_clause
...
<span id="sourcetableclause" name="sourcetableclause"></span>

### source\_table\_clause
...
该语句为YashanDB通用[条件语句](../通用SQL语法/condition)，用于指定MERGE选择下一步操作的条件，对目标表里的每行记录执行此条件判断，为真（MATCHED）时，将执行merge\_update\_clause；为假（NOT MATCHED）时，将执行merge\_insert\_clause。
...
<span id="mergeupdateclause" name="mergeupdateclause"></span>
...
<span id="mergeinsertclause" name="mergeinsertclause"></span>
