...
```ebnf
= ALTER [PUBLIC] OUTLINE outline_name (REBUILD|RENAME TO new_outline_name|CHANGE CATEGORY TO category_name|(ENABLE | DISABLE | SET ID UPGRADE)).
```
...
### RENAME TO new\_outline\_name

对OUTLINE重命名。重命名的OUTLINE名称不能带模式名指定，且需符合YashanDB的[对象命名规范](../基本SQL元素/标识符.md)。

### CHANGE CATEGORY TO category\_name

更改OUTLINE对应的类别。类别名称可以是已经存在的或者新指定的，不能带模式名指定，且需符合YashanDB的[对象命名规范](../基本SQL元素/标识符.md)。
