...
该语句用于开启或关闭表级附加日志属性。表级附加日志只对当前的表生效，数据库级附加日志请参考[ALTER DATABASE](./ALTER DATABASE.md#supplementallogclauses)。

存算一体分布式集群部署中用户无法执行此操作。

如需使用[YStream服务](../../PL参考手册/内置高级包/DBMS_YSTREAM_ADM.md)，应先开启附加日志（库级或表级）再创建YStream服务，若先创建了YStream服务再开启附加日志会报错。
...
通过[dba\_log\_groups](../../../参考手册/系统视图/DBA视图/DBA_LOG_GROUPS.md)、[all\_log\_groups](../../../参考手册/系统视图/ALL视图/ALL_视图.md)、[user_log_groups](../../../参考手册/系统视图/USER视图.md)视图可以查看当前已开启的附加日志信息。
...
由于收缩数据涉及到物理位置变更，执行本操作前必须开启行迁移，具体操作请查阅[row\_movement\_clause](#rowmovementclause)。
...
- 在共享集群/分布式集群部署中，如果发生实例启停、实例加入或共享集群故障，会把所有nologging表都标记为corrupted。
...
该语句用于修改表的parallel属性，parallel属性用于指定查询该表的并行度，语法同CREATE TABLE中的[parallel_clause](CREATE TABLE.md#parallelclause)。
...
为增加的列字段指定一个符合该列字段数据类型的缺省值，default\_expr可以为字面量、运算式、函数等表达式。
...
该语句设置指定列的身份属性，配置语法请参考[identity_clause](#identityclause)。

###### inline\_constraint

该语句用于在增加列字段时同时定义表的行内约束项。关于约束项的详细描述请参考通用SQL语法[constraint](../通用SQL语法/constraint)。
...
该语句用于在增加列字段时同时定义列的加密属性，规则同[CREATE TABLE](./CREATE TABLE.md#columnencryptionclause)。
...
该语句为增加的LOB/JSON字段指定存储属性，语法同[CREATE TABLE](./CREATE TABLE)中lob\_clause子句描述。
...
修改列字段的数据类型为YashanDB认可的[数据类型](../数据类型/00数据类型)。
...
- **原数据类型为LOB/JSON/UDT的其他列**：不允许修改其数据类型。
- **原数据类型为字符型的其他空列**：允许将其修改为除LOB/JSON/UDT外的其他数据类型。
- **原数据类型为字符型的其他非空列**：允许修改为不同字符型，但仍需遵顼以下规则：
    ...
    - HEAP表不允许跨存储方式修改数据类型，例如HEAP表不允许将列数据类型从VARCHAR(3200)（采用普通字符串存储）修改为VARCHAR(8004)（会自动转换为LOB型存储），而TAC表允许此操作。存储方式介绍请查阅[字符型](../数据类型/字符型)。

    - HEAP表允许修改以LOB类型存储的列的数据类型长度属性，例如HEAP表可以将列数据类型从VARCHAR(32000)（会自动转换为LOB类型存储）修改为VARCHAR(1000)，但修改后列数据存储方式仍保持为LOB类型。

- **原数据类型为其他数据类型的其他空列**：允许将其修改为除LOB/JSON/UDT外的其他数据类型。
...
修改或删除列字段的缺省值，default\_expr可以为字面量、运算式、函数等表达式。
...
该语句修改指定列的身份属性，配置语法请参考[identity_clause](#identityclause)。

##### inline\_constraint

修改列字段的约束项，遵循通用[constraint](../通用SQL语法/constraint)里的行内约束所定义语法要求。
...
- 不允许对不符合创建主键要求的列字段执行modify PRIMARY KEY，创建主键的要求请参考[constraint](../通用SQL语法/constraint)描述。
...
- 不允许对不符合创建外键要求的列字段执行modify FOREIGN KEY，创建外键的要求请参考[constraint](../通用SQL语法/constraint)描述。
...
该语句中range_subpartition_desc、list_subpartition_desc、individual_hash_subparts的描述请参考[CREATE TABLE](./CREATE TABLE.md)。
...
该语句能将一个范围或者列表分区重新拆分为多个分区。当一个分区过于臃肿影响到了查询、备份等性能时，可以考虑SPLIT PARTITION。
...
可以指定拆分分区时是否同时更新索引，可省略，省略表示拆分时不对索引进行更新。

- 指定UPDATE INDEXES表示拆分分区的同时更新本地分区索引和全局索引。

- 指定UPDATE GLOBAL INDEXES表示拆分分区的同时更新全局索引，使其不失效。若未指定，全局分区可能会失效：

    - 若原分区不存在数据或原分区的数据能完全划分到某一个新分区，则拆分分区不会改变全局索引的状态。

    - 若原分区存在数据但不能完全划分到某一个新分区，拆分分区后会失效全局索引，需手动[重建（rebuild_clause）](./ALTER INDEX.md#rebuildclause)使其恢复。
...
- 指定UPDATE INDEXES表示合并分区的同时更新本地分区索引和全局索引。

- 指定UPDATE GLOBAL INDEXES表示合并分区的同时更新全局索引，使其不失效。若未指定，全局分区可能会失效：

    - 若原分区存在数据，合并分区后会失效全局索引，需手动[重建（rebuild_clause）](./ALTER INDEX.md#rebuildclause)使其恢复。

    - 若原分区不存在数据，合并分区不会改变全局索引的状态。
...
该语句的使用规则同[merge\_table\_partitions](#mergetablepartitions)。
...
该语句用于交换分区表的某个分区与一个非分区表的数据，其中CASCADE仅用于语法兼容，无实际含义。

该语句须遵循如下限制：

- 仅适用于HEAP表，且目标表不能为分区表。

- 要求用户对目标分区表和目标常规表具备读写权限。

- 分区和目标表的列定义和约束必须完全相同。

- 不允许存在NESTED TABLE类型的列。

- 如需同时交换索引（指定INCLUDING INDEXES），分区上的本地索引必须能与目标表上的索引一一对应且定义一致。
...
指定在交换数据时是否同时交换索引，可省略，省略则默认不交换。

- INCLUDING INDEXES：交换数据时额外检查分区表的本地索引与常规表的索引是否完全相同，校验通过则执行交换，交换分区数据的同时会交换对应的本地索引分区数据和普通索引数据，校验不通过则报错。

- EXCLUDING INDEXES：不校验索引的定义，交换数据后本地索引对应分区与常规表的索引均变为不可用状态（UNUSABLE）。

##### WITH|WITHOUT VALIDATION

指定在交换数据时是否校验分区表换入的数据满足分区定义与否，可省略，省略则默认校验。

- WITH VALIDATION：校验换入数据是否满足分区定义，不满足则报错。

- WITHOUT VALIDATION：不校验换入数据，有可能会换入不满足分区定义的数据。

##### UPDATE [GLOBAL] INDEXES

指定在交换数据时是否同时更新全局索引，指定UPDATE INDEXES或UPDATE GLOBAL INDEXES均表示更新全局索引。可省略，省略则默认不更新。

- 更新：换出的数据从全局索引删除，换入的数据插入全局索引。
...
约束项，关于约束项的详细描述请参考通用SQL语法[constraint](../通用SQL语法/constraint)。
...
如某个约束项在其创建时未指定名称，可以从系统提供视图（如user\_constraints）中查询到其默认名称后再执行删除操作。
...
启用或停用指定的约束项（含义见[constraint](../通用SQL语法/constraint)中的ENABLE|DISABLE描述）。
...
启用或停用约束检查（含义见[constraint](../通用SQL语法/constraint)中的VALIDATE|NOVALIDATE描述）。
...
在启用主键/唯一约束项时，使用本语句可对系统自动复用或创建的索引进行人工指定，详细语法定义和描述见[constraint](../通用SQL语法/constraint)中的using_index_clause。
...
* START WITH用于指定序列号生成器的起始值，该值需介于MINVALUE和MAXVALUE之间。  
如果 START WITH LIMIT VALUE，则需要先判断表是否为空。 如果表为空，则起始值为MINVALUE（升序序列号生成器）或 MAXVALUE（降序序列号生成器），如果表不为空，则起始值为当前表中最大值+增量（升序序列号生成器）或当前表中最小值+增量（降序序列号生成器）。  
...
* MINVALUE/NOMINVALUE用于指定序列号生成器的最小值，对于升序序列号生成器，NOMINVALUE表示为1；对于降序序列号生成器，NOMAXVALUE表示为操作系统的最大负整数数值。  
