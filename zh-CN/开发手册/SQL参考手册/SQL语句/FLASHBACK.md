...
- 确保已开启[全库闪回](ALTER DATABASE.md#flashbackdatabaseclauses)功能。
...
- 执行全库闪回时数据库需处于MOUNT阶段，闪回完成后必须完成相应的后置操作，详细的数据库闪回操作请查阅[全库闪回](../../../数据库管理/故障处理/闪回/全库闪回/00全库闪回.md)。

    - 若在非主备高可用部署中执行的闪回，必须执行[ALTER DATABASE OPEN RESETLOGS](ALTER DATABASE.md#startupclauses)重置redo时间线。

    - 若在主备高可用部署中的主库/主集群上执行的闪回，必须执行[ALTER DATABASE OPEN RESETLOGS](ALTER DATABASE.md#startupclauses)重置主库/主集群的redo时间线，并执行备库/备集群闪回或构建使其追平主库。

    - 若在主备高可用部署中的备库/备集群上执行的闪回，会自动停止redo日志回放，需手动[开启回放](ALTER DATABASE.md#start_apply)。
