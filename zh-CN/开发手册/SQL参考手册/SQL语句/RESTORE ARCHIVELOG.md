...
指定FROM指令恢复归档日志时，恢复的终点为当前数据库的日志恢复点的上一个归档文件，具体取值可以查询V$DATABASE视图的RCY\_POINT字段中的ASN。
...
指定UNTIL指令恢复归档日志时，恢复的起点为当前数据库的日志刷盘点ASN，具体取值可以查询V$DATABASE视图的FLUSH\_POINT字段中的ASN。
