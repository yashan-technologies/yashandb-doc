...
当YashanDB部署为容器数据库（配置参数ENABLE_PLUGGABLE_DATABASE=TRUE）时，若连接根容器使用该语句，表示进行全局操作，即涵盖根容器和所有PDB。若直连某个PDB使用该语句，则表示对该PDB进行操作。但配置数据库[全库闪回](#flashbackdatabaseclauses)功能除外，该配置在根容器和各个PDB中相互独立。

存算一体分布式集群部署中，仅支持如下语法，推荐使用[yasboot](../../../工具手册/yasboot/yasboot命令介绍/00yasboot命令介绍)管理数据库。 
...
MOUNT是数据库挂载物理文件后的状态，OPEN是数据库打开后的状态，详细数据库启动介绍请参考[实例启停](../../../数据库管理/实例管理/实例启停)。 
...
关闭某个数据文件的自动扩展，同时该数据文件的NEXT\_SIZE和MAX\_SIZE被设置为0。
...
将用户创建的表空间中某个数据文件立即脱机（不等待数据刷盘），数据文件脱机后不可读写，且不能对该文件进行resize或设置自动扩展开关等操作。

将数据文件脱机会同时将其所属表空间脱机，该表空间内的所有数据文件全被置为RECOVER状态，其效果等同于执行[OFFLINE IMMEDIATE](./ALTER TABLESPACE.md#offline_option)表空间。

数据文件脱机功能的使用规则如下：

- 需在数据库处于OPEN状态（推荐）或MOUNT阶段时执行该操作。若在MOUNT阶段执行该操作，需额外注意：

    - 某些情况下（例如启动过程中回放了创建该文件所属表空间的redo日志），数据库会修复已被该操作脱机的数据文件及其所属表空间，并将它们置为ONLINE状态。
    
    - 执行该操作后再打开数据库，查询V$DATAFILE将无法正常显示该文件的创建时间，除非再次将其联机。

- 在主备高可用部署中，建议在主库上执行该操作，通过redo日志同步将备库的相同文件脱机。如需在备库执行该操作，则只能在MOUNT阶段执行，备库启动后会变成need repair状态。

- 单次操作只能指定1个数据文件，且不允许操作内置表空间的数据文件、不允许操作影子文件。

- 脱机表空间及其数据文件仍为数据库的一部分，因此不能创建与之同名的表空间或数据文件。

该操作常用于以下业务场景：

- 因数据文件损坏或丢失导致无法打开数据库时，可在MOUNT阶段先将目标文件脱机再启动数据库至OPEN。

- 需要进行数据隔离时，可临时将某个数据文件脱机。

- 因数据文件损坏导致数据库处于ABNORMAL状态（可通过V$DIAG_INCIDENT视图的ERROR_COMMENTS字段排查）时，可先将目标文件脱机再修复数据库状态。

##### DROP

在数据库未开启归档模式时，执行数据文件脱机必须指定此选项。在开启归档模式的情况下，此选项被忽略，即OFFLINE DROP = OFFLINE。
...
> - 可能导致备库不可用，后续可通过build database修复，请查阅[修复异常备库](../../../高可用/修复异常备库)。
...
该语句用于执行主备库之间的切换。主备库详细操作描述请参考[高可用](../../../高可用/YashanDB高可用概述)。
...
- 只能在备库或备集群的master实例上执行该语句。

    - GV$DATABASE或V$DATABASE视图中DATABASE_ROLE=STANDBY表示备库/备集群。

    - GV$INSTANCE视图中INSTANCE_ROLE=MASTER表示master实例。

- 当YashanDB部署为容器数据库（配置参数ENABLE_PLUGGABLE_DATABASE=TRUE）时，若连接备根容器使用该语句表示进行全局切换，即涵盖根容器和所有PDB。若直连某个备PDB使用该语句，则表示进行PDB级别的切换。  


- 主备库/集群必须均处于OPEN状态，对于共享集群/分布式集群，主集群要求至少master实例处于OPEN状态（其他实例无要求）。

- 主备之间的redo传输链路连接正常，且备库日志同步状态正常。主库查询视图V$ARCHIVE_DEST_STATUS或备库查询视图V$REPLICATION_STATUS可确认相应信息。

> **Note**: 
>
> 共享集群/分布式集群部署中，switchover的性能受checkpoint的影响，会等待非1号实例完成全量checkpoint再切换。
...
- 当前备库/备集群必须处于OPEN状态，对于共享集群/分布式集群，备集群要求至少master实例处于OPEN状态（其他实例无要求，直接保持OPEN即可）。
...
手动清理归档日志的筛选条件由ARCHIVELOG_DELETION_POLICY或ARCH_CLEAN_IGNORE_MODE参数决定，清理条件的具体描述请查阅[归档管理](../../../数据库管理/实例管理/归档管理)。 
...
数据库级附加日志对选定类型的所有用户表生效，可以通过动态视图[V$DATABASE](../../../参考手册/系统视图/动态视图/V$DATABASE.md)查看数据库级别的附加日志生效状态。若目标表开启了[加密](./CREATE TABLE.md#columnencryptionclause)，数据库级附加日志仍会根据选定表类型显示生效状态但实际redo里并不会记录该表相关的附加信息。

表级附加日志请参考[ALTER TABLE](./ALTER TABLE.md#addsupplementalloggingclause)。
...
开启全库闪回后，数据库允许在没有备份集的情况下[闪回](FLASHBACK.md#databaseclauses)到指定时间点。
...
- 当YashanDB部署为容器数据库（配置参数ENABLE_PLUGGABLE_DATABASE=TRUE）时，根容器和各个PDB中全库闪回功能的配置相互独立、互不影响。

- 在高可用部署场景中，主备之间的全库闪回功能完全独立。如需在备库/备集群上开启闪回功能，执行该语句前需先[暂停redo日志回放](#stop_apply)。
...
该语句用于为共享集群新增数据库实例，语句中logfiles_clause、datafiles_clause等的语法及规则同[CREATE DATABASE](CREATE DATABASE.md)对应子句。

> **Caution**:
>
> 如需进行[共享集群实例扩容](../../../安装和升级/扩缩容/共享集群扩缩容/集群实例扩缩容)，推荐使用yasboot工具提供的一键式命令，**不建议**单独执行该语句新增数据库实例。
...
- 新增实例后，最终实例总数不能超过当前集群的[MAXINSTANCES](CREATE DATABASE.md#maxinstances)值。

<span id="removeinstanceclauses" name="removeinstanceclauses"></span>

### remove\_instance\_clauses

该语句用于为共享集群删除数据库实例，语句中instance id可查看[GV$INSTANCE](../../../参考手册/系统视图/动态视图/GV$INSTANCE.md)视图。

> **Caution**:
>
> 如需进行[共享集群实例缩容](../../../安装和升级/扩缩容/共享集群扩缩容/集群实例扩缩容)，推荐使用yasboot工具提供的一键式命令，**不建议**单独执行该语句删除数据库实例。
