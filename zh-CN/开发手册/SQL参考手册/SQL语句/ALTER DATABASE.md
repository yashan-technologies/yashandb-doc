...
推荐使用yasboot运维工具管理分布式集群，具体见[yasboot命令介绍](../../../工具手册/yasboot/yasboot命令介绍/00yasboot命令介绍)章节描述。 
...
在高可用部署场景中，全库闪回主要是在主库上开启与执行。对于备库，目前仅适用于特定的故障处理场景，更多详情请查阅[全库闪回](../../../数据库管理/故障处理/闪回/全库闪回/00全库闪回.md)。
...
- 在共享集群/分布式集群部署中，全库闪回相关操作只能在主实例（[GV$INSTANCE](../../../参考手册/系统视图/动态视图/GV$INSTANCE)视图的INSTANCE_ROLE = MASTER_ROLE）上执行。
