...
- 在共享集群/分布式集群部署中，只能在主实例（GV$INSTANCE视图中INSTANCE_ROLE=MASTER的实例）上对LSC表删除数据。
...
对于被子表定义了外键约束的父表，如果要被删除的外键列项数据值已在子表中存在，则无法删除该父表的此行数据（除非该外键约束被定义了ON DELETE），详细描述请参考通用SQL语法[constraint](../通用SQL语法/constraint)的FOREIGN KEY。
...
该语句用于指定要删除记录的对象，可以为表名称（包括本地数据库的表或[远端表](../通用SQL语法/dblink/DBLink语法说明.md)）、表分区名称或视图名称。
...
该语句表示要删除的是远端表，详细说明请查阅[dblink](../通用SQL语法/dblink/DBLink语法说明.md)。
...
该语句用于指定[condition](../通用SQL语法/condition)，按此条件过滤出的记录行被执行删除操作。可省略，则表示删除表的所有行，但该表及表上索引所占的数据空间仍然被保留。
