通用描述
----
CREATE ACCESS CONSTRAINT用于在指定的表上创建[AC对象](../基本SQL元素/对象)。只能基于LSC表执行本语句创建AC对象，且该LSC表不可以为组合分区表或INTERVAL类型的分区表。

创建AC后，系统将在后台异步生成AC数据（运行[强制转换](ALTER TABLE.md#forcexfmrclause)可即时查看数据），AC数据可被用于：
...
该语句用于指定创建的AC的名称，不可省略，且需符合YashanDB的[对象命名规范](../基本SQL元素/标识符)。
