...
- 当指定为唯一索引时，不能同时指定为[BITMAP索引](#BITMAP)。

- 在LSC表上创建索引时，必须指定该关键字。

- 在存算一体分布式集群部署中，分区列必须是唯一索引关键列的子集，否则报错。
...
- 当指定为列式索引时，不能同时指定为[函数索引](#columnexpression)或[反向索引](#REVERSE)，且不能在线（ONLINE）[创建](#ONLINE)和[重建](./ALTER INDEX.md#rebuildclause)。
...
- 只能为[ST_Geometry类型](../数据类型/ST_GEOMETRY)的列创建RTree索引，且每个RTree索引只能指定一个列。
...
- BITMAP索引不适用于存算一体分布式集群部署。

- BITMAP索引仅适用于HEAP表。

- 当指定为BITMAP索引时，不能同时指定为[唯一索引](#UNIQUE)、[函数索引](#columnexpression)或[反向索引](#REVERSE)，且不能在线（ONLINE）[创建](#ONLINE)和[重建](./ALTER INDEX)（rebuild_clause）。
...
该语句用于指定创建的索引的名称，不可省略，且需符合YashanDB的[对象命名规范](../基本SQL元素/标识符)。
...
- [LOB大对象型](../数据类型/大对象型.md)、[UDT](../数据类型/用户自定义类型.md)的列均不能作为索引列。

- [RAW类型](../数据类型/RAW.md)的列不能作为列式索引列。


- [ST_Geometry类型](../数据类型/ST_GEOMETRY)的列上只能创建RTree索引，且每个RTree索引只能指定一个列。


- 每个全文索引只能指定一个列，且只能为CHAR、VARCHAR或CLOB列。

- 每个BITMAP索引最多可指定30个列，且不能为存在约束（[constraint](../通用SQL语法/constraint.md)）的列。

- 除RTree索引/全文索引只能1列、BITMAP索引最多30列外，其他索引最多可指定32个列。索引键值长度（即指定的列名称集总长度）不得超过以下限制：

    - 当数据块大小DB_BLOCK_SIZE为8K时，索引键值长度最大值为6400字节。

    - 当数据块大小DB_BLOCK_SIZE为16K时，索引键值长度最大值为12960字节。
    
    - 当数据块大小DB_BLOCK_SIZE为32K时，索引键值长度最大值为26510字节。

    > **Note**:
    >
    > 对于VARCHAR、NVARCHAR或RAW类型的列，若该列当前无数据，则其长度按1字节参与检验。后续插入或更新变长列的值时，将按实际数据长度参与校验。
...
- 当指定为[RTree索引](#RTREE)或[BITMAP索引](#BITMAP)时，不能同时指定为函数索引。
...
- column_expression中包含UDF时，还额外存在以下约束规则：

    - 索引的所有者必须具备相应UDF的执行权限。

    - UDF必须是确定性函数。

    - 索引所依赖的UDF必须为可用状态（DBA_OBJECTS视图中STATUS=VALID），UDF不可用会导致执行该索引的相关操作（DROP INDEX除外）报错，例如UDF中所使用的表或UDT对象被删除将导致UDF不可用从而导致对应索引无法正常使用。此时，需先删除该索引、更新UDF后再创建新的索引才能正常使用。

    - 索引所依赖的UDF及其过程体不允许删除或替换，如需删除或替换必须先删除对应的索引。
...
创建[BITMAP索引](#BITMAP)时，无论指定为DESC、ASC或不指定，始终按升序排序。
...
该语句用于指定索引的存储属性，省略则INITRANS/PCTFREE默认为2/8，其他存储属性请参考通用SQL语法[storage](../通用SQL语法/storage_clause)描述。
...
- 创建[BITMAP索引](#BITMAP)时，不能指定该关键字。

- 在分布表上创建索引时，不能指定该关键字，否则将会报错。

- 其他场景下，若不指定该关键字，YashanDB也将默认创建全局索引。
...
- 不允许为LSC表和临时表在线创建索引。
- 不允许在线创建[列式索引](#COLUMNAR)、[RTree索引](#RTREE)以及[BITMAP索引](#BITMAP)。
- 在共享集群/分布式集群部署下，正在执行的create/rebuild index online操作在遇到实例启停时会失败。
...
该语句用于创建分区索引（Local Partitioned Index）。

该语句的使用规则如下：

- 只能在分区表上建立分区索引，且索引的分区数量、分区界值必须与表的分区相同。

- 创建分区唯一索引时，需保证一级分区键和二级分区键的并集是索引键的子集。
