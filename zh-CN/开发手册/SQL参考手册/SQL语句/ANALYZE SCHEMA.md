...
ANALYZE SCHEMA用于收集存算一体分布式集群部署中指定用户下所有对象（表、AC、列、索引）的统计信息，与之对应的是[DBMS_STATS](../../PL参考手册/内置高级包/DBMS_STATS)高级包的GATHER_SCHEMA_STATS程序。
...
```ebnf
= ANALYZE SCHEMA OWNER [(ESTIMATE_PERCENT estimate_value|BLOCK_SAMPLE block_value|METHOD_OPTION method_clause|PARALLEL_DEGREE parallel_value|GRANULARITY "'" graularity_value "'"|INDEX_CASCADE index_value) 
