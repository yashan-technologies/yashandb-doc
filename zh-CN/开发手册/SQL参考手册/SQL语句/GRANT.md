...
当YashanDB部署为容器数据库（配置参数ENABLE_PLUGGABLE_DATABASE=TRUE）时，对用户和角色的授权操作默认仅在执行容器生效。如需实现全局用户、全局角色的全局权限管理，须连接根容器操作并指定container=all，此时执行结果将对根容器以及所有PDB生效。若直连某个PDB对全局用户、全局角色进行权限管理则仅对该PDB生效。对本地用户、本地角色进行权限管理则必须直连其所属的PDB执行。
...
```ebnf
= (grant_system_privilege|grant_object_privilege|grant_role) [CONTAINER "=" (CURRENT|ALL)].
```
