...
```ebnf
= ALTER PROFILE profile_name LIMIT  (password_parameters | resource_parameters | tcp_ip_parameters) {" " (password_parameters | resource_parameters | tcp_ip_parameters)}.
```
...
语法及规则同CREATE PROFILE中的[password_parameters](CREATE PROFILE.md#pswd)。
...
语法及规则同CREATE PROFILE中的[resource_parameters](CREATE PROFILE.md#res)。
...
语法及规则同CREATE PROFILE中的[tcp_ip_parameters](CREATE PROFILE.md#ip)。
