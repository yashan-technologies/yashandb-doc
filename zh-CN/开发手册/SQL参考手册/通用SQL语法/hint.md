...
- 使用BULKLOAD提示项的插入操作会阻塞ALTER STABLE/COMPACT操作，直到该BULKLOAD插入事务提交。
...
-- 此时只会回滚UPDATE语句
COMMIT;
-- 提交事务后查看area表可以发现('08','EastChina','Shanghai')数据已成功插入
...
使用MAX_WORKERS_PER_EXEC提示项时，无需指定表名，该提示项仅适用于存算一体分布式数据库的TAC表和LSC表的查询和子查询语句，指定可以同时执行的stage的数量。
