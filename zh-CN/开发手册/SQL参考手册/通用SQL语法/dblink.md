...
使用dblink功能时，崖山数据库会启动[沙箱进程](../../../数据库管理/会话和计划管理/yex_server沙箱进程管理)进行相关数据操作。

- 通过dblink操作远端数据库的表时，需满足如下要求：
  >**Note**:
  >
  > 在存算一体分布式集群部署中，操作远端表时采用列存，除下表所列数据类型要求外，还应**遵循列存的[数据类型](../数据类型（yashan模式）/00数据类型（yashan模式）)支持范围**。
...
指定yex_server进程使用的字符集，默认为UTF8。重新创建database时，如果使用了其他的字符集，yex_server.ini的内容不会同步更新，需手动修改CHARACTER_SET参数值与database字符集保持一致。
