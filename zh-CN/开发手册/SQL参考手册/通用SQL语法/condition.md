...
*   IS EMPTY|IS NOT EMPTY（expr必须为嵌套表类型）
...
RLIKE语法实现功能与[REGEXP_LIKE](../内置函数/REGEXP_LIKE)函数相同，通过正则表达式匹配算法将字符表达式expr与正则表达式regexp做匹配，匹配成功返回TRUE，匹配失败返回FALSE。其完整语法形式为：
...
ROWNUM是系统顺序分配的查询返回的行编号，返回的第一行分配的是1，第二行是2，依此类推。该语句通过ROWNUM伪列限制查询返回的总行数，具体使用规则[ROWNUM伪列](../基本SQL元素/伪列)。
