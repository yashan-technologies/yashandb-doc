...
> 微秒的特殊处理：
>
> - 在定义时间戳类型时，微秒精度可定义范围为0 ~ 9，但实际存储的精度最大值为6。
> - 当微秒格式为'FF'时，系统可以接收0 ~ 9位数字的微秒输入，但会将其补齐或四舍五入到精度指定的位数。
...
> 微秒的特殊处理：
>
> - 在定义本地时区类型时，微秒精度可定义范围为0 ~ 9，但实际存储的精度最大值为6。
> - 当微秒格式为'FF'时，系统可以接收0 ~ 9位数字的微秒输入，但会将其补齐或四舍五入到精度指定的位数。
>
> 与TIMESTAMP类型差异：
>
...
> 微秒的特殊处理：
>
> - 在定义时区类型时，微秒精度可定义范围为0 ~ 9，但实际存储的精度最大值为6。
> - 当微秒格式为'FF'时，系统可以接收0 ~ 9位数字的微秒输入，但会将其补齐或四舍五入到精度指定的位数。
>
> 与TIMESTAMP WITH LOCAL TIME ZONE类型差异：
>
> - TZ类型包括时间戳和时区，而LTZ类型仅包含时间戳。
> - TZ类型使用格式符TIMESTAMP\_TZ\_FORMAT，而LTZ类型使用TIMESTAMP\_FORMAT格式符。

### 转换时区

通过AT TIME ZONE语法可以修改/指定数据的时区：

- 为TZ类型的数据修改时区（数据类型不变）。
- 为TIMESTAMP、TIMESTAMP LTZ类型的数据指定时区（会将其转换为TZ类型，效果类似于[FROM_TZ](../内置函数/FROM_TZ)与[TO_TIMESTAMP_TZ](../内置函数/TO_TIMESTAMP_TZ)函数）。
...
待修改/指定时区的原数据，需为TIMESTAMP、TIMESTAMP LTZ或TIMESTAMP TZ类型的数据或常量NULL。

**expr2**

指定时区信息，需为字符串类型或常量NULL，格式为'TZH:TZM'，取值范围为-15:59 ~ 15:59。
...
年到月间隔类型的定义格式为INTERVAL YEAR \[(year\_precision)\] TO MONTH。year\_precision：表示年的精度，取值范围\[0,9\]，省略时取默认值2。
...
*   day\_precision：表示天的精度，语法支持\[0,9\]，实际取值范围\[0,8\]。
*   fractional\_seconds\_precision：表示微秒的精度，取值范围\[0,9\]。
