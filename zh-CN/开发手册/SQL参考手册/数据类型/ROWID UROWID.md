YashanDB单机/共享集群/分布式集群部署中表对象的默认存储方式为HEAP方式，数据按行（Row）组织，系统根据每一行数据所在物理地址信息生成一个全局唯一记录，即ROWID。ROWID用于检索表数据时的寻址，也可以用作每一行数据的唯一标识。
...
<span id="Format" name="Format"></span>
...
data object id：行所在的Segment的ID，该值可从user\_objects等视图中的DATA_OBJECT_ID字段查询获得。
