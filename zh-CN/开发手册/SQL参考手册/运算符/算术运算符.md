...
*   算术运算符：/、%
*   内置函数：[MOD](../内置函数（yashan模式）/MOD)、[DIV](../内置函数（yashan模式）/DIV)

**%**

取模运算，与[MOD](../内置函数（yashan模式）/MOD)函数同义。
...
| \-  | 二元  | \*   时间间隔<br>\*   时间 | \*   将两个间隔类型相减，返回更短的间隔类型。<br>\*   将DATE、TIMESTAMP、LTZ、TZ与TIME相减，返回在时间轴上更靠前的TIMESTAMP、LTZ、TZ。<br>\*   将两个DATE相减，返回数值，表示相差的天数。<br>\*   将DATE与TIMESTAMP相减，或TIMESTAMP与TIMESTAMP相减，或LTZ与LTZ相减，或TZ与TZ相减，返回DS_INTERVAL。<br>\*   将DATE或TIMESTAMP或LTZ或TZ与数值相减，数值被解释为天数（小数部分表示不满一天的时间），返回在时间轴上更靠前的DATE。 |
