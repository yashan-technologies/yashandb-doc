...
*   CASE语句
*   udt_expr：对于Object UDT列，udt_expr格式为表别名.属性名；对于Varray UDT和Nested Table UDT列，udt_expr格式为表别名.*。详细使用方法和示例见[用户自定义类型](../数据类型（yashan模式）/用户自定义类型)中描述。
...
可用于查询Varray UDT或Nested Table UDT数据，UDT数据类型的详细描述请参考[用户自定义类型](../数据类型（yashan模式）/用户自定义类型)。
...
可使用表函数进行查询，表函数的具体描述可见[内置函数](../内置函数（yashan模式）/00内置函数（yashan模式）)章节中内置表函数部分描述。
