...
回收站开启（配置参数RECYCLEBIN_ENABLED为ON）的场景下执行DROP语句删除表时，目标表及其索引、约束、触发器、LOB列等数据都将进入回收站。

DROP 操作删除了表对象的所有相关定义，如需再次使用该表，只能按创建新表的方式进行重建。

DROP 操作不能回滚（Rollback），也不能通过[flashback_query_clause](./SELECT)获得操作之前的数据。如果指定了PURGE语句，也无法从回收站恢复。
...
回收站功能仅适用于HEAP表，是否使用回收站由配置参数RECYCLYBIN_ENABLED（默认值为ON）控制，当设置为ON时，对HEAP表执行DROP操作时系统会将表对象及数据放入回收站（表名称被修改为BIN开头的系统生成字符串）便于误操作之后的恢复，对TAC表、LSC表以及外部表执行DROP操作则始终表现为彻底删除。
