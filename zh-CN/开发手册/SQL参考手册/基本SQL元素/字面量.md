...
日期字面量是以DATE关键字开始，直至单引号`''`包围的字符串右引号结束的值，采用'yyyy-mm-dd'进行格式匹配，例如`DATE '2020-01-01'`。其声明方式如下图所示，其中date\_str表示符合DATE类型标准格式的字符串。
...
日期时间戳字面量是以TIMESTAMP开始，直至单引号`''`包围的字符串右引号结束的值，采用'yyyy-mm-dd hh24:mi:ss.ff'进行格式匹配，时分秒可缺省，例如`TIMESTAMP '2020-01-01 13:08:28'`。其声明方式如下图所示，其中timestamp\_str表示符合TIMESTAMP类型标准格式的字符串。
...
年到月字面量的声明方式如下图所示，其中YMInterval\_str表示符合INTERVAL YEAR TO MONTH类型标准格式的字符串，year\_precision表示年的精度（默认值为2）。
...
天到秒字面量的声明方式如下图所示，其中DSInterval\_str表示符合INTERVAL DAY TO SECOND类型标准格式的字符串，day\_precision表示天的精度（默认值为2），second\_precision表示秒的精度（默认值为6）。
...
二进制字面量是以字符b开始，直至单引号`''`包围的字符串右引号结束的值。其声明方式如下，其中bit\_str表示符合二进制类型标准格式的字符串。
...
国家字符集字面量是以字符N开始，直至单引号`''`包围的字符串右引号结束的值，其中nchar\_str是英文字母、中文汉字、数字字符和特殊字符的组合，具体声明方式如下。
