...
- ROWSCN不能用于[多表条件插入](../SQL语句（yashan模式）/INSERT.html#conditionalinsertclause)的WHEN条件子句、insert_values_clause以及子查询。
...
ROWID伪列表示行所在的地址。ROWID伪列的类型为ROWID类型，使用限制等更多说明请查阅[ROWID UROWID](../数据类型（yashan模式）/ROWID UROWID)。

- 该伪列仅适用于HEAP表。

- ROWID不能用于[多表条件插入](../SQL语句（yashan模式）/INSERT.html#conditionalinsertclause)的WHEN条件子句、insert_values_clause以及子查询。
