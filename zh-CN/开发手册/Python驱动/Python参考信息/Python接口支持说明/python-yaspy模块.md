...
| paramstyle   | 参数标记的格式风格<br/>目前固定为named，支持命名绑定（`:name`格式）和位置绑定（`:1`、`:2`格式） | named |
...
# 方法3：多地址连接（primary类型，故障转移）
# conn = yaspy.connect(
#     dsn="sales/sales@192.168.1.2:1688,192.168.1.2:1688,192.168.1.2:1688"
# )

# 方法4：多地址连接（loadBalance类型，负载均衡）
# conn = yaspy.connect(
#     dsn="sales/sales@LOADBALANCE:192.168.1.2:1688,192.168.1.2:1688"
# )
...
- Data inserted successfully：数据插入成功
- Retrieved: id=1, name=Product A, value=99.99：查询返回的结果
