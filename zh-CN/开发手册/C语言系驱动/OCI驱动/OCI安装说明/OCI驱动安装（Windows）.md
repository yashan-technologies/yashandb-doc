...
使用YashanDB OCI驱动需要先安装YashanDB C驱动并设置环境变量，YashanDB C驱动安装文件集成在YashanDB客户端安装包中，安装时需获取对应的客户端安装包。
...
5.单击【新建】，输入C驱动的库文件所在文件夹，例如D:\yasdb-driver-c\lib。
...
1. 从[Oracle官网](https://www.oracle.com/database/technologies/instant-client/downloads.html)下载Instant Client基础包，获取`oci.h`、`oratypes.h`及相关头文件。下载后解压到任意目录，例如`D:\yasdb-driver-c\lib`。

2. 编译并运行示例程序：

```shell
# 设置Oracle Instant Client路径（包含oci.h和oratypes.h头文件）
export YASHAN_HOME=D:\yasdb-driver-c\lib

# 编译OCI示例程序
gcc -o ociexample ociexample.c -I${YASHAN_HOME}/sdk/include -L${YASHAN_HOME} -lclntsh -lpthread -ldl

# 运行程序（需要设置LD_LIBRARY_PATH）
