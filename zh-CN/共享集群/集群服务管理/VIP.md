...
- VIP必须与实例监听地址（LISTEN_ADDR）处于相同子网、相同网卡，且端口号一致。
...
2. 检查确认每个数据库实例的监听端口号（LISTEN_ADDR）与对应的集群节点服务端口号（service port）是否一致。
```shell
# 查看数据库实例配置的LISTEN_ADDR监听端口
...
$ ycsctl show config
    ...
    Node name: yas2, yascs/yasfs inter connect URL: 192.168.7.134:1788, Node ID: 2
        public service port: 1601           # 节点yas2已配置服务端口号为1601，但与LISTEN_ADDR监听端口1688不一致
        yasdb instance name:yasdb-1-2, yasdb instance id:1
```

3. 若端口号配置不满足配置规则，执行如下操作。
```shell
# 停止集群
