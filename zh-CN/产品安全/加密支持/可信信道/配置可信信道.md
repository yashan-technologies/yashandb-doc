在配置可信信道前，请先自行了解其[配置规则](00可信信道.md)，并根据实际需求选择需要配置的可信信道以及选择合适的加密协议。

配置可信信道后，后续需关注相应证书有效期，并在证书过期前及时[替换证书](./管理证书.md#expire)。
...
- 已生成[SSL协议相关证书](管理证书.md#getSSLCert)。
...
3. 开启SSL可信信道开关，配置服务器各类证书路径。
    ...
    > **Warn**:
    >
    > 开启客户端<->服务端可信信道开关后，必须正确配置服务端各类证书路径相关的数据库参数，否则会导致数据库启动失败。
    >
...
- 已生成[TLCP协议相关证书](管理证书.md#getTLCPCert)。
