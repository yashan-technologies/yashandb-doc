...
启用三权分立后，可进一步要求管理员用户通过yasql登录数据库时声明其角色（默认不强制，如需启用该功能请联系我们的技术支持），除原有认证要求外，还需角色匹配才能登录成功。

## 配置三权分立

>**Note**:
>
> 建议预先规划确认是否使用三权分立，如需启用建议在安装/建库操作时一并开启，不建议频繁切换权限模式。若建库时未开启，但后续确需开启，应先清理并整理当前已有用户的权限集合，再开启三权分立。
