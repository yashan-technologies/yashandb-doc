...
通过DBA\_SYS\_PRIVS视图可查看当前系统中所有授权的系统特权信息。
...
- 对象操作类特权：管理表、索引、视图、序列、同义词、自定义类型、存储过程、触发器、自定义库、定时任务、数据库链接和物化视图等对象。
...
|  权限名称| 描述|
| --- | --- |
| ALL PRIVILEGES              | 具备所有的系统特权（SELECT ANY DICTIONARY除外）                |
| **数据库与会话**：          |                 |
...
| DEBUG CONNECT SESSION | 仅用于兼容，无实际含义 |
...
| **序列**：                      |                           |
| CREATE SEQUENCE              | 具备在用户自己的schema下创建序列的权限                           |
| CREATE ANY SEQUENCE          | 具备在任意schema下创建序列（sys schema除外）的权限               |
| ALTER ANY SEQUENCE           | 具备对数据库中任意序列修改属性（sys schema除外）的权限           |
| DROP ANY SEQUENCE            | 具备删除数据库中任意序列（sys schema除外）的权限                 |
| SELECT ANY SEQUENCE          | 具备对数据库中任意序列发起查询（sys schema除外）的权限           |
| **同义词**：                     |                           |
...
| **定时任务**： |  |
| CREATE ANY JOB | 具备在任意schema下创建和管理定时任务（sys schema除外）的权限（即调用DBMS_SCHEDULER、DBMS_JOB）  |
| CREATE JOB | 具备在用户自己的schema下创建定时任务的权限（即调用DBMS_SCHEDULER.CREATE_JOB、DBMS_JOB.SUBMIT） |
| MANAGE SCHEDULER | 仅用于兼容，无实际含义 |
| **数据库链接**：                   |                         |
...
| **上下文**:                   |                                |
| CREATE ANY CONTEXT           | 具备创建任意上下文对象的权限                                        |
| DROP ANY CONTEXT             | 具备删除任意上下文对象的权限                                        |
