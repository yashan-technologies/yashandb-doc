...
| *-s,--sql*       | 指定yasrman运行的SQL，SQL用法参考[yasrman使用指导](../yasrman/yasrman使用指导/00yasrman使用指导.md) |
...
> - 共享集群/分布式集群部署：选择第一个节点进行备份，选择master role节点进行恢复。

示例

yasrman提供了对接第三方XBSA API的接口，需要在SQL语句中添加PARAMS参数，PARAMS请参见[yasrman params简介](../yasrman/yasrman使用指导/00yasrman使用指导.md)。
