...
2. 进入安装目录，对数据库的所有节点进行以下检查，判断是否所有节点都开启了归档模式（ARCHIVELOG）。若未开启，请参照[归档管理](../../数据库管理/基本数据库管理/归档管理)开启归档。
...
4. 获取yasom服务的连接token，配置位于${YASDB_HOME}/om/\<Cluster_name>/conf/tls.toml，并对yasbak进行初始化操作。
