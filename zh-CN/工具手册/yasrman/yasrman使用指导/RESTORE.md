...
在单机部署和共享集群部署中，执行RESTORE操作时系统会先自动进行环境预检查，检查项目包括待恢复文件路径是否可达、目标地址空余磁盘空间是否充足。检查通过再进行RESTORE，检查失败则直接退出。
...
使用本命令与[RESTORE DATABASE](../../../开发手册/SQL参考手册/SQL语句（yashan模式）/RESTORE DATABASE)语句所执行的操作类似，但使用本命令恢复备份集后会启动当前节点至open状态，存算一体分布式集群部署中还会build备库（若存在）。
