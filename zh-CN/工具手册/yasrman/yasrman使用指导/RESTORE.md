使用RESTORE命令可基于yasrman生成的备份集进行恢复，包括全库恢复、某一表空间、归档日志的恢复操作。
...
- 所有节点上实例的数据文件目录、归档日志文件目录、bucket目录已清空，并启动至NOMOUNT状态。
...
- 在共享集群/分布式集群部署中，执行恢复时连接的目标节点必须为MASTER ROLE节点（可通过查询视图V$INSTANCE的INSTANCE\_ROLE字段确定），否则无法恢复数据库。
...
| 适用场景 | 关键字 | 功能描述 |
|-----------------|-------------|-------------------------|
| 非容器数据库 | DATABASE  | 用于恢复单机部署或共享集群部署的整个数据库。 |
| 非容器数据库 | CLUSTER | 用于恢复存算一体分布式集群部署的整个数据库。 |
| 容器数据库 | DATABASE | 用于恢复整个数据库或单个PDB，具体取决于使用yasrman所连接登录的对象：<br/>* 连接根容器时：恢复根容器、种子容器以及所有PDB。若通过指定数据库ID执行PITR（`RESTORE DATABASE DBID id UNTIL TIME/SCN`），等价于单独对根容器执行PITR，即仅恢复根容器至目标时刻。<br/>* 连接某个PDB时：恢复该PDB，无法执行PITR恢复。 |
| 容器数据库 | DATABASE ROOT | 用于恢复根容器，必须连接根容器执行。  |
| 容器数据库 | PLUGGABLE DATABASE | 用于恢复指定PDB，必须采用具备指定PDB的恢复和连接权限的用户连接根容器执行。<br />无法通过指定数据库ID执行PITR（`RESTORE PLUGGABLE DATABASE DBID id UNTIL TIME/SCN`）。 |

### FROM TAG

指定执行全库恢复的备份集，必须指定为catalog中存在的全库备份集。

### PITR恢复

#### FROM TAG

指定执行PITR恢复的备份集，必须指定为catalog中存在的全库备份集。省略时，yasrman会自动从catalog中获取合适的全库备份集，按备份集生成顺序进行扫描直至找到合适的全库备份集后开始执行恢复。

恢复过程中，若目标全库备份集中的归档日志不足以支撑此次操作的恢复范围，yasrman会按备份集生成顺序扫描catalog中记录的所有归档日志备份集，获取合适的归档日志并恢复。

#### DBID

指定执行PITR恢复的数据库ID。ID值在建库后不会发生变化，用于标识唯一的数据库，可通过V$DATABASE视图的DATABASE_ID字段获取。

yasrman会从catalog中搜索指定数据库ID的备份集并进行恢复。

在容器数据库中指定数据库ID进行恢复时，始终只会恢复执行当前操作的容器。

#### UNTIL

UNTIL选项用于PITR恢复。当使用PITR恢复时，可以指定数据库备份集，也可以指定DBID自动搜索时间最近的数据库备份集。如果不指定TAG和DBID，将搜索时间最近目标点的任意备份集。

执行PITR恢复时，首先恢复数据库备份集，然后尝试恢复目标范围内的归档日志文件备份，最后尝试复用数据库本地的归档文件，尽可能恢复到目标点。因此如果目标点以内的归档日志文件全部在yasrman备份，或者在数据库本地都存在，都可以恢复到目标点。

在容器数据库中，执行整个CDB的PITR恢复时，操作会分为两个阶段：

- 阶段一：先对根容器执行PITR恢复，恢复成功后进入阶段二，否则整个恢复操作中断。

- 阶段二：并行恢复种子容器和所有PDB，每个数据库对象的恢复进程相互独立、互不干扰。

	若指定的SCN/TIME过大，部分PDB无法恢复至目标回放点，相应PDB的恢复进程会中断（不会影响其他PDB），此时目标PDB只会还原至MOUNT阶段，需手动将其启动至OPEN状态并重置日志时间线（例如登录根容器并执行[ALTER PLUGGABLE DATABASE](../../../开发手册/SQL参考手册/SQL语句/ALTER PLUGGABLE DATABASE.md) pdbname_list OPEN RESETLOGS语句）。可通过指定[IGNORE PITR TARGET ERROR](#IGNORE)自动忽略部分PDB报错并自动将其OPEN RESETLOGS，无需额外手动干预。

在容器数据库中，执行批量PDB的PITR恢复（RESTORE PLUGGABLE DATABASE pdbname_list）与上述阶段二无异。

PITR不适用于存算一体分布式集群部署。

#### UNTIL TIME

该语句用于将数据库恢复到此指定的时间点，即PITR恢复，time_string必须是时间格式的字符串，格式需与TIMESTAMP\_FORMAT参数定义的格式一致。
...
该语句用于忽略因SCN/TIME过大而回放不到目标点的错误。

指定该关键字后，系统会继续将目标数据库从MOUNT阶段启动至OPEN状态并重置日志时间线（即执行OPEN RESETLOGS）。
...
> **Note**: 
>
> - 最小时间点是保证数据库恢复到一致性的时间点，该时间点对应与备份完datafile，开始备份归档日志时的scn对应的时间点，略小于备份完成时间。
...
执行数据库恢复时，默认将所有文件恢复至原路径。如需调整数据文件或REDO日志文件存放位置，可以先根据规划[准备路径映射文件](./LIST.md#LISTBACKUPSET)再基于此映射文件进行恢复，此时相应的数据文件或REDO日志文件将会存放至新的路径。
...
使用本关键字表示对数据库归档日志文件执行恢复。

### RESTORE ARCHIVELOG ROOT

在容器数据库中，指定该关键字表示在根容器上执行归档日志文件恢复操作。
...
指定执行归档日志恢复的备份集，必须指定为catalog中存在的归档日志备份集或全库备份集。


当恢复范围指定为ALL时，必须通过FROM TAG指定备份集。


其他场景可省略FROM TAG，省略时yasrman会自动从catalog中获取合适的归档日志备份集或全库备份集，按备份集生成顺序进行扫描直至找到包含恢复范围内的归档日志的备份集后进行恢复。
