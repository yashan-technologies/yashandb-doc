...
执行PITR恢复时，首先恢复数据库备份集，然后尝试恢复目标范围内的归档日志文件备份，最后尝试复用数据库本地的归档文件，尽可能恢复到目标点。因此如果目标点以内的归档文件全部在yasrman备份，或者在数据库本地都存在，都可以恢复到目标点。
...
指定执行归档日志恢复的备份集，必须指定为catalog中存在的归档日志备份集。

当恢复范围指定为ALL时，必须通过FROM TAG指定备份集。

其他场景可省略FROM TAG，yasrman会自动从catalog中获取合适的归档日志备份集，按备份集生成顺序进行扫描直至找到包含恢复范围内的归档日志的备份集后进行恢复。
