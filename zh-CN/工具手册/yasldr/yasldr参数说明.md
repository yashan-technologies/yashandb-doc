...
| MODE	                    | [BASIC,BATCH]                     | 控制导入模式，支持BASIC和BATCH两种导入模式，默认值为BATCH。<br>* BASIC：采用INSERT INTO绑定参数执行，视图、GIS列、interval分区表、存算一体分布式集群部署中的复制表仅支持该模式导入。<br>* BATCH：采用BATCH INSERT协议执行，目前无法导入range interval分区表。                                                                                                                                          |
