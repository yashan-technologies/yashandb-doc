...
- 待操作的目标磁盘组已挂载，且其所有存量磁盘均不处于离线状态。

- 如果指定的故障组名称已存在，则向已有故障组内添加磁盘。

- 如果指定的故障组名称不存在，或未指定故障组名称，则创建新的故障组并为其添加磁盘。

- 需确保添加磁盘后故障组数量满足磁盘组冗余度要求，且各故障组的磁盘数量一致。若数量无法达到要求，YFS会阻止添加操作（操作无法成功）。
...
- 需确保删除磁盘后剩余故障组数量满足磁盘组冗余度要求，且剩余各故障组的磁盘数量一致。若剩余数量无法达到要求，YFS会阻止删除操作（操作无法成功）。
...
- 如需对系统磁盘组（名为SYSTEM）执行该操作，需确保系统磁盘组不处于保护状态（stat ≠ PROTECT）。
...
该语句用于将磁盘组中已离线（stat = OFFLINE）的磁盘重新上线。
...
- 若原磁盘因硬件故障而离线，则根据磁盘组类型不同操作略有差异：

    - 对于系统磁盘组：必须先修复硬件问题并确保**新磁盘的路径、磁盘名与旧磁盘完全一致**，才能通过上线操作完成修复。

    - 对于数据磁盘组：

        - 若修复硬件问题时磁盘路径和名称均可保障与原磁盘一致，则可通过上线操作完成修复。

        - 若修复硬件问题时磁盘路径或名称无法保障与原磁盘一致，则只能通过[更换（REPLACE）操作](#replace_disk_clause)完成修复。
...
该语句用于添加新磁盘更换磁盘组中已离线（stat = OFFLINE）的磁盘。
...
要删除的磁盘组必须处于已挂载状态，否则删除失败，可通过ALTER DISKGROUP语句改变磁盘组的状态。
