...
- 如需将磁盘组的冗余度规划为Normal或High，同一磁盘组中所有磁盘的可用**容量**、每个故障组下的磁盘**数量**均要求一致。
...
```ebnf
= ALTER DISKGROUP dgname (DISMOUNT [FORCE] | MOUNT | add_disk_clause | drop_disk_clause | rename_disk_clause | offline_disk_clause |  online_disk_clause |  replace_disk_clause | rebalance_modify).
```
...
- 需确保删除磁盘后仍满足以下数量要求，若无法达到要求，YFS会阻止删除操作（操作无法成功）：

    - 剩余故障组数量满足磁盘组冗余度要求。
    
    - 冗余度为Normal或High时，剩余各故障组的磁盘数量一致。
