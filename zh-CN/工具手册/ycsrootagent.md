...
若通过yasboot部署集群，其会在用户提供了sudo账密或配置了sudo免密时自动启动YCSRA。若YCSRA进程异常，请按示例手动启动YCSRA以恢复服务。
...
在需要自动化检测存储设备是否支持SCSI I/O Fencing的场景中，yasboot会自行调用fenceScsiCheck脚本从而调用执行本命令的相关子命令。如需手动进行相应检测，请直接执行[fenceScsiCheck脚本](../共享集群/集群服务管理/IO Fencing/基于SCSI持久预留的IO Fencing.html#fenceScsiCheck_usage)，**不建议用户直接执行本命令**。
...
在需要自动化检测存储设备是否支持SCSI I/O Fencing的场景中，yasboot会自行调用fenceScsiCheck脚本从而调用执行本命令。如需手动进行相应检测，请直接执行[fenceScsiCheck脚本](../共享集群/集群服务管理/IO Fencing/基于SCSI持久预留的IO Fencing.html#fenceScsiCheck_usage)，**不建议用户直接执行本命令**。
