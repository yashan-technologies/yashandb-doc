...
| *--enable-pluggable-database*  | 是否部署为容器数据库，默认为false<br/>指定该参数时，不能同时使用-m（或--mode）参数指定语法模式，PDB的语法模式在创建时指定即可|
| *--enable-branch*  | 是否部署为分支数据库，默认为false<br/>指定该参数时，不能同时使用-m（或--mode）参数指定语法模式|
| *--create-cgroup*     | 是否创建资源管理cgroup目录，创建需使用sudo权限，要求至少指定-u，-p或者-su，-sp其中之一<br/>安装容器数据库时默认创建，非容器数据库则默认不创建     |
...
| *--disk-found-path*   | 存储网络中的磁盘发现路径（默认为/dev/yfs），仅部署分支数据库时可用          |
| *--system-data*       | 存储网络中的系统盘，其路径必须在磁盘发现路径下，多个系统盘信息间用逗号`,`隔开，仅部署分支数据库时可用       |
| *--data*              | 存储网络中的数据盘，其路径必须在磁盘发现路径下，多个数据盘信息间用逗号`,`隔开，仅部署分支数据库时可用         |
| **节点规模** |  |
...
| *--scale-factor*      | 诸如TPCH等测试场景的数据量（单位：GB），默认为100<br />须配合--recommend-param使用 |
| **其他参数** |  |
| *-h,--help*        | 查看当前命令的帮助信息  |
...
| *--listen-cidr*       | 数据库的监听地址（默认与IP网段一致）              |
| *--replica-cidr*      | 主备复制链路地址（默认与IP网段一致）          ||
| *--disk-found-path*   | 存储网络中的磁盘发现路径（默认为/dev/yfs）          |
| *--system-data*       | 存储网络中的系统盘，其路径必须在磁盘发现路径下，多个系统盘信息间用逗号`,`隔开       |
| *--data*              | 存储网络中的数据盘，其路径必须在磁盘发现路径下，多个数据盘信息间用逗号`,`隔开         |
...
| *--node-host-map* | 在测试环境中部署共享集群时，可通过配置文件指定数据库实例与服务器的分配关系<br />文件及其内容要求如下：<br />* 文件名后缀需为`.toml`<br />* 内容格式为`服务器称名='实例节点ID1,…,实例节点IDn'`，例如`host0001='1-1,1-2'`，每行指定1台服务器<br /><br />**不建议**在生产环境中使用，默认按--ip参数指定的服务器列表顺序在每台服务器上分配一个实例 |
| **数据库性能相关** | 若安装时不指定此类参数，安装完成后可按需通过调用[DBMS_PARAM](../../../开发手册/PL参考手册/内置高级包/DBMS_PARAM.md).OPTIMIZE进行调整 |
| *--recommend-param*   | 是否开启推荐参数功能                       |
| *--memory-limit*      | 服务器可供数据库使用内存百分比上限，默认为80，可用量上限计算公式为`服务器总内存 * memory-limit`<br />须配合--recommend-param使用 |
| *--cpu-limit*         | 服务器可供数据库使用CPU百分比上限，默认为100，可用量上限计算公式为`服务器总CPU * cpu-limit`<br />须配合--recommend-param使用 ||
...
| *--nvme-reuse*   | 控制是否检查并复用已有的NVMe-oF磁盘，默认不启用复用机制。若不复用，检测到残留的NvmeOf时会抛出警告，需由用户二次确认；复用的前提条件为IP地址一致，且在对应的NQN文件中可查询到与部署时传入的盘符名称完全相同的记录。                      |
| *--connect-param*   | NVMe-oF的连接参数，具体可以参考命令`nvme connect --help`<br/>参数前需使用`\`符进行续行，例如`--connect-param \ -i\ 4\ -Q\ 32\ -l\ -1`                    |
...
| *--db_type*           | 数据库的类型，可选[disk,mem]，disk为磁盘库，mem为内存库，默认为disk |
...
| *--mn*                | MN节点的部署规模，默认为1           |
| *--cn*                | CN节点的部署规模，默认为1           |
| *--dn*                | DN组及其节点规模，例如1-3表示1一个组3个节点（组内一主两备）                    |
| **数据库性能相关** | 若安装时不指定此类参数，安装完成后可按需通过调用[DBMS_PARAM](../../../开发手册/PL参考手册/内置高级包/DBMS_PARAM.md).OPTIMIZE进行调整 |
| *--recommend-param*   | 是否开启推荐参数功能                       |
| *--memory-limit*      | 服务器可供数据库使用内存百分比上限，默认为80，可用量上限计算公式为`服务器总内存 * memory-limit`<br />须配合--recommend-param使用 |
| *--cpu-limit*         | 服务器可供数据库使用CPU百分比上限，默认为100，可用量上限计算公式为`服务器总CPU * cpu-limit`<br />须配合--recommend-param使用  ||
| *--scene*             | 参数调优的场景，可选[NORMAL,BATCH,TPCH,AIM]，默认为NORMAL。须配合--recommend-param使用<br>- NORMAL：通用混合负载场景，适用于所有表类型 <br>- BATCH：批量处理场景，仅当表类型为HEAP时可设置为BATCH。当业务主要为OLTP业务使用了HEAP表时，如遇到周期性跑批场景性能不佳的情况，可考虑使用BATCH场景参数进行调优<br />- TPCH：复杂分析查询场景，仅当表类型为LSC时可设置为TPCH。使用TPCH场景进行调优推荐时，必须确保所有节点都执行参数推荐，否则可能重启时可能会因为连接参数不匹配而导致失败<br />- AIM：全内存优化场景，仅当部署为分布式内存数据库（即--db_type指定为mem）时可设置为AIM |
| *--scale-factor*      | 诸如TPCH等测试场景的数据量（单位：GB），默认为100<br />须配合--recommend-param使用 |
| *--mms_limit*         | 内存数据库中，缓存内存表空间的数据可用的内存百分比上限，默认为80 ，可用量上限计算公式为`物理总内存 * memory-limit * mms_limi`，其余部分则用于计算、缓存非内存表空间数据等<br />仅当--db_type指定为mem且scene指定为AIM时，该参数才生效，且仅在DN节点上生效 |
| **其他参数** |  |
| *-h,--help*        | 查看当前命令的帮助信息  |
| *--ssl-protocol*      | 指定配置数据库[客户端<->服务器可信信道](../../../产品安全/加密支持/可信信道/00可信信道.md)的协议，目前仅支持SSL <br/> 须配合--ssl-path使用   |
| *--ssl-path*          | 指定SSL相关证书的存放路径，须配合--ssl-protocol使用 <br/> 执行数据库安装时，会将该路径下的所有文件全部拷贝至每台服务器的$YASDB_HOME/ssl路径下                    |
...
该命令已不再维护，请换用package se/de/ce gen命令。
