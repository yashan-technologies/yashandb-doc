...
- 单机主备部署中的备库、级联备库；双复制组主备部署中主复制组的备库、备复制组中的非首节点（即级联备）
...
| --------------- |----------------------------------------------------------------------------------------------------------------|
| *-c, --cluster* | YashanDB的集群名（必传参数）                                                                                             |
| *-n, --node-id* | 待删除的节点ID（例如`1-2`，可通过yasboot cluster status命令查看数据库信息取`nodeid`冒号前的数字串），仅允许指定一个ID <br/>如需一次性删除多个节点请使用--node-ids参数 |
| *--node-ids*    | 待删除的节点ID清单（例如`1-3,1-4`，可通过yasboot cluster status命令查看数据库信息取`nodeid`冒号前的数字串），允许指定一个或多个ID，ID间使用逗号分隔               |
| *-f, --force*   | 跳过确认直接执行命令                                                                                                     |
| *--purge*   | 删除目标节点的data数据                                                                                                  |
| *-w, --nowait*  | 运行后不等待执行命令结果                                                                                                   |
| *-d, --child*   | 展示任务以及子任务信息                                                                                                    |
...
| *-u, --username*     | 指定数据库用户，必须为sys用户，省略时默认使用sys用户                                                                                  |
| *-p, --password*  | 数据库用户的密码<br/>若使用sys用户且已开启[操作系统认证](../../../产品安全/身份标识与鉴别/操作系统认证/00操作系统认证.md)（安装后默认开启）则无需指定密码                    |
...
本命令用于双复制组主备部署，旨在对备复制组的首节点（即异地备）进行角色切换，使其转换为级联备，同时将指定的非首节点（即级联备）升为新的首节点。

|  选项| 含义|
| --------------- | ---------------------------------------- |
| *-c, --cluster* | YashanDB的集群名（必传参数）                        |
| *-n, --node-id* | 待切换为异地备库的节点ID（例如2-2，可通过yasboot cluster status命令查看数据库信息取nodeid冒号前的数字串），仅允许指定一个ID |
| *-f, --force*   | 跳过确认直接执行命令                     |
| *-w, --nowait*  | 运行后不等待执行命令结果                 |
| *-d, --child*   | 展示任务以及子任务信息                   |
| *-u, --username*     | 指定数据库用户，不指定则默认使用sys用户           |
...
本命令用于安全停止指定节点，执行本命令等价于在数据库中依次执行以下操作：

1. `ALTER SYSTEM LISTENER STOP`：停止接收新连接
2. `ALTER SYSTEM KILL SESSION WITH TRANSACTION`：等待活跃事务结束后终止所有会话
3. `SHUTDOWN IMMEDIATE`：关闭节点
...
本命令用于通过批量终止会话来平衡节点负载。

|  选项| 含义|
| --------------- | ----------------------------------------- |
| *-c, --cluster* | YashanDB的集群名（必传参数）                          |
| *-n, --node-id* | 目标节点的ID（例如`1-2`，可通过yasboot cluster status命令查看数据库信息取`nodeid`冒号前的数字串）|
| *--with-transaction* | 终止会话时包含有活跃事务的会话，这些会话会等待事务结束后再断开 |
| *--count* | 最多终止的会话数量，默认为0，表示终止所有符合条件的会话 |
| *-w, --nowait*  | 运行后不等待执行命令结果                  |
| *-d, --child*   | 展示任务以及子任务信息                    |
| *--disable*     | 屏蔽运行的进度信息                        |
| *-u, --username*     | 指定数据库用户，不指定则默认使用sys用户           |
