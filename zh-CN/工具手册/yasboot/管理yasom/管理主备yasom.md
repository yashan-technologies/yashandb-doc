...
目前无法直接部署备yasom，只能通过[recover](../yasboot命令介绍/yasboot process)命令在当前服务器上新增一个备yasom，要求当前服务器上不存在已有的yasom进程。
...
2. 执行如下命令，执行如下命令在当前服务器上新增一个备yasom。
...
yasom降备操作主要用于手动修复yasom多主问题，推荐使用[yasom自修复](./yasom自修复配置)减少人工运维成本，若自修复仍未解决问题，则需手动将多余的主yasom降备或[清理](#clean)。
