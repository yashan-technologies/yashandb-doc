...
连接模式（多IP场景）仅在使用驱动连接YashanDB服务端时可以进行配置，mysql模式仅支持[JDBC驱动](../开发手册/JDBC驱动/JDBC驱动使用介绍/00JDBC驱动使用介绍)和[C驱动](../开发手册/C语言系驱动/C驱动/C驱动接口说明/连接、授权和初始化函数/yacConnect)，且要求YashanDB服务端存在多个节点。
...
- 需确保发起连接的设备IP不在[黑名单](../产品安全/连接管理/管理IP黑白名单)中。
