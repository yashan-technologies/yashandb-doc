...
| TABLE ACCESS BY USER ROWID  | 指定单个ROWID（`ROWID = ?`或`ROWID in ('9912:0:0:5108:4', '9912:0:0:5108:8')`）访问表中的数据，ROWID的数据格式请查阅[ROWID数据格式](../../../开发手册/SQL参考手册/数据类型/ROWID UROWID.html#Format)。                          | 行、列       | ×        |
| TABLE ACCESS BY ROWID RANGE   | 指定ROWID的范围（`ROWID BETWEEN ... AND ...`或`ROWID > ?`）访问表中的多行数据，ROWID的数据格式请查阅[ROWID数据格式](../../../开发手册/SQL参考手册/数据类型/ROWID UROWID.html#Format)。                          | 行          |  √    |
...
| PART SCAN COMBINED ITERATOR | 指定二级分区扫描。 | 行、列        | √          |
