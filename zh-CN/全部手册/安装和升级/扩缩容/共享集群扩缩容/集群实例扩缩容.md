...
> **Note**: 
>
> 执行扩缩容操作前，请仔细阅读[扩缩容注意事项](../扩缩容注意事项)。
>
> 如果扩缩容失败，请参考[扩缩容异常处理](../扩缩容异常处理)进行处理。
...
6. 将hosts_add.toml中的[[host]]内容复制粘贴到hosts.toml的末尾，以免升级时仍用旧的hosts信息。

7. 若扩容前其他服务器均已配置开机自启动，还需登录所有新服务器[配置开机自启动](../../安装部署/安装后初始环境/配置开机自启动)。
...
6. （可选）若缩容时指定了--with-host参数（即移除了空服务器），缩容完成后可对移除的服务器进行环境清理：
