YashanDB支持对备库进行在线扩展/删减，执行具体操作前请仔细阅读相关[注意事项](../扩缩容注意事项)。
...
5. 在新服务器上安装YashanDB软件并完成其他要求各主备库配置一致的操作。
    ...
    1. ) 执行如下命令，安装YashanDB。
        ...
        ```
    2. ) 若扩容前数据库已开启[资源管理](../../../数据库管理/资源管理/00资源管理)功能，新服务器上也需执行[yasboot host cgroup命令](../../../工具手册/yasboot/yasboot命令介绍/yasboot host)创建cgroup目录。
...
9. 若扩容前其他服务器均已配置开机自启动，还需登录所有新服务器[配置开机自启动](../../安装部署/安装后初始环境/配置开机自启动)。

10. 可按需开启自动选主功能，保障业务连续性：

    - 一主一备环境：可按需手动开启[yasom仲裁选主](../../../高可用/自动选主配置/一主一备yasom仲裁选主)。
...
4. 执行如下命令，删除备库。
    ...
    - 场景2：删除断连备库
      ...
      删除备库后，不会停止该备库，如果该备库被monit监控，也不会关闭对该备库的进程监控。
