...
如需进行[滚动升级](./升级操作/滚动升级)，须确保当前数据库满足以下条件：  
...
- 数据库为yashan模式。  
    ...
    若当前环境为mysql模式，请使用[离线升级](./升级操作/离线升级)。  
...
- 数据库不存在身份列。
    ...
    若当前数据库中存在身份列，请使用[离线升级](./升级操作/离线升级)。  
...
1. 在共享存储上规划系统盘，具体操作请查阅[磁盘划分](../安装部署/安装前准备/配置存储设备.html#DiskPartitioning)。
...
如需使用默认的备份路径，无需手动创建，但需确保升级操作用户具备创建路径的权限，以及默认备份路径所在父目录${旧版本的YASDB_HOME}具备足够的容量。

| 部署形态 | 备份位置 |
|--------------------|-------------|
| 单机部署<br/>存算一体分布式部署 | 每台服务器的${旧版本的YASDB_HOME}/upgrade_tmp/backup |
| 共享集群部署 | 主实例所在服务器的${旧版本的YASDB_HOME}/upgrade_tmp/backup |
