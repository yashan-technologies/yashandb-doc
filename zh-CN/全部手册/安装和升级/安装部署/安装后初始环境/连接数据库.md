...
连接模式（多IP场景）仅在使用驱动连接YashanDB服务端时可以进行配置，例如[JDBC驱动](../../../开发手册/JDBC驱动/JDBC驱动使用介绍)、[C驱动](../../../开发手册/C语言系驱动/C驱动/C驱动接口说明/连接、授权和初始化函数/yacConnect)、[OCI驱动](../../../开发手册/C语言系驱动/OCI驱动/OCI连接配置说明)、[ODBC驱动](../../../开发手册/C语言系驱动/ODBC驱动/ODBC安装说明/00ODBC安装说明)，且要求YashanDB服务端存在多个节点。
...
- 已完成[YashanDB客户端安装](../../安装部署/YashanDB客户端安装/00YashanDB客户端安装)和环境变量配置，如在服务端所在服务器的安装用户下连接本地数据库，则无需重复安装yasql。

- 需确保用户已具有[CREATE SESSION权限](../../../产品安全/数据访问控制/特权与角色管理/系统特权)（或[CONNECT角色](../../../产品安全/数据访问控制/特权与角色管理/角色)）。
...
当通过驱动连接数据库时，可参考[JDBC驱动](../../../mysql模式参考手册/快速入门/JDBC应用工程对接示例-MySQL驱动)和[C驱动](../../../开发手册/C语言系驱动/C驱动/00C驱动)了解详细的连接方式。
