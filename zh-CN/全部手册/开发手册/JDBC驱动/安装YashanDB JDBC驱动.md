...
1. 参考[YashanDB软件包清单](../../安装和升级/安装部署/安装前准备/下载软件包)获取YashanDB JDBC驱动安装包yasdb-jdbc-*版本号*.jar，例如yasdb-jdbc-{版本号}.jar。

2. 将jar文件下载到本地路径，例如/path/yasdb-jdbc-{版本号}.jar。
...
3. 将YashanDB JDBC驱动加入到JVM环境中：
    ...
    ```shell
    # 预先加入方式
    export CLASSPATH=/path/yasdb-jdbc-{版本号}.jar:${CLASSPATH} 

    # JVM运行时加入方式，Jdbcexample为应用程序名称
    java -cp .:/path/yasdb-jdbc-{版本号}.jar Jdbcexample
    ```
