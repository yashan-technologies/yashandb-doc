...
对于不能确保获得一行结果集的SELECT语句，需通过[游标](../PL语言基础/变量/游标)实现结果集赋值。
...
结果集可以包含一列或多列数据，每一列为一个[YashanDB通用表达式](../../SQL参考手册/通用SQL语法/expr)，但不能为序列、伪列、聚集函数、窗口函数、子语句。

在此种用法中，INSERT语句只能为[insert_values_clause](../../SQL参考手册/SQL语句（yashan模式）/INSERT)语法，不可以为多行或者子查询插入。

在此种用法中，[returning_clause](../../SQL参考手册/SQL语句（yashan模式）/INSERT)语法，不可返回变量。
...
结果集可以包含一列或多列数据，每一列为一个[YashanDB通用表达式](../../SQL参考手册/通用SQL语法/expr)，但不能为序列、伪列、聚集函数、窗口函数、变量、子语句。

在此种用法中，[returning_clause](../../SQL参考手册/SQL语句（yashan模式）/UPDATE.html#returningclause)语法，不可返回变量。
