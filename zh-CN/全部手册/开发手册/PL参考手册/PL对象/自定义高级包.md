...
用于定义已声明的存储过程，语法同[存储过程](./存储过程.html#procedurebodyclause)。

##### 5.2.2. function\_body\_clause

用于定义已声明的自定义函数，语法同[自定义函数](./存储过程.html#procedurebodyclause)。

若某个自定义函数在声明和定义时指定的PIPELINED、PARALLEL_ENABLE以及DETERMINISTIC标识不同，生效效果会因该函数的公有/私有属性不同而存在差异：

- 若该函数为公有成员：

  - [PIPELINED](./自定义函数.html#pipelined)：声明和定义时必须保持一致，否则报错。

  - PARALLEL_ENABLE：始终仅为语法兼容，若声明未指定但定义时指定则报错。

  - [DETERMINISTIC](./自定义函数.html#deterministic)：按声明时指定的生效，若声明未指定但定义时指定则报错。
