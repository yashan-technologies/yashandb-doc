...
PACKAGE HEAD和PACKAGE BODY通过不同语法分别定义，且YashanDB并不严格要求HEAD和BODY的创建顺序，如在HEAD还未创建时先定义BODY，系统将仍然创建该高级包，但会因为HEAD声明不存在而抛出编译错误，该高级包也无法被执行和调用，直到HEAD被创建。
...
在YashanDB中，自定义高级包的变量与自定义高级包进行绑定管理，如果自定义高级包的HEAD或BODY发生变更，该高级包的所有变量将被重置。
