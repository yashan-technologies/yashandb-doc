...
在使用分布式内存数据库时，由于需要严格对数据和业务进行分区管理，为保证数据一致性，要求PL对象的DDL操作在CN执行，PL对象的DML执行操作在DN执行。实际业务应用中，DDL执行操作可以由应用端配置连接串直接连接到DN，也可以参考[JDBC驱动对数据分片的支持](../JDBC驱动/JDBC高级功能说明/内存数据库对数据分片的支持)配置CN连接串然后由CN自动分发。
