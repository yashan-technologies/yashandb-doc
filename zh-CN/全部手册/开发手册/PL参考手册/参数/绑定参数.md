...
在23.1之前的版本，没有变量窥视，会将待绑定的unknow变量从最近的运算数据类型中推导一个类型。如该例子里，会将var传入之前的unknow数据类型推导为与data相同的类型，即char类型。待var变量传入以后，会将var变量转为char类型再与data比较。此时使用的是char类型与char类型的比较规则，空格是不敏感的，具体可查看[字符型](../../SQL参考手册/数据类型/字符型)，所以用例where条件匹配成功，结果为1。

在23.1及之后的版本，有变量窥视，会将var数据类型的真实数据类型传入，data与var类型比较时，是char类型与varchar类型比较，此时空格是敏感的，具体可查看[字符型](../../SQL参考手册/数据类型/字符型)。所以用例where条件匹配失败，结果为0。
