...
- 游标所绑定的SQL中包含有FOR UPDATE语句时，不支持dblink，其他约束同[SQL层的限制](../../../SQL参考手册/SQL语句/SELECT.html#forupdateclause)，例如多表连接查询时仅对通过OF column_name指定列对应的表进行更新。
...
[SELECT](../../../SQL参考手册/SQL语句/SELECT)查询语句，不可指定占位符（绑定参数）。为兼容语法，此处可以使用INTO子句，无实际作用。
