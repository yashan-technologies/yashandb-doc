...
| os_memory_limit| 内存限制百分比，默认为100，即分配所有内存，建议按需设置 |
...
| scene| 参数调优的场景，可选[NORMAL,BATCH,TPCH,AIM]，默认为NORMAL<br>当业务主要为OLTP业务使用了HEAP表时，如遇到周期性跑批场景性能不佳的情况，可考虑使用BATCH场景参数进行调优；<br>只有在存算一体分布式集群部署且表类型为LSC时可设置为TPCH，否则会报错；当使用TPCH场景进行调优推荐时，必须确保所有节点都执行参数推荐，否则可能重启时可能会因为连接参数不匹配而导致失败；<br>当业务使用了分布式内存数据库时，可配置为AIM进行调优推荐 |
| scale_factor| 诸如TPCH等测试场景的数据规模，默认100，表示100G测试数据。取值范围[1, 100000]，仅用于内存足够的情况下辅助部分参数值的推荐，内存不足时优先保证不超配，该参数会自动降级 |
| mms_limit| 内存库内存表空间使用进程内存限制百分比，默认80，剩下20用作计算内存分配。如果设置MMS占用内存过大，则将按数据库最小可用内存1.5G进行计算。scene为AIM的场景下生效，仅分布式内存库下DN生效。取值范围[0, 100] 。 |
