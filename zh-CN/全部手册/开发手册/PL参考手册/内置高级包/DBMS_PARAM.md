...
| scene| 参数调优的场景，可选[NORMAL,BATCH,TPCH]，默认为NORMAL<br>当业务主要为OLTP业务使用了HEAP表时，如遇到周期性跑批场景性能不佳的情况，可考虑使用BATCH场景参数进行调优；<br>只有在存算一体分布式集群部署且表类型为LSC时可设置为TPCH，否则会报错；当使用TPCH场景进行调优推荐时，必须确保所有节点都执行参数推荐，否则可能重启时可能会因为连接参数不匹配而导致失败 |
