...
本函数依赖密码相关的[系统变量](../../../参考手册/系统变量（mysql模式）)，函数返回值会因此类变量的取值变化而有所差异。基于密码的字节长度Length、[密码策略](../../../参考手册/系统变量（mysql模式）.html#validate_password_policy)配置以及是否匹配密码字典等因素，本函数的返回规则如下：

* Length < 4，返回0。
* 4 <= Length < [VALIDATE_PASSWORD_LENGTH](../../../参考手册/系统变量（mysql模式）.html#validate_password_length)，返回25。
* 满足Low策略，返回50。
