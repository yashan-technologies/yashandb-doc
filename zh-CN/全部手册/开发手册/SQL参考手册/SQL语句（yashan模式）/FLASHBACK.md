...
- 执行全库闪回时数据库需处于NOMOUNT状态，闪回完成后必须[ALTER DATABASE OPEN RESETLOGS](ALTER DATABASE.html#startupclauses)重置redo时间线。
...
- 仅适用于HEAP表。
- FLASHBACK TO SCN不允许使用于指定SCN点至今发生过以下改变表结构的DDL的场景：
