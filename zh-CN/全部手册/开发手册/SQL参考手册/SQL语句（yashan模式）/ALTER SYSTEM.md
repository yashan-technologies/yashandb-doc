...
GLOBAL_SESSION_ID（即存算一体分布式集群部署中的全局会话ID）可通过USERENV函数或查询GV$SESSION视图获取。在具有相同GLOBAL_SESSION_ID的会话集中有且仅有1个会话连接客户端与服务端CN节点，其他则为服务端各节点间的内部会话（其生存周期与客户端连接会话同步），关于会话的详细内容可参考[会话管理](../../../数据库管理/会话和计划管理/会话管理)。
