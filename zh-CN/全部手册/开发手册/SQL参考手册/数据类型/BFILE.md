...
- BFILE类型使用时需要创建文件路径别名，规格可参考[创建目录](../SQL语句/CREATE DIRECTORY)。
...
BFILE类型通过内置函数[BFILENAME](../内置函数/BFILENAME)定义BFILE定位器，进行插入操作，并通过[DBMS_LOB](../../PL参考手册/内置高级包/DBMS_LOB.html#dbms_lob_bfile)相关高级包功能进行文件数据的操作。
