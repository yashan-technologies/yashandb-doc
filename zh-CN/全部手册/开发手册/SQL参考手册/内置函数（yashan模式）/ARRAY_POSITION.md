...
ARRAY_POSITION函数以start_locate为起点查找数组（[数组变量](../../PL参考手册/PL语言基础/变量/集合变量)，[数组对象](../../PL参考手册/PL对象/自定义类型)，[数组类型](../数据类型（yashan模式）/用户自定义类型)）array_var的成员中第一个出现compare_member的位置，并返回INT类型的该位置值。
