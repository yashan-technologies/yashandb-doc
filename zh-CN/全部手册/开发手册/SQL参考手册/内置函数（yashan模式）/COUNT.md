...
COUNT函数按给定参数[expr](../通用SQL语法/expr)统计记录行数，本函数返回一个BIGINT的数值，且永远不会返回NULL。
...
当指定OVER关键字时，COUNT将作为[窗口函数](00内置函数（yashan模式）.html#WindowFunction)，并支持滑动窗口，返回多行的统计行数。
...
KEEP聚集函数通用语法，详细请查阅[KEEP聚集函数](00内置函数（yashan模式）.html#KeepAggregateFunction)。
