...
STRING_TO_ARRAY函数将字符串src_string以split_string作为分隔符进行切分生成一个数组，当被切分的成员与replace_string相同时，则将被切分成员替换为NULL后再生成数组（[数组变量](../../PL参考手册/PL语言基础/变量/集合变量)，[数组对象](../../PL参考手册/PL对象/自定义类型)，[数组类型](../数据类型/用户自定义类型)）并返回。
