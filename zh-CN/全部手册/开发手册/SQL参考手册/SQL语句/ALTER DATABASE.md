...
在共享集群部署中，本地TEMP/SWAP表空间的数据文件大小调整无法跨实例操作，即只能RESIZE当前实例所在服务器上的本地文件。 
...
在共享集群部署中，redo日志文件是各个实例独享的，各个实例可以给自己添加redo日志文件，主实例则可以通过指定THREAD integer为离线的实例添加redo日志文件。
...
在共享集群部署中，redo日志文件是各个实例独享的，各个实例可以给自己删除redo日志文件，主实例则可以通过指定THREAD integer为离线的实例删除redo日志文件，其中integer为实例ID。
...
> - 共享集群部署下，需在备集群的master实例执行该操作，备集群的其他存活实例保持OPEN状态即可。
...
在高可用部署场景中，全库闪回主要是在主库上开启与执行。对于备库，目前仅适用于特定的故障处理场景，更多详情请查阅[全库闪回](../../../数据库管理/闪回/全库闪回/00全库闪回)。
