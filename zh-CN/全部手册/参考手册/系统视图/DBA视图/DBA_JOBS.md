...
| LAST_DATE   | TIMESTAMP(6) WITH TIME ZONE | JOB上一次执行的时间                                     |
| LAST_SEC    | VARCHAR(8)               | 同LAST_DATE                                             |
| THIS_DATE   | TIMESTAMP(6) WITH TIME ZONE | 如果JOB正在执行，则为本次开始执行的时间。通常为NULL     |
| THIS_SEC    | VARCHAR(8)               | 同THIS_DATE                                             |
| NEXT_DATE   | TIMESTAMP(6) WITH TIME ZONE | JOB下次执行的时间                                       |
