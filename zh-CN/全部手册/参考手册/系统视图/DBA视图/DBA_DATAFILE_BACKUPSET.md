...
| TYPE             |    VARCHAR(10)   | 备份类型<br>\* FULL：全量备份 <br>\* INCREMENTAL：增量备份 |
...
| TAG              |   VARCHAR(64)   | 唯一标签   |
| IDENT            |   BIGINT        | 数据文件唯一ID |
| TRUNC_LSN        |   BIGINT        | 该数据文件基线日志序列号，该序列号之前的redo日志不需要回放 |
| FILE#            |   BIGINT        | 数据文件ID |
| TS#              |   BIGINT        | 数据文件所属表空间ID |
| TSNAME           |   VARCHAR(64)   | 数据文件所属表空间名称 |
...
| RCY_BEGIN_ASN    |   INTEGER       | 备份集回放开始点对应的ASN：归档序列号，共享集群中为所有节点中最小的ASN |
| RCY_BEGIN_LFN    |   BIGINT        | 备份集回放开始点对应的LFN：日志序列号 |
| RCY_END_ASN      |   INTEGER       | 备份集回放结束点对应的ASN：归档序列号，共享集群中为所有节点中最大的ASN|
| RCY_END_LFN      |   BIGINT        | 备份集回放结束点对应的LFN：日志序列号 |
| BS_KEY           |   BIGINT        | 当前备份集的唯一身份标识 |
| IS_BUCKET        |   BOOLEAN       | 标识是否是SLICE文件备份集 |
| RESTORE_TIME     |   DATE          | 当前备份集产生时，数据库最近一次restore的时间，若数据库未执行过RESTORE操作，显示为空 |
| CHECKPOINT_TIME  |  TIMESTAMP(6)     | 数据库一致性恢复的时间点，若其显示为空，则该数据库为旧版本升级至新版本，无需参考该项内容 |
