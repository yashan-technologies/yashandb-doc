...
| PERSISTENT_MEM | INTEGER | 在SQL缓存池中占用的页面内存减去尾部未使用的内存 |
| RUNTIME_MEM | INTEGER | 此SQL在执行过程中申请的内存 |
...
| PX_SERVERS_EXECUTIONS | BIGINT | 以并行方式执行的总次数（保留字段） |
| END_OF_FETCH_COUNT | BIGINT | 光标被带到库缓存中后，该光标完全执行的次数 |
| ROWS_PROCESSED | BIGINT | SQL语句返回的总行数 |
...
| DIRECT_WRITES | BIGINT | 游标直接写的次数（保留字段） |
| DIRECT_READS | BIGINT | 游标直接读的次数（保留字段） |
...
| IM_SCANS | BIGINT | 内存列存储段扫描的数量（保留字段） |
| IM_SCAN_BYTES_UNCOMPRESSED | BIGINT | 内存列存储段扫描的未压缩字节数（保留字段） |
| IM_SCAN_BYTES_INMEMORY | BIGINT | 内存列存储段扫描的字节数（保留字段） |
...
| OPTIMIZER_MODE | VARCHAR(10) | SQL语句的优化器模型（保留字段） |
...
| COMPAT_MODE | INTEGER | 当前SQL的兼容模式 |
| COMPAT_ATTRS | BIGINT | 当前SQL的兼容模式属性 |
