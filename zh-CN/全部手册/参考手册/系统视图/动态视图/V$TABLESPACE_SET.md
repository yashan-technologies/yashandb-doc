本视图显示当前节点的所有表空间集信息，仅适用于存算一体分布式集群部署。
...
|DATAFILES_COUNT          | NUMBER       | 该表空间集中数据文件的总数|
|DATAFILES_MAX_SIZE       | NUMBER       | 该表空间集的数据文件总的最大SIZE|
|DATAFILES_SIZE           | NUMBER       | 该表空间集中所有datafile的实际大小的总和|
|DATAFILES_FREE_BLOCKS    | NUMBER       | 该表空间集中所有datafile的空闲可用数据块总数|
|DATABUCKETS_COUNT        | NUMBER       | 该表空间集中DATABUCKET文件的总数|
