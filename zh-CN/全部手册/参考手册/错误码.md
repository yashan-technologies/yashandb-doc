...
**Action**：容器的任务是串行执行的，需要等待上一个任务执行完成后再执行，可查询视图[V$CONTAINER_TASK](./系统视图/动态视图/V$CONTAINER_TASK)视图查看任务的执行情况。

### YAS-02914
**Message**: the current pluggable database is unavailable

**Action**：当前PDB因RESTORE失败导致不可用，请检查RESTORE环境后重新执行该PDB的RESTORE操作。
...
**Action**：BLOB， TEXT， GEOMETRY 或者 JSON 列不能设置 DEFAULT 'XXX'。
