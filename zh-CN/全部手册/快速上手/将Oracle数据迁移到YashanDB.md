...
- 数据迁移平台-YMP：YMP（YashanDB Migration Platform）是YashanDB提供的数据库迁移产品，支持数据源类型包括Oracle、MySQL、DM及YashanDB，支持元数据和数据一键迁移到崖山数据库。具体的适配版本可参考[YMP产品规格](https://doc.yashandb.com/ymp/23.5/zh/Product-Introduction/Specifications.html)。
...
本章节以YMP迁移小规格Oracle数据库到YashanDB为例介绍完整的迁移过程。迁移过程中，YMP会进行业务数据的暂存和处理，迁移数据量越大、迁移速度要求越高对部署YMP的服务器资源要求越高，因此本案例作为迁移示例，建议Oracle源数据库不超过30张表，单表数据量控制在百万级。对于生产环境数据迁移请参考[YMP数据迁移平台产品文档](https://doc.yashandb.com/ymp/23.5/zh/Product-Introduction/Summarize.html)进行操作。
...
本次示例仅考虑迁移Oracle数据库中某一schema的业务数据，且该schema中的对象对其他schema中的对象没有依赖关系，如需全量迁移Oracle数据库，需参考[数据迁移平台-YMP](https://doc.yashandb.com/ymp/23.5/zh/Product-Introduction/Summarize.html)文档了解相关权限要求。

### 目标端数据库信息收集及配置

针对目标端YashanDB需收集下列信息。本章节使用的示例信息为[快速体验YashanDB](快速体验YashanDB)章节完成的配置示例，实际迁移时需根据真实环境信息调整相关参数。

- YashanDB23.4版本已支持兼容mysql模式，如需将源端MySQL迁移到YashanDB，需要。
- 服务器IP及端口。
- 数据库名称和Schema名称。
- 登录连接用户名和密码，该连接用户需拥有DBA权限。在[快速体验YashanDB](快速体验YashanDB)章节，已完成测试用户sales并赋权。
...
4. 以ymp用户将[快速体验YashanDB](快速体验YashanDB)中部署的数据库安装包和YMP安装包yashan-migrate-platform-{版本号}-linux-x86-64.zip上传到/home/ymp路径，解压后执行安装命令。
...
1. 选择【数据源管理】页面，单击【添加数据源】按钮，依次将源端Oracle数据库和目标端YashanDB添加作为数据源，Oracle的连接配置需勾选**是否为普通用户**选项，崖山数据库配置连接用户可使用[快速体验YashanDB](快速体验YashanDB)章节创建的测试用户sales。
