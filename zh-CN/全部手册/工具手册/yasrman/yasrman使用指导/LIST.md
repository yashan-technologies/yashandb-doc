...
该语句用于列出指定PDB的备份集信息，完整语句为LIST BACKUP OF PLUGGABLE DATABASE pdb_name [, pdb_name]，无法与其他LIST语法同时使用。
...
mapfile内容示例如下：
```shell
database_node_id: 1-1  # 数据库节点ID，存算一体分布式集群部署中用该ID确定该节点的恢复路径，不可手动更改。
