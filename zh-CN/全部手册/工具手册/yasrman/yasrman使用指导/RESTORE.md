...
```ebnf+diagram
syntax::= RESTORE dbObjectSpecifier ([FROM TAG tag_name|DBID database_id] (UNTIL ((SCN integer)|(TIME time_string)))|FROM TAG tag_name)
...
```ebnf+diagram
syntax::= DATABASE [ROOT]|PLUGGABLE DATABASE ((pdb_name) {"," (pdb_name)})
```
...
| 容器数据库 | DATABASE | 用于恢复整个数据库或单个PDB，具体取决于使用yasrman所连接登录的对象：<br/>* 连接根容器时：恢复根容器、种子容器以及所有PDB。<br/>* 连接某个PDB时：恢复该PDB。 |
| 容器数据库 | DATABASE ROOT | 用于恢复根容器，必须连接根容器执行。  |
| 容器数据库 | PLUGGABLE DATABASE | 用于恢复指定PDB，必须采用具备指定PDB的恢复和连接权限的用户连接根容器执行。 |
...
# 连接根容器，恢复指定PDB到指定时间点
$ yasrman sys/********@192.168.1.2:1688 -c "restore pluggable database PDB1 until time '2023-11-13 18:54:32:657892'" -D /home/yashan/catalog
...
```ebnf+diagram
syntax::= RESTORE ARCHIVELOG [ROOT] archivelogRangeSpecifier [FROM TAG tag_name] [SKIP VALIDATE][OVERWRITE] [restoreCommonSpecifier]
```
