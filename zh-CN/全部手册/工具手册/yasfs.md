yasfs是单实例的YFS服务程序，在共享集群/分布式集群安装部署、离线升级以及离线修改集群配置时提供文件系统服务。

>**Warn**:
>
> 仅允许在使用ycsctl离线修改集群配置、离线新增PDB时启动单实例yasfs服务，ycsctl离线操作结束后，必须先停止yasfs服务再启动集群。
>
> 单实例yasfs服务运行时，同一共享集群/分布式集群的其他节点不得启动YCS或yasfs服务，**否则会造成元数据损坏或数据丢失**。
