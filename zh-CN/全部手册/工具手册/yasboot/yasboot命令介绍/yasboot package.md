...
| *--cpu-limit*         | 服务器的可使用CPU百分比上限，默认为100，须配合--recommend-param使用           |
| *--scale-factor*      | 诸如TPCH等测试场景的数据规模，默认为100，表示100G测试数据须配合--recommend-param使用         |
| *--scene*             | 参数调优的场景，可选[NORMAL,TPCH,AIM]，默认为NORMAL。<br>使用分布式列存表执行TPC-H性能测试时，可考虑使用TPCH场景参数进行调优，且必须确保所有节点都执行参数推荐，否则可能重启时可能会因为连接参数不匹配而导致失败。<br>当业务使用了分布式内存数据库时，可配置为AIM进行调优推荐须配合--recommend-param使用     |
| *--mms_limit*         | 内存库模式下，内存表空间大小占进程内存的百分比，只有scene模式为AIM时才生效。取值范围[0,100]，默认为80     |
