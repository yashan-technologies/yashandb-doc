...
| *--ignore-hostname*   | 忽略服务器名称（隐藏参数）<br/>* 不指定时，默认采用服务器名称作为数据库各节点的hostid<br/>* 指定该选项时，yasboot将自行生成默认字符串（yas1，yas2……）作为hostid<br/>当共享集群部署的不同实例的服务器同名且不允许修改时需指定该选项，其他场景非必要**不推荐使用**                |
...
| ------------------ | ---------------------------------------------------- |
| *-c, --cluster*    | YashanDB的集群名（必传参数）                         |
| *-m, --start-mode* | 启动数据库实例的模式，可选[open&#124;nomount&#124;mount]，默认open（共享集群部署暂无法启动到mount模式） |
| *-w, --nowait*     | 运行后不等待执行命令结果                             |
| *-d, --child*      | 展示任务以及子任务信息                               |
| *--disable*        | 屏蔽任务进度条展示                                   |
| *-u, --username*     | 指定数据库用户，不指定则默认使用sys用户           |
...
| *-n, --new-password* | 新的sys密码（必传参数），密码需遵循[密码强度](../../../产品安全/身份标识与鉴别/密码认证/密码策略.html#password_complexity)和[密码策略](../../../产品安全/身份标识与鉴别/密码认证/密码策略)要求<br/>若密码中含有OS命令相关的特殊字符（例如`@`、`/`、`.`、`!`、`$`、`'`等）需进行转义                     |
| *-w, --nowait*       | 运行后不等待执行命令结果                                  |
| *-d, --child*        | 展示任务以及子任务信息                                    |
| *--disable*          | 屏蔽任务进度条展示                                        |
...
| *--backup-reserved-space* |  离线升级时的临时备份路径的预留容量，单位可以为B/KB/MB/GB/TB/PB/EB（B可省略，例如2GB或2G），默认为5G |
...
| *--disk-config* | 共享集群由23.2.x.x及以前版本升级至23.4.x.x及以上版本需指定的磁盘信息，格式见[共享集群升级配置文件](../配置文件/共享集群升级配置文件)           |
...
建议首次执行本命令时不指定-t参数，此时会在当前目录生成[数据库拓扑文件](../配置文件/数据库拓扑文件)记录数据库拓扑信息。若发现首次加载后，数据库信息与实际不符，需修正[数据库拓扑文件](../配置文件/数据库拓扑文件)并指定-t参数再次进行加载。

|  选项| 含义|
| --------------- | -------------------------------- |
| *-c, --cluster* | 集群名称（必传参数）             |
| *-t,--toml*     | 从[数据库拓扑文件](../配置文件/数据库拓扑文件)中加载数据库信息。 |
