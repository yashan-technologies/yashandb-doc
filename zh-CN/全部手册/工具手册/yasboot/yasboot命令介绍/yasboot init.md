...
| --ce-data           | 共享集群/分布式集群数据盘，支持输入多个，使用逗号分隔        |
| --disk-found-path   | 共享集群/分布式集群磁盘发现路径，支持输入多个，使用逗号分隔                                  |
...
| --group             | 共享集群/分布式集群组的个数                                |
| --standby-node      | 共享集群备集群节点的部署规模                 |
| -fg,--failgroup     | 共享集群/分布式集群磁盘组的故障组数量                       |
