...
> - 关闭数据库实例的脚本内容通常是：yasql登录到数据库实例并执行"shutdown immediate"命令。为了提高安全性，推荐完成[操作系统认证配置](../../产品安全/身份标识与鉴别/操作系统认证/配置操作系统认证)并通过yasql[免密登录](../yasql/yasql使用指导)。
...
指定数据库实例进行密钥管理的[钱包的密码](../../产品安全/加密支持/存储加密/密钥管理)。当共享集群已有加密对象且需要直接启动数据库到MOUNT或OPEN模式时必须指定，否则数据库实例会启动失败。
...
> **Note**: 
>
> 关于YCS OS Watcher的详细介绍，请查阅[操作系统负载监控](../../数据库管理/运行监控/YCS OS Watcher)。
...
参数名称，来自于YCS所允许的[集群参数](../../数据库管理/集群管理/集群参数配置)，当前可指定设置如下参数：
...
目标参数值，必须为YCS所允许范围内的值，详情请查阅[集群参数配置](../../数据库管理/集群管理/集群参数配置)。
