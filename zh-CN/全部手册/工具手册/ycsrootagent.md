...
在需要自动化检测存储设备是否支持SCSI I/O Fencing的场景中，yasboot会自行调用fenceScsiCheck脚本从而调用执行本命令的相关子命令。如需手动进行相应检测，请直接执行[fenceScsiCheck脚本](../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing.html#fenceScsiCheck_usage)，**不建议用户直接执行本命令**。
...
| *--all* | 仅对clear子命令有效，若带此选项，将清理此前进行过持久预留操作的所有磁盘 |

## ycsrootagent pingdisk

在需要自动化检测存储设备是否支持SCSI I/O Fencing的场景中，yasboot会自行调用fenceScsiCheck脚本从而调用执行本命令。如需手动进行相应检测，请直接执行[fenceScsiCheck脚本](../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing.html#fenceScsiCheck_usage)，**不建议用户直接执行本命令**。
...
|  选项| 含义|
| ------------------ | ---------------------------------------------------- |
| *-f* | 若带此选项，将跳过风险提示并强制执行 |

## ycsrootagent network verify

本命令用于校验当前服务器是否可以访问特定网络，以及对应的网卡状态是否正常。

|  选项| 含义|
| ------------------ | ---------------------------------------------------- |
| *-subnet* | 待校验的网络的子网地址、掩码等信息，格式为subnet/netmask[/interface] |
