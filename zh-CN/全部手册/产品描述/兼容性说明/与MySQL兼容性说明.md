...
- [mysql模式](../../mysql模式参考手册/mysql模式概述)：在该模式下用户输入的SQL语句将优先按照MySQL的语法体系进行解析，适用于需要密切适配MySQL数据库的使用场景。
...
详情请查阅[数据类型（mysql模式）](../../mysql模式参考手册/SQL参考手册/数据类型/00数据类型)。
...
| SQL_MODE                | YashanDB mysql模式已实现了部分SQL_MODE语法开关，例如ANSI_QUOTES、NO_AUTO_VALUE_ON_ZERO和NO_BACKSLASH_ESCAPES等，详细兼容项说明请参考[SQL_MODE](../../mysql模式参考手册/SQL参考手册/SQL_MODE)。对于未完成实际能力兼容的开关，无论如何修改参数值，均不会对数据库的行为产生影响，实际执行效果与yashan模式一致。 |
