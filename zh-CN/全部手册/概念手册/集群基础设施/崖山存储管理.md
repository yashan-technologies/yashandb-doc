...
YFS的冗余度分为EXTERNAL、NORMAL和HIGH，冗余级别依次增强。若指定为EXTERNAL则无数据冗余，数据的可靠性依赖外部能力实现，例如RAID。  

| 冗余级别 | 系统DiskGroup中文件副本数 | 数据DiskGroup中用户数据副本数 | 数据DiskGroup中YFS元数据副本数 |
| ---------- | ------ | ----------- | --------------- |
| EXTERNAL   | 1      | 1           | 1               |
| NORMAL     | 2 ~ 3  | 2           | 2 ~ 3           |
| HIGH       | 5      | 3           | 3 ~ 5           |
...
YFS支持的AU size颗粒度包括：1M（默认值）、4M、8M、16M以及32M，用户可以在创建DiskGroup时指定该DiskGroup的AU size或使用默认的AU size。
