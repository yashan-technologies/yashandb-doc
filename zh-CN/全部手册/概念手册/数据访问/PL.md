...
PACKAGE HEAD和PACKAGE BODY通过不同语法分别定义，YashanDB对HEAD和BODY存在一定约束，但YashanDB不严格要求HEAD和BODY的创建顺序，即使在创建HEAD前先定义BODY，仍然允许创建package，但会因为HEAD声明不存在而抛出编译错误，该自定义高级包也无法被执行和调用，直至完成其HEAD创建。约束如下：
