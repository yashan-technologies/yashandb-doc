...
所有SQL语句都是由优化器依据YashanDB特定的代价模型生成最优的执行计划，该计划受统计信息、代价计算公式、HINT、OUTLINE等功能影响。
...
数据定义语言（DDL，Data Definition Language）提供对数据库对象的创建、修改、删除操作。数据库对象包括用户、表空间、表、索引、视图、序列、同义词、存储过程、自定义函数、触发器、高级包等数据库结构，还包括数据库本身。
...
数据操作语言（DML，Data Manipulation Language）提供了对数据库进行INSERT插入、UPDATE更新、DELETE删除、MERGE合并数据以及SELECT查询的能力，允许单表、多表数据操作。
...
事务控制语句可以对事务进行提交（COMMIT）、回滚（ROLLLBACK）、设置回滚保存点（SAVEPOINT）以及基于SAVEPOINT的回滚（ROLLLBACK TO SAVEPOINT）操作。
