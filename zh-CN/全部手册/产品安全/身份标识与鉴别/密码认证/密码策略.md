...
- FAILED_LOGIN_ATTEMPTS、FAILED_LOGIN_INTERVAL与PASSWORD_LOCK_TIME配合使用，可达到“密码错误时等待X秒后可重试，连续重试Y次后用户被锁定Z天无法登录（除非手动[解锁用户](../管理用户.html#alteruser)）”的效果。
...
执行PROFILE修改操作的用户，需具备ALTER PROFILE权限，授权操作请查阅[权限授予与收回](../../数据访问控制/特权与角色管理/权限授予与收回)。
...
执行PROFILE删除操作的用户，需具备DROP PROFILE权限，授权操作请查阅[权限授予与收回](../../数据访问控制/特权与角色管理/权限授予与收回)。
