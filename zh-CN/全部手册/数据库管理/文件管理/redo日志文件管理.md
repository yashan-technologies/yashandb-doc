...
若初始配置无法满足业务/性能需求，可通过[ALTER DATABASE](../../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER DATABASE.html#add_logfile)语句自定义添加redo日志文件。
...
- 手动切换：
    ...
    - 单切换：执行[ALTER SYSTEM SWITCH LOGFILE](../../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER SYSTEM.html#switch_logfile)语句可强制切换redo日志文件，即使数据库处于归档模式，该操作仍不会自动归档。

    - 归档 + 切换：数据库处于归档模式时，执行[ALTER SYSTEM ARCHIVE LOG CURRENT](../../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER SYSTEM.html#archive_logfile)语句可归档当前CURRENT状态的redo日志文件，同时会切换redo日志文件。
...
如发生下述情况，可通过[ALTER DATABASE](../../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER DATABASE.html#drop_logfile)语句删除redo日志：
