...
| 控制文件名称 | 单机部署、存算一体分布式部署：ctrl1、ctrl2、ctrl3<br/>共享集群部署：ctrlfile0、ctrlfile1、ctrlfile2 |
| 控制文件存储路径 | 单机部署、存算一体分布式部署：$YASDB_DATA/dbfiles<br/>共享集群部署：+DG0/dbfiles |
...
- 安装完成后
  ...
  - 新建数据库时，在执行[CREATE DATABASE](../../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE DATABASE)时可通过CONTROLFILES关键字指定控制文件的数量及各个文件的存放路径。
...
在YashanDB中，备份与恢复控制文件通过备份与恢复整个数据库实现，具体操作请查阅[备份恢复](../备份恢复/00备份恢复)。
