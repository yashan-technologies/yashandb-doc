...
此方法需要YCSRA进程在线以提供特权操作代理服务，请确保YCSRA进程在每个服务器上都已启动，否则请参考[ycsrootagent工具](../../../工具手册/ycsrootagent)手动sudo拉起。为简化运维，建议在每台服务器上为该进程配置开机自启动，具体操作请查阅[配置开机自启动](../../../安装和升级/安装部署/安装后初始环境/配置开机自启动)。
...
- 注意事项：若已通过多路径的方式挂载磁阵，请在配置此方法前[修改多路径配置文件](../../../安装和升级/安装部署/安装前准备/配置存储设备.html#modify_multipath_conf)。
...
- 错误码YAS-00341：磁盘是SCSI设备但残留了非本共享集群注册或预留的信息，请在确认安全后谨慎使用[ycsrootagent scsi clear命令](../../../工具手册/ycsrootagent.html#scsi_clear)清理残留信息。
