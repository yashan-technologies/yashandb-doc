...
OS Watcher默认会随YCS启动而启动，用户可按需通过YCS配置参数[OSW_AUTO_START](../集群管理/集群参数配置.html#osw_auto_start)配置是否自启动。
...
默认配置下，OS Watcher收集信息的时间间隔为磁盘心跳超时时间DISK_HB_KEEP_ALIVE的三分之一（结果向下取整数）。用户可按需通过YCS配置参数[OSW_INTERVAL](../集群管理/集群参数配置.html#osw_interval)指定时间间隔。
...
OS Watcher支持指定能保存文件数量和单个文件大小，通过YCS配置参数[OSW_FILE_NUM](../集群管理/集群参数配置.html#osw_file_num)和[OSW_FILE_SIZE](../集群管理/集群参数配置.html#osw_file_size)控制。
