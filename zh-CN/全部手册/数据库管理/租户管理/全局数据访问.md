...
YashanDB部署为容器数据库（配置参数ENABLE_PLUGGABLE_DATABASE=TRUE）时提供了一组[CDB视图](../../参考手册/系统视图/CDB视图)，供DBA在根容器上获取全局信息，例如查询CDB_TABLES视图可以获取当前环境中所有的表、查询CDB_ROLE_PRIVS视图可以获取当前环境中所有基于角色授权的记录等。
