...
在使用[CREATE TABLE](../../开发手册/SQL参考手册/SQL语句/CREATE TABLE)创建表对象时，不使用TABLESPACE指定表空间，默认为用户的所属表空间，表数据进入规划的存储空间内，其中临时表数据将进入系统临时表空间内。
...
数据库管理员可以通过SQL语句管理表的存储空间，详细语法描述请参考开发手册[CREATE TABLE](../../开发手册/SQL参考手册/SQL语句/CREATE TABLE)、[ALTER TABLE](../../开发手册/SQL参考手册/SQL语句/ALTER TABLE)。
