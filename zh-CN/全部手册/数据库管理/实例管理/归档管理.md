...
归档可以延长数据库的redo历史操作记录的保存时间，用于[备份与恢复](../备份与恢复/00备份与恢复)保障数据安全、高可用部署时的主备同步。
...
如在开启归档模式后再调整该路径，原路径中已经存在的归档日志文件不会发生改变，可以按需根据清理规则进行自动或手动清理，清理操作请查阅[归档日志文件管理](../存储管理/数据库文件管理/归档日志文件管理.html#cleanup)。
...
如果YashanDB默认的自动清理策略无法满足实际需求，数据库管理员可以按需[调整](#auto_cleanup_config)对应参数，或临时执行[手动清理](../存储管理/数据库文件管理/归档日志文件管理.html#cleanup_manual)操作。
...
手动归档（[ALTER SYSTEM ARCHIVE LOG CURRENT](../../开发手册/SQL参考手册/SQL语句/ALTER SYSTEM.html#archive_logfile)）只能归档CURRENT状态的redo日志文件，归档的同时还会切换redo日志文件，具体操作查阅[切换并归档redo日志文件](../存储管理/数据库文件管理/redo日志文件管理.html#SWITCH)。

手动归档所生成的归档日志文件与自动归档生成的文件在管理操作上不存在区别，同样受自动清理策略所管理，也同样可以按需进行[文件管理](../存储管理/数据库文件管理/归档日志文件管理)。
