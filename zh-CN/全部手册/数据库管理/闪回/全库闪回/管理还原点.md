...
| 恢复类操作对还原点的影响 | * 对于[备份与恢复](../../备份恢复/00备份恢复)：恢复后，会自动删除所有还原点<br/>* 对于[全库闪回](./闪回恢复数据库)：全库闪回完成后，创建时间晚于恢复目标时间点的还原点将被自动删除  | 同永久还原点 |
| 其他 | * 强制保留该还原点的[归档日志文件](../../基本数据库管理/归档管理.html#auto_cleanup)<br/>* 通常存在永久还原点时无法[关闭全库闪回功能](./全库闪回基础配置.html#flashback_off)，但执行SWITCHOVER时会强制自动关闭旧主的闪回功能（即使存在永久还原点） | - |
