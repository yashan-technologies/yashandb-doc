...
YashanDB中，数据库每个实例节点上的控制文件数及文件存储路径配置相互独立，相关配置由配置参数[CONTROL_FILES](../../../参考手册/配置参数.html#CONTROL_FILES)指定，默认配置如下：
...
- 安装数据库时
  ...
  - （推荐）在{集群名称}.toml文件中指定配置参数[CONTROL_FILES](../../../参考手册/配置参数.html#CONTROL_FILES)配置控制文件的数量及各个文件的存放路径。

  - 单机或存算一体分布式集群部署时，可以在{集群名称}.toml文件中指定建库参数[CTRL_FILE_NUM](../../../工具手册/yasboot/建库参数)配置控制文件的数量，但单纯只指定CTRL_FILE_NUM时所有控制文件的存放路径均为$YASDB_DATA/dbfiles。

- 安装完成后
  
  - 对于已有数据库，可通过配置参数[CONTROL_FILES](../../../参考手册/配置参数.html#CONTROL_FILES)调整控制文件的数量及各个文件的存放路径。
...
在YashanDB中，备份与恢复控制文件通过备份与恢复整个数据库实现，具体操作请查阅[备份与恢复](../../备份与恢复/00备份与恢复)。
