数据库运行于[归档模式](../../实例管理/归档管理)时，redo日志文件进行归档后生成的二进制文件称为归档日志文件。

## 查看归档日志文件

### 查看物理文件

使用安装用户登录数据库所在服务器，进入归档日志文件的存放路径查看对应的物理文件，归档日志文件默认存放在$YASDB_DATA/archive目录，支持自定义[配置归档路径](../../实例管理/归档管理.html#archive_local_dest)。
...
通过[V$ARCHIVED_LOG](../../../参考手册/系统视图/动态视图/V$ARCHIVED_LOG)视图可以查看归档日志文件的信息，例如归档日志的序列号、归档日志文件名和归档时间等。
...
具体的备份与恢复操作步骤，请查阅[备份与恢复](../../备份与恢复/00备份与恢复)。

<span id="cleanup_manual" name="cleanup_manual" class="yaslink"></span>

## 手动清理归档日志文件

YashanDB提供了[自动清理机制](../../实例管理/归档管理.html#auto_cleanup)，在参数配置符合业务需求的情况下，通常无需人为介入。若遇偶发需要更多磁盘空闲容量等场景，数据库管理员可以对归档日志文件进行手动清理，手动清理操作不会妨碍自动清理机制。

手动清理归档日志文件是指通过执行SQL语句进行清理，**请勿直接在操作系统路径下删除相关文件**。清理时，支持基于以下维度指定归档日志文档范围（实际清理仍遵循清理原则与策略），相应的范围依据均可通过查询[V$ARCHIVED_LOG](../../../参考手册/系统视图/动态视图/V$ARCHIVED_LOG)视图获取：
...
2. 查看ARCHIVELOG_DELETION_POLICY和ARCH_CLEAN_IGNORE_MODE参数配置。
...
4. 为避免归档日志文件被误清理，建议先对其进行备份。
    ...
    在单机/共享集群/分布式集群部署中，可以单独备份归档日志文件，具体操作步骤请查阅[备份归档日志](../../备份与恢复/SQL语句备份恢复/备份.html#backup_arch)。
...
```
    ...
    在存算一体分布式集群部署中，只能通过备份整个数据库从而备份归档日志文件，具体操作步骤请查阅[备份全库](../../备份与恢复/yasrman工具备份恢复/备份)。
...
5. 手动清理归档日志，具体语法请查阅[ALTER DATABASE](../../../开发手册/SQL参考手册/SQL语句/ALTER DATABASE.html#deletearchivelogclauses)。
