...
- 如果备库transport_lag或apply_lag不为0（即备库有待接收和回放的redo），Switchover过程耗时会增加，可通过查询[V$RECOVERY_PROGRESS](../参考手册/系统视图/动态视图/V$RECOVERY_PROGRESS)视图了解剩余回放时间。
...
- 主备库同步正常，可通过[V$REPLICATION_STATUS](../参考手册/系统视图/动态视图/V$REPLICATION_STATUS)视图获取同步状态。
...
HA_ELECTION_ENABLED = TRUE表示已开启自动选主，需[关闭自动选主](./自动选主配置/一主多备自动选主.html#close_election)后才能执行后续操作。
...
- 全局切换：通过根容器执行[ALTER DATABASE](../开发手册/SQL参考手册/SQL语句/ALTER DATABASE)或[yasboot](../工具手册/yasboot/yasboot命令介绍/yasboot node)命令进行Switchover或Failover，实现整个CDB（根容器和所有PDB）的统一角色转换，最终切换结果为新主根容器上的所有PDB均为主库。
...
- 批量PDB精准切换：通过根容器执行[ALTER PLUGGABLE DATABASE](../开发手册/SQL参考手册/SQL语句/ALTER PLUGGABLE DATABASE)语句或[yasboot](../工具手册/yasboot/yasboot命令介绍/yasboot node)命令定向操作指定单个或多个PDB进行独立Switchover或Failover。
