...
- 自动选主机制：主库节点不可用时，从备库节点自动选举升主，实现发生故障时的自动切换，包括[一主多备自动选主](./自动选主配置/一主多备自动选主)和[一主一备yasom仲裁选主](./自动选主配置/一主一备yasom仲裁选主)。
...
主备复制分物理复制和逻辑复制，物理复制将主库的物理存储内容复制到备库，逻辑复制基于逻辑变更记录（例如SQL语句）在备库上执行的方式重现主库数据，其主要区别在于日志应用的方式。

- 物理备库：使用redo apply机制，通过物理回放从主库接收的redo数据与主库保持同步。物理备库是主库的物理副本，其磁盘数据库结构与主库逐块相同、数据库架构（包括索引）与主库完全相同。

- [逻辑备库](./逻辑备库/00逻辑备库)：使用SQL Apply机制，先将从主库收到的redo数据转换为SQL语句，然后再执行对应SQL语句。逻辑备库包含与主库相同的逻辑信息，即两者包含完全相同的业务数据与元信息，但允许在物理存储结构（例如数据文件分布、索引组织形式）上存在差异。

<span id="protection_mode" name="protection_mode" class="yaslink"></span>

### 保护模式

主备复制基于网络传输，势必存在一定的延迟和不稳定性，YashanDB提供三种不同的保护模式供选择配置，由用户根据自身对性能或对数据安全的需求与偏好来决策采用合适的保护模式，具体操作请查阅[管理保护模式](./管理保护模式)。
...
在单机部署或存算一体分布式集群部署中，用户可以根据实际业务需求和性能考虑定义最大保护模式下具体的[同步备](./定义同步备)相应配置，从而灵活调整主库事务提交成功的同步机制。
...
在不同机房/地域部署两套单机主备环境，每套主备环境作为一个复制组，数据从一个复制组（主复制组）复制同步到另一个复制组（备复制组）构成异地容灾架构，部署操作请查阅[双复制组主备部署](../安装和升级/安装部署/YashanDB服务端安装（命令行）/单机（主备）部署.html#Manual)。
...
可根据需要选择是否启用[yasom仲裁选主](自动选主配置/一主一备yasom仲裁选主)。
...
当YashanDB部署为容器数据库（配置参数ENABLE_PLUGGABLE_DATABASE=TRUE）时，支持PDB级别的主备复制机制，通过在多个独立CDB之间建立PDB级别的主备关系，在保障数据高可用性的同时，为CDB环境提供了灵活的容灾解决方案，实现了真正的跨容器数据保护。

![](./image/P-SinCDB.png)

### 主备CDB 

在单机主备部署中，CDB将分为主CDB和备CDB，而CDB中各个PDB的角色可以与根容器本身的主备角色不一致。

- 关联关系约束：CDB间PDB主备关系的建立依赖于主备根容器间的连接配置。只有当主备根容器完成必要的关联配置后，PDB才能与另一个CDB内的对应PDB建立主备关系，确保架构的一致性和安全性。

- 元数据同步：主CDB与备CDB保持PDB数量以及元数据的完全同步。

### PDB级高可用 

在主备部署的CDB中，提供了PDB级精细化高可用保障能力：

- 故障隔离：PDB级独立复制和主备切换机制实现真正故障隔离，当某个PDB发生故障时，不影响同一CDB中其他PDB的正常运行，确保CDB环境下各PDB业务的独立性和连续性，避免“一损俱损”风险。

- 精准恢复与资源优化：支持以PDB为单位进行独立故障检测、恢复和主备切换操作，无需对整个CDB进行重启或恢复。既能缩短故障恢复时间，又能避免不必要资源消耗，提高系统整体资源利用效率。

- PDB自治的高可用服务：每个PDB可根据其业务重要性配置独立高可用策略和主备切换阈值。重要PDB可配置更敏感故障检测和更快切换响应，一般PDB可采用相对宽松高可用配置，实现差异化服务保障。

- 灵活灾备策略：支持为不同PDB配置不同灾备策略和恢复目标，既可实现同城双活，也支持异地容灾，满足不同业务场景灾备需求。

- 运维效率提升：管理员可针对特定PDB进行独立高可用维护和测试，不影响其他PDB正常服务，可显著提升CDB环境管理效率和维护灵活性。

YashanDB支持为不同PDB配置差异化的高可用策略：

- 保护模式：每个PDB可独立选择适合的保护级别，满足不同业务场景的可靠性要求。

- 自动选主策略：每个PDB可配置个性化的自动选主策略，实现业务级别的灵活容灾。

- 多样化主备复制方式：支持物理复制和逻辑复制，用户可根据具体业务需求进行独立选择与配置。

### 主备运维管理 

在单机主备部署中，YashanDB为PDB提供了完整的运维管理能力：

- 元数据同步管理：PDB的创建与删除操作仅能在主CDB执行，自动同步至备CDB，确保CDB间元数据一致性。

- 本地化运维：除创建和删除PDB操作之外，主备CDB均可独立执行PDB的日常运维操作，提升运维效率，例如打开关闭PDB。常见的本地化运维操作包括备PDB的构建和主备切换。

#### 备PDB的构建 

当出现备PDB故障等异常情况时，需要重建备PDB。PDB的构建可以由PDB管理员自行操作，也可以通过根容器进行统一管理，具体操作请查阅[备PDB异常修复](./修复异常备库.html#buildpdb)。

#### 主备切换 

为满足不同业务场景需求、提高管理效率简化运维，支持以下主备切换方式：

- CDB级全局切换：通过根容器执行Switchover或Failover，实现整个CDB（根容器和所有PDB）的统一角色转换，最终切换结果为新主根容器上的所有PDB均为主库。适用于大规模维护场景。

  ![](./image/CDBswitch.png)

- PDB级精准切换：通过根容器定向操作指定单个或多个PDB进行独立Switchover或Failover，或直接连接目标PDB执行Switchover或Failover。
