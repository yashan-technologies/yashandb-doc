...
- 一主多备（不含级联备）：yasboot会调整保护模式为最大保护，并开启[自动选主](自动选主配置/一主多备自动选主)。
...
- 3个及以上节点：yasboot会调整保护模式为最大保护，并开启[自动选主](自动选主配置/一主多备自动选主)。
...
执行[ALTER DATABASE SET STANDBY DATABASE TO](../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER DATABASE.html#set_standby_clause)语句可按需切换数据库保护模式。
