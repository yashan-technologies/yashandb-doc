...
YashanDB认可的[通用表达式](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)，timestamp_expr的值必须为DATE、TIMESTAMP、TIME或者字符型数据，当为字符型时，必须确保字符串符合当前TIMESTAMP类型的格式要求。

当timestamp_expr为NULL时，函数返回NULL。

**time_expr**

YashanDB认可的[通用表达式](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)，time_expr的值必须为DATE、TIMESTAMP、TIME或者字符型数据，当为字符型时，必须确保字符串符合当前TIME类型的格式要求。
