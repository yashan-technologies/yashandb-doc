...
REGEXP_COUNT为正则表达式函数，用于在源字符串[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)中按正则模式匹配目标字符串regexp，返回匹配到的次数，返回值为BIGINT类型。若没有匹配到，函数返回0。
...
指定一个[RegExp](../../全部手册/开发手册/SQL参考手册/通用SQL语法/RegExp)，长度不超过512字节。
...
match_para参数指定为可选值以外的其他值时，函数将返回YAS-04363错误。
