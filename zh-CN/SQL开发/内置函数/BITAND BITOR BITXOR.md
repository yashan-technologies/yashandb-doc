...
[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)1、[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)2的值为TINYINT、SMALLINT、INT、BIGINT、NUMBER、TIME类型数据，或可以转换为NUMBER类型的字符型数据（转换失败返回类型转换错误）。对于其他类型，返回类型不支持错误。
