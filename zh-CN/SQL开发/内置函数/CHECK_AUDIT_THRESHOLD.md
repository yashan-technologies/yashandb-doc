...
- 执行本函数所得实时结果可能存在偏差，最精准的结果应以[统计信息](../../全部手册/性能调优/性能调优特性与工具/统计信息)为准。

执行本函数后，系统会根据函数结果自动触发以下操作：

- 返回值 > 给定阈值（[AUDIT_RECORD_THRESHOLD](../../全部手册/参考手册/配置参数.html#auditthreshold)参数），记录1条ACTION为“AUDIT RECORD THRESHOLD”的审计日志。

- 返回值 > 80，在告警日志alert.log中记录“AuditRecordTableFull”并判断[AUDIT_RECORD_TABLE](../../全部手册/参考手册/配置参数.html#auditdest)参数的值：
