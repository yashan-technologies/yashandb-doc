...
ARRAY_LENGTH函数对数组（[数组变量](../../全部手册/开发手册/PL参考手册/PL语言基础/变量/集合变量)，[数组对象](../../全部手册/开发手册/PL参考手册/PL对象/自定义类型)，[数组类型](../../全部手册/开发手册/SQL参考手册/数据类型/用户自定义类型)）array_var计算按dimension指定维度的数组长度（即成员数量），返回一个INT类型的数值。
