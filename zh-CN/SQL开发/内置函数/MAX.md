...
MAX函数计算给定参数[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)的最大值。
...
当指定OVER关键字时，MAX将作为[窗口函数](../../全部手册/开发手册/SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）.html#WindowFunction)，并支持滑动窗口，返回多行的最大值。
