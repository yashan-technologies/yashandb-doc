...
*   对于小数（FLOAT/DOUBLE/NUMBER），与[算术运算符](../../全部手册/开发手册/SQL参考手册/运算符/算术运算符)/算法一致。
*   对于整数，作整除算术运算并返回商数。

在算术运算时，YashanDB通过隐式数据转换，将参与运算的数据类型统一到某个数据类型，并按此数据类型返回运算结果，具体规则请参考[算术运算符](../../全部手册/开发手册/SQL参考手册/运算符/算术运算符)里的数据类型描述。

[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)1、[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)2的值为数值型或可以转换为NUMBER类型的字符型（转换失败返回Invalid number错误）。对于其他类型，函数返回不支持该运算。
