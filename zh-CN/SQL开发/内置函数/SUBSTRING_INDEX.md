...
SUBSTRING_INDEX函数用于在源字符串[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)中提取分隔符delim出现第count次前的子字符串。
...
分隔符，用于分割源字符串expr。delim为与expr相同的通用表达式，须为字符型数据或除JSON、LOB类型外的可转化为字符型的其他类型数据。
