...
LISTAGG函数将多行的[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)执行拼接操作，并通过分隔符分隔，返回一行VARCHAR/RAW类型的字符串。

expr不能为BIT、BOOLEAN、NCHAR、NVARCHAR、NCLOB、时区类型和UDT类型。
...
当指定OVER关键字时，LISTAGG将作为[窗口函数](../../全部手册/开发手册/SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）.html#WindowFunction)，将每个分组窗口内的行进行拼接，分组窗口内每一行的拼接结果都相同。
