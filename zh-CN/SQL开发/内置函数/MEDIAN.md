...
MEDIAN函数计算给定参数[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)的中位数，仅支持expr类型为[数值型](../../全部手册/开发手册/SQL参考手册/数据类型（yashan模式）/数值型)或者[日期时间型](../../全部手册/开发手册/SQL参考手册/数据类型（yashan模式）/日期时间型)，使用其他数据类型会返回错误。
...
expr只支持INT、NUMBER与时间类型。partition by的参数支持除LOB、JSON、UDT外的其它数据类型。
