...
string可以为通用表达式[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)或查询列为单列且返回行为单行的子查询，string的值必须为CHAR、VARCHAR或除JSON、NCLOB和时区类型外可转换为CHAR或VARCHAR的其他类型。
...
当指定OVER关键字时，WM_CONCAT将作为[窗口函数](../../全部手册/开发手册/SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）.html#WindowFunction)，返回多行的数值和，返回类型为CLOB。

- WM_CONCAT作为窗口函数时，string不能为时间类型，NCHAR，NVARCHAR，NCLOB类型。
