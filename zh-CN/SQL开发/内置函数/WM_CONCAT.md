...
string可以为通用表达式[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)或查询列为单列且返回行为单行的子查询，string的值必须为字符型或除JSON时区类型外可转换为字符型的其他类型。
...
当指定OVER关键字时，WM_CONCAT将作为[窗口函数](00内置函数.html#WindowFunction)，返回多行的数值和，返回类型为CLOB。
