...
IFNULL函数有2个[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)参数，当expr1不为NULL时返回expr1，否则返回expr2。

expr1、expr2的值可以为任意数据类型，但当expr1、expr2其中之一为RAW、JSON、CLOB、NCLOB、BLOB、BFILE类型时，不允许与其它类型搭配使用（即expr1和expr2的数据类型必须相同），否则函数返回错误。
...
- 当expr1和expr2的数据类型不相同，函数将先进行隐式类型转换后再返回结果，基本规则如下：
  ...
  - expr1与expr2分属于数值型（除BIT外）、日期时间型、字符型或ROWID不同大类组时：

    - 若不包含NCHAR、NVARCHAR，函数返回值为VARCHAR类型。

    - 若包含NCHAR、NVARCHAR，函数返回值为NVARCHAR类型。

  - expr1与expr2其中之一为布尔型：

    - 另一个属于非数值型的其他大类时，函数返回VARCHAR类型。

    - 另一个属于数值型（除BIT外）时，函数返回对应的数值类型。

    - 另一个为BIT类型时，函数返回BIGINT类型。

  - expr1与expr2其中之一为BIT类型：
