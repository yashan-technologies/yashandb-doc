...
`JSON_SERIALIZE` 函数将二进制[json](../../全部手册/开发手册/SQL参考手册/通用SQL语法/json)数据按照pretty格式或compact格式序列化为字符串，且序列化后的字符串长度不超过 `returning_clause` 定义的限制值。
...
EXTENDED关键字用于设置使用扩展模式输出JSON数据，详见[JSON扩展格式输出](../../全部手册/开发手册/SQL参考手册/通用SQL语法/json)。
