...
DENSE_RANK为[窗口函数](../../全部手册/开发手册/SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）.html#WindowFunction)，用于对数据的实时分析。

本函数不支持向量化计算。

本函数的语法描述及约束与[RANK函数](./RANK)，[ROW_NUMBER函数](./ROW_NUMBER)一致，区别在于排序时对并列值的处理：
