...
JSON函数用于将[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)的值转换为二进制[JSON](../../全部手册/开发手册/SQL参考手册/通用SQL语法/json)数据。当转换后的JSON数据为JSON Object类型时，函数返回按key值进行排序后的结果。
...
EXTENDED关键字用于指定使用扩展模式解析JSON字符串，详情请查阅[JSON扩展格式解析](../../全部手册/开发手册/SQL参考手册/通用SQL语法/json)。
