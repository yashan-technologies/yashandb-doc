...
- 用法一：用于将源字符串[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)中的所有的from_string中的每一个字符一对一替换为to_string中的字符，返回新的字符串。
...
需要被替换的字符串，from_string为与expr相同的通用表达式，须为字符型数据或可转换为字符型的其他类型数据（LOB类型支持隐式转换）。

当from_string的值为NULL时，函数返回NULL。

**to_string**

将要替换成的字符串，按此字符串的值进行一对一替换，to_string为与expr相同的通用表达式，须为字符型数据或可转换为字符型的其他类型数据（LOB类型支持隐式转换）。
