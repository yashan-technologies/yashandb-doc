...
YashanDB认可的[通用表达式](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)，其值须为DATE/TIMESTAMP/时区类型或可转换为TIMESTAMP类型的字符型。
...
YashanDB认可的[通用表达式](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)，其值须为数值型或字符型，对于非整数的数值型数据，函数对其进行向下取整。
