...
LEAD为[窗口函数](../../全部手册/开发手册/SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）.html#WindowFunction)。本函数根据窗口查询返回的一系列行和游标的位置，提供对超出该位置的给定物理偏移量的行的访问，并返回该行记录对应[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)的值。
