...
REGEXP_REPLACE为正则表达式函数，用于在源字符串[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)按正则模式匹配目标字符串regexp，并使用replace字符串替换第occurrence次匹配到的regexp子串，最后返回替换后的完整字符串，返回值为VARCHAR类型。若没有匹配到，函数返回源字符串expr。
