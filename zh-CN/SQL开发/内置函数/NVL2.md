...
expr1、expr2、expr3均为YashanDB认可的[通用表达式](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)。

expr1、expr2、expr3均支持LOB及BFILE类型隐式转换。
...
* expr2和expr3均不为空且二者数据类型不同时，函数会先执行[数据类型转换](../../全部手册/开发手册/SQL参考手册/数据类型（yashan模式）/数据类型转换)并返回转换后的数据类型，若二者数据类型无法进行转换则返回类型转换错误。二者数据类型支持转换时，转换规则如下：
