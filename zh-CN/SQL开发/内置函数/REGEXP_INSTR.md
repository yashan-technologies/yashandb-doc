...
REGEXP_INSTR为正则表达式函数，用于在源字符串[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)按正则模式匹配目标字符串regexp，返回第occurrence次匹配到regexp的第subexpr个子字符串的return_opt（开始或结束）位置，返回值为BIGINT类型。若没有匹配到，函数返回0。
