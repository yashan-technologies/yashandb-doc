...
| l_inst_num | 实例标识符<br/>* 在单机部署中，该参数值为1<br/>* 在共享集群部署中，该参数值取自GV$INSTANCE.INSTANCE_NUMBER<br/>* 在存算一体分布式集群部署中，该参数值取自GV$INSTANCE.GROUP_ID和GROUP_NODE_ID的拼接字符串，输入格式形如'3-1' |
| l_bid      | 起始快照ID                                                   |
| l_eid      | 终止快照ID                                                   |
| l_options  | 保留字段                                                     |
| l_format   | 保留字段                                                     |
