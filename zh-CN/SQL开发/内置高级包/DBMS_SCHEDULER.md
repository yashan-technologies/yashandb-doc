DBMS_SCHEDULER包提供了一组内置的存储过程，用于创建和管理[定时任务](../../全部手册/开发手册/PL参考手册/PL对象/定时任务)。
...
|  参数| 描述|
| :------------------ | :----------------------------------------------------------- |
| job_name            | 定时任务名称，可以为schema.job_name格式，且需符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。 |
