...
YashanDB会在当前用户下自动创建一个自定义库对象，对象的名称由输入的路径按照一定规则生成，区分大小写，需要符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。
...
INSERTING函数用于在INSERT语句中执行的触发器内返回TRUE，该函数无参数，使用请参考[触发器](../../全部手册/开发手册/PL参考手册/PL对象/触发器)中的例程。

## DELETING

DELETING函数用于在DELETE语句中执行的触发器内返回TRUE，该函数无参数，使用请参考[触发器](../../全部手册/开发手册/PL参考手册/PL对象/触发器)中的例程。
...
使用请参考[触发器](../../全部手册/开发手册/PL参考手册/PL对象/触发器)中的例程。
