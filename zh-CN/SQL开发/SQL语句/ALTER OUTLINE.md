...
对OUTLINE重命名。重命名的OUTLINE名称不能带模式名指定，且需符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。

### 5. CHANGE CATEGORY TO category\_name

更改OUTLINE对应的类别。类别名称可以是已经存在的或者新指定的，不能带模式名指定，且需符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。
