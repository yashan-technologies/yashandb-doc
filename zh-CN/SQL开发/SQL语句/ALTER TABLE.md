...
不能对[AC对象](../../全部手册/开发手册/SQL参考手册/基本SQL元素/对象)以及AC对象的源表执行ALTER TABLE操作。
...
虚拟列语法规格说明请参考[virtual_column_definition](CREATE TABLE.html#virtualcoldef)。
...
该语句用于修改表名，指定的新名称不能为空且必须符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。
...
通过[DBA_LOG_GROUPS](../../全部手册/参考手册/系统视图/DBA视图/DBA_LOG_GROUPS)、[ALL_LOG_GROUPS](../../全部手册/参考手册/系统视图/ALL视图/ALL_视图)、[USER_LOG_GROUPS](../../全部手册/参考手册/系统视图/USER视图)视图可以查看当前已开启的附加日志信息。
...
注意当客户端返回成功时并不保证转换一定成功，只代表启动线程成功。后续转换仍有可能失败。转换结果通过[运行日志](../../全部手册/数据库管理/运行监控/日志管理/运行日志管理)记载。
...
该语句用于修改表的parallel属性，parallel属性用于指定查询该表的并行度，语法同CREATE TABLE中的[parallel_clause](CREATE TABLE.html#parallelclause)。
...
- 对于LSC表的新增列字段，该列为LOB型时不能指定缺省值；为其他类型时，缺省值不能定义为[Sequence伪列](../../全部手册/开发手册/SQL参考手册/基本SQL元素/伪列)。
...
该语句用于在增加列字段时同时定义表的行内约束项。关于约束项的详细描述请参考通用SQL语法[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint)。
...
该语句用于对列字段进行重命名，指定的新名称不允许为空且必须符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。
...
修改列字段的数据类型为YashanDB认可的[数据类型](../../全部手册/开发手册/SQL参考手册/数据类型/00数据类型)。
...
- **索引所在列**：
    ...
    - 若为空表（即无数据），允许将其修改为除LOB/JSON/UDT外的数据类型，但修改结果还需满足[索引列的相关规则](CREATE INDEX.html#indexexpr)。
...
- **原数据类型为字符型的其他非空列**：允许修改为不同字符型，但仍需遵顼以下规则：
    ...
    - HEAP表不允许跨存储方式修改数据类型，例如HEAP表不允许将列数据类型从VARCHAR(3200)（采用普通字符串存储）修改为VARCHAR(8004)（会自动转换为LOB型存储），而TAC表允许此操作。存储方式介绍请查阅[字符型](../../全部手册/开发手册/SQL参考手册/数据类型/字符型)。
...
修改列字段的约束项，遵循通用[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint)里的行内约束所定义语法要求。
...
- 不允许对不符合创建主键要求的列字段执行modify PRIMARY KEY，创建主键的要求请参考[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint)描述。
...
- 不允许对不符合创建外键要求的列字段执行modify FOREIGN KEY，创建外键的要求请参考[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint)描述。
...
分区列表值可以指定为通用表达式（[expr](../../全部手册/开发手册/SQL参考手册/通用SQL语法/expr)）。
...
约束项，关于约束项的详细描述请参考通用SQL语法[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint)。
...
启用或停用指定的约束项（含义见[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint)中的ENABLE|DISABLE描述）。
...
启用或停用约束检查（含义见[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint)中的VALIDATE|NOVALIDATE描述）。
...
在启用主键/唯一约束项时，使用本语句可对系统自动复用或创建的索引进行人工指定，详细语法定义和描述见[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint)中的using_index_clause。
...
* START WITH 用于指定序列号生成器的起始值，该值需介于MINVALUE和MAXVALUE之间。  
...
* MINVALUE/NOMINVALUE 用于指定序列号生成器的最小值，对于升序序列号生成器，NOMINVALUE表示为1；对于降序序列号生成器，NOMAXVALUE表示为操作系统的最大负整数数值。  
