...
**[inline_constraint定义](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)**
...
**[out_of_line_constraint定义](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)**
...
指定列字段的数据类型。查看YashanDB的[数据类型描述](../../全部手册/开发手册/SQL参考手册/数据类型（yashan模式）/00数据类型（yashan模式）)。
...
该语句用于在增加列字段时同时定义表的行内约束项。关于约束项的详细描述请参考通用SQL语法[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)。
...
修改列字段的数据类型为YashanDB认可的[数据类型](../../全部手册/开发手册/SQL参考手册/数据类型（yashan模式）/00数据类型（yashan模式）)。
...
- **原数据类型为字符型的其他非空列**：允许修改为不同字符型，但仍需遵顼以下规则：
    ...
    - HEAP表不允许跨存储方式修改数据类型，例如HEAP表不允许将列数据类型从VARCHAR(3200)（采用普通字符串存储）修改为VARCHAR(8004)（会自动转换为LOB型存储），而TAC表允许此操作。存储方式介绍请查阅[字符型](../../全部手册/开发手册/SQL参考手册/数据类型（yashan模式）/字符型)。
...
修改列字段的约束项，遵循通用[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)里的行内约束所定义语法要求。
...
- 不允许对不符合创建主键要求的列字段执行modify PRIMARY KEY，创建主键的要求请参考[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)描述。
...
- 不允许对不符合创建外键要求的列字段执行modify FOREIGN KEY，创建外键的要求请参考[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)描述。
...
约束项，关于约束项的详细描述请参考通用SQL语法[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)。
...
启用或停用指定的约束项（含义见[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)中的ENABLE|DISABLE描述）。
...
启用或停用约束检查（含义见[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)中的VALIDATE|NOVALIDATE描述）。
...
在启用主键/唯一约束项时，使用本语句可对系统自动复用或创建的索引进行人工指定，详细语法定义和描述见[constraint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/constraint（yashan模式）)中的using_index_clause。
