...
CREATE PROFILE用于创建一个profile，profile为系统对用户的一组资源限制的集合。在YashanDB中，profile可用于定义[密码使用策略](../../全部手册/产品安全/身份标识与鉴别/密码认证/密码策略（yashan模式）.html#password_policy)相关、[会话资源](../../全部手册/产品安全/资源控制/会话资源控制)相关以及[连接请求IP](../../全部手册/产品安全/连接管理/IP黑白名单管理)相关限制。
...
定义[密码使用策略](../../全部手册/产品安全/身份标识与鉴别/密码认证/密码策略（yashan模式）.html#password_policy)相关的配置，包括密码策略参数和参数值，同时定义多个参数以空格分隔。
...
参数值支持数学运算，结果满足参数值规则即可。

|  策略参数| 参数说明| 参数值规则|
| --------------------- | --------------------- | --------------------------------------- |
| FAILED_LOGIN_ATTEMPTS | 最大连续错误登录次数 | 取值需为整数，取值范围为(0,2147483647)                               |
| FAILED_LOGIN_INTERVAL | 密码错误时再次尝试需要等待的时间间隔，单位为秒|取值需为整数，取值范围为[1,2147483646]       |
| PASSWORD_LOCK_TIME    | 达到最大连续错误登录次数需要锁定对应用户的时间，单位为天|取值范围为(0,24855]  |
| PASSWORD_LIFE_TIME    | 密码生命周期，单位为天|取值范围为(0,24855]                                 |
| PASSWORD_GRACE_TIME   | 密码过期宽限期，单位为天|取值范围为[0,24855]                                |
| PASSWORD_REUSE_TIME   | 密码不能被复用的时间间隔，单位为天|取值范围为(0,24855]                          |
| PASSWORD_REUSE_MAX    | 密码被复用需要经历的变更次数|取值需为整数，取值范围为(0,2147483647)                       |
