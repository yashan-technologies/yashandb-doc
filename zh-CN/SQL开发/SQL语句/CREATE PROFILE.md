...
CREATE PROFILE用于创建一个profile，profile为系统对用户的一组资源限制的集合。在YashanDB中，profile可用于定义[密码使用策略](../../全部手册/产品安全/身份标识与鉴别/密码认证/密码策略.html#password_policy)相关、[会话资源](../../全部手册/产品安全/资源控制/会话资源控制)相关以及[连接请求IP](../../全部手册/产品安全/连接管理/IP黑白名单管理)相关限制。
...
profile名称，不可省略，且需符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。


在容器数据库中，全局profile名称必须以前缀开头，默认前缀为C##，前缀值可在创建全局角色、全局用户以及全局profile前通过COMMON_USER_PREFIX参数配置。本地profile名称始终不能以C##以及自定义指定的前缀开头。  
...
定义[密码使用策略](../../全部手册/产品安全/身份标识与鉴别/密码认证/密码策略.html#password_policy)相关的配置，包括密码策略参数和参数值，同时定义多个参数以空格分隔。
...
- FAILED_LOGIN_ATTEMPTS、FAILED_LOGIN_INTERVAL与PASSWORD_LOCK_TIME配合使用，可达到“密码错误时等待X秒后可重试，连续重试Y次后用户被锁定Z天无法登录（除非手动[解锁用户](../../全部手册/产品安全/身份标识与鉴别/管理用户.html#alteruser)）”的效果。
...
定义[会话资源](../../全部手册/产品安全/资源控制/会话资源控制)相关的配置，同时定义多个参数以空格分隔。
...
定义[连接请求IP](../../全部手册/产品安全/连接管理/IP黑白名单管理)相关的配置，同时定义多个参数以空格分隔。
