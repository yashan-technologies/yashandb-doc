...
- 共享集群/分布式集群部署中，必须使用CREATE CLUSTER DATABASE创建一个新的多实例数据库，且redo、undo、swap等所有数据文件须指定为[YFS](../../全部手册/数据库管理/存储管理/集群文件系统管理/00集群文件系统管理)路径，否则返回错误。
...
指定要创建的数据库的名称，不可省略，且需符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。
...
- 如需定义种子容器的属性，应在SEED关键字后连续定义完种子容器的属性。尤其是内置表空间，若同时指定种子容器和根容器的某个内置表空间属性（即语句中出现两次同名内置表空间属性子句），SEED关键字后第2次出现的内置表空间属性及其后所有表空间定义都将视作根容器的属性。
...
-- 指定种子容器部分属性和其他通用属性
-- SEED关键字后的第2个SYSTEM及其后的表空间定义均视作根容器的属性
