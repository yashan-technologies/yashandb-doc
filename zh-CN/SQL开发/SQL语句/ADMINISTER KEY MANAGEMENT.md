...
创建钱包与配置[WALLET_ROOT](../../全部手册/参考手册/配置参数.html#wallet)参数不存在绝对的先后顺序约束，但钱包文件的路径要求如下：
...
-- 场景1：不指定keystore_location创建钱包，系统会自动创建子目录/tde
ADMINISTER KEY MANAGEMENT CREATE KEYSTORE IDENTIFIED BY admin123;

-- 场景2：指定keystore_location创建钱包，必须指定至tde层级
...
该语句用于打开`WALLET_ROOT路径/tde`目录下的钱包，只能通过IDENTIFIED BY keystore_password指定密码进行钱包信息认证。
