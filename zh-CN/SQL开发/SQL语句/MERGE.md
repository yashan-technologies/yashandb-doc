...
该语句为YashanDB通用[条件语句](../../全部手册/开发手册/SQL参考手册/通用SQL语法/condition)，用于指定MERGE选择下一步操作的条件，对目标表里的每行记录执行此条件判断，为真（MATCHED）时，将执行merge_update_clause；为假（NOT MATCHED）时，将执行merge_insert_clause。
