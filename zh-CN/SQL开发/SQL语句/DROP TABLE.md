...
在[回收站开启](../../全部手册/数据库管理/基本数据库管理/回收站管理)（配置参数RECYCLEBIN_ENABLED为ON）的场景下执行DROP语句删除HEAP表且未指定PURGE关键字，该表及其索引、约束、触发器、LOB列等数据都将进入回收站（表名称被修改为BIN开头的系统生成字符串）；删除TAC表、LSC表以及外部表则始终表现为彻底删除。
