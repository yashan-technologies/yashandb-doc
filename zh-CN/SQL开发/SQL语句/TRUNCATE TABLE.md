...
TRUNCATE操作将会一次性删除表的所有数据，不能回滚（Rollback），也不能通过[flashback_query_clause](./SELECT)获得操作之前的数据。
...
```ebnf+diagram
syntax::= TRUNCATE TABLE [schema "."] table_name [DROP STORAGE|REUSE STORAGE | AS subquery_clause]
```
