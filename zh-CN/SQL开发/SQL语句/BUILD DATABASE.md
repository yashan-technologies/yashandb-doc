...
BUILD DATABASE用于在主备高可用部署环境中，在备库上创建standby数据库，有关主备高可用架构的详细操作描述请参考[高可用](../../全部手册/高可用/YashanDB高可用概述)。
...
在单机/共享集群/分布式集群部署中，执行BUILD操作时系统会先自动进行环境预检查，检查项目包括待恢复文件路径是否可达、目标地址空余磁盘空间是否充足。检查通过再执行BUILD，检查失败则直接退出。
...
build过程中跳过磁盘检查。默认在build过程中执行数据build检查，检查项包含目标备库文件还原目标路径、还原目标磁盘空间是否足够、还原目标是否存在同名文件等。

### 3. OVERWRITE

build过程中还原目标若存在同名文件即删除该同名文件并创建同名文件至还原目标路径。REDO文件和归档文件使用该参数仍然不可覆盖，若存在同名文件请手动清除。
...
本操作每次只能指定一个备库，且指定的备库必须是某些异常类型（通过V$REPLICATION_STATUS视图查看备库的异常类型）的need repair状态，例如主库归档被清理等，其他的异常只能通过全量BUILD修复，更详细的描述请查看高可用手册[修复异常备库](../../全部手册/高可用/修复异常备库)。
