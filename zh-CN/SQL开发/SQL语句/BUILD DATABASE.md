...
*   主库已OPEN，并处于归档模式。
* 需先确保主库不处于最大保护模式（MAXIMIZE PROTECTION），可查询V$DATABASE视图的PROTECTION_MODE字段确认。如果是MAXIMIZE PROTECTION，必须先[切换](./ALTER DATABASE.html#set_standby_clause)为其他模式再发起BUILD，BUILD完成后可按需切换回最大保护模式。
