...
该语句用于指定创建的目录的名称，不可省略，且需符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。

### 2. path\_name

该语句用于指定创建的目录的地址。支持指定本地文件路径与YFS文件路径（YFS文件路径参见[YFS文件管理](../../全部手册/数据库管理/存储管理/集群文件系统管理/文件管理)）。
