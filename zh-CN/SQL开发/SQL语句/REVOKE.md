...
REVOKE用于对某一用户或角色收回授予其的权限，可收回的内容包括[系统特权SYSTEM PRIVILEGE](../../全部手册/产品安全/数据访问控制/特权与角色管理（yashan模式）/系统特权)、[对象特权OBJECT PRIVILEGE](../../全部手册/产品安全/数据访问控制/特权与角色管理（yashan模式）/对象特权)和[角色ROLE](../../全部手册/产品安全/数据访问控制/特权与角色管理（yashan模式）/角色)。
...
YashanDB支持在单机和共享集群部署中收回授予给角色的角色，在存算一体分布式集群部署中执行此操作则会返回错误。

单机或集群部署时一条REVOKE语句最多可以回收300个权限；存算一体分布式集群部署时如果回收系统权限或对象权限则单条REVOKE语句最多可以回收300个权限，回收角色权限则单条REVOKE语句最多回收8个角色权限。

YashanDB的权限体系管理具体描述请查阅[特权与角色管理](../../全部手册/产品安全/数据访问控制/特权与角色管理（yashan模式）/00特权与角色管理（yashan模式）)。
