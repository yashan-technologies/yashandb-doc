...
存算一体分布式集群部署中，COMMIT等同于COMMIT WORK WRITE WAIT。
...
GTID：从[GV$2PC_PENDING视图](../../全部手册/参考手册/系统视图/动态视图/GV$2PC_PENDING)中获得。该参数对FORCE语句不可省略。
...
在存算一体分布式集群部署中，不论是否使用WRITE子句或采用WRITE子句的任意组合，其效果全等同于COMMIT WORK WRITE WAIT IMMEDIATE。
