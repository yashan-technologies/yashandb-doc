...
CREATE USER用于新建一个数据库用户，YashanDB的用户管理体系请参考[用户管理](../../全部手册/产品安全/身份标识与鉴别/管理用户)。


在容器数据库（配置参数ENABLE_PLUGGABLE_DATABASE=TRUE）中，若连接根容器使用本语句，表示创建全局用户。
...
该语句用于指定创建的用户的名称，不可省略，且需符合YashanDB的[对象命名规范](../../全部手册/开发手册/SQL参考手册/基本SQL元素/标识符)。
...
在容器数据库中，全局用户的名称必须以前缀开头，默认前缀为C##，前缀值可在创建全局角色、全局用户以及全局profile前通过COMMON_USER_PREFIX参数配置。本地用户的名称始终不能以C##以及自定义指定的前缀开头。  
...
*   区分大小写。
*   密码字符串长度不超过64。
*   遵循[密码强度](../../全部手册/产品安全/身份标识与鉴别/密码认证/密码策略.html#password_complexity)和[密码策略](../../全部手册/产品安全/身份标识与鉴别/密码认证/密码策略.html#password_policy)要求。
