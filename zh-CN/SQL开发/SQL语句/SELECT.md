...
在存算一体分布式集群部署中，[全局动态视图](../../全部手册/参考手册/系统视图/动态视图/00动态视图)相关的操作均在各节点本地执行，最后再汇总至CN。
...
该语句用于提出给定的方案到优化器（Optimizer），使其按照此方案生成语句的执行计划，详情请查阅[hint](../../全部手册/开发手册/SQL参考手册/通用SQL语法/hint)。
...
*   CASE语句
*   udt_expr：对于Object UDT列，udt_expr格式为表别名.属性名；对于Varray UDT和Nested Table UDT列，udt_expr格式为表别名.*。详细使用方法和示例见[用户自定义类型](../../全部手册/开发手册/SQL参考手册/数据类型/用户自定义类型)中描述。
...
*   [query_name](#queryname)：表、视图、同义词、分区、子分区、AC等
...
*   [dblink](../../全部手册/开发手册/SQL参考手册/通用SQL语法/dblink/dblink语法说明)：远端表
...
可用于查询Varray UDT或Nested Table UDT数据，UDT数据类型的详细描述请参考[用户自定义类型](../../全部手册/开发手册/SQL参考手册/数据类型/用户自定义类型)。
...
可使用表函数table()进行查询，表函数的具体描述请查阅[内置表函数](../内置函数/00内置函数.html#tableFunc)。
...
该语句用于指定查询的[condition](../../全部手册/开发手册/SQL参考手册/通用SQL语法/condition)。
...
通过CONNECT BY后跟的[condition](../../全部手册/开发手册/SQL参考手册/通用SQL语法/condition)，指定层次关系查询条件，其中必须至少有一个条件用于指定父子关系，该条件通过PRIOR标识符识别。 
...
通过START WITH后跟的[condition](../../全部手册/开发手册/SQL参考手册/通用SQL语法/condition)，指定以满足该条件的数据作为层次关系中根节点，其LEVEL为1。condition中不允许使用rownum。
...
* HAVING后的condition是一个布尔表达式，语法同WHERE子句中的filter_clause，但是它只能包含分组列、聚集函数（可以与select_list中的聚集函数不一致）、字面量和子查询（子查询中的列不需要为分组列）。其中存算一体分布式集群部署中不可以使用子查询。
...
当排序键expr为[子查询](../../全部手册/开发手册/SQL参考手册/基本SQL元素/子查询)时，需遵顼如下规则：
...
通过指定具体的列字段名来锁定列所在的表，而无需将FROM后出现的所有表都锁定。常用于多表连接查询时仅对部分特定的表（即OF column_name指定列对应的表）执行更新。省略时，将锁定查询涉及到的所有表对象对应的记录。
