...
用户登录数据库后，当前Schema默认为登录用户的同名Schema，允许通过执行ALTER SESSION语句手动切换current_schema。如需确认当前会话的Schema（即current_schema的参数值）可以通过[DBMS_UTILITY](../内置高级包/DBMS_UTILITY).OLD_CURRENT_SCHEMA()查询。
