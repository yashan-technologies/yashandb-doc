OPEN Statement为打开[游标](../../全部手册/开发手册/PL参考手册/PL语言基础/变量/游标)的语句，打开游标表示执行游标所绑定（静态绑定或动态绑定）的SQL语句，并获得查询结果集。
...
其中，cursor_variable为游标变量名称；select_statement为该游标要绑定的SQL语句，该SQL语句可以是静态SQL语句，也可以是通过变量承载的动态SQL语句。当使用静态SQL语句时可以使用INTO子句，此时只做语法兼容，没有实际作用。当使用动态SQL语句时可以使用USING子句加入[绑定参数](../../全部手册/开发手册/PL参考手册/参数/绑定参数)。此时语法格式变为：
