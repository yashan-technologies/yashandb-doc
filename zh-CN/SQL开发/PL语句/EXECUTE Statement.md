...
a1,a2...为变量或常量形式的[绑定参数](../../全部手册/开发手册/PL参考手册/参数/绑定参数)。
...
RETURING语法请查阅[UPDATE](../../全部手册/开发手册/SQL参考手册/SQL语句（yashan模式）/UPDATE.html#returningclause)。
...
执行v_sql字符串里包含的SQL语句（只能是UPDATE语句），并将v_sql中returning指定的表达式的结果赋值给变量，`v1,v2...`与UPDATE语句的RETURNING项一一对应。
...
执行v_sql字符串里包含的SQL语句（只能是UPDATE语句，UPDATE语句中不能使用"BULK COLLECT"），并将v_sql中retruning 表达式的结果赋值给集合变量，`v1,v2...`与UPDATE语句的RETURNING 项一一对应。
