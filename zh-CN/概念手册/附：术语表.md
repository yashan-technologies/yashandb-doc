...
控制组：Linux内核提供的一种资源管理机制，将一系列的任务及其子任务进行集合（或划分）到不同的分组内，并以此限制、记录和隔离不同分组的资源使用情况。
...
数据操作语言：在ANSI SQL标准中，将SQL划分为DDL、DML、DCL等类型，其中DML类的SQL语句用于操作数据库中对象包含的数据，即记录。
...
key preserved关系：子查询的结果集能唯一对应上基表的一行，即说明该基表具有kp关系。
...
​NVMe规范名称：NVMe协议中用于唯一标识控制器、子系统或命名空间的标准化命名体系，遵循RFC 4122规范，采用分层命名空间结构。
...
虚拟IP：YashanDB共享集群中，虚拟IP可以在故障实例和正常实例之间迁移，使得实例级故障对用户透明。
