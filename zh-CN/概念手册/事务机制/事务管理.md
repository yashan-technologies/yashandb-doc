...
事务启动时，YashanDB会为其分配运行期间所需的资源，包括事务运行需要的内存、锁区、UNDO等。同时会为其分配一个全局唯一的事务ID，事务ID由事务区编号XEXT、事务在该事务区的编号XNODE以及事务的版本号XSN组成。
...
在分布式系统中，事务参与者涉及多个实例，需要通过分布式事务管理来保证分布式环境下的事务特性。分布式事务管理通过去中心化、全局高精度逻辑时钟同步技术，提供分布式多实例数据实时强一致性能力，避免全局中心事务管理（GTM）的扩展瓶颈问题。

![dstb-tm](./image/dstb-tm.png)

- 事务协调模块（TC，Transaction Coordinator）

  - 提供分布式事务与本地事务的整体状态收集与呈现的能力。

  - 提供异常情况下，残留分布式事务的恢复能力。

- 事务管理模块（TM，Transaction Manager）

  - 提供两阶段提交的能力。

  - 提供正常场景下，分布式事务（commit、rollback、savepoint）的能力。

- 资源管理模块（RM，Resource Manager）
...
- 全局时钟服务模块（GTS，Global Time Service）
