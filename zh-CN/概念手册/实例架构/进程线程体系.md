...
- 数据库写盘线程（DBWR）
   ...
   DBWR线程的主要功能是将脏数据块（Dirty Blocks）从数据库写盘缓存（每个线程的缓存区大小通过DBWR_BUFFER_SIZE参数配置）写回到磁盘上的数据文件。该线程在数据库启动到NOMOUNT阶段时启动，生命周期与数据库实例一致。DBWR数量默认为2个，最多为16个，线程数通过DBWR_COUNT参数配置。具体功能如下：

   - 脏数据块刷新：DBWR线程定期或在特定条件下将脏数据块刷新到磁盘上的数据文件。

   - 数据写回策略：DBWR线程根据一些策略来决定哪些脏数据块需要被写回磁盘。

   - 写回优化：DBWR线程会尽量合并多个数据块（允许合并的数量通过DBWR_FLUSH_NEIGHBORS_COUNT参数配置）的写回操作，以减少磁盘I/O操作的次数，提高写入性能。
...
- MMNL后台线程（MMNL）
   ...
   MMNL后台线程定时的调用ASH采集接口采集ASH数据，定时时间间隔由_ASH_SAMPLE_INTERVAL配置项决定。

- M000后台线程（M000）

   M000线程是MMON线程的从属线程，用于进行ASH内存数据的持久化。
...
- 闪回日志写入线程（RVWR）
   ...
   RVWR线程的主要功能是定期将数据库闪回日志写入到闪回日志文件。该线程在数据库开启全库闪回时首次创建，关闭全库闪回时关闭。若始终保持全库闪回开启状态，则该线程的生命周期与数据库实例一致。
...
内部网络通信相关线程在分布式集群部署和共享集群部署中启动，相关线程有：
...
分布式集群部署采用存储集群和计算集群分离部署的架构，主要进程有：

- 计算集群由多个计算节点（Compute Node，简称CN）组成，每个计算节点由如下进程组成:

    - CN进程

      CN（Compute Node）进程负责对外提供接口，接收用户请求，生成分布式查询计划，并协调多实例完成并行计算。

- 存储集群由多个存储节点（Data Node，简称DN）组成，每个存储节点由如下进程组成:

    - DN进程

      DN（Data Node）进程主要负责存储管理和监控、数据生命周期管理、资源亲和性计算、计算下推的执行等。

### 主要线程

- 并行执行协调线程（WORKER）

  WORKER主要负责接收业务请求，生成分布式执行计划，作为执行协调线程协调本实例以及其他计算实例的并行执行，生命周期与数据库实例一致。
...
- 磁盘监控线程（DISK_MONITOR）
  ...
  DISK_MONITOR线程负责接收磁盘发现和监控，生命周期与DN实例一致。
