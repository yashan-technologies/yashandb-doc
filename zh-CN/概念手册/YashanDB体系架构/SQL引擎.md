...
- 协调实例：任一计算节点（CN）可以接收SQL请求，成为当前SQL执行的协调实例，负责解析SQL并生成分布式执行计划，并向包括自己的所有协调实例分发执行计划进行执行并汇总执行结果。

- 计算实例：所有计算节点CN都可能成为一个SQL执行的计算实例。由协调实例根据执行的SQL和数据分布、数据亲和性以及并行度，选择一组（可能是全部，包括当前协调实例自身）计算实例共同完成当前SQL的执行。

分布式SQL引擎将用户文本形式的SQL语句进行解析、验证、优化、协调实例向计算实例分发执行计划、多个计算实例并行执行，并最终返回查询结果集给用户。
...
- **Distributed Interconnect Network**
    ...
    采用异步网络通讯框架的分布式通信组件，负责节点之间的网络通信，包括执行计划的分发，以及各节点间的数据交换。

- **数据交换机制**

    分布式集群数据库的每一个计算实例均可访问所有的数据，为了充分利用多实例的资源加速海量数据的计算，以数据分区为粒度进行分组，每个分区数据只由一个计算实例负责计算，协调节点结合并行度和数据亲和性进行执行计划的调度。当某一个SQL计算的数据源来自于不同分区时，需要由特定的PX并行执行算子，将数据按指定方式，搬运到指定位置。

    在一个分布式SQL的执行过程中，可能会发生如下几种情形的数据交换：

    - 计算实例的数据向协调实例上汇聚成分布式SQL的查询结果。

    - 某个计算实例上的计算需使用其他计算实例上的数据，需要将其他计算实例上的数据搬运过来。
    
    - 某个stage向同计算实例内的其他stage传递数据。

- **并行执行**

	YashanDB的分布式SQL的并行执行分为两级：

    - **第一级：节点间并行**

        协调实例上的优化器根据并行度和表数据的亲和性，将一个复杂查询分为多个计划片段（stage），发送到不同的计算实例，各计算实例/计划片段（stage）之间并行执行。

    - **第二级：节点内并行**

        协调实例上的优化器切分后产生的计划片段（stage），在计算实例上根据数据分片信息和并行度等将stage划分成多个pipeline任务实例，并由多个工作线程并行执行。
