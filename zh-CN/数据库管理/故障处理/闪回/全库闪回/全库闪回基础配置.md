...
- 在共享集群/分布式集群中，全库闪回相关操作只能在主实例（[GV$INSTANCE](../../../../参考手册/系统视图/动态视图/GV$INSTANCE)视图的INSTANCE_ROLE = MASTER_ROLE）上执行。
...
通过备份整个数据库实现对闪回日志文件的备份，具体操作请查阅[备份与恢复](../../../备份与恢复/00备份与恢复.md)。
