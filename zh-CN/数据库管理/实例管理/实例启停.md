...
*   **NOMOUNT**：启动数据库实例，读取[配置参数文件](../存储管理/数据库文件管理/配置参数文件与密码文件管理.md)，但不加载数据库；本阶段仅允许sys用户登录。

*   **MOUNT**：启动数据库实例，读取[控制文件](../存储管理/数据库文件管理/控制文件管理.md)，加载数据库，但数据库处于关闭状态；本阶段仅允许sys用户登录。
...
主备部署时，还需关注主备角色的DATABASE\_ROLE字段。
