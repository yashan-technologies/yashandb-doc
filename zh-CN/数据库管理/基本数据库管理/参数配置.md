...
>
> - 存算一体分布式集群部署中，推荐使用YCM图形化界面或[yasboot](../../工具手册/yasboot/yasboot命令介绍/yasboot config)命令行方式来进行多节点统一的参数配置。如需使用SQL语句修改配置参数时应依据开发手册[ALTER SYSTEM](../../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER SYSTEM)中type和node所描述内容实现多节点同步修改或仅本节点修改。
>
> - 对于过时参数，配置时具体规则如下：
