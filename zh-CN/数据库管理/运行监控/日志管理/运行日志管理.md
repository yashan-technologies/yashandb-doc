数据库运行日志通过日志级别控制记录开关和记录策略，级别为OFF时不记录运行日志。
...
```sql
ALTER SYSTEM SET RUN_LOG_FILE_COUNT=11 SCOPE=SPFILE;
```
...
WARN级别记录数据库运行过程中出现的警告信息，这些信息表示可能存在潜在问题，但不影响数据库正常运行。
...
```verilog
//当REDO日志文件需要切换复用时，如果对应的数据页未完成刷盘：
...
TRACE级别记录数据库运行过程中的详细跟踪信息，用于深入分析问题原因。该级别会输出非常详细的日志，包括函数调用链、性能数据等。
