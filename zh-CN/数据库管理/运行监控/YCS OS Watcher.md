...
OS Watcher默认会随YCS启动而启动，用户可按需通过YCS配置参数[OSW_AUTO_START](../集群管理/集群参数配置.md#osw_auto_start)配置是否自启动。
...
默认配置下，OS Watcher收集信息的时间间隔为磁盘心跳超时时间DISK_HB_KEEP_ALIVE的三分之一（结果向下取整数）。用户可按需通过YCS配置参数[OSW_INTERVAL](../集群管理/集群参数配置.md#osw_interval)指定时间间隔。
...
OS Watcher支持指定能保存文件数量和单个文件大小，通过YCS配置参数[OSW_FILE_NUM](../集群管理/集群参数配置.md#osw_file_num)和[OSW_FILE_SIZE](../集群管理/集群参数配置.md#osw_file_size)控制。
...
本文以[典型部署](../../安装和升级/安装部署/YashanDB服务端安装（命令行）/共享集群部署.md)为例，每20秒收集一次数据，文件大小的增长速度参考值如下表所示，下表数据仅供参考，具体请以实际为准。
