...
通过SQL语句执行的备份与恢复操作适用于单机/共享集群/分布式集群部署的数据库。
...
1. 检查并开启数据库的归档模式。
      ...
      LOG_MODE为ARCHIVELOG表示归档模式，直接执行后续操作即可。若LOG_MODE为NOARCHIVELOG表示非归档模式，需先将数据库[切换为归档模式](../../实例管理/归档管理.md)。
...
通过SQL语句执行的归档日志备份与恢复操作适用于单机/共享集群/分布式集群部署的数据库。
...
除指定SEQUENCE区间外，也可指定为时间、SCN区间，详情请查阅[BACKUP ARCHIVELOG](../../../开发手册/SQL参考手册/SQL语句/BACKUP ARCHIVELOG)。
