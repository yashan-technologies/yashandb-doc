...
- 当前安装的YashanDB版本为v23.5或以上，若为主备集群部署则需v23.5.2或以上。

- YashanDB采用单机部署、共享集群部署或分布式集群部署。

- 已执行DROP DATABASE语句[删除当前数据库](../../实例管理/数据库删除.md)。
...
3. 若instance_status=close，需[启动数据库](../../实例管理/实例启停.md)至NOMOUNT状态。
...
在单机部署中，CDB启停等同于[实例启停](../../实例管理/实例启停.md)，会同时启停根容器和所有PDB。

在共享集群/分布式集群部署中，CDB启停等同于[集群启停](../../集群管理/集群启停.md)，会同时启停根容器和所有PDB。
...
在后续日常使用中，可以通过[V$CONTAINERS](../../../参考手册/系统视图/动态视图/V$CONTAINERS.md)/[GV$CONTAINERS](../../../参考手册/系统视图/动态视图/GV$CONTAINERS.md)、[V$PDBS](../../../参考手册/系统视图/动态视图/V$PDBS)/[GV$PDBS](../../../参考手册/系统视图/动态视图/GV$PDBS)查看容器信息和状态。
