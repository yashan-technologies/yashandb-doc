...
根目录`+`下一级目录是DiskGroup的虚拟目录，不能通过`mkdir`在此创建目录。仅可以通过创建DiskGroup实现在此创建目录的目的。
...
通过`snapshot`命令可以为任何YFS文件创建快照。快照仅保存文件在某一时刻的状态（存储分配单元列表），不复制实际数据。
...
单个文件只能记录一个快照版本，多次创建仍只会关联最后一次创建的快照。删除文件时，如果该文件存在快照，快照会自动被删除。

<span id="reset" name="reset"></span>

### 借助快照闪回文件

对于存在快照的文件，通过`reset`命令将其快速恢复到快照时的状态。
...
始终只能闪回到最近一次创建的快照，无法指定快照版本。闪回后，原文件与快照文件共享存储分配单元数据。

### 创建克隆文件

YFS具备文件克隆功能，通过执行`cp -C`命令可以为源文件以克隆方式在同一磁盘组内创建一个即时副本。
...
在创建克隆文件时，并不会即刻复制文件数据，而是仅与源文件共享存储分配单元。只有当对源文件或克隆文件执行写入操作时，克隆文件才会进行数据复制，即采用写时复制（COW， Copy-on-Write）机制。

在创建克隆文件时，会自动为其创建一次快照，如需手动创建新版本的快照，请查阅[创建文件快照](#snapshot)。

借助快照可以将克隆文件快速闪回至快照创建时刻的版本，具体操作请查阅[借助快照闪回文件](#reset)。
