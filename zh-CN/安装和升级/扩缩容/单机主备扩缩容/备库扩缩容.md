...
10. 可按需开启自动选主功能，保障业务连续性：
    ...
    - 单机主备部署（非级联备）环境：可按需手动开启[yasom仲裁选主](../../../高可用/自动选主配置/yasom仲裁选主)。

    - 一主多备（非级联备）环境：yasboot会自动开启[自动选主](../../../高可用/自动选主配置/一主多备自动选主.md)。
...
3. 执行如下命令，查看待删除的目标备库ID。

    ```shell
    $ yasboot cluster status -c yashandb -d
    # nodeid中冒号前的部分即为节点ID，例如1-3:3对应的节点ID为1-3
    ```
    
    **不允许**删除存在级联备的备库，如需删除需先参考[级联备扩缩容](./级联备扩缩容.md)删除其所有级联备。
...
7. 删除备库后，可根据数据库高可用配置情况，可按需开启自动选主功能相关配置：

    - 单机主备部署（非级联备）环境：可按需手动开启[yasom仲裁选主](../../../高可用/自动选主配置/yasom仲裁选主)。

    - 一主多备（非级联备）环境：可以开启[自动选主](../../../高可用/自动选主配置/一主多备自动选主.md)。
