...
8.  （可选）若当前环境为主备集群部署且需为备集群扩容实例，还需执行以下操作：

    1. ）在上述生成配置文件的服务器上执行如下命令为备集群生成对应的配置文件：
    ...
    3. ）若存在多个备集群且需要为其扩容实例，重复步骤1.）~2.）即可。
...
> **Caution**:
>
> 在主备集群部署中，删除主集群的某个实例（例如1-1实例）时，会同步删除每个备集群中的相应实例（例如2-1、3-1）。
...
4. 执行如下命令，删除实例。如需显式指定执行该操作的数据库用户，必须指定为sys用户。
...
6. （可选）若缩容时指定了--with-host参数（即移除了空服务器），缩容完成后需要更新hosts.toml文件的[[host]]内容，删除对应服务器相关信息，以免升级时仍用旧的hosts信息。
