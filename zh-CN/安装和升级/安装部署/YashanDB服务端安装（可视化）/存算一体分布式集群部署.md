...
4. 在弹出的对话框中，配置其他服务器信息，单击【确定】保存配置。
   ...
   - 安装路径：数据库安装路径，建议配置为规划的[HOME目录](../安装前准备/创建用户和目录.md#HOMEDirectory)/{版本号}（此处不校验/{版本号}子目录是否存在，安装过程中会自动创建），支持数字、字母（区分大小写）以及部分符号（`/`、`-`、`_`、`.`），例如/data/yashan/yasdb_home/{版本号}。

   - 日志路径：[运行日志目录](../安装前准备/创建用户和目录.md#run_log_path)，例如/data/yashan/log。
...
1. 在弹出的节点规模配置对话框中，根据[实际规划](../安装前准备/服务器准备)的节点数调整相关配置，单击【确定】保存信息。
   ...
   - 起始端口：填写数据库监听端口的起始值，若存在多个监听端口系统会根据[端口划分规则](../安装前准备/网络准备.md#openports)自行计算，默认值为1688。
