本步骤适用于使用共享集群部署、分布式集群部署和分支数据库部署场景，其他场景请略过。

在共享集群部署中，采用分布式NVMe存储设备时，服务器被分为两类：部署实例的服务器（以下简称实例服务器）和挂载NVMe SSD的服务器（以下简称存储服务器）。

在分布式集群部署中，服务器则被分为计算节点服务器（以下简称CN服务器）和数据节点服务器（以下简称DN服务器）两类，其中DN服务器上将挂载NVMe SSD。

在分支数据库部署中，服务器上需配置一个系统盘和一个数据盘，用于支持数据库分支功能。

下表将列出不同子场景下配置存储设备所需的步骤和要求，请严格遵循指导操作（-表示跳过该步骤）：

| 步骤| 共享集群（共享存储）部署| 共享集群（分布式存储）部署| 分布式集群（分布式存储）部署| 分支数据库部署|
| ----------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 磁盘划分          | 按YFS磁盘规划划分LUN                                         | 按YFS磁盘规划划分命名空间                                    | 按YFS磁盘规划划分命名空间                                    | 按YFS磁盘规划划分磁盘                                         |
| 挂载磁盘         | 多路径方式挂载至所有服务器                                   | 挂载至所有存储服务器                                         | 挂载至所有DN服务器                                           | 挂载至服务器                                                 |
| 磁盘映射          | -                                                            | 在所有存储服务器上执行映射                                   | 在所有DN服务器上执行映射                                     | -                                                            |
| 连接磁盘          | -                                                            | 在所有实例服务器上执行连接                                   | 在所有CN服务器上执行连接                                     | -                                                            |
| 绑定磁盘         | 在所有服务器上执行绑定                                       | 在所有实例服务器上执行绑定                                   | 在所有CN服务器上执行绑定                                     | 在服务器上执行绑定                                           |

若为主备集群部署，则仅支持存储设备为集中式共享存储，且需为主集群和备集群分别规划并绑定LUN。

>**Caution**:
>
> 本文仅以示例步骤介绍操作流程，实际使用过程中可能存在细微差异或执行结果不一致，建议参考对应操作系统的官方指引或联系系统服务商解决。
>
> 实际部署环境可能与示例不同，请参考示例调整，请勿将以下配置直接用于生产环境。

<span id="DiskPartitioning" name="DiskPartitioning"></span>

## 磁盘划分

崖山文件系统管理（YFS）以磁盘作为存储管理的逻辑单元，并根据使用目的区别为如下两类：

- **系统盘**：用于集群管理，此类磁盘的绝对路径（由本文后续配置生成）将作为--system-data参数的值，以`,`分隔多个磁盘进行配置，例如`--system-data /dev/yfs/sys0,/dev/yfs/sys1,/dev/yfs/sys2`。
  
   仅支持1、2、3或5块**相同大小**的磁盘，每块磁盘容量至少1G（当系统盘数量为1块且AU Size需配置为32M时，要求系统盘大小至少为2G），规划的系统盘个数会影响系统盘组的[冗余度（Redundancy）](../../../概念手册/集群基础设施/崖山存储管理.md#redundancy)，1、2、3或5块系统盘分别对应的冗余度为EXTERNAL、NORMAL或HIGH。
	
	系统盘配置在**安装完成后无法再调整**，请在此步骤根据需求确定相应配置规格（磁盘数量、磁盘容量）并完成后续相应的操作，如无法确定可咨询我们的技术支持。

- **数据盘**：用于保存集群业务数据，此类磁盘的绝对路径（由本文后续配置生成）将作为--data参数的值，以`,`分隔多个磁盘进行配置，例如`--data /dev/yfs/data0,/dev/yfs/data1,/dev/yfs/data2`。

    支持1个或多个磁盘，详细配置规格请查阅[YFS规格](../../../产品描述/产品规格/物理规格.md#YFS)。若将磁盘组的冗余度规划为Normal或High，同一磁盘组中所有磁盘的可用**容量**、每个故障组下的磁盘**数量**均要求一致，请根据实际业务需求规划其数量和容量。
	
    考虑到数据盘组的冗余度，可按实际需求量的倍数规划数据盘的数量。也可在后期运行过程中再根据情况增加数据盘。

在对共享磁盘阵列或NVMe SSD做磁盘划分时，划分单元的用途和容量应与上述两类磁盘进行对应，例如规划3个2G大小系统盘，则需要在共享磁盘阵列划分3个2G大小的LUN，或者在NVMe SSD上划分3个2G大小的命名空间。本文不对存储设备的磁盘划分提供操作指引，请根据厂商提供的安装指导操作。

## 挂载磁盘

共享集群（共享存储）部署时，需要将所有划分好的磁盘（下文称为LUN）以多路径的方式挂载至集群中的所有服务器。

共享集群（分布式存储）部署时，需要将所有划分好的磁盘（下文称为NVMe-oF盘）挂载至集群中的所有实例服务器。

分布式集群部署时，需要将所有划分好的磁盘（下文称为NVMe-oF盘）挂载至集群中的所有DN服务器。

本文不对存储设备的挂载提供操作指引，请管理员根据厂商提供的安装指导操作。

本文假设存储设备已按下表所示进行划分和挂载：

::: tabs

== 共享集群（共享存储）部署

|  LUN| 磁盘用途| 所在服务器|
|------------|--------|--------------------------------------|
| /dev/mapper/lun03-sys0 | 系统盘 | 172.16.1.2/172.16.1.3 |
| /dev/mapper/lun04-sys1 | 系统盘 | 172.16.1.2/172.16.1.3 |
| /dev/mapper/lun05-sys2 | 系统盘 | 172.16.1.2/172.16.1.3 |
| /dev/mapper/lun01-data0 | 数据盘 | 172.16.1.2/172.16.1.3 |

== 共享集群（分布式存储）部署

| NVMe-oF盘路径| 磁盘用途| 所在服务器|
|--------|--------------------------------------|--------|
| /dev/nvme0n1 | 系统盘 | 172.16.1.4 |
| /dev/nvme0n2 | 系统盘 | 172.16.1.4 |
| /dev/nvme0n1 | 系统盘 | 172.16.1.5 |
| /dev/nvme0n3 | 数据盘 | 172.16.1.5 |

== 分布式集群部署

| NVMe-oF盘路径| 磁盘用途| 所在服务器|
|--------|--------------------------------------|--------|
| /dev/nvme0n1 | 系统盘 | 172.16.1.4 |
| /dev/nvme0n2 | 系统盘 | 172.16.1.4 |
| /dev/nvme0n1 | 系统盘 | 172.16.1.5 |
| /dev/nvme0n3 | 数据盘 | 172.16.1.5 |

== 分支数据库部署

|  LUN| 磁盘用途| 所在服务器|
|------------|--------|--------------------------------------|
| /dev/mapper/lun03-sys0 | 系统盘 | 172.16.1.2 |
| /dev/mapper/lun01-data0 | 数据盘 | 172.16.1.2 |

:::

## 共享集群（共享存储）部署下绑定磁盘

请在共享集群部署中的**所有服务器**上执行下述操作。本标题下所述操作执行结束即完成共享集群（共享存储）部署下的存储设备配置，本文档其他内容请略过。

在将所有的LUN挂载到服务器后，需要按下述操作指导将这些LUN绑定映射到服务器的同一目录下，该目录将作为--disk-found-path参数的值，本文采用默认值/dev/yfs作为示例。  

下表为绑定路径示例：

|  LUN| 磁盘用途| 规划绑定路径（示例值）|
|------------|--------|--------------------------------------|
| /dev/mapper/lun03-sys0 | 系统盘 | /dev/yfs/sys0 |
| /dev/mapper/lun04-sys1 | 系统盘 | /dev/yfs/sys1 |
| /dev/mapper/lun05-sys2 | 系统盘 | /dev/yfs/sys2 |
| /dev/mapper/lun01-data0 | 数据盘 | /dev/yfs/data0 |

绑定目录（/dev/yfs）与磁盘名称（data0、sys0等）均为示例值，请根据实际业务需求调整，但命名时需注意：

- 绑定后磁盘的绝对路径（例如`/dev/yfs/sys0`）长度不得超过31字节。
- 绑定目录和磁盘的绝对路径均将作为`yasboot package ce gen`命令的相应参数值，如有修改，请在执行`yasboot package ce gen`时使用实际值。
- 同一个LUN绑定到同一集群的每台服务器上所使用的绑定目录与磁盘名称须保持一致。

### 首次绑定

本文将介绍通过多路径绑定（推荐）和通过SCSI设备的WWID绑定两种方式，主要差异在于：

|  序号| 步骤| 差异说明|
|------------|--------|--------------------------------------|
| 0 | 前提条件 | 通过SCSI设备的WWID绑定存储设备时，要求目标路径（例如/dev）下各设备的WWID唯一，否则会绑定不成功。 |
| 1 | 步骤1：生成配置文件 | 二者genDevRuleByUUID.sh脚本内容不同，对应着执行脚本生成的配置文件yashan-device-rule.rules内容亦不同。 |
| 2 | 步骤4（可选）：修改多路径配置文件 | 如果通过多路径绑定存储设备且需要使用[基于SCSI持久预留的IO Fencing](../../../数据库管理/集群管理/IO Fencing/基于预留的IO Fencing)功能，必须先在共享集群的每台服务器上修改多路径配置文件。<br /> |

>**Caution**:
>
> - 通过SCSI设备的WWID绑定存储设备不满足高可用需求，不建议用于生产环境。
>
> - 本文仅以CentOS和KylinOS V10为例介绍操作流程，实际使用过程中可能存在细微差异或执行结果不一致，建议参考对应操作系统的官方指引或联系系统服务商解决。
>
>  - 采用其他方法绑定磁盘时，可能会存在重启服务器引起磁盘的权限变化从而导致数据库无法启动的问题（此类问题会被归类为[ERROR][errno=05579]记录在[运行日志run.log](../../../数据库管理/运行监控/日志管理/运行日志管理.md)中）。如遇此类问题，需手动调整磁盘的权限（归属：安装用户、权限：至少0666）。

请选择合适的方式，以root用户在共享集群的所有服务器上执行对应操作绑定LUN。

<span id="first" name="first"></span>

#### 方式一：通过多路径绑定 

##### 步骤1：生成配置文件

1. 以root用户登录服务器。

2. 在root目录下创建名称为`genDevRuleByUUID.sh`的脚本：

   ```shell
   # cd ~
   # vi genDevRuleByUUID.sh
   ```

3. 输入如下内容，并根据部署环境修改`DEVICES`为各设备路径，修改`TARGETS`为各设备绑定名称，名称应与设备路径一一对应，修改`YFS_DISK_DIR`为绑定目录，保存并退出。

   ::: tabs
   == CentOS

```shell
#/bin/bash

# 请修改为各设备路径
DEVICES=("/dev/mapper/lun01-data0" "/dev/mapper/lun03-sys0" "/dev/mapper/lun04-sys1" "/dev/mapper/lun05-sys2")
# 请按DEVICES数组顺序指定这些设备绑定的名称，且建议指定为与业务相关的名称
TARGETS=("data0"                   "sys0"                   "sys1"                   "sys2")
# 指定绑定的目录
YFS_DISK_DIR="yfs"

# 请保持默认，请确保yashan用户、YASDBA用户组已存在
USER="yashan"
GROUP="YASDBA"
UGROUP="$USER:$GROUP"

if [ ${#TARGETS[@]} != ${#DEVICES[@]} ]
then
    echo "DEVICES and TARGETS count does not match"
    exit 1
fi

if [ ${#TARGETS[@]} == 0 ]
then
    echo "no targets found"
    exit 1
fi

curDir=$(cd "$(dirname "$0")";pwd)
rm -f $curDir/yashan-device-rule.rules

count=${#TARGETS[@]}

for ((i = 0; i < $count; i ++))
do
    device=${DEVICES[$i]}
    target_name=${TARGETS[$i]}
    target="${YFS_DISK_DIR}/${target_name}"
    
    str1="$device:`udevadm info --query=all --name=$device | grep DM_UUID`"
    str2=${str1#*=}
    echo "KERNEL==\"dm-*\",ENV{DM_UUID}==\"${str2}\",SYMLINK+=\"$target\",OWNER=\"$USER\",GROUP=\"$GROUP\",MODE=\"0666\",OPTIONS:=\"nowatch\",RUN+=\"/bin/sh -c 'chown -R $UGROUP $target'\""  >> $curDir/yashan-device-rule.rules
done
```

    == KylinOS V10

```shell
#/bin/bash

# 请修改为各设备路径
DEVICES=("/dev/mapper/lun01-data0" "/dev/mapper/lun03-sys0" "/dev/mapper/lun04-sys1" "/dev/mapper/lun05-sys2")
# 请按DEVICES数组顺序指定这些设备绑定的名称，且建议指定为与业务相关的名称
TARGETS=("data0"                   "sys0"                   "sys1"                   "sys2")
# 指定绑定的目录
YFS_DISK_DIR="yfs"

# 请保持默认，请确保yashan用户、YASDBA用户组已存在
USER="yashan"
GROUP="YASDBA"
UGROUP="$USER:$GROUP"

if [ ${#TARGETS[@]} != ${#DEVICES[@]} ]
then
    echo "DEVICES and TARGETS count does not match"
    exit 1
fi

if [ ${#TARGETS[@]} == 0 ]
then
    echo "no targets found"
    exit 1
fi

curDir=$(cd "$(dirname "$0")";pwd)
rm -f $curDir/yashan-device-rule.rules

count=${#TARGETS[@]}

for ((i = 0; i < $count; i ++))
do
    device=${DEVICES[$i]}
    target_name=${TARGETS[$i]}
    target="${YFS_DISK_DIR}/${target_name}"
    
    str1="$device:`udevadm info --query=all --name=$device | grep DM_UUID`"
    str2=${str1#*=}
    echo "KERNEL==\"sd*\",ENV{ID_SERIAL}==\"${str2}\",SYMLINK+=\"$target\",OWNER=\"$USER\",GROUP=\"$GROUP\",MODE=\"0666\",OPTIONS:=\"nowatch\"" >> $curDir/yashan-device-rule.rules
done
```
    :::

4. 执行脚本，生成配置文件：

   ```shell
   # sh ./genDevRuleByUUID.sh
   ```

   脚本执行成功后，会在当前路径生成名为yashan-device-rule.rules的绑定配置文件，内容如下，实际部署时内容可能有差异：

   ::: tabs
   == CentOS

```shell
# cat yashan-device-rule.rules
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a2231f0000e7d9668d7b4a",SYMLINK+="yfs/data0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/data0'"
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a2231f0000e7dd668d7b68",SYMLINK+="yfs/sys0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/sys0'"
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a233b90000e358668d7caf",SYMLINK+="yfs/sys1",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/sys1'"
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a233b90000e35a668d7cbf",SYMLINK+="yfs/sys2",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/sys2'"
```

   == KylinOS V10

```shell
# cat yashan-device-rule.rules
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd50",SYMLINK+="yfs/data0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd58",SYMLINK+="yfs/sys0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd66",SYMLINK+="yfs/sys1",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd78",SYMLINK+="yfs/sys2",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
```
   :::

   SYMLINK为绑定设备的别名，检查是否为预期绑定目标。

##### 步骤2：绑定存储设备

1. 将配置文件移动至/etc/udev/rules.d目录下。

   ```shell
   # cp yashan-device-rule.rules /etc/udev/rules.d/99-yashan-device-rule.rules
   ```

2. 加载udev配置。

   ```shell
   # udevadm control --reload-rules
   ```

3. 绑定存储设备。

   ```shell
   ## 场景1：绑定规则文件中的所有设备
   # udevadm trigger
   
   ## 场景2：绑定规则文件中的某一设备
   # udevadm trigger --sysname-match=sdf
   ```

##### 步骤3：检查绑定结果

1. 执行如下命令，确认所有绑定已生效，用户为`yashan`、组为`YASDBA`：

   ```shell
   # ls -l /dev/yfs/*
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 data0 -> ../dm-5
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys0 -> ../dm-7
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys1 -> ../dm-4
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys2 -> ../dm-6
   ```

    并检查它们指向设备的用户为`yashan`、组为`YASDBA`、权限为`0666`：

    ```shell
    # ls -l /dev/dm-4
    brw-rw-rw- 1 yashan YASDBA 253, 4 Jul 11 14:44 /dev/dm-4

    # 其他设备类似
    ```

2. 在集群所有服务器上执行以下命令，确认各服务器上**所有**绑定的多路径WWID一致。

   ```shell
   # 对比各服务器上的输出结果是否对应一致
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/data
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys0
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys1
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys2
   ```
   若两台服务器上同一路经所对应的WWID不一致，说明其挂载的磁阵不是同一个LUN，需联系网络管理员处理。

<span id="modify_multipath_conf" name="modify_multipath_conf"></span>

##### 步骤4（可选）：修改多路径配置文件

通过多路径绑定存储设备时，在使用[基于SCSI持久预留的IO Fencing](../../../数据库管理/集群管理/IO Fencing/基于预留的IO Fencing)功能前，必须在共享集群的每台服务器上完成本操作。

1. 确认已安装mpathpersist工具。

   ```shell
   # mpathpersist -h
   ```

2. 修改每个服务器的/etc/multipath.conf文件，添加"reservation_key file"一项到“defaults”部分。

   示例非完整文件内容，实际修改时仅按说明添加选项，请勿修改其他配置项。

   ```shell
   defaults {
       reservation_key file
   }
   ```

#### 方式二：通过SCSI设备的WWID绑定 

##### 步骤1：生成配置文件

1. 以root用户登录服务器。

2. 在root目录下创建名称为`genDevRuleByUUID.sh`的脚本：

   ```shell
   # cd ~
   # vi genDevRuleByUUID.sh
   ```

3. 输入如下内容，并根据部署环境修改`DEVICES`为各设备路径，修改`TARGETS`为各设备绑定名称，名称应与设备路径一一对应，修改`YFS_DISK_DIR`为绑定目录，保存并退出。

   ```shell
   #/bin/bash

   # 请修改为各设备路径
   DEVICES=("/dev/sdc" "/dev/sdd" "/dev/sde" "/dev/sdf")
   # 请按DEVICES数组顺序指定这些设备绑定的名称，且建议指定为与业务相关的名称
   TARGETS=("data0"     "sys0"    "sys1"     "sys2")
   # 指定绑定的目录
   YFS_DISK_DIR="yfs"

    # 请保持默认，请确保yashan用户、YASDBA用户组已存在
    USER="yashan"
    GROUP="YASDBA"
    UGROUP="$USER:$GROUP"

   if [ ${#TARGETS[@]} != ${#DEVICES[@]} ]
   then
       echo "DEVICES and TARGETS count does not match"
       exit 1
   fi

   if [ ${#TARGETS[@]} == 0 ]
   then
       echo "no targets found"
       exit 1
   fi

   curDir=$(cd "$(dirname "$0")";pwd)
   rm -f $curDir/yashan-device-rule.rules

   count=${#TARGETS[@]}

   for ((i = 0; i < $count; i ++))
   do
       device=${DEVICES[$i]}
       target_name=${TARGETS[$i]}
       target="${YFS_DISK_DIR}/${target_name}"
       
       echo "KERNEL==\"sd*\", SUBSYSTEM==\"block\", PROGRAM==\"/usr/lib/udev/scsi_id -g -u -d /dev/\$parent\", RESULT==\"`/usr/lib/udev/scsi_id -g -u -d $device`\",SYMLINK+=\"$target\",OWNER=\"$USER\", GROUP=\"$GROUP\", MODE=\"0666\", OPTIONS:=\"nowatch\", RUN+=\"/bin/sh -c 'chown -R $UGROUP /dev/$target'\"" >> $curDir/yashan-device-rule.rules
   done
   ```

4. 执行脚本，生成配置文件：

   ```shell
   # sh ./genDevRuleByUUID.sh
   ```

   脚本执行成功后，会在当前路径生成名为yashan-device-rule.rules的绑定配置文件，内容如下，实际部署时内容可能有差异：

    

    ```shell
    # cat yashan-device-rule.rules
    KERNEL=="sd*", SUBSYSTEM=="block", PROGRAM=="/usr/lib/udev/scsi_id -g -u -d /dev/$parent", RESULT=="36a01c8d100cd2c1b777f88dd000002b4",SYMLINK+="yfs/data0",OWNER="yashan", GROUP="YASDBA", MODE="0666", OPTIONS:="nowatch", RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/data0'"
    KERNEL=="sd*", SUBSYSTEM=="block", PROGRAM=="/usr/lib/udev/scsi_id -g -u -d /dev/$parent", RESULT=="36a01c8d100cd2c1b777f88be000002b3",SYMLINK+="yfs/sys0",OWNER="yashan", GROUP="YASDBA", MODE="0666", OPTIONS:="nowatch", RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/data0'"
    KERNEL=="sd*", SUBSYSTEM=="block", PROGRAM=="/usr/lib/udev/scsi_id -g -u -d /dev/$parent", RESULT=="36a01c8d100cd2c1b777f88be000002b5",SYMLINK+="yfs/sys1",OWNER="yashan", GROUP="YASDBA", MODE="0666", OPTIONS:="nowatch", RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/data0'"
    KERNEL=="sd*", SUBSYSTEM=="block", PROGRAM=="/usr/lib/udev/scsi_id -g -u -d /dev/$parent", RESULT=="36a01c8d100cd2c1b777f88be000002b6",SYMLINK+="yfs/sys2",OWNER="yashan", GROUP="YASDBA", MODE="0666", OPTIONS:="nowatch", RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/data0'"
    ```
<span id="rebinding" name="rebinding"></span>

##### 步骤2：绑定存储设备

1. 将配置文件移动至/etc/udev/rules.d目录下。

   ```shell
   # cp yashan-device-rule.rules /etc/udev/rules.d/99-yashan-device-rule.rules
   ```

2. 加载udev配置。

   ```shell
   # udevadm control --reload-rules
   ```

3. 绑定存储设备。

   ```shell
   ## 场景1：绑定规则文件中的所有设备
   # udevadm trigger
   
   ## 场景2：绑定规则文件中的某一设备
   # udevadm trigger --sysname-match=sdf
   ```

##### 步骤3：检查绑定结果

1. 执行如下命令，确认所有绑定已生效，用户为`yashan`、组为`YASDBA`：

    ```shell
    # ls -l /dev/yfs/*
    lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 data0 -> ../dm-5
    lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys0 -> ../dm-7
    lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys1 -> ../dm-4
    lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys2 -> ../dm-6
    ```

    并检查它们指向设备的用户为`yashan`、组为`YASDBA`、权限为`0666`：

    ```shell
    # ls -l /dev/dm-4
    brw-rw-rw- 1 yashan YASDBA 253, 4 Jul 11 14:44 /dev/dm-4

    # 其他设备类似
    ```

2. 在集群所有服务器上执行以下命令，确认各服务器上**所有**绑定的多路径WWID一致。

   ```shell
   # 对比各服务器上的输出结果是否对应一致
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/data
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys0
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys1
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys2
   ```
   若两台服务器上同一路经所对应的WWID不一致，说明其挂载的磁阵不是同一个LUN，需联系网络管理员处理。

### 重新绑定

若在数据库运行过程中，需要重新绑定存储设备，可参考以下步骤进行操作：

1. 以root用户登录目标服务器。

2. 查看当前的配置规则（即yashan-device-rule.rules文件），确保该文件存在且包含了正确的匹配条件和操作。

   ```shell
   # cat /etc/udev/rules.d/yashan-device-rule.rules
   ```

   - 如果规则文件没有问题，直接执行后续操作。

   - 如果规则文件不正确，可参考[首次绑定](#first)中的“步骤1：生成配置文件”重新生成该文件，再执行后续操作。

3. 重新加载udev配置。

   ```shell
   # udevadm control --reload-rules
   ```

4. 重新绑定存储设备。

   ```shell
   ## 场景1：重新绑定规则文件中的所有设备
   # udevadm trigger

   ## 场景2：重新绑定规则文件中的某一设备
   # udevadm trigger --sysname-match=sdf
   ```

5. 检查设备状态。

   ```shell
   # udevadm info --query=all --name=/dev/sdf
   ```

## 共享集群（分布式存储）/分布式集群部署下磁盘映射

共享集群（分布式存储）部署时，需要以root用户或具备sudo权限的用户登录所有存储服务器，对服务器上已挂载的所有NVMe-oF盘进行映射。

分布式集群部署时，需要以root用户或具备sudo权限的用户登录所有DN服务器，对服务器上已挂载的所有NVMe-oF盘进行映射。

假设NVMe SSD已按如下所示划分磁盘并挂载至存储服务器/DN服务器：

| NVMe-oF盘路径| 磁盘用途| 所在服务器| 规划命名空间路径（示例值）  |
|--------|-----------------|---------------------|--------|
| /dev/nvme0n1 | 系统盘 | 172.16.1.4 | /dev/nvme1n1  |
| /dev/nvme0n2 | 系统盘 | 172.16.1.4 | /dev/nvme2n1  |
| /dev/nvme0n1 | 系统盘 | 172.16.1.5 | /dev/nvme3n1  |
| /dev/nvme0n3 | 数据盘 | 172.16.1.5 | /dev/nvme4n1  |

### 手动执行 

手动映射NVMe-oF盘需要安装nvme-cli或nvmetcli工具，本文以使用nvmetcli工具为例。  

1. 加载NVMe相关模块和驱动。

   ::: tabs
   == 使用RDMA协议

```shell
# modprobe nvme_core
# modprobe nvmet
# modprobe nvmet_rdma
# modprobe nvme_fabrics
# lsmod | grep nvme
```
   == 使用TCP协议

```shell
# modprobe nvme_core
# modprobe nvmet
# modprobe nvmet_tcp
```
   :::

2. 安装NVMe管理工具nvmetcli，若已安装无需重复操作。

   ```shell
   # yum install -y nvmetcli
   ```

3. 启动nvmetcli配置目标端的子系统和NVMe端口。

   ```shell
   # nvmetcli
   ```

4. 进入subsystems目录，创建NVMe-oF子系统。

   > **Caution**:
   >
   > 子系统的名称必须为NVMe规范名称（NQN，NVMe Qualified Name），建议省略名称由系统自动生成。

   ```shell
   > cd /subsystems
   > create
   ```

   创建完成后，可以通过`ls`命令查看子系统信息，请记录新建的子系统的名称便于后续操作使用。  

   ```shell
   > ls
   /subsystems
   └──nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742
   ```

5. 允许所有服务器访问上一步新建的子系统（子系统名称仅为示例）。

   ```shell
   > cd nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742
   > set attr allow_any_host=1
   ```

6. 在新子系统下添加命名空间，命名空间ID必须为正整数。

   ```shell
   > cd namespaces/
   > create nsid=100
   ```

7. 设置新命名空间对应的设备路径，并将其启用。

   ```shell
   > cd 100
   > set device path=/dev/nvme0n1
   > enable
   ```

8. 给新子系统添加子系统端口并配置端口的IP协议版本`adrfam`、监听地址`traddr`、传输类型`trtype`以及传输端口`trsvcid`。

   其中，子系统端口ID必须为正整数，监听地址必须为[组网规划](./网络准备.md)子网中的IP地址（例如上表中的172.16.1.4）。传输端口`trsvcid`必须为按[端口规划](./网络准备.md#openports)中开放的端口号。

   ::: tabs
   == 使用RDMA协议

```shell
> cd /ports
> create portid=100
> cd 100/
> set addr adrfam=ipv4
> set addr traddr=172.16.1.4
> set addr trtype=rdma
> set addr trsvcid=1701
```
   == 使用TCP协议

```shell
> cd /ports
> create portid=100
> cd 100/
> set addr adrfam=ipv4
> set addr traddr=172.16.1.4   
> set addr trtype=tcp
> set addr trsvcid=1701
```
   :::

9. 关联NVMe-oF子系统和子系统端口，其中NVMe-oF子系统名称需为上面步骤中创建的子系统实际名称。

   ```shell
   > cd subsystems
   > create nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742
   ```

10. 重复上述步骤4 - 9，映射其他NVMe-oF盘，建议为每个NVMe-oF盘创建一个独立的NVMe-oF子系统，命名空间ID唯一，便于后续识别。

   如希望取消已关联和已建立的子系统，参照以下命令执行：

   ```shell
   > cd /ports
   > delete 100
   > cd subsystems
   > delete nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742
   ```

11. 重复上述所有步骤，在所有存储服务器（共享集群部署时）或DN服务器（分布式集群部署时）上完成本机器上的NVMe-oF盘映射。

### 通过脚本操作 

YashanDB安装包中会提供用于映射NVMe-oF盘的脚本nvmfAdd.sh，可以登录安装服务器（本文以172.16.1.2为例）获取该脚本至本服务器，通过脚本快速完成NVMe-oF盘映射操作。

nvmfAdd.sh脚本提供如下参数：

| 参数 | 是否必填 | 含义|
| ---------- | ------------ | ------------------------------------------------------------ |
| -h         | 否           | 查看帮助。 |
| -d         | 是           | 指定要添加的NVMe SSD硬盘路径。                                 |
| -i         | 是           | 指定本机的NVMe-oF所使用的IP地址。                              |
| -p         | 是           | 指定本机的NVMe-oF所使用的端口号。                              |
| -t         | 否           | 指定NVMe-oF的传输类型，可选为TCP或RDMA。默认情况下，如果操作系统内核版本高于5.0且加载了nvmet_tcp模块则为TCP，否则为RDMA。 |
| -nqn       | 否           | 指定待创建的NVMe-oF子系统名称，必须为NVMe规范名称。建议省略该参数，由脚本自动生成UUID4字符串再与前缀`nqn.2014-08.org.nvmexpress:NVMf:uuid:`拼接组成子系统名称。 |
| -nsid      | 否           | 指定待添加的命名空间ID，必须为正整数，省略时默认值为1。            |
| -nvmf-port | 否           | 指定待添加的子系统端口号，省略时默认为已使用的最大端口号+1，若无已使用的最大端口号则默认取1。 |

1. 登录安装服务器，获取nvmfAdd.sh脚本，上传到本服务器。

2. 加载NVMe相关模块和驱动。

   ::: tabs
   == 使用RDMA协议

```shell
# modprobe nvme_core
# modprobe nvmet
# modprobe nvmet_rdma
# modprobe nvme_fabrics
# lsmod | grep nvme
```
   == 使用TCP协议

```shell
# modprobe nvme_core
# modprobe nvmet
# modprobe nvmet_tcp
```
   :::

3. 进入nvmfAdd.sh脚本所在目录，检查脚本的执行权限，执行脚本映射NVMe-oF盘。

   其中，子系统端口ID必须为正整数，监听地址必须为[组网规划](./网络准备.md)子网中的IP地址（例如上表中的172.16.1.4）。传输端口必须为按[端口规划](./网络准备.md#openports)中开放的端口号。

   ::: tabs
   == 使用RDMA协议

```shell
# ls -l nvmfAdd.sh
# chmod +x nvmfAdd.sh
# nvmfAdd.sh -d /dev/nvme0n1 -i 172.16.1.4 -p 1701 -t rdma -nsid 100 -nvmf-port 100
# nvmfAdd.sh -d /dev/nvme0n2 -i 172.16.1.4 -p 1702 -t rdma -nsid 101 -nvmf-port 101   
```
   == 使用TCP协议

```shell
# ls -l nvmfAdd.sh
# chmod +x nvmfAdd.sh
# nvmfAdd.sh -d /dev/nvme0n1 -i 172.16.1.4 -p 1701 -t tcp -nsid 100 -nvmf-port 100
# nvmfAdd.sh -d /dev/nvme0n2 -i 172.16.1.4 -p 1702 -t tcp -nsid 101 -nvmf-port 101   
```
   :::

4. 重复上述所有步骤，在所有存储服务器（共享集群部署时）或DN服务器（分布式集群部署时）上完成本机器上的NVMe-oF盘映射。

	如希望取消已关联和已建立的子系统，参照以下命令执行：
	
   ```shell
   > cd /ports
   > delete 100
   > cd subsystems
   > delete nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742
   ```

## 共享集群（分布式存储）/分布式集群部署下连接磁盘

共享集群（分布式存储）部署时，需要以root用户或具备sudo权限的用户登录每一台实例服务器，对已完成磁盘映射的所有NVMe-oF盘（包括所有存储服务器上的已映射NVMe-oF盘）进行连接设置。

分布式集群部署时，要以root用户或具备sudo权限的用户登录每一台CN服务器，对已完成磁盘映射的所有NVMe-oF盘（包括所有DN服务器上的已映射NVMe-oF盘）进行连接设置。

1. 加载NVMe相关模块和驱动。
   
   ::: tabs
   == 使用RDMA协议

```shell
# modprobe nvme_core
# modprobe nvmet
# modprobe nvmet_rdma
# modprobe nvme_fabrics
# lsmod | grep nvme
```
   == 使用TCP协议

```shell
# modprobe nvme_core
# modprobe nvmet
# modprobe nvmet_tcp
```
   :::

2. 发现NVMe-oF盘。

   ::: tabs
   == 使用RDMA协议

```shell
# nvme discover -t rdma -q nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742 -a 172.16.1.4 -s 1701
# nvme discover -t rdma -q nqn.2014-08.org.nvmexpress:NVMf:uuid:ca8a0467-4b69-4eb2-9f44-6a1928e68065 -a 172.16.1.4 -s 1702
# nvme discover -t rdma -q nqn.2014-08.org.nvmexpress:NVMf:uuid:d5a8594a-f7ab-4b26-acd3-c440eebdd1b0 -a 172.16.1.5 -s 1703
# nvme discover -t rdma -q nqn.2014-08.org.nvmexpress:NVMf:uuid:210e4d0c-025c-4c19-9cbf-47882b669d5d -a 172.16.1.5 -s 1700
```
   == 使用TCP协议

```shell
# nvme discover -t tcp -q nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742 -a 172.16.1.4 -s 1701
# nvme discover -t tcp -q nqn.2014-08.org.nvmexpress:NVMf:uuid:ca8a0467-4b69-4eb2-9f44-6a1928e68065 -a 172.16.1.4 -s 1702
# nvme discover -t tcp -q nqn.2014-08.org.nvmexpress:NVMf:uuid:d5a8594a-f7ab-4b26-acd3-c440eebdd1b0 -a 172.16.1.5 -s 1703
# nvme discover -t tcp -q nqn.2014-08.org.nvmexpress:NVMf:uuid:210e4d0c-025c-4c19-9cbf-47882b669d5d -a 172.16.1.5 -s 1700
```
   :::

   
   - -t：指定为磁盘映射中对应的传输类型。
   
   - -q：指定为磁盘映射中对应的NVMe-oF子系统名称NQN。
   
   - -a：指定为磁盘映射中对应的监听地址。
   
   - -s：指定为磁盘映射中对应的传输端口。

3. 连接已发现的NVMe-oF盘。

    连接参数可执行nvme connect --help命令获取，本文示例语句如下，未指定的其他参数（例如-i、-Q等）将采用默认值。
   
   ::: tabs
   == 使用RDMA协议

```shell
## 示例命令
# nvme connect -t rdma -n nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742 -a 172.16.1.4 -w 172.16.1.4 -s 1701
# nvme connect -t rdma -n nqn.2014-08.org.nvmexpress:NVMf:uuid:ca8a0467-4b69-4eb2-9f44-6a1928e68065 -a 172.16.1.4 -w 172.16.1.4 -s 1702
# nvme connect -t rdma -n nqn.2014-08.org.nvmexpress:NVMf:uuid:d5a8594a-f7ab-4b26-acd3-c440eebdd1b0 -a 172.16.1.5 -w 172.16.1.4 -s 1703
# nvme connect -t rdma -n nqn.2014-08.org.nvmexpress:NVMf:uuid:210e4d0c-025c-4c19-9cbf-47882b669d5d -a 172.16.1.5 -w 172.16.1.4 -s 1700
```
   == 使用TCP协议

```shell
## 示例命令
# nvme connect -t tcp -n nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742 -a 172.16.1.4 -w 172.16.1.4 -s 1701
# nvme connect -t tcp -n nqn.2014-08.org.nvmexpress:NVMf:uuid:ca8a0467-4b69-4eb2-9f44-6a1928e68065 -a 172.16.1.4 -w 172.16.1.4 -s 1702
# nvme connect -t tcp -n nqn.2014-08.org.nvmexpress:NVMf:uuid:d5a8594a-f7ab-4b26-acd3-c440eebdd1b0 -a 172.16.1.5 -w 172.16.1.4 -s 1703
# nvme connect -t tcp -n nqn.2014-08.org.nvmexpress:NVMf:uuid:210e4d0c-025c-4c19-9cbf-47882b669d5d -a 172.16.1.5 -w 172.16.1.4 -s 1700
```
   :::
   
   - -t：指定为磁盘映射中对应的传输类型。
   
   - -n：指定为磁盘映射中对应的NVMe-oF子系统名称NQN。
   
   - -a：指定为磁盘映射中对应的监听地址。
   
   - -s：指定为磁盘映射中对应的传输端口。
      
   - -w：指定为本机IP地址。

4. 检查NVMe-oF盘信息。

	其中，Node名称由nvme自动生成，并作为本机对已连接的NVMe-oF盘的名称标识。

   ```shell
   # nvme list
   Node             SN                   Model                                    Namespace Usage                      Format           FW Rev
   ---------------- -------------------- ---------------------------------------- --------- -------------------------- ---------------- --------
   /dev/nvme1n1     a4dbcda30ff2204b     Linux                                    100         0.00  GB /   2.00  GB    512   B +  0 B   3.10.0-1
   /dev/nvme2n1     7f20f5ab5e40f0b7     Linux                                    101         0.00  GB /   2.00  GB    512   B +  0 B   3.10.0-1
   /dev/nvme3n1     0455bf6ba394c03a     Linux                                    101         0.00  GB /   2.00  GB    512   B +  0 B   3.10.0-1
   /dev/nvme4n1     8bba31ba2b54b137     Linux                                    100         0.00  GB /   1.02  TB    512   B +  0 B   3.10.0-1
   ```

5. 为安装用户（yashan）授予所有NVMe-oF盘的读写权限。

    ```shell
    # chown -R yashan:YASDBA /dev/nvme1n1 && chmod 666 /dev/nvme1n1
    # chown -R yashan:YASDBA /dev/nvme2n1 && chmod 666 /dev/nvme2n1
    # chown -R yashan:YASDBA /dev/nvme3n1 && chmod 666 /dev/nvme3n1
    # chown -R yashan:YASDBA /dev/nvme4n1 && chmod 666 /dev/nvme4n1
    ```

	如希望断开已成功连接的NVMe-oF盘，请参照如下命令执行：
	
   ```shell
   # nvme disconnect -n nqn.2014-08.org.nvmexpress:NVMf:uuid:5834f6e0-cb79-40ac-a7cc-f02524340742
   ```

## 共享集群（分布式存储）/分布式集群部署下绑定磁盘

共享集群（分布式存储）部署时，需要以root用户或具备sudo权限的用户登录每一台实例服务器，对已成功连接的所有NVMe-oF盘进行绑定。

分布式集群部署时，要以root用户或具备sudo权限的用户登录每一台CN服务器，对已成功连接的所有NVMe-oF盘进行绑定。

所有NVMe-oF盘应被绑定到服务器的同一目录下，该目录将作为--disk-found-path参数的值，本文采用默认值/dev/yfs作为示例。  

下表为绑定路径示例：

|  NVMe-oF盘| 磁盘用途| 规划绑定路径（示例值）|
|------------|--------------------------------------|--------|
| /dev/nvme1n1 | 系统盘 | /dev/yfs/sys0 |
| /dev/nvme2n1 | 系统盘 | /dev/yfs/sys1 |
| /dev/nvme3n1 | 系统盘 | /dev/yfs/sys2 |
| /dev/nvme4n1 | 数据盘 | /dev/yfs/data0 |

绑定目录（/dev/yfs）与磁盘名称（data0、sys0等）均为示例值，请根据实际业务需求调整，但命名时需注意：

- 绑定后磁盘的绝对路径（例如`/dev/yfs/sys0`）长度不得超过31字节。
- 绑定目录和磁盘的绝对路径均将作为`yasboot package ce gen`命令的相应参数值，如有修改，请在执行`yasboot package ce gen`时使用实际值。
- 同一个LUN绑定到同一集群的每台服务器上所使用的绑定目录与磁盘名称须保持一致。

### 步骤1：生成配置文件

1. 以root用户登录服务器。

2. 在root目录下创建名称为`genDevRuleByUUID.sh`的脚本：

   ```shell
   # cd ~
   # vi genDevRuleByUUID.sh
   ```

3. 输入如下内容，并根据部署环境修改`DEVICES`为各设备路径，修改`TARGETS`为各设备绑定名称，名称应与设备路径一一对应，修改`YFS_DISK_DIR`为绑定目录，保存并退出。

   ::: tabs
   == CentOS

```shell
#/bin/bash

# 请修改为各设备路径
DEVICES=("/dev/nvme4n1" "/dev/nvme1n1" "/dev/nvme2n1" "/dev/nvme3n1")
# 请按DEVICES数组顺序指定这些设备绑定的名称，且建议指定为与业务相关的名称
TARGETS=("data0"                   "sys0"                   "sys1"                   "sys2")
# 指定绑定的目录
YFS_DISK_DIR="yfs"

# 请保持默认，请确保yashan用户、YASDBA用户组已存在
USER="yashan"
GROUP="YASDBA"
UGROUP="$USER:$GROUP"

if [ ${#TARGETS[@]} != ${#DEVICES[@]} ]
then
    echo "DEVICES and TARGETS count does not match"
    exit 1
fi

if [ ${#TARGETS[@]} == 0 ]
then
    echo "no targets found"
    exit 1
fi

curDir=$(cd "$(dirname "$0")";pwd)
rm -f $curDir/yashan-device-rule.rules

count=${#TARGETS[@]}

for ((i = 0; i < $count; i ++))
do
    device=${DEVICES[$i]}
    target_name=${TARGETS[$i]}
    target="${YFS_DISK_DIR}/${target_name}"
    
    str1="$device:`udevadm info --query=all --name=$device | grep DM_UUID`"
    str2=${str1#*=}
    echo "KERNEL==\"dm-*\",ENV{DM_UUID}==\"${str2}\",SYMLINK+=\"$target\",OWNER=\"$USER\",GROUP=\"$GROUP\",MODE=\"0666\",OPTIONS:=\"nowatch\",RUN+=\"/bin/sh -c 'chown -R $UGROUP $target'\""  >> $curDir/yashan-device-rule.rules
done
```

    == KylinOS V10

```shell
#/bin/bash

# 请修改为各设备路径
DEVICES=("/dev/nvme4n1" "/dev/nvme1n1" "/dev/nvme2n1" "/dev/nvme3n1")
# 请按DEVICES数组顺序指定这些设备绑定的名称，且建议指定为与业务相关的名称
TARGETS=("data0"                   "sys0"                   "sys1"                   "sys2")
# 指定绑定的目录
YFS_DISK_DIR="yfs"

# 请保持默认，请确保yashan用户、YASDBA用户组已存在
USER="yashan"
GROUP="YASDBA"
UGROUP="$USER:$GROUP"

if [ ${#TARGETS[@]} != ${#DEVICES[@]} ]
then
    echo "DEVICES and TARGETS count does not match"
    exit 1
fi

if [ ${#TARGETS[@]} == 0 ]
then
    echo "no targets found"
    exit 1
fi

curDir=$(cd "$(dirname "$0")";pwd)
rm -f $curDir/yashan-device-rule.rules

count=${#TARGETS[@]}

for ((i = 0; i < $count; i ++))
do
    device=${DEVICES[$i]}
    target_name=${TARGETS[$i]}
    target="${YFS_DISK_DIR}/${target_name}"
    
    str1="$device:`udevadm info --query=all --name=$device | grep DM_UUID`"
    str2=${str1#*=}
    echo "KERNEL==\"sd*\",ENV{ID_SERIAL}==\"${str2}\",SYMLINK+=\"$target\",OWNER=\"$USER\",GROUP=\"$GROUP\",MODE=\"0666\",OPTIONS:=\"nowatch\"" >> $curDir/yashan-device-rule.rules
done
```
    :::

4. 执行脚本，生成配置文件：

   ```shell
   # sh ./genDevRuleByUUID.sh
   ```

   脚本执行成功后，会在当前路径生成名为yashan-device-rule.rules的绑定配置文件，内容如下，实际部署时内容可能有差异：
   
   ::: tabs
   == CentOS

```shell
# cat yashan-device-rule.rules
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a2231f0000e7d9668d7b4a",SYMLINK+="yfs/data0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/data0'"
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a2231f0000e7dd668d7b68",SYMLINK+="yfs/sys0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/sys0'"
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a233b90000e358668d7caf",SYMLINK+="yfs/sys1",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/sys1'"
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a233b90000e35a668d7cbf",SYMLINK+="yfs/sys2",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/sys2'"
```

   == KylinOS V10

```shell
# cat yashan-device-rule.rules
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd50",SYMLINK+="yfs/data0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd58",SYMLINK+="yfs/sys0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd66",SYMLINK+="yfs/sys1",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd78",SYMLINK+="yfs/sys2",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
```
   :::

   SYMLINK为绑定设备的别名，检查是否为预期绑定目标。

### 步骤2：绑定存储设备

1. 将配置文件移动至/etc/udev/rules.d目录下。

   ```shell
   # cp yashan-device-rule.rules /etc/udev/rules.d/99-yashan-device-rule.rules
   ```

2. 加载udev配置。

   ```shell
   # udevadm control --reload-rules
   ```

3. 绑定存储设备。

   ```shell
   ## 场景1：绑定规则文件中的所有设备
   # udevadm trigger
   
   ## 场景2：绑定规则文件中的某一设备
   # udevadm trigger --sysname-match=sdf
   ```

### 步骤3：检查绑定结果

1. 执行如下命令，确认所有绑定已生效，用户和组均为`yashan`：

   ```shell
   # ls -l /dev/yfs/*
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 data0 -> ../dm-5
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys0 -> ../dm-7
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys1 -> ../dm-4
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys2 -> ../dm-6
   ```

   并检查它们指向设备的用户为`yashan`、组为`YASDBA`、权限为`0666`：

   ```shell
   # ls -l /dev/dm-4
   brw-rw-rw- 1 yashan YASDBA 253, 4 Jul 11 14:44 /dev/dm-4

   # 其他设备类似
   ```

2. 在集群所有服务器上执行以下命令，确认各服务器上**所有**绑定的多路径WWID一致。

   ```shell
   # 对比各服务器上的输出结果是否对应一致
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/data
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys0
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys1
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys2
   ```
   若两台服务器上同一路经所对应的WWID不一致，说明其挂载的NVMe-oF盘不是同一个NVMe-oF盘，需联系网络管理员处理。

完成上述操作后，本服务器的存储设备在此步完成配置，可切换至剩余服务器重复上面步骤。

如需重新绑定，请参照共享集群（共享存储）部署下的[重新绑定](#rebinding)进行操作。

## 分支数据库部署下绑定磁盘

分支数据库部署的磁盘配置与共享集群（共享存储）部署类似，区别在于分支数据库仅需配置一个系统盘和一个数据盘，且不涉及多服务器场景。

### 步骤1：生成配置文件

1. 以root用户登录服务器。

2. 在root目录下创建名称为`genDevRuleByUUID.sh`的脚本：

   ```shell
   # cd ~
   # vi genDevRuleByUUID.sh
   ```

3. 输入如下内容，并根据部署环境修改`DEVICES`为各设备路径，修改`TARGETS`为各设备绑定名称，名称应与设备路径一一对应，修改`YFS_DISK_DIR`为绑定目录，保存并退出。

   ::: tabs
   == CentOS

```shell
#/bin/bash

# 请修改为各设备路径
DEVICES=("/dev/mapper/lun01-data0" "/dev/mapper/lun03-sys0")
# 请按DEVICES数组顺序指定这些设备绑定的名称，且建议指定为与业务相关的名称
TARGETS=("data0"                   "sys0")
# 指定绑定的目录
YFS_DISK_DIR="yfs"

# 请保持默认，请确保yashan用户、YASDBA用户组已存在
USER="yashan"
GROUP="YASDBA"
UGROUP="$USER:$GROUP"

if [ ${#TARGETS[@]} != ${#DEVICES[@]} ]
then
    echo "DEVICES and TARGETS count does not match"
    exit 1
fi

if [ ${#TARGETS[@]} == 0 ]
then
    echo "no targets found"
    exit 1
fi

curDir=$(cd "$(dirname "$0")";pwd)
rm -f $curDir/yashan-device-rule.rules

count=${#TARGETS[@]}

for ((i = 0; i < $count; i ++))
do
    device=${DEVICES[$i]}
    target_name=${TARGETS[$i]}
    target="${YFS_DISK_DIR}/${target_name}"
    
    str1="$device:`udevadm info --query=all --name=$device | grep DM_UUID`"
    str2=${str1#*=}
    echo "KERNEL==\"dm-*\",ENV{DM_UUID}==\"${str2}\",SYMLINK+=\"$target\",OWNER=\"$USER\",GROUP=\"$GROUP\",MODE=\"0666\",OPTIONS:=\"nowatch\",RUN+=\"/bin/sh -c 'chown -R $UGROUP $target'\""  >> $curDir/yashan-device-rule.rules
done
```

    == KylinOS V10

```shell
#/bin/bash

# 请修改为各设备路径
DEVICES=("/dev/mapper/lun01-data0" "/dev/mapper/lun03-sys0")
# 请按DEVICES数组顺序指定这些设备绑定的名称，且建议指定为与业务相关的名称
TARGETS=("data0"                   "sys0")
# 指定绑定的目录
YFS_DISK_DIR="yfs"

# 请保持默认，请确保yashan用户、YASDBA用户组已存在
USER="yashan"
GROUP="YASDBA"
UGROUP="$USER:$GROUP"

if [ ${#TARGETS[@]} != ${#DEVICES[@]} ]
then
    echo "DEVICES and TARGETS count does not match"
    exit 1
fi

if [ ${#TARGETS[@]} == 0 ]
then
    echo "no targets found"
    exit 1
fi

curDir=$(cd "$(dirname "$0")";pwd)
rm -f $curDir/yashan-device-rule.rules

count=${#TARGETS[@]}

for ((i = 0; i < $count; i ++))
do
    device=${DEVICES[$i]}
    target_name=${TARGETS[$i]}
    target="${YFS_DISK_DIR}/${target_name}"
    
    str1="$device:`udevadm info --query=all --name=$device | grep DM_UUID`"
    str2=${str1#*=}
    echo "KERNEL==\"sd*\",ENV{ID_SERIAL}==\"${str2}\",SYMLINK+=\"$target\",OWNER=\"$USER\",GROUP=\"$GROUP\",MODE=\"0666\",OPTIONS:=\"nowatch\"" >> $curDir/yashan-device-rule.rules
done
```
    :::

4. 执行脚本，生成配置文件：

   ```shell
   # sh ./genDevRuleByUUID.sh
   ```

   脚本执行成功后，会在当前路径生成名为yashan-device-rule.rules的绑定配置文件，内容如下，实际部署时内容可能有差异：

   ::: tabs
   == CentOS

```shell
# cat yashan-device-rule.rules
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a2231f0000e7d9668d7b4a",SYMLINK+="yfs/data0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/data0'"
KERNEL=="dm-*",ENV{DM_UUID}=="mpath-36d039ea000a2231f0000e7dd668d7b68",SYMLINK+="yfs/sys0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch",RUN+="/bin/sh -c 'chown -R yashan:YASDBA /dev/yfs/sys0'"
```

   == KylinOS V10

```shell
# cat yashan-device-rule.rules
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd50",SYMLINK+="yfs/data0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
KERNEL=="sd*",ENV{ID_SERIAL}=="3684302d47042ab09d42082949ee0fd58",SYMLINK+="yfs/sys0",OWNER="yashan",GROUP="YASDBA",MODE="0666",OPTIONS:="nowatch"
```
   :::

   SYMLINK为绑定设备的别名，检查是否为预期绑定目标。

### 步骤2：绑定存储设备

1. 将配置文件移动至/etc/udev/rules.d目录下。

   ```shell
   # cp yashan-device-rule.rules /etc/udev/rules.d/99-yashan-device-rule.rules
   ```

2. 加载udev配置。

   ```shell
   # udevadm control --reload-rules
   ```

3. 绑定存储设备。

   ```shell
   ## 场景1：绑定规则文件中的所有设备
   # udevadm trigger
   
   ## 场景2：绑定规则文件中的某一设备
   # udevadm trigger --sysname-match=sdf
   ```

### 步骤3：检查绑定结果

1. 执行如下命令，确认所有绑定已生效，用户为`yashan`、组为`YASDBA`：

   ```shell
   # ls -l /dev/yfs/*
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 data0 -> ../dm-5
   lrw-rw-rw- 1 yashan YASDBA 7 Jul 11 14:44 sys0 -> ../dm-7
   ```

    并检查它们指向设备的用户为`yashan`、组为`YASDBA`、权限为`0666`：

    ```shell
    # ls -l /dev/dm-4
    brw-rw-rw- 1 yashan YASDBA 253, 4 Jul 11 14:44 /dev/dm-4

    # 其他设备类似
    ```

2. 在集群所有服务器上执行以下命令，确认各服务器上**所有**绑定的多路径WWID一致。

   ```shell
   # 对比各服务器上的输出结果是否对应一致
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/data
   # /lib/udev/scsi_id --whitelisted  --device=/dev/yfs/sys0
   ```
   若两台服务器上同一路经所对应的WWID不一致，说明其挂载的磁阵不是同一个LUN，需联系网络管理员处理。

<span id="modify_multipath_conf" name="modify_multipath_conf"></span>

如需重新绑定，请参照共享集群（共享存储）部署下的[重新绑定](#rebinding)进行操作。
