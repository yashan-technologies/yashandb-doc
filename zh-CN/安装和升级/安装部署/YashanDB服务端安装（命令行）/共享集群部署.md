...
| 部署形态| 部署场景|
| ----------------------------------------- | ------------------------------------------------------------ |
| 单集群<br>主备集群                        | 单一网段/多网段<br>SCAN和VIP         |



本文[第二部分](#installation)所述各部署形态的安装过程将默认按公网私网位于同一网段内的场景进行示例演示。请仔细阅读本文[第一部分](#InstallationScenario)对各种安装场景的说明，并参照指导对示例命令进行调整后再运行安装。
