...
| DB_BLOCK_SIZE  | 数据块大小                                                   |
| YAS_{建库参数} | 支持所有[建库参数](../../工具手册/yasboot/建库参数.md)，格式为`YAS_参数名`（例如参数REDO_FILE_SIZE需写成YAS_REDO_FILE_SIZE），此处配置过的参数建库后不可修改 |
...
2. 使用[yasql](../../工具手册/yasql/00yasql.md)工具连接数据库，查看实例状态。
...
进入容器后，可使用[yasql](../../工具手册/yasql/00yasql.md)、[yasboot](../../工具手册/yasboot/00yasboot.md)等工具连接或管理数据库。
