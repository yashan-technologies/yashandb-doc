...
7. 检测共享存储等设备/环境，确认是否可以使用[基于SCSI持久预留的I/O Fencing](../安装前准备/服务器准备.md)。
...
- 配置monit守护进程以及开机自启动
   ...
   该操作仅配置数据库开机自启动，若已开启[基于SCSI持久预留的I/O Fencing功能](../../../数据库管理/集群管理/IO Fencing/基于SCSI持久预留的IO Fencing.md)，还需配置ycsrootagent进程的开机自启动，操作步骤请查阅[配置开机自启动](../安装后初始环境/配置开机自启动.md)。
