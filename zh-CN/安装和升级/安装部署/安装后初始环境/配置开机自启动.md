...
| ycsrootagent进程 | 已开启[SCAN](../../../数据库管理/集群管理/SCAN管理.md)、[VIP](../../../数据库管理/集群管理/VIP管理.md)或[基于预留的I/O Fencing功能](../../../数据库管理/集群管理/IO Fencing/基于预留的IO Fencing.md) |用于为基于预留的I/O Fencing功能提供相应服务。<br/>该进程无法被共享集群节点启停命令管理，也无法被yascsm进程守护。安装部署时yasboot会自动尝试sudo启动ycsrootagent，后续使用中若该进程出现异常或服务器重启后需要手动sudo启动（需要root权限）。  |
