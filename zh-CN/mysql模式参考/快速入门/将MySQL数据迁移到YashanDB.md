...
- 数据迁移平台-YMP：YMP（YashanDB Migration Platform）是YashanDB提供的数据库迁移产品，支持数据源类型包括Oracle、MySQL、DM、PG及YashanDB，支持元数据和数据一键迁移到崖山数据库。YMP迁移MySQL到YashanDB与迁移Oracle到YashanDB类似，可参考[将Oracle数据迁移到YashanDB.md](../../全部手册/快速上手/将Oracle数据迁移到YashanDB)
