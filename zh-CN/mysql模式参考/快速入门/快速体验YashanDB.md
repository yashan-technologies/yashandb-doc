...
3. 通过命令行工具yasboot安装单机数据库。
   ...
   - 此处的hosts.toml、yashandb.toml文件为./bin/yasboot package se gen命令按照服务器本地化部署默认部署参数自动生成；在部署mysql语法模式时，yashandb.toml文件中参数`mysql_addr = "ip_addr:1690"`为MySQL协议的监听地址，默认端口为1690（单机及单机主备）或1691（共享集群），可使用MySQL客户端或驱动通过该监听地址连接到YashanDB。如需自定义部署参数，可参考[yasboot package](../../全部手册/工具手册/yasboot/yasboot命令介绍/yasboot package)修改参数。
