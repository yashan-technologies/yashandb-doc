Outline通过固定SQL语句执行计划确保执行计划稳定性。使用[CREATE OUTLINE](../../../开发手册/SQL参考手册/SQL语句/CREATE OUTLINE.md)创建指定存储纲要，SQL_TEXT语法会根据SQL语句对应的当前执行计划生成对应的Hint信息，SQL_ID语法为具体某条SQL语句设置Hint信息。当存储纲要可用时，数据库将自动根据这些Hint信息生成对应SQL语句的执行计划。
...
Outline由用户来定义和维护，包括创建、修改和删除，具体操作请参考[CREATE OUTLINE](../../../开发手册/SQL参考手册/SQL语句/CREATE OUTLINE.md)、[ALTER OUTLINE](../../../开发手册/SQL参考手册/SQL语句/ALTER OUTLINE.md)、[DROP OUTLINE](../../../开发手册/SQL参考手册/SQL语句/DROP OUTLINE.md)。
...
当前outline只支持生成SCAN类与JOIN类的hint，即[hint](../../../开发手册/SQL参考手册/通用SQL语法/hint.md) 中的 FULL，INDEX，USE_NL，USE_MERGE，USE_HASH这几类。
