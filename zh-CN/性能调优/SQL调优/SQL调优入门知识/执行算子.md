...
| RESULT COLLECTOR | 批量执行开启pipeline并行标识。                            | 行          | 仅批量执行 |
| CLUSTER COORDINATOR  | 集群部署中多机并行执行的标识。表示此标识以下的算子都为多机并行执行。 | 行 | 仅批量执行        |
| UNPIVOT          | 将列旋转为行数据。                                        | 行          | ×        |
...
| FIXED TABLE FIXED INDEX   |  使用FIXED INDEX对FIXED TABLE扫描，仅适用于WHERE条件是等值查询的SQL。<br/>该算子不受索引相关hint的控制。  | All         | ×        |
...
| HNSW INDEX SCAN                 | 向量查询HNSW索引扫描。 | 行 | ×        |
...
| AFFINITY PART SCAN ALL | 亲和性分区全扫描。 | 行 | 仅批量执行       |
| AFFINITY PART SCAN ITERATOR | 指定一组分区亲和扫描。 | 行 | 仅批量执行        |
| AFFINITY PART SCAN SINGLE | 指定单个分区亲和扫描。 | 行 | 仅批量执行        |
