...
| TABLE ACCESS BY INDEX ROWID | 根据索引查找到对应数据块的rowid（根据rowid回表）。 | √          | √          |
| TABLE ACCESS BY USER ROWID  | 指定rowid进行数据块的查找。                          | √          | √          |
...
| NESTED LOOPS INNER          | NESTED LOOP 内连接。两表通过循环遍历方式查找满足join条件的数据。 | √          | √          |
...
| DISTRIBUTED COORDINATOR                | 存算一体分布式集群部署中并行执行的标识，表示此标识以下的算子都为并行执行。      | √          | √          |
