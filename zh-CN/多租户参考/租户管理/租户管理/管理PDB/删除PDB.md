...
| 方法 | 功能说明 |
| -------------------------------- | -------------------------------------------- |
| 连接根容器执行[DROP PLUGGABLE DATABASE](../../../../全部手册/开发手册/SQL参考手册/SQL语句/DROP PLUGGABLE DATABASE)语句 | 单次可以删除1个PDB，可按需选择是否保留目标PDB的redo日志文件、归档日志文件以及PDB元数据。 |
| [yasboot工具](../../../../全部手册/工具手册/yasboot/yasboot命令介绍/yasboot pdb) | 单次可以单个或批量删除PDB，但该操作属于彻底销毁PDB，无法自选保留部分持久化文件。 |
