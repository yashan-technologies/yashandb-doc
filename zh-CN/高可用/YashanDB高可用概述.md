...
级联备，即备库的备库，普通备库从主库接收日志，而级联备则是从其父级备库接收日志。

当某个级联备的父级备库升为主库，该级联备转为普通备库。
...
由于级联备未直接与主库连接，因此不能对其执行Switchover，只能执行Failover。但如果级联备的父级备库执行了Switchover，则该级联备的角色将发生变化，而主备切换以系统中当前的角色状态为准，如下图：
...
- 备库：主复制组中的所有备库，负责接收主库发送的redo日志并回放。

- 异地备：备复制组中的首节点，负责接收主库发送的redo日志并回放，还作为备复制组中其他节点的父级备库，将接收到的redo日志发送给备复制组中的其他节点。

- 级联备：备复制组中除首节点外的其他节点，接收首节点发送的redo日志并回放。

以下图中的双复制组主备部署为例，GROUP 1为主复制组：NODE1-1为主库、NODE1-2和1-3为同步备库，GROUP 2为备复制组：NODE2-1为异地备库（备复制组的首节点）、NODE2-2和NODE2-3为级联备库。
...
通过`yasboot node switch-remote-standby`命令可以更换备复制组的首节点。
...
yasom仲裁选主是在单机主备部署（非级联备）、主备集群部署或存算一体分布式集群部署（DN组内节点为一主一备）中，当主节点发生异常不能对外服务时，系统通过yasom仲裁，将备节点升为主节点，并将原主节点降为备节点。
...
并行build由主库指定多台备库发起build操作，主库同时向多台备库发送数据（满足主备部署允许的最大备库数据量限制），多台备库同时执行restore。级联备的并行build则由其父级备库发起。
