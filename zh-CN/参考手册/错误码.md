...
**Action**：设备有残留的预留信息，请在确认操作安全后sudo执行ycsrootagent resv clear -d <device>命令进行清理。

### YAS-00342:ERR_CMM_NVME_COMMAND

**Message**: NVMe command failed for: %s, SCT: %d, SC: %d, DNR: %d

**Action**：NVMe命令执行失败，请联系我们的技术支持解决。
...
**Action**：表空间的空间不足，请打开表空间自动扩展或给表空间添加空闲数据文件。
...
**Action**：Buffer Cache在线扩展已至上限，不能再执行ALTER SYSTEM EXTEND DATA_BUFFER ... 或ALTER SYSTEM SET DATA_BUFFER_SIZE ... SCOPE=MEMORY|BOTH语句进行在线扩展操作。
...
**Action**：请按照正确的格式输入有效的用户名和密码组合。若确认用户名和密码无误，可能因当前环境已开启[三权分立](../产品安全/数据访问控制/三权分立.md)且要求管理员用户登录时声明角色（[as role_name](../工具手册/yasql/yasql使用指导.md#role_name)，省略则默认为普通用户）但未正确声明角色导致登录失败。
...
**Action**：指定属于该表空间的DATA BUCKET进行操作。
...
**Action**：不允许为以LOB类型存储的列添加外键约束，也不允许外键约束引用此类列。
...
**Action**：切换快照备库的操作尚未全部完成，请继续执行剩余的与切换快照备库相关的操作。只有在完成快照备库的切换后，方可再次切回到物理备库。
...
**Action**：当前yasrman版本和数据库版本不匹配，请检查版本信息后使用正确的版本执行操作命令。
...
**Action**：yasrman工具异常退出，具体原因请参考yasrman的详细报错信息。
...
**Action**：指定的归档区间定义不合法，请按照提示信息输入合法的归档区间，多租户模式下，root实例仅支持指定归档ALL、UNTIL SCN或UNTIL TIME区间。

### YAS-02582:ERR_RMAN_BAK_EXEC_FAILED

**Message**: yasrman failed to perform %s on container %s

**Action**：备份恢复工具yasrman执行指定的操作失败，请按照其余错误码指示修正后重新执行该操作。
...
**Action**：连接至PDB时不允许指定PDB定义。请检查并修正命令。

### YAS-02584:ERR_RMAN_DELETE_SET_FAILED

**Message**: failed to delete backup set, reason: %s

**Action**：删除备份集失败，请按照错误码原因提示，选择合适的删除指令。

- 提示“this backup set must be deleted on the PDB it belongs to”：指定删除的备份集为单个PDB生成的，必须连接生成该备份集的PDB执行删除命令。

- 提示“this backup set must be deleted on ROOT”：指定删除的备份集是在根容器生成的，必须连接根容器执行删除命令。

### YAS-02585:ERR_RMAN_CMD_ALLOWED_CDB

**Message**: command only allowed in a container database

**Action**：该命令只允许在容器数据库中执行，请检查执行命令和执行的目标数据库部署模式。

### YAS-02586:ERR_RMAN_CATALOG_INCOMPLETE

**Message**: the catalog file collection is incomplete

**Action**：指定的catalog文件集合不完整，请确认并使用正确的catalog文件集重新执行操作命令。
...
**Action**：不允许本地表空间同时包含YFS和本地文件系统路径。
...
**Action**：请不要创建超过最大数量的本地数据文件，本地表空间的最大文件个数为表空间最大文件数除以实例个数并向下取整。
...
**Action**：请等待该redo文件归档后重试，或使用UNARCHIVED字段操作（该操作可能会导致备份集不可用、备库同步异常等）。
...
**Action**：无法闪回到不支持的操作以前的scn，请调整目标值，如果建议scn为18446744073709551615，则表示此时无法进行闪回，请稍后重试。
...
**Action**：请修正FILE_NAME_CONVERT clause并重试语句。
...
**Action**：请检查配置参数ENABLE_PLUGGABLE_DATABASE是否与实际数据库环境相匹配。

### YAS-02908: ERR_ANK_INVALID_ADMIN_USER

**Message**: invalid admin user name

**Action**：管理用户名与其它用户名冲突，或者名称中带有全局前缀。
...
**Action**：当前DROP操作的目标PDB和实际操作的元数据内容不符，请检查控制文件中记录的目标PDB的HOME路径是否和实际HOME路径一致，检查项具体所在位置为：

- 在单机部署中，请检查$YASDB_DATA/config/yasdb.ini文件的CONTROL_FILES配置项。

- 在共享集群/分布式集群部署中，请检查YFS中的$PDB_DATA/config文件。

### YAS-02915: ERR_ANK_CONTAINER_INCOMPLETE

**Message**: not allow to open incomplete container '%s', please drop and create again

**Action**：目标容器数据库创建不完整，无法开启，请先删除并正确重建后再启动。
...
**Action**：VM Buffer在线扩展次数已至上限，不能再执行ALTER SYSTEM SET VM_BUFFER_SIZE ... SCOPE=MEMORY|BOTH语句进行在线扩展操作。
...
**Action**：构建HNSW向量索引时内存不足，索引构建将自动切换至基于表空间存储的模式，这会影响构建性能。请结合实际情况，分析是MEX POOL内存不足，还是HNSW_BUILD_MEM_SIZE配置过小，并据此进行相应调整。
...
**Action**：本次冷数据的插入因提示的原因失败而将整个事务进行了回滚。
...
- 提示“result of string concatenation”、“overflow indicator”、“overflow separator”、“listagg const”：listagg函数的使用存在规格上限，请调整listagg函数的用法。
...
**Action**：请检查fvecs文件的数据内容，并在修正数据后再重新导入。
...
**Action**：多实例任务执行过程中，某个实例出现执行失败。可以通过查看底层返回的错误码来准确定位和分析失败原因。
...
**Action**：磁盘组内故障组的数量与其冗余度不匹配。请配置足够数量的故障组。
...
**Action**：磁盘size超出合法范围，请修改后重试。size参数可省略，省略时YFS会使用整块磁盘的空间。
...
**Action**：该磁盘组下的磁盘未找到，请先排查主备节点磁盘是否正确挂载、是否能正常读写。若无法解决，请联系我们的技术支持解决。
...
**Action**：数据库正处于主备切换过程中，暂无法建立连接。待切换完成，客户端会自动重试连接。您也可选择稍后手动重新发起连接请求。
...
**Action**：审计策略的判断条件不合理，请检查创建审计策略时指定的判断条件是否正确。
...
**Action**：无效VECTOR描述，请创建有效VECTOR描述。

### YAS-08080:ERR_YAC_VECTOR_DATA_INVALID

**Message**: invalid vector data

**Action**：无效VECTOR数据，请创建或获取有效VECTOR数据。
...
**Action**：剩余内存不足，请关闭其它容器或调大CDB_MAX_MEMORY_SIZE。
...
**Action**：无法在线调整CDB_MAX_MEMORY_SIZE参数，请关闭资源管理功能后再尝试或设置成重启生效。
