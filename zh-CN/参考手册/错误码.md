...
**Action**：不支持当前类型间转换，请查阅[数据类型转换](../开发手册/SQL参考手册/数据类型（yashan模式）/数据类型转换)了解转换规则。若在mysql模式下使用[无符号数值型](../开发手册/SQL参考手册/数据类型（mysql模式）/00数据类型（mysql模式）)时出现该问题，可能是由于yasql客户端版本过低，请检查并升级客户端。
...
- 报driver memory pool不足时，可通过调整yex_server.ini配置文件中的参数YDBC_BUFFER_SIZE，具体操作请查阅[调整内存池大小](../数据库管理/会话和计划管理/yex_server沙箱进程管理)。
...
**Action**：请选择合适的文件系统，支持的文件系统可以参考[CREATE_TABLESPACE](../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE TABLESPACE)。

### YAS-00324
**Message**: the page size of file system must be %s

**Action**：请选择满足page size的文件系统，支持的文件系统可以参考[CREATE_TABLESPACE](../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE TABLESPACE)。
...
**Action**：钱包文件pkcs12文件写入失败，请根据具体报错排查故障。
...
**Action**：可以减小参数DBWR_COUNT或DBWR_BUFFER_SIZE的值，也可以调大双写文件的大小，双写文件的大小约束可以参考[CREATE DATABASE](../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE DATABASE)。
...
**Action**：表中含有Nested Table UDT列或属性，请为其定义存储子句，参考[CREATE TABLE](../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE TABLE)中的nested_table_clauses。
...
**Action**：单次授予的角色超过上限，请减少将授予的角色名后重试。
...
**Action**：根据[CREATE ACCESS CONSTRAINT](../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE ACCESS CONSTRAINT)语法图添加y值列。
...
**Action**：使用ALTER TABLE语句开启build ac开关，参考ALTER TABLE文档中的[enable_xfmr_clause](../开发手册/SQL参考手册/SQL语句（yashan模式）/ALTER TABLE.html#enablexfmrclause)。
...
**Action**：输入的参数无效，请参考[内置函数](../开发手册/SQL参考手册/内置函数（yashan模式）/00内置函数（yashan模式）)或[内置高级包](../开发手册/PL参考手册/内置高级包/00内置高级包)，修改参数并重新执行。
...
**Action**：请指定正确的lob storage选项，参考[CREATE TABLE](../开发手册/SQL参考手册/SQL语句（yashan模式）/CREATE TABLE)中的lob_clause子句描述。
...
**Action**：请将options语句里的参数修改为符合要求的值，参考[LOAD DATA](../开发手册/SQL参考手册/SQL语句（yashan模式）/LOAD DATA)。
...
**Action**：输入的参数无效，请参考[LOAD DATA](../开发手册/SQL参考手册/SQL语句（yashan模式）/LOAD DATA)。
...
**Action**：依据[数据类型转换](../开发手册/SQL参考手册/数据类型（yashan模式）/数据类型转换)，修改赋值语句。
...
**Action**：共享集群模式下，请通过`ycsctl`停止服务。 
...
**Action**：YCS未启动VIP功能，请确保所有集群节点都配置了VIP且已启动成功。
...
**Action**：请根据[SET AUTOTRACE](../开发手册/SQL参考手册/SQL语句（yashan模式）/SET AUTOTRACE)语法介绍修正AUTOTRACE相关选项为有效值后重试。
