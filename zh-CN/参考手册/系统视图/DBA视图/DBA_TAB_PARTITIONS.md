...
| NUM\_ROWS | BIGINT | 分区的行数<br/>仅在收集完对象的统计信息后才有值 |
| BLOCKS | BIGINT | 分区的block数量<br/>仅在收集完对象的统计信息后才有值 |
| EMPTY\_BLOCKS | BIGINT | 分区的空block数量<br/>仅在收集完对象的统计信息后才有值 |
| AVG\_SPACE | INTEGER | 分区block的平均空闲空间（单位：字节）<br/>仅在收集完对象的统计信息后才有值 |
| CHAIN\_CNT | BIGINT | 分区的行链接数量<br/>仅在收集完对象的统计信息后才有值 |
| AVG\_ROW\_LEN | INTEGER | 分区的平均行长度（单位：字节）<br/>仅在收集完对象的统计信息后才有值 |
