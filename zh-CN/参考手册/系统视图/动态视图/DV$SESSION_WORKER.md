本视图显示分布式集群中所有节点分布式执行使用session worker池的汇总信息。
...
| MAX_TASK_RUN_TIME | BIGINT | worker执行任务的最长时间（单位：微秒） |
| MIN_TASK_RUN_TIME | BIGINT | worker执行任务的最短时间（单位：微秒） |
| AVG_TASK_RUN_TIME | BIGINT | worker任务平均执行时间（单位：微秒） |
