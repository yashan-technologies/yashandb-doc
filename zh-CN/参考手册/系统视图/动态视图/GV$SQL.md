...
| RUNTIME\_MEM | INTEGER | 此SQL在执行过程中申请的内存 |
...
| PHYSICAL\_WRITE\_REQUESTS | BIGINT | SQL发出的物理写入I/O请求数（SQL执行不会立即写盘，而是由dbwr线程写盘，因此从SQL层面观察不到该统计项的值变更，建议从V$SYSSTAT/V$SESSTAT/V$MYSTAT视图观察） |
| PHYSICAL\_WRITE\_BYTES | BIGINT | SQL写入磁盘的字节数（SQL执行不会立即写盘，而是由dbwr线程写盘，因此从SQL层面观察不到该统计项的值变更，建议从V$SYSSTAT/V$SESSTAT/V$MYSTAT视图观察） |
