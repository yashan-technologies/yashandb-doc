存算一体分布式集群部署中，本视图显示所有节点的内部链路的汇总信息。

单机/共享集群/分布式集群部署中，本视图为空。
...
| ADDRESS | VARCHAR(108) | 本端地址 |
| PEER_ENDPOINT | SMALLINT | 对端通讯ID |
| PEER_ADDRESS | VARCHAR(108) | 对端通讯地址 |
...
| LINK_CREATE_TIME | TIMESTAMP(6) | 连接创建时间 |
...
| LAST_SEND_TIME | TIMESTAMP(6) | 链路最后发送时间 |
| LAST_RECV_TIME | TIMESTAMP(6) | 链路最后接收时间|
...
| THROUGHPUT | BIGINT | 链路吞吐量（单位：字节/分钟） |
