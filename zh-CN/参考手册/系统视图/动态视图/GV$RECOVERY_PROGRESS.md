...
| Latest Redo Merge Time | Millisecond | 最近一次合并日志的耗时（当遇到DDL日志时，会在合并日志期间回放DDL，该时间会偏大） |
| Latest Analysis Time | Millisecond | 最近一次日志分析的耗时 |
| Latest Redo Merge Time | Millisecond | 最近一次合并日志的耗时（若日志中包含DDL操作，合并期间需回放 DDL，会导致耗时显著增加）   |
