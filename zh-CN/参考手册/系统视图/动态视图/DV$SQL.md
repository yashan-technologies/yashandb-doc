...
| PERSISTENT\_MEM | INTEGER | 在SQL缓存池中占用的页面内存减去尾部未使用的内存（单位：字节） |
| RUNTIME\_MEM | INTEGER | 此SQL在执行过程中申请的内存（单位：字节） |
...
| ROW\_PROCESSED | BIGINT | SQL语句返回的总行数 |
...
| IM\_SCANS | BIGINT | 内存列存储段扫描的数量 |
| IM\_SCAN\_BYTES\_UNCOMPRESSED | BIGINT | 内存列存储段扫描的未压缩字节数 |
| IM\_SCAN\_BYTES\_INMEMORY | BIGINT | 内存列存储段扫描的字节数 |
...
| OPTIMIZER\_MODE | VARCHAR(10) | SQL语句的优化器模型 |
