...
说明：database writer执行对象级checkpoint次数。

### DBWR OBJECT WRITES

说明：database writer执行对象级checkpoint时刷的block数量。
...
说明：集群字段、自动扩展GCS锁池的次数。

### GCS AUTO EXTEND LOCK POOL PERCENTS

说明：集群字段、自动扩展GCS锁池的比例（相较于数据块数量）。
