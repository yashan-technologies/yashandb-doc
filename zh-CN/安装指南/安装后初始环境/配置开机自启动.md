...
| ycsrootagent进程 | 已开启[VIP](../../全部手册/共享集群/集群服务管理/VIP)或[基于SCSI持久预留的I/O Fencing功能](../../全部手册/共享集群/集群服务管理/IO Fencing/基于SCSI持久预留的IO Fencing) |用于为基于SCSI持久预留的I/O Fencing功能提供相应服务。<br/>该进程无法被共享集群节点启停命令管理，也无法被yascsm进程守护。安装部署时yasboot会自动尝试sudo启动ycsrootagent，后续使用中若该进程出现异常或服务器重启后需要手动sudo启动（需要root权限）。  |
