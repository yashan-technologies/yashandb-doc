完成安装后，所有节点的数据库安装用户下将添加如下环境变量信息：

```properties
# 具体路径以实际值为准
export YASDB_HOME=/data/yashan/yasdb_home/{版本号}
export PATH=$YASDB_HOME/bin:$PATH 
export LD_LIBRARY_PATH=$YASDB_HOME/lib:$LD_LIBRARY_PATH
export YASDB_DATA=/data/yashan/yasdb_data/db-1-1   
```

- **YASDB_HOME**：指向路径为YashanDB的HOME目录，包含产品相关程序文件。直接查看集群名称.bashrc环境变量文件时，YASDB_HOME会显示为其他路径（通过软链接指向`/data/yashan/yasdb_home/{版本号}`），**不建议手动**修改该软链接。

- **$YASDB_HOME/bin**：包含YashanDB配套工具，将其加入PATH系统路径使其可以直接运行。

- **LD_LIBRARY_PATH**：指向路径为YashanDB的LIB目录，包含产品运行所需库文件。

- **YASDB_DATA**：指向路径为YashanDB的DATA目录，包含产品相关数据文件。

部分功能还需额外的环境变量，可按需配置：

- 在共享集群和分布式集群中，还需在每台服务器上配置并生效如下环境变量。

  ```shell
  # 如下路径需更换为实际的节点路径
  # 实例1-1
  $ export YASCS_HOME=/data/yashan/yasdb_data/ycs/ce-1-1

  # 实例1-2
  $ export YASCS_HOME=/data/yashan/yasdb_data/ycs/ce-1-2
  ```

- 如需使用[外置UDF（Java）](../../全部手册/开发手册/PL参考手册/PL对象/自定义函数.html#ext_udf)，还需配置并生效如下环境变量。

  ```shell
  # 如下路径需更换为实际的JDK安装路径
  $ export LD_LIBRARY_PATH=/etc/jdk-18.0.2/lib/server:$LD_LIBRARY_PATH
  ```
