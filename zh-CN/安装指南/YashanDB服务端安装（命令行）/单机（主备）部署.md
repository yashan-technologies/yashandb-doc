...
3. 选择需要安装的插件，指定多个值时使用逗号`,`隔开。
    ...
    | 1/none  |  不安装插件  |
    | 2/all  |  安装所有插件，默认值  |
    | 3/gis  |  安装用于内置[GIS](../../全部手册/开发手册/SQL参考手册/内置函数（yashan模式）/GIS Function/00GIS Function)函数的插件  |
    | 4/dblink  |  安装用于DBLink相关功能的插件  |
    | 5/udf  |  安装用于[自定义函数](../../全部手册/开发手册/PL参考手册/PL对象/自定义函数)相关功能的插件  |
    | 6/s3  |  安装用于S3 bucket相关功能的插件  |
    | 7/listagg  |  安装用于内置[LSFA_LISTAGG](../../全部手册/开发手册/SQL参考手册/内置函数（yashan模式）/LSFA_LISTAGG)函数的插件  |
    | 8/xml  |  安装用于内置[XML](../../全部手册/开发手册/SQL参考手册/内置函数（yashan模式）/XML Function/00XML Function)函数的插件  |
...
根据需要部署的数据库规模（例如单库、1主2备等）不同，生成配置文件时需要指定的参数略有差异，常用参数如下表所示，参数全集请查阅[yasboot package se gen命令](../../全部手册/工具手册/yasboot/yasboot命令介绍/yasboot package)。
...
2. 根据实际需要，手动修改配置文件中的相关参数：
    ...
    - 数据库集群的配置文件yashandb.toml：可在group级别设置YashanDB的所有[建库参数](../../全部手册/工具手册/yasboot/建库参数)，可在node级别设置YashanDB的所有[配置参数](../../全部手册/参考手册/配置参数)。
...
2. 根据实际需要，手动修改配置文件中的相关参数：
    ...
    - 数据库集群的配置文件yashandb.toml：可在group级别设置YashanDB的所有[建库参数](../../全部手册/工具手册/yasboot/建库参数)，可在node级别设置YashanDB的所有[配置参数](../../全部手册/参考手册/配置参数)。
      ...
      |  参数名称| 参数说明| 配置方法|
      |--------------|-------------------------|--------------|
      | [REPLICATION_ADDR](../../全部手册/参考手册/配置参数.html#REPLICATION_ADDR) | 当前数据库/节点的IP与同步复制端口，端口号 = 监听端口 + 1。| 自动配置 |
      | [ARCHIVE_DEST_*](../../全部手册/参考手册/配置参数.html#ARCHIVE_DEST) | 对端数据库/节点链路（该类参数有32个，分别为ARCHIVE_DEST_1 ~ 32，每个参数表示1个备库的对端数据库链路），需在REPLICATION_ADDR参数配置生效后再进行配置。<br/>ARCHIVE_DEST_*参数配置后立即生效，相应复制链路会被占用且不允许直接修改（直接修改将报YAS-02425错误）。如需重新配置该参数，应先将该参数置空，释放链路后再重新设置为正确的值。| 自动配置 |
      | [DB_FILE_NAME_CONVERT](../../全部手册/参考手册/配置参数.html#DB_FILE_NAME_CONVERT)<br/>[REDO_FILE_NAME_CONVERT](../../全部手册/参考手册/配置参数.html#REDO_FILE_NAME_CONVERT)<br/>[DB_BUCKET_NAME_CONVERT](../../全部手册/参考手册/配置参数.html#DB_BUCKET_NAME_CONVERT) |  分别为数据文件转换路径、日志文件转换路径以及LSC表的bucket转换路径。<br/>当主备库的对应路径不同时，所有节点均需设置转换路径，只需父路径匹配即可将子文件夹全部转换到目标文件夹下。| 自动配置 |
      | [HA_ELECTION_ENABLED](../../全部手册/参考手册/配置参数.html#HA_ELECTION_ENABLED)  | 一主多备[自动选主](../../全部手册/高可用/自动选主配置/一主多备自动选主)功能的总开关。 | 部署规模为3节点及以上时该参数自动且强制开启，如需关闭只能在安装完成后手动操作|
      | [HA_SSL_ENABLE](../../全部手册/参考手册/配置参数.html#HA_SSL_ENABLE)（[SSL_ENABLE](../../全部手册/参考手册/配置参数.html#SSL_ENABLE)）| 主备间SSL可信信道开关，主备之间的开关参数必须一致。<br/>当REPLICATION_ADDR和LISTEN_ADDR共享端口，主备可信信道参照SSL_ENABLE，否则参照HA_SSL_ENABLE。<br/>开启可信信道时，主备需要配置SSL_CERT_FILE/SSL_KEY_FILE/SSL_DH_PARAM_FILE参数。 | 建议安装后手动按需配置，配置方式请查阅[HA节点间SSL可信信道](../../全部手册/数据库管理/基本数据库管理/SSL可信信道配置/HA节点间SSL可信信道)|
...
[双复制组主备部署](../../全部手册/高可用/YashanDB高可用概述.html#dual_rep_group)中分为主复制组和备复制组，每个组对应的服务器应安装在不同的地域/机房。
