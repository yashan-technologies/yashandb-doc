...
- **DATA目录**：YashanDB的数据目录，用于存放数据库的各类系统数据文件、部分日志文件（具体请查阅[日志管理](../../全部手册/数据库管理/日志管理/00日志管理)）和配置文件，用户数据也默认存储在该目录下。但对于共享集群，所有的数据文件和redo文件均需保存在共享存储上，DATA目录将只用于存储实例运行相关的配置文件、日志文件等数据。在安装操作中，该目录将作为--data-path参数的值。安装完成后YashanDB会在该目录下按规则生成$YASDB_DATA。

    > **Note**:
    >
    > DATA目录过长可能会影响部分功能的正常使用，例如[UDS本地连接](../../全部手册/产品安全/连接管理/00连接管理)以及基于UDS的其他功能等，建议该路径长度不超过64字节。
...
- 按规划统一存放
    ...
    1. 规划一个目录，用于存放run.log、slow.log以及yasom和yasagent的日志。
        ...
        ```shell
        # 创建log目录，完整的日志目录为/data/yashan/log
        $ cd /data/yashan
        $ mkdir log
        ```
...
- 按规划分开存放
    ...
    2. 在安装操作中，可以将其中一个目录（例如yasom和yasagent的日志的规划目录）将作为--log-path参数的值，生成[配置文件](../../全部手册/工具手册/yasboot/配置文件/00配置文件){集群名称}.toml后按需修改中另外两类日志目录相关参数再执行后续的安装部署操作，也可以等安装完成后修改对应的[配置参数](../../全部手册/参考手册/配置参数)调整run.log和slow.log的存放目录。
...
| 2 | 步骤4（可选）：修改多路径配置文件 | 如果通过多路径绑定存储设备且需要使用[基于SCSI持久预留的IO Fencing](../../全部手册/共享集群/集群服务管理/IO Fencing/基于SCSI持久预留的IO Fencing)功能，使用该功能前必须先在共享集群的每个服务器上修改多路径配置文件。 |
...
通过多路径绑定存储设备时，在使用[基于SCSI持久预留的IO Fencing](../../全部手册/共享集群/集群服务管理/IO Fencing/基于SCSI持久预留的IO Fencing)功能前，必须在共享集群的每台服务器上完成本操作。
