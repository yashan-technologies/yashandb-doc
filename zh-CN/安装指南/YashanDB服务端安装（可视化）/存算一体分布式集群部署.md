...
1. 根据实际情况，配置数据库基本信息：
   ...
   - 数据库部署形态：选择数据库部署形态，例如存算一体分布式集群部署。
...
1. 在数据库配置区域，可以配置以下功能：
   ...
   - 创建cgroup：开启表示创建用于YashanDB CPU资源管理的cgoup目录，并需填写服务器其他配置区域的cgroup目录。仅需安装可开启CPU资源管理的数据库（非级联备）时需配置该参数，具体请查阅[配置资源管理](../../全部手册/数据库管理/资源管理/配置资源管理)。
...
1. 在弹出的节点规模配置对话框中，根据[实际规划](../安装前准备/服务器准备)的节点数调整相关配置，单击【确定】保存信息。
   ...
   - 是否开启推荐配置：开启推荐配置时，yasom将调用[DBMS_PARAM](../../全部手册/开发手册/PL参考手册/内置高级包/DBMS_PARAM)高级包生成推荐参数覆盖同名配置参数，默认为开启。开启时，还需配置以下参数：
...
在【数据库建库参数】页面，可参考[存算一体分布式集群配置文件](../../全部手册/工具手册/yasboot/配置文件/存算一体分布式集群配置文件)按需增/删/改各节点组的参数，确认信息无误后，单击【下一步】。
...
1. 使用[yasql](../../全部手册/工具手册/yasql/yasql使用指导)工具连接数据库，查看实例状态。
...
2. （可选）创建数据库用户并赋权，更多操作请查阅[用户管理](../../全部手册/产品安全/身份标识与鉴别/管理用户)。
...
若DN节点数量配置为2（即DN节点采用一主一备规格），需使用[yasom仲裁选主](../../全部手册/工具手册/yasboot/yasboot命令介绍/yasboot election)才能实现DN组内节点的主备自动切换。
