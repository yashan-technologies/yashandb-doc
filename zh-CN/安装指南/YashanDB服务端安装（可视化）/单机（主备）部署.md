...
1. 在数据库配置区域，可以配置以下功能：
   ...
   - 创建cgroup：开启表示创建用于YashanDB CPU资源管理的cgoup目录，并需填写服务器其他配置区域的cgroup目录。仅需安装可开启CPU资源管理的单机数据库（非级联备）时需配置该参数，具体请查阅[配置资源管理](../../全部手册/数据库管理/资源管理/配置资源管理)。
...
在【数据库建库参数】页面，可参考[单机配置文件](../../全部手册/工具手册/yasboot/配置文件/单机配置文件)按需增/删/改建库参数，确认信息无误后，单击【下一步】。
