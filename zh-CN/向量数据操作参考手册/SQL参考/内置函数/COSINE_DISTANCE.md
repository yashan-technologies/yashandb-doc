...
COSINE_DISTANCE函数用于两个向量之间的余弦距离，函数返回值为DOUBLE类型。

COSINE_DISTANCE函数是VECTOR_DISTANCE函数**metric**配置为`COSINE`时的简化用法。

使用COSINE_DISTANCE函数查询时，如果对应列上创建了基于余弦算法的索引才可以通过索引加速查询。
