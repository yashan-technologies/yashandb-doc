...
mysql模式下创建的用户，可以被授予/收回的权限是yashan模式和mysql模式两套权限机制的并集（[msyql模式相关权限](00特权管理.md)与[yashan模式相关权限](../../../产品安全/数据访问控制/特权与角色管理/00特权与角色管理.md)），请根据实际需求合理授权。
...
3. 执行[GRANT](../../SQL参考手册/SQL语句/GRANT)语句，对目标用户发起授权，可选权限集为[msyql模式相关权限](00特权管理.md)。
