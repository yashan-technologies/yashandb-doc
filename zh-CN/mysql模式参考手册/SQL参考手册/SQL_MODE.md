...
|ONLY_FULL_GROUP_BY          |该选项要求启用严格遵守SQL标准的GROUP BY语法，要求SELECT子句中出现的所有列必须出现在聚合函数或GROUP BY子句中；取消该选项后，支持不在GROUP BY列表中的属性列出现在查询列和HAVING列表中。<br>该选项无法对ORDER BY语法进行约束，即使取消该选项，也需要严格遵守SQL标准来编写ORDER BY语句。 |是      |
