...
LOCATE函数从源字符串[expr](../通用SQL语法/expr.md)的第position位开始查找目标字符串sub_character，比较规则为大小写敏感，返回第1次出现sub_character的位置值，返回值为BIGINT类型，未查找到则返回0。
...
- 当sub\_character的值为NULL时，函数返回NULL。  
