...
REPLACE函数将源数据[expr](../通用SQL语法/expr.md)中所有的search_expr指定内容替换为replace_expr指定内容并返回替换后的新数据。
...
需要被替换掉的目标内容，search_expr为与expr相同的通用表达式，须为字符型或可转化为字符型的其他类型。

当search_expr的值为NULL时，函数返回NULL。 

**replace_expr**

按此字符串的值进行替换，replace_expr为与expr相同的通用表达式。
