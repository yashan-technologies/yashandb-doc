...
RLIKE语法通过正则表达式匹配算法将字符表达式expr与正则表达式regexp做匹配。与[REGEXP_LIKE](../内置函数/REGEXP REGEXP_LIKE RLIKE)函数省略了第三个入参时效果相同，匹配成功返回1，匹配失败返回0。其完整语法形式为：

RLIKE条件表达式与REGEXP条件表达式是同义词，返回类型为TINYINT。
