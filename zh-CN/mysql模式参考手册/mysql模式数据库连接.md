...
连接模式（多IP场景）仅在使用驱动连接YashanDB服务端时可以进行配置，mysql模式仅支持[JDBC驱动](../开发手册/JDBC驱动/JDBC驱动使用介绍/00JDBC驱动使用介绍.md)和[C驱动](../开发手册/C语言系驱动/C驱动/C驱动接口说明/连接、授权和初始化函数/yacConnect.md)，且要求YashanDB服务端存在多个节点。
