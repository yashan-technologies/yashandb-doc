...
本小节将介绍以下两种场景数据导入导出的操作步骤，您也可以参考[yasldr](../../全部手册/工具手册/yasldr/00yasldr)、[imp](../../全部手册/工具手册/imp/00imp)、[exp](../../全部手册/工具手册/exp/00exp)对应文档了解详细的操作说明。
...
4. 以数据库安装用户在数据库服务器上执行shell命令，将`datafile`文件中数据导入至`yasldr_user`用户的`loadData`表中：
   ...
   YashanDB Loader Enterprise Edition Release {版本号} x86_64 297f388
...
3. 以安装用户登录数据库所在服务器，执行如下命令将`sales`schema下所有元数据导出至`export.owner.export`文件中，请将`********`更改成设置的sys用户密码：
   ...
   YashanDB Export Release {版本号} x86_64 297f388
...
为方便演示，仍然以[快速体验YashanDB](快速体验YashanDB)创建的数据库为例演示导入元数据操作，在实际使用中，目标库与源库为不同的数据库，则需要在目标库上创建用户并授予DBA权限。
...
4. 以安装用户登录数据库所在服务器，执行如下命令将元数据文件导入至`sales`用户，请将`********`更改成设置的用户密码：
   ...
   YashanDB Import Release {版本号} x86_64 297f388
