...
3. 通过命令行工具yasboot安装单机数据库。
   ...
   - 此处的hosts.toml、yashandb.toml文件为./bin/yasboot package se gen命令按照服务器本地化部署默认部署参数自动生成；在部署mysql语法模式时，yashandb.toml文件中参数`mysql_addr = "ip_addr:1690"`为MySQL协议的监听地址，1690为默认端口，可使用MySQL客户端或驱动通过该监听地址连接到YashanDB。如需自定义部署参数，可参考[yasboot package](../../全部手册/工具手册/yasboot/yasboot命令介绍/yasboot package)修改参数。
...
5. 以sys用户通过服务器本地的数据库客户端工具yasql连接数据库，查看数据库运行状态为open表示数据库正常运行，数据库默认连接端口为1688。
   ...
   为了方便您通过远端的Windows或Linux设备访问YashanDB，可参考[YashanDB客户端安装](../../全部手册/安装和升级/安装部署/YashanDB客户端安装/00YashanDB客户端安装)将数据库客户端工具yasql部署在远端设备上。
