...
当您需要部署生产环境数据库时，请严格参考[安装和升级](../全部手册/安装和升级/安装部署/00安装部署)手册进行操作。
...
3. 通过命令行工具yasboot安装单机数据库。
   ...
   - 此处的hosts.toml、yashandb.toml文件为./bin/yasboot package se gen命令按照服务器本地化部署默认部署参数自动生成，如需自定义部署参数，可参考[yasboot package](../全部手册/工具手册/yasboot/yasboot命令介绍/yasboot package)修改参数。
...
5. 以sys用户通过服务器本地的数据库客户端工具yasql连接数据库，查看数据库运行状态为open表示数据库正常运行，数据库默认连接端口为1688。
   ...
   为了方便您通过远端的Windows或Linux设备访问崖山数据库服务，可参考[YashanDB客户端安装](../全部手册/安装和升级/安装部署/YashanDB客户端安装/00YashanDB客户端安装)将数据库客户端工具yasql部署在远端设备上。
...
1. 准备服务器（以Linux CentOS 7.6系统为例），创建数据库安装用户yashan，并创建容器挂载目录。
   ...
   以安装用户yashan登录安装服务器并在根目录下创建2个目录用于映射YashanDB的数据目录和yasboot工具目录：
...
为了方便您通过远端的Windows或Linux设备访问数据库服务，可参考[YashanDB客户端安装](../全部手册/安装和升级/安装部署/YashanDB客户端安装/00YashanDB客户端安装)将数据库客户端工具yasql部署在远端设备上。连接数据库容器时，使用上述命令指定的sys用户密码及主机映射端口进行连接。
...
可参考[用户管理](../全部手册/产品安全/身份标识与鉴别/管理用户)了解详细的用户管理体系。
